...
#基础参数，需要模型审视修改
#网络名称，同目录名称
Network="improved-bagan-gp_ID3132_for_TensorFlow2.X"
#Device数量，单卡默认为1
RANK_SIZE=1
#训练参数
batch_size=128
...
for para in $*; do
    ...
    elif [[ $para == --epochs_steps* ]]; then
        epochs_steps=$(echo ${para#*=})
    elif [[ $para == --learning_step* ]]; then
        learning_step=$(echo ${para#*=})
...
#输出性能FPS，需要模型审视修改
sum_FPS=$(grep 'TimeHistory:' $cur_path/test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log | awk 'NR>1 {sum += $4};END {print sum}')
num_FPS=$(grep 'TimeHistory:' $cur_path/test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log | awk 'NR>1' | wc -l)
...
#输出训练精度,需要模型审视修改
train_accuracy=$(grep 'g_loss:' $cur_path/test/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log | awk 'END {print$NF}')
#打印，不需要修改
...
grep 'g_loss:' $cur_path/test/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log | awk '{print$NF}' >$cur_path/test/output/$ASCEND_DEVICE_ID/train_${CaseName}_loss.txt
#最后一个迭代loss值，不需要修改
