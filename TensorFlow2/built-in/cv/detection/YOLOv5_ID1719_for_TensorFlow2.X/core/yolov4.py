...
def YOLO(input_layer, NUM_CLASS, model='yolov4', is_tiny=False):
    struc = [[0.33, 0.5], [0.67, 0.75], [1, 1], [1.33, 1.25]]
    if model == 'yolov5s':
        return YOLOV5(input_layer, NUM_CLASS, *struc[0])
    if model == 'yolov5m':
        return YOLOV5(input_layer, NUM_CLASS, *struc[1])
    if model == 'yolov5l':
        return YOLOV5(input_layer, NUM_CLASS, *struc[2])
    if model == 'yolov5x':
        return YOLOV5(input_layer, NUM_CLASS, *struc[3])
    if is_tiny:
        if model == 'yolov4':
...
def YOLOV5(input_layer, NUM_CLASS, depth_gain, width_gain):
    route_1, route_2, conv = backbone.yolov5CSP(input_layer, depth_gain, width_gain)
    conv = common.convolutional(conv, filters_shape=(1, 1, 1024 * width_gain, 512 * width_gain), activate_type='mish')
    route = conv # 19 * 19 * 256
    conv = common.upsample(conv)
    conv = tf.concat([route_2, conv], axis=-1)
    conv = common.C3(conv, 1024 * width_gain, 512 * width_gain, 3 * depth_gain, False)
    conv = common.convolutional(conv, filters_shape=(1, 1, 512 * width_gain, 256 * width_gain), activate_type='mish')
    route_2 = conv # 38 * 38 * 128
    conv = common.upsample(conv)
    conv = tf.concat([route_1, conv], axis=-1)
    conv = common.C3(conv, 512 * width_gain, 256 * width_gain, 3 * depth_gain, False) # 76 * 76 * 128
    
    conv_sbbox = common.convolutional(conv, filters_shape=(1, 1, 256 * width_gain, 3 * (NUM_CLASS + 5)), activate=False, bn=False)
    
    conv = common.convolutional(conv, filters_shape=(3, 3, 256 * width_gain, 256 * width_gain), downsample=True, activate_type='mish')
    conv = tf.concat([route_2, conv], axis=-1)
    conv = common.C3(conv, 512 * width_gain, 512 * width_gain, 3 * depth_gain, False)
    
    conv_mbbox = common.convolutional(conv, filters_shape=(1, 1, 512 * width_gain, 3 * (NUM_CLASS + 5)), activate=False, bn=False)
    
    conv = common.convolutional(conv, filters_shape=(3, 3, 512 * width_gain, 512 * width_gain), downsample=True, activate_type='mish')
    conv = tf.concat([route, conv], axis=-1)
    conv = common.C3(conv, 1024 * width_gain, 1024 * width_gain, 3 * depth_gain, False)
    
    conv_lbbox = common.convolutional(conv, filters_shape=(1, 1, 1024 * width_gain, 3 * (NUM_CLASS + 5)), activate=False, bn=False)
...
def decode_train(conv_output, output_size, NUM_CLASS, STRIDES, ANCHORS, i=0, XYSCALE=[1, 1, 1]):
    ...
    pred_xy = ((tf.sigmoid(conv_raw_dxdy) * 2 - 0.5) + xy_grid) * STRIDES[i]
    pred_wh = (tf.sigmoid(conv_raw_dwdh) * 2) ** 2 * ANCHORS[i]
...
def compute_loss(pred, conv, label, bboxes, STRIDES, NUM_CLASS, IOU_LOSS_THRESH, i=0):
    ...
    diou = tf.expand_dims(utils.bbox_diou(pred_xywh, label_xywh), axis=-1)
    ...
    conf_loss = conf_focal * (
            0.75 * respond_bbox * tf.nn.sigmoid_cross_entropy_with_logits(labels=respond_bbox, logits=conv_raw_conf)
            +
            0.25 * respond_bgd * tf.nn.sigmoid_cross_entropy_with_logits(labels=respond_bbox, logits=conv_raw_conf)
    )
    ...
    return (diou_loss, conf_loss, prob_loss) if i == 0 else(ciou_loss, conf_loss, prob_loss)