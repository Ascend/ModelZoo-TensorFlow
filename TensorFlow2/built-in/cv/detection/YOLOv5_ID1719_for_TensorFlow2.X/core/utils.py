...
def load_freeze_layer(model='yolov4', tiny=False):
    if tiny:
        ...
        if model == 'yolov3':
            ...
            freeze_layouts = ['conv2d_47', 'conv2d_54', 'conv2d_61']
...
def load_config(FLAGS):
    if FLAGS.tiny:
        ...
        STRIDES = np.array(cfg.YOLO.STRIDES, dtype=np.int32)
        if FLAGS.model == 'yolov4':
            ANCHORS = get_anchors(cfg.YOLO.ANCHORS, FLAGS.tiny)
        elif FLAGS.model in['yolov3', 'yolov5s', 'yolov5m', 'yolov5l', 'yolov5x']:
...
def bbox_iou_(bboxes1, bboxes2):
    bboxes1_area = bboxes1[..., 2] * bboxes1[..., 3]
    bboxes2_area = bboxes2[..., 2] * bboxes2[..., 3]

    bboxes1_coor = tf.concat([bboxes1[..., :2], bboxes1[..., :2]],axis=-1)
    bboxes2_coor = tf.concat([bboxes2[..., :2], bboxes2[..., :2]],axis=-1)
    left_up = tf.maximum(bboxes1_coor[..., :2], bboxes2_coor[..., :2])
...
def bbox_giou(bboxes1, bboxes2):
    ...
    iou = tf.math.divide_no_nan(inter_area, union_area + 1e-3)
...
def bbox_diou(boxes1, boxes2):
    ...
    iou = 1.0 * inter_area / (union_area + 1e-4)
    ...
    diou = iou - 1.0 * center_distance / (enclose_diagonal + 1e-4)
...
def bbox_ciou(bboxes1, bboxes2):
    ...
    diou = iou - tf.math.divide_no_nan(rho_2, c_2 + 1e-3)

    v = (
        (
            tf.math.atan(
                tf.math.divide_no_nan(bboxes1[..., 2], bboxes1[..., 3] + 1e-3)
            )
            - tf.math.atan(
                tf.math.divide_no_nan(bboxes2[..., 2], bboxes2[..., 3] + 1e-3)
            )
        )
        * 2
        / np.pi
    ...
    alpha = tf.math.divide_no_nan(v, 1 - iou + v + 1e-3)
...
def nms(bboxes, iou_threshold, sigma=0.3, method='nms'):
    ...
    for cls in classes_in_img:
        ...
        while len(cls_bboxes) > 0:
            ...
            iou = bbox_ciou(best_bbox[np.newaxis, :4], cls_bboxes[:, :4])
