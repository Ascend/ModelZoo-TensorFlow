...
echo "------------------ Final result ------------------"
#输出性能FPS。需要模型审视修改
steps_per_epoch=`grep steps_per_epoch $cur_path/output/0/train_0.log | awk '{print $2}'`
epoch_duration=`grep epoch_duration $cur_path/output/0/train_0.log | awk '{print $2}'`
FPS=$(($steps_per_epoch / $epoch_duration * $batch_siz * 8))
...
ActualFPS=${FPS}
#单迭代训练时长
TrainingTime=$(($epoch_duration / $steps_per_epoch))
