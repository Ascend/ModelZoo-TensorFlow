...
class SwinTransformerBlock(tf.keras.layers.Layer):
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0, mlp_ratio=4.,
        ...
        self.norm1 = norm_layer(epsilon=1e-6, name=f'{self.prefix}/norm1')
        ...
        self.norm2 = norm_layer(epsilon=1e-6, name=f'{self.prefix}/norm2')
...
class PatchMerging(tf.keras.layers.Layer):
    def __init__(self, input_resolution, dim, norm_layer=LayerNormalization, prefix=''):
        ...
        self.norm = norm_layer(epsilon=1e-6, name=f'{prefix}/downsample/norm')
    ...
    def call(self, x):
        ...
        '''
        x = tf.reshape(x, shape=(-1, H // 2, 2, W // 2, 2, C))
        x0 = x[:, :, 0:1, :, 0:1, :]
        x1 = x[:, :, 1:2, :, 0:1, :]
        x2 = x[:, :, 0:1, :, 1:2, :]
        x3 = x[:, :, 1:2, :, 1:2, :]
...
def SwinTransformer(ckpt_path, model_name='swin_large_224', num_classes=1000, include_top=True, pretrained=True, use_tpu=False, cfgs=CFGS):
    cfg = cfgs[model_name]
    ...
    if pretrained is True:
        ...
        pretrained_ckpt = f'{ckpt_path}/{model_name}'
    else:
