...
parser.add_argument("--nproc_per_node", default=1, type=int, help="npu_device_nums")
...
TOTAL_BATCH_SIZE=args.nproc_per_node*args.batch_size
...
def load_dataset(filenames, labeled=True, ordered=False):
    ...
    #dataset= dataset.interleave(tf.data.TFRecordDataset,cycle_length=10,num_parallel_calls=1)
    dataset = dataset.with_options(ignore_order) # uses data as soon as it streams in, rather than in its original order
    #dataset= dataset.interleave(tf.data.TFRecordDataset,cycle_length=10,num_parallel_calls=10)
...
def get_training_dataset():
    ...
    dataset, BATCH_SIZE = npu_device.distribute.shard_and_rebatch_dataset(dataset,TOTAL_BATCH_SIZE)
    print('111111111111122221',str(BATCH_SIZE))
...
opt = npu_device.distribute.npu_distributed_keras_optimizer_wrapper(opt)
model.compile(
    optimizer=opt,
