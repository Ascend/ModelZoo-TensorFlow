...
#参数配置
data_path="/npu/traindata/dataset_swin/"
ckpt_path="/npu/traindata/pretrain/"
model_name=""
...
do	
	...
	python3 ../swintransformer/train_main_16p.py \
        --data_path=${data_path} \
        --ckpt_path=${ckpt_path} \
	...
	--batch_size 16 \
	--nproc_per_node=8 >$cur_path/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log 2>&1 &
done
wait
end=$(date +%s)
e2etime=$(( $end - $start ))
