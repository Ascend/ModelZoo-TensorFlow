...
#网络名称，同目录名称
Network="YoloV3_ID0363_for_TensorFlow2.X"
...
#输出性能FPS，需要模型审视修改
TrainingTime=`grep "perf" $cur_path/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log|awk 'END {print $15}'`
wait
...
grep "loss:" $cur_path/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log|awk '{print $13}'|sed 's/,//g'|sed '/^$/d' > $cur_path/output/$ASCEND_DEVICE_ID/train_${CaseName}_loss.txt
