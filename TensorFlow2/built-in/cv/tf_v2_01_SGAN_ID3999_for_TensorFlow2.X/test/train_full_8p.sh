...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
TIME=`grep Time $cur_path/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log | awk '{if(NR>1)sum+=$NF}END{print sum/(NR-1)}'`
...
grep Time $cur_path/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log | awk '{print $6}' > $cur_path/output/$ASCEND_DEVICE_ID/train_${CaseName}_loss.txt
