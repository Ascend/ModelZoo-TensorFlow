...
batch_size=128
#网络名称，同目录名称
Network="tf_v2_02_CGAN_ID4000_for_TensorFlow2.X"
#Device数量，单卡默认为1
RankSize=1
#训练epoch，可选
train_epochs=50
...
start=$(date +%s)
python3 train.py --train_epochs=${train_epochs} \
...
wait
#输出性能FPS，需要模型审视修改
# single_batch_step_sec=`grep TimeHistory  $cur_path/test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|awk '{sum+=$4}END{printf "%.4f\n", sum/NR}'`
# FPS=`awk 'BEGIN{printf "%.2f\n",'${single_batch_step_sec}'}'`
TIME=$(grep Time $cur_path/test/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log | awk '{if(NR>1)sum+=$NF}END{print sum/(NR-1)}')
FPS=$(awk 'BEGIN{printf "%.2f\n", '${batch_size}'/'${TIME}'}')
# 打印，不需要修改
...
# grep loss: $cur_path/test/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log | awk '{print $NF}' > $cur_path/test/output/$ASCEND_DEVICE_ID/train_${CaseName}_loss.txt
grep Time $cur_path/test/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log | awk '{print $6}' >$cur_path/test/output/$ASCEND_DEVICE_ID/train_${CaseName}_loss.txt
...
# ModelStatus="图执行FAIL"
# DTS_Number="DTS2022060806756"
# error_msg="E19999  Param:in_data_anchor is nullptr, check invalid"
