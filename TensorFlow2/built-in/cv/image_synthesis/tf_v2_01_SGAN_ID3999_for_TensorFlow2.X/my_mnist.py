...
def loadMinistLable(filename):
    ...
    if head[0]!=2049:
        ...
        return labels
def load_data(data_path, get_new=True,normalization=False,one_hot=False,detype=np.float32):
    if get_new==True:
        train_images = loadMinistImage(os.path.join(data_path, "train-images.idx3-ubyte"))
        train_labels = loadMinistLable(os.path.join(data_path, "train-labels.idx1-ubyte"))
        test_images = loadMinistImage(os.path.join(data_path, "t10k-images.idx3-ubyte"))
        test_labels = loadMinistLable(os.path.join(data_path, "t10k-labels.idx1-ubyte"))
        np.savez(os.path.join(data_path, "mnist.npz"),k1=train_images,k2=train_labels,k3=test_images,k4=test_labels)
    else:
        npzfile=np.load(os.path.join(data_path, 'mnist.npz'))
