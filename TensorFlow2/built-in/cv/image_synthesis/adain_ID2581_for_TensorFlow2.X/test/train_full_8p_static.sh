...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
sum_FPS=$(grep 'TimeHistory:' $cur_path/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log | awk 'NR>1 {sum += $4};END {print sum}')
num_FPS=$(grep 'TimeHistory:' $cur_path/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log | awk 'NR>1' | wc -l)
...
#输出训练精度,需要模型审视修改
train_accuracy=$(grep '50/50' $cur_path/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log | awk 'END {print$12}')
#打印，不需要修改
...
grep '50/50' $cur_path/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log | awk '{print$12}' >$cur_path/output/$ASCEND_DEVICE_ID/train_${CaseName}_loss.txt
