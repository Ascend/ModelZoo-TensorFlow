...
#输出性能FPS，需要模型审视修改
sum_FPS=$(grep 'TimeHistory:' $cur_path/test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log |tail -n 4 | awk '{sum += $4};END {print sum}')
num_FPS=$(grep 'TimeHistory:' $cur_path/test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log |tail -n 4 | wc -l)
