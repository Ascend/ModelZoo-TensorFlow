...
class CNN(object):
    ...
    def __init__(self, height, width, channel, num_class, rank_size, leaning_rate=1e-3, ckpt_dir='./Checkpoint'):
        ...
        if self.rank_size != 1:
            self.optimizer = npu_device.distribute.npu_distributed_keras_optimizer_wrapper(tf.optimizers.Adam(self.leaning_rate))
        else:
    ...
    @tf.function
    def compat_tape(self,x,y,train):
    ...
    def step(self, x, y, iteration=0, train=False):
        ...
        loss, accuracy, score = self.compat_tape(x,y,train)

        return loss, accuracy, score

    def save_params(self):
        # self.model.save_weights("%s/model.h5" %(self.ckpt_dir))
        self.model.save_weights(filepath=os.path.join(self.ckpt_dir,'model'),save_format='tf')

    def load_params(self):
        # self.model.load_weights("%s/model.h5" %(self.ckpt_dir))
        self.model.load_weights(filepath=os.path.join(self.ckpt_dir, 'model'))
