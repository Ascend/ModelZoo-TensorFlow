...
class Dataset(object):
    ...
    def __init__(self, rank_size, device_id, data_path,normalize=True):
        ...
        f = np.load(os.path.join(self.data_path,'mnist.npz'))
        x_tr, y_tr,x_te, y_te = f['x_train'],f['y_train'],f['x_test'],f['y_test']
        if rank_size != 1:
            self.x_tr = np.split(x_tr, rank_size)
            self.x_tr = self.x_tr[device_id]

            self.y_tr = np.split(y_tr, rank_size)
            self.y_tr = self.y_tr[device_id]

            self.x_te = np.split(x_te, rank_size)
            self.x_te = self.x_te[device_id]

            self.y_te = np.split(y_te, rank_size)
            self.y_te = self.y_te[device_id]
        else:
        ...
        self.example_num = self.x_tr.shape[0]
