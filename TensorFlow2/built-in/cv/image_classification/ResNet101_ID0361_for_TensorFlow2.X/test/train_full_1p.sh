...
#网络名称，同目录名称
Network="ResNet101_ID0361_for_TensorFlow2.X"
...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
TrainingTime=`grep "perf" $cur_path/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log|tail -2|head -1|awk '{print $10}'`
wait
...
#输出训练精度,需要模型审视修改
train_accuracy=`grep "valid accuracy:" $cur_path/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log|awk 'END {print $8}'|awk -F ',' '{print $1}'`
#打印，不需要修改
...
grep "loss:" $cur_path/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log|grep -v "train loss"|awk '{print $6}'|sed 's/,//g'|sed '/^$/d' >> $cur_path/output/$ASCEND_DEVICE_ID/train_${CaseName}_loss.txt
