...
batch_size=64
#网络名称，同目录名称
Network="Densenet_smplified_ID3541_for_TensorFlow2.X"
#Device数量，单卡默认为1
RANK_SIZE=1
#训练epoch，可选
train_epochs=2
...
#学习率
learning_rate=1e-4
log_steps=2 # 0.5 * steps_per_epoch
train_data_size=500
test_data_size=128
#参数配置
data_path=""
...
for para in $*; do
   ...
   elif [[ $para == --train_data_size* ]]; then
      train_data_size=$(echo ${para#*=})
   elif [[ $para == --test_data_size* ]]; then
      test_data_size=$(echo ${para#*=})
...
start=$(date +%s)
nohup python3 cifar10_densenet_classification.py \
   --data_path=$data_path \
...
#输出训练精度,需要模型审视修改
train_accuracy=$(grep "val_accuracy" $cur_path/test/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log | awk 'END {print $15}')
#打印，不需要修改
