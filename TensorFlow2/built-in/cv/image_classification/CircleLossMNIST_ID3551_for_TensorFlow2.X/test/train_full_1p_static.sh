...
batch_size=256
#网络名称，同目录名称
Network="CircleLossMNIST_ID3551_for_TensorFlow2.X"
#Device数量，单卡默认为1
RANK_SIZE=1
#训练epoch，可选
train_epochs=30
...
#学习率
#learning_rate=1e-4
log_steps=54 # 0.5 * steps_per_epoch
#参数配置
data_path=""
...
#输出性能FPS，需要模型审视修改
sum_FPS=$(grep 'FPS:' $cur_path/test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log | awk 'NR>1 {sum += $8};END {print sum}')
num_FPS=$(grep 'FPS:' $cur_path/test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log | awk 'NR>1' | wc -l)
FPS=$(awk 'BEGIN{printf "%2.f\n",'${sum_FPS}'/'${num_FPS}'}')
#打印，不需要修改
echo "Final Performance images/sec : $FPS"

#输出训练精度,需要模型审视修改
train_accuracy=$(grep "val_acc" $cur_path/test/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log | awk 'END {print $2}')
#打印，不需要修改
