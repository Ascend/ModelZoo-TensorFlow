...
start=$(date +%s)
python3 mnist_AQUAVS.py --data_dir=${data_path} \
...
#输出训练精度,需要模型审视修改
train_accuracy=$(grep '1500/1500' $cur_path/test/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log | awk 'END {print$6}')
#打印，不需要修改
...
grep '1500/1500' $cur_path/test/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log | awk '{print$6}' >$cur_path/test/output/$ASCEND_DEVICE_ID/train_${CaseName}_loss.txt
#最后一个迭代loss值，不需要修改
