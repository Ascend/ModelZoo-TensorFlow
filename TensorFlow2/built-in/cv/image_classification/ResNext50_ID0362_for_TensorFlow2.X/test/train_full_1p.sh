...
batch_size=8
#网络名称，同目录名称
Network="Basic_CNNs_TF2_ID0362_for_TensorFlow"
#Device数量，单卡默认为1
RANK_SIZE=1
#训练epoch，可选
train_epochs=1
...
#输出训练精度,需要模型审视修改
train_accuracy=`grep "accuracy:" $cur_path/test/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log|awk 'END {print $8}'| sed 's/,//g'`
#打印，不需要修改
