...
batch_size=8
#网络名称，同目录名称
Network="MobileNetV3_ID0364_for_TensorFlow2.X"
#Device数量，单卡默认为1
RANK_SIZE=1
#训练epoch，可选
train_epochs=50
...
# 帮助信息，不需要修改
if [[ $1 == --help || $1 == -h ]];then
    echo"usage:./train_full_1p_MobileNetV3.sh <args>"
    echo " "
...
##############执行训练##########
cd $cur_path
sed -i "s|EPOCHS = 1|EPOCHS = ${train_epochs}|g" configuration.py
sed -i "s|dataset/|${data_path}/dataset/|g" configuration.py
sed -i "s|model_index = 1|model_index = 3|g" configuration.py
wait
...
#参数回改
sed -i "s|EPOCHS = ${train_epochs}|EPOCHS = 1|g" configuration.py
sed -i "s|${data_path}/dataset/|dataset/|g" configuration.py
sed -i "s|model_index = 3|model_index = 1|g" configuration.py
