...
#基础参数 需要模型审视修改
#网络名称，同目录名称
Network="aquavs_ID3120_for_TensorFlow2.X"
#训练batch_size
batch_size=256
#训练step
#train_steps=1000
...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
FPS=$(grep 'TimeHistory' $cur_path/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log | awk 'END {print$4}')
...
#输出训练精度,需要模型审视修改
train_accuracy=$(grep '1500/1500' $cur_path/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log | awk 'END {print$6}')
#打印，不需要修改
...
grep '1500/1500' $cur_path/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log | awk '{print$6}' >$cur_path/output/$ASCEND_DEVICE_ID/train_${CaseName}_loss.txt
#最后一个迭代loss值，不需要修改
