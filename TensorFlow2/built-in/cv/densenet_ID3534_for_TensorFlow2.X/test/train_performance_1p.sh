...
batch_size=100
#网络名称，同目录名称
Network="densenet_ID3534_for_TensorFlow2.X"
#Device数量，单卡默认为1
RANK_SIZE=1
#训练epoch，可选
train_epochs=2
log_steps=120
#每个epoch的步长，测试性能，每个epoch只跑1/3，仅在测试性能时使用
steps_per_epoch=480
...
#输出性能FPS，需要模型审视修改
single_batch_step_sec=`grep TimeHistory  $cur_path/test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log | awk -F " " '{print$4}' | awk 'BEGIN{count=0}{if(NR%4!=1){sum+=$NF;count+=1}}END{printf "%.4f\n", sum/count}'`
FPS=`awk 'BEGIN{printf "%.2f\n",'${single_batch_step_sec}'}'`
#打印，不需要修改
echo "Final Performance images/sec : $FPS"

#输出训练精度,需要模型审视修改
train_accuracy=$(grep "loss" $cur_path/test/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log | awk -F " " 'END{print$15}')
#打印，不需要修改
