...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
sum_FPS=$(grep 'FPS:' $cur_path/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log | awk 'NR>1 {sum += $8};END {print sum}')
num_FPS=$(grep 'FPS:' $cur_path/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log | awk 'NR>1' | wc -l)
FPS=$(awk 'BEGIN{printf "%2.f\n",'${sum_FPS}'/'${num_FPS}'}')
#打印，不需要修改
echo "Final Performance images/sec : $FPS"

#输出训练精度,需要模型审视修改
train_accuracy=$(grep "val_acc" $cur_path/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log | awk 'END {print $2}')
#打印，不需要修改
