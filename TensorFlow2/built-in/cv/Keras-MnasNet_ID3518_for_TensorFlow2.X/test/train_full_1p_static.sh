...
#输出性能FPS，需要模型审视修改
FPS=`grep "examples/second" $cur_path/test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log | awk -F " " '{print$4}' | awk '{if(NR!=1){sum+=$NF}}END{printf "%.4f\n", sum/(NR-1)}'`
FPS=`printf "%.4f\n" ${FPS}`
#打印，不需要修改
echo "Final Performance images/sec : $FPS"

#输出训练精度,需要模型审视修改
train_accuracy=$(grep "accuracy" $cur_path/test/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log | awk -F " " 'END{print$9}')
#打印，不需要修改
...
grep 'accuracy' $cur_path/test/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log | awk -F " " '{print$6}' >$cur_path/test/output/$ASCEND_DEVICE_ID/train_${CaseName}_loss.txt
#最后一个迭代loss值，不需要修改
