...
batch_size=32
#网络名称，同目录名称
Network="keras-transformer_ID3469_for_TensorFlow2.X"
#Device数量，单卡默认为1
RANK_SIZE=1
#训练epoch，可选
train_epochs=15
...
#学习率
#learning_rate=1e-5
log_steps=500 # 0.5 * steps_per_epoch
#参数配置
data_path=""
...
single_batch_step_sec=`grep TimeHistory  $cur_path/test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log | awk -F " " '{print$4}' | awk 'BEGIN{count=0}{if(NR%5!=1){sum+=$NF;count+=1}}END{printf "%.4f\n", sum/count}'`
