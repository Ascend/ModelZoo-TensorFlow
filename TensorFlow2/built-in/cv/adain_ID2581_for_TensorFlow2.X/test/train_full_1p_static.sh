...
#基础参数，需要模型审视修改
#网络名称，同目录名称
Network="adain_ID2581_for_TensorFlow2.X"
#Device数量，单卡默认为1
RANK_SIZE=1
#Batch Size
batch_size=64
...
steps_per_epoch=50
drop_remainder=True
log_steps=25 # 0.5 * steps_per_epoch
#参数配置
data_path=""
...
for para in $*; do
    ...
    elif [[ $para == --drop_remainder* ]]; then
        drop_remainder=$(echo ${para#*=})
...
start=$(date +%s)
python3 adain.py --data_dir=${data_path} \
...
#输出训练精度,需要模型审视修改
train_accuracy=$(grep '50/50' $cur_path/test/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log | awk 'END {print$12}')
#打印，不需要修改
...
grep '50/50' $cur_path/test/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log | awk '{print$12}' >$cur_path/test/output/$ASCEND_DEVICE_ID/train_${CaseName}_loss.txt
#最后一个迭代loss值，不需要修改
