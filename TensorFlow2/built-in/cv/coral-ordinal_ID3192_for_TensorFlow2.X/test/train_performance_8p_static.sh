...
#基础参数 需要模型审视修改
#网络名称，同目录名称
Network="coral-ordinal_ID3192_for_TensorFlow2.X"
#训练batch_size
batch_size=1024
#训练step
#train_steps=1000
...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
sum_FPS=$(grep 'TimeHistory:' $cur_path/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log |tail -n 3 | awk '{sum += $4};END {print sum}')
num_FPS=$(grep 'TimeHistory:' $cur_path/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log |tail -n 3 | wc -l)
...
#输出训练精度,需要模型审视修改
train_accuracy=$(grep ' loss:' $cur_path/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log | awk 'END {print$6}')
#train_accuracy=`grep "train_acc " $cur_path/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log|awk 'END {print $12}'|sed 's/,//g'`
#打印，不需要修改
