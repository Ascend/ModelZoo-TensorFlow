...
def FLEN(linear_feature_columns,
    ...
    dnn_logit = ts.layers.Dense(1, use_bias=False, kernel_initializer=tf.keras.initializers.glorot_normal(seed))(
        concat_func([fm_mf_out, dnn_output]))

    final_logit = add_func([linear_logit, dnn_logit])
    output = PredictionLayer(task)(final_logit)

    model = ts.models.Model(inputs=inputs_list, outputs=output)
    return model
