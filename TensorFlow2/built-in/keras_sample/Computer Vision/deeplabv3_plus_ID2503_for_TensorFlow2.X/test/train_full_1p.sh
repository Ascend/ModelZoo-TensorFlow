...
batch_size=4
#网络名称，同目录名称
Network="deeplabv3_plus_ID2503_for_TensorFlow2.X"
#Device数量，单卡默认为1
RANK_SIZE=1
#训练epoch，可选
train_epochs=25
...
##############执行训练##########
cd $cur_path
cp $data_path/../resnet50_weights_tf_dim_ordering_tf_kernels_notop.h5 /root/.keras/models/
...
#输出训练精度,需要模型审视修改
train_accuracy=`grep val_loss $cur_path/test/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log | awk '{print$NF}' | awk 'BEGIN {max=0} {if ($1+0 > max+0) max=$1} END {print max}'`
#打印，不需要修改
...
grep '250/250' $cur_path/test/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log| awk -F  " " '{print $6}' > $cur_path/test/output/$ASCEND_DEVICE_ID/train_${CaseName}_loss.txt 
#最后一个迭代loss值，不需要修改
