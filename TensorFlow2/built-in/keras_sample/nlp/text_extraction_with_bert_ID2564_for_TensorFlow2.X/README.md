...
**描述（Description）：基于TensorFlow框架的微调预训练BERT的文字提取模型训练代码**

<h2 id="概述.md">概述</h2>

## 简述<a name="section194554031510"></a>

本项目使基于TensorFlow2.X框架的微调预训练BERT的文字提取模型，目标是在所使用的斯坦福问答数据集中的问答问题的段落中找到文本的跨度。我们使用“精确匹配”的指标评估我们在这些数据上的表现，该指标衡量与任何一个真实答案完全匹配的预测百分比。
...
- 参考实现：
  ...
  https://github.com/keras-team/keras-io/blob/master/examples/nlp/text_extraction_with_bert.py
...
1、用户自行准备好数据集，本网络使用的数据集是SQuAD数据集（斯坦福问答数据集）
...
- 开始训练。
    ...
    2. 单卡训练
        ...
        2. 1单卡训练指令（脚本位于text_extraction_with_bert_ID2564_for_TensorFlow2.X/test/train_full.sh）,需要先使用cd命令进入test目录下，再使用下面的命令启动训练。请确保下面例子中的“--data_path”修改为用户的数据路径,这里选择将数据文件夹放在home目录下。
...
├──text_extraction_with_bert.py    				#训练代码
