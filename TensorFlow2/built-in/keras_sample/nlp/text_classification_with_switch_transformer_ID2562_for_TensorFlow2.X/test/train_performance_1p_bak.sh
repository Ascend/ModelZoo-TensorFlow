...
batch_size=50
#网络名称，同目录名称
Network="test_classification_with_switch_transformer_ID2562_for_TensorFlow2.X"
#Device数量，单卡默认为1
RankSize=1
#训练epoch，可选
train_epochs=5
...
start=$(date +%s)
nohup python3 text_classification_with_switch_transformer.py --data_dir=$data_path --epochs=$train_epochs --batch_size=$batch_size >$cur_path/test/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log 2>&1 &
wait
end=$(date +%s)
e2etime=$(( $end - $start ))
...
##获取错误信息
#系统错误信息
error_msg="model/transformer_block/multi_head_attention/attention_output/einsum/Einsum/BatchMatMul"
