...
eval_batch_size=16
#网络名称，同目录名称
Network="BERT_ID3228_for_TensorFlow2.X"
#Device数量，单卡默认为1
RANK_SIZE=1
#训练epoch，可选
epochs=2
...
SQUAD_VERSION=v1.1
BERT_BASE_DIR=${data_path}/download/google_pretrained_weights/uncased_L-12_H-768_A-12/
SQUAD_DIR=${data_path}/download/squad/v1.1
...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
FPS=`grep Perf $cur_path/test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|tail -n 2|awk '{sum += $6} END {printf "NR = %d,Average = %3.3f\n", NR, sum/NR}' | awk '{print $5}'`
#打印，不需要修改
