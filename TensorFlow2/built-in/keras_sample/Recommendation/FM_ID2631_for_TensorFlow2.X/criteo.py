...
def create_criteo_dataset(file, embed_dim=8, read_part=True, sample_num=100000, test_size=0.2,static=0):
    """
    ...
    if static==1:
        train_X = train[features].values[:3686400].astype('int32')
        train_y = train['label'].values[:3686400].astype('int32')
        print("train_X.shape",train_X.shape,flush=True)
        print("train_y.shape",train_y.shape,flush=True)
        test_X = test[features].values[:999424].astype('int32')
        test_y = test['label'].values[:999424].astype('int32')
