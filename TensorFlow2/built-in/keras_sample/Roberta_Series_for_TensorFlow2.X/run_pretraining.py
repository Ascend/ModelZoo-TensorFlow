...
flags.DEFINE_bool("distributed", False, "Use multiple NPUs for training using Horovod.")
...
def model_fn_builder(bert_config, init_checkpoint, learning_rate,
                     ...
                     use_one_hot_embeddings, distributed):
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      train_op = optimization.create_optimizer(
          total_loss, learning_rate, num_train_steps, num_warmup_steps, use_tpu, distributed)

      logging_hook = tf.estimator.LoggingTensorHook({"loss":total_loss,"global_step":tf.compat.v1.train.get_global_step()},every_n_iter=1)
      ...
      def metric_fn(masked_lm_example_loss, masked_lm_log_probs, masked_lm_ids,
        ...
        masked_lm_predictions = tf.argmax(input=masked_lm_log_probs, axis=-1, output_type=tf.int32)
...
def main(_):
  ...
  if FLAGS.distributed:
    # print()
    # FLAGS.output_dir = FLAGS.output_dir if get_npu_rank_id() == 0 else \
        # os.path.join(FLAGS.output_dir, str(get_npu_rank_id()))
    # Horovod: adjust number of steps based on number of GPUs.
    FLAGS.num_train_steps = FLAGS.num_train_steps // get_npu_rank_size()
    FLAGS.num_warmup_steps = FLAGS.num_warmup_steps // get_npu_rank_size()
  ...
  if FLAGS.distributed:
    ...
    run_config = tf.compat.v1.estimator.tpu.RunConfig(
        ...
        save_checkpoints_steps=FLAGS.save_checkpoints_steps if get_rank_id() ==7 else 0,
        # save_checkpoints_steps=FLAGS.save_checkpoints_steps,
        tpu_config=tf.compat.v1.estimator.tpu.TPUConfig(
            iterations_per_loop=FLAGS.iterations_per_loop,
            num_shards=FLAGS.num_tpu_cores,
            per_host_input_for_training=is_per_host),
        session_config=session_config, save_summary_steps=0)
  else:
    run_config = tf.compat.v1.estimator.tpu.RunConfig(
        cluster=tpu_cluster_resolver,
  ...
  if FLAGS.do_train:
    ...
    if FLAGS.distributed:
        ...
        hooks = []
    estimator.train(input_fn=train_input_fn, max_steps=FLAGS.num_train_steps, hooks=npu_hooks_append(hooks_list=hooks))

  if FLAGS.do_eval and int(os.getenv('RANK_ID')) == 7:
#   if FLAGS.do_eval:
