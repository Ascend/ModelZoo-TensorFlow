...
start=$(date +%s)
nohup python3 gradient_centralization.py --data_path=$data_path --epochs=${train_epochs} --batch_size=${batch_size} --drop_remainder=True >$cur_path/test/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log 2>&1 &
wait
end=$(date +%s)
e2etime=$(( $end - $start ))
