...
#基础参数，需要模型审视修改
#网络名称，同目录名称
Network="DIN_ID2641_for_TensorFlow2.X"
#训练epoch
train_epochs=5
#训练batch_size
batch_size=4096
...
#输出训练精度,需要模型审视修改
train_accuracy=`cat $cur_path/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log| grep loss | grep auc | awk 'END{print $NF}'`
#打印，不需要修改
