...
batch_size=4096
#网络名称，同目录名称
Network="WDL_ID2633_for_TensorFlow2.X"
#Device数量，单卡默认为1
RankSize=1
#训练epoch，可选
#train_epochs=5
...
Time=`grep val_loss $cur_path/test/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log | tail -n +2 | awk '{print $3}' | tr -d s | awk '{sum+=$1} END {print sum/NR}'`
...
train_accuracy=`grep val_loss $cur_path/test/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log |awk '{print $15}'|tail -1`
#打印，不需要修改
...
grep val_loss: $cur_path/test/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log|awk -F "loss:" '{print $2}'|awk -F "- auc:" '{print $1}' | tr -d ' ' > $cur_path/test/output/$ASCEND_DEVICE_ID/train_${CaseName}_loss.txt
#打印，不需要修改
