...
class TransformerTask(object):
  ...
  def train(self):
    ...
    set_split_strategy_by_size([50, 25, 25])
    ...
    while current_step < flags_obj.train_steps:
      ...
      if params["use_ctl"]:
        ...
        with summary_writer.as_default():
          ...
          logging.info("Train Step: %d/%d / loss = %s / loss_scale = %s / learning_rate = %s / not_overflow_status = %s", current_step,
                       flags_obj.train_steps, train_loss, opt.loss_scale.numpy(), opt.inner_optimizer.learning_rate(current_step).numpy(), opt.last_step_finite.numpy())
        ...
        logging.info("loss_scale = %s / learning_rate = %s / not_overflow_status = %s", opt.loss_scale.numpy(), opt.inner_optimizer.learning_rate(current_step).numpy(), opt.last_step_finite.numpy())
