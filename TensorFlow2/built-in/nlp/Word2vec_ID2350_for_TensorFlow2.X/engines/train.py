...
def train(data_manager, logger):
    ...
    static = classifier_config['static']
    ...
    for i in range(epoch):
        ...
        for step, batch in train_dataset.shuffle(len(train_dataset)).batch(batch_size, drop_remainder=False).enumerate():
            ...
            if step % print_per_batch == 0 and step != 0:
                ...
                logger.info('training batch: %5d, trainloss: %.5f, %s, Perf: %.5f' % (step, loss, res_str, cost_time))
        ...
        for val_batch in tqdm(val_dataset.batch(batch_size, drop_remainder=static)):
            if embedding_method == 'Bert':
