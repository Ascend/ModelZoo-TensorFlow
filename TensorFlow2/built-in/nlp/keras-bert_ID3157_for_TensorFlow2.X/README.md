...
**描述（Description）：训练一种非官方实现的BEART网络进行图像分类**

<h2 id="概述.md">概述</h2>

- BEART非官方实现，加载预训练模型，进行图像分类训练。
...
- 适配昇腾 AI 处理器的实现：
    ...
    [https://gitee.com/ascend/ModelZoo-TensorFlow/tree/master/TensorFlow2/built-in/nlp/keras-bert_ID3157_for_TensorFlow2.X](https://gitee.com/ascend/ModelZoo-TensorFlow/tree/master/TensorFlow2/built-in/nlp/keras-bert_ID3157_for_TensorFlow2.X)
...
- 开始训练。
    ...
    2. 单卡训练
        ...
        2.1 设置单卡训练参数（脚本train_full_1p.sh），示例如下。
        ...
        数据集应有如下结构（数据切分可能不同）
        data
        ├── train_indices.npy        # 已保存的训练数据
        ├── train_sentiments.npy     # 已保存的训练数据
        ├── adImdb                   # 原始训练数据
        |    |—— test
        |    |—— train
        |    ```
