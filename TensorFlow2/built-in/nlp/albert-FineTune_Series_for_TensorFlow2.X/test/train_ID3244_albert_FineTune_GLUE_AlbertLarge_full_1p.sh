...
#基础参数，需要模型审视修改
#网络名称，同目录名称
Network="albert_FineTune_GLUE_AlbertLarge_ID3244_for_TensorFlow2.X"
batch_size=4
TASK=MNLI
...
if [[ $1 == --help || $1 == --h ]];then
   echo "usage:./train_full_1p.sh --data_path=$data_path "
   exit 1
fi
...
cd $cur_path/..
start=$(date +%s)
python3 ./official/nlp/albert/run_classifier.py \
  --mode='train_and_eval' \
  ...
  --bert_config_file=$data_path/albert_large/albert_config.json \
  --init_checkpoint=$data_path/albert_large/bert_model.ckpt \
