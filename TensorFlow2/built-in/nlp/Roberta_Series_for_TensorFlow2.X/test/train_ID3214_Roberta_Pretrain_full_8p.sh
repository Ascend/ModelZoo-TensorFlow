...
#基础参数，需要模型审视修改
#网络名称，同目录名称
Network="Roberta_Pretrain_ID3214_for_TensorFlow2.X"
batch_size=128
...
do
    ...
    nohup ${bind_core} python3 -u run_pretraining.py \
    ...
    --distributed=True > ${cur_path}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log 2>&1 &
done 
wait
...
#输出训练精度,需要模型审视修改
train_accuracy=`grep masked_lm_accuracy ${cur_path}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log | grep run_pretraining | awk -F'masked_lm_accuracy = ' '{print $2}'`
#打印，不需要修改
