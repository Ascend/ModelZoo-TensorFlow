...
if [[ $1 == --help || $1 == --h ]];then
   echo "usage:./train_full_1p.sh $data_path"
   exit 1
fi
...
#输出训练精度,需要模型审视修改
train_accuracy=`grep 232/232 $cur_path/test/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log |awk 'END{print $NF}'`
#打印，不需要修改
...
grep 232/232 $cur_path/test/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log | awk '{print $(NF-9)}' > $cur_path/test/output/$ASCEND_DEVICE_ID/train_${CaseName}_loss.txt
#最后一个迭代loss值，不需要修改
