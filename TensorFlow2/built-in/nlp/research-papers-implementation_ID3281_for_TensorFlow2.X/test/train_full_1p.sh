...
batch_size=64
#网络名称，同目录名称
Network="research-papers-implementation_ID3281_for_TensorFlow2.X"
#Device数量，单卡默认为1
RANK_SIZE=1
#训练epoch，可选
train_epochs=30
...
#学习率
#learning_rate=1e-4
log_steps=50 # 0.5 * steps_per_epoch
#参数配置
data_path=""
...
#输出训练精度,需要模型审视修改
train_accuracy=$(grep "106/106" $cur_path/test/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log | awk 'END {print $15}')
#打印，不需要修改
...
grep "106/106" $cur_path/test/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log | awk '{print $6}' >$cur_path/test/output/$ASCEND_DEVICE_ID/train_${CaseName}_loss.txt
