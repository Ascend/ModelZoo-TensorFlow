...
class Attention(tf.keras.layers.Layer):
 ...
 def call(self, inputs):
    ...
    # add by zz
    seq_length = inputs[3] if len(inputs) == 4 else None
    ...
    # add by zz
    query_tensor = tf.reshape(query_tensor, [-1, seq_length, self._num_heads, self._head_size])
    # `query_tensor` = [B, N, F, H]
    query_tensor = tf.transpose(query_tensor, [0, 2, 1, 3])

    key_tensor = tf.reshape(key_tensor, [-1, seq_length, self._num_heads, self._head_size])
    # `key_tensor` = [B, N, T, H]
    ...
    # add by zz
    value_tensor = tf.reshape(value_tensor, [-1, seq_length, self._num_heads, self._head_size])
    value_tensor = tf.transpose(value_tensor, [0, 2, 1, 3])
    context_layer = tf.matmul(attention_probs, value_tensor)
    context_layer = tf.transpose(context_layer, [0, 2, 1, 3])
    context_layer = tf.reshape(context_layer, [-1, self._num_heads * self._head_size])
