...
class Transformer(tf.keras.layers.Layer):
  ...
  def build(self, input_shape):
    ...
    self._attention_dropout = bert_dropout.Dropout(rate=self._dropout_rate)
    ...
    self._output_dropout = bert_dropout.Dropout(rate=self._dropout_rate)
    # Use float32 in layernorm for numeric stability.
  ...
  def call(self, inputs):
    ...
    if attention_mask is not None:
      ...
      attention_inputs.append(seq_length)
    ...
    with tf.name_scope(self.name):
      ...
      # add by zz
      layer_output = tf.reshape(layer_output, [-1, seq_length, hidden_size])

      return layer_output


@tf.keras.utils.register_keras_serializable(package="Text")
class TransformerNPU(tf.keras.layers.Layer):
  """Transformer layer.
  ...
  def __init__(self,
               ...
               bias_constraint=None,
               **kwargs):
    super(TransformerNPU, self).__init__(**kwargs)
  ...
  def build(self, input_shape):
    ...
    self._attention_layer_norm = (
        # tf.keras.layers.LayerNormalization(
        bert_layernorm.LayerNormalization(
            name="self_attention_layer_norm",
            axis=-1,
            epsilon=util.LAYER_NORM_EPSILON,
            dtype=tf.float32))
    ...
    # Use float32 in layernorm for numeric stability.
    # self._output_layer_norm = tf.keras.layers.LayerNormalization(
    self._output_layer_norm = bert_layernorm.LayerNormalization(
        name="output_layer_norm",
        axis=-1,
        epsilon=util.LAYER_NORM_EPSILON,
        dtype=tf.float32)

    super(TransformerNPU, self).build(input_shape)
  ...
  def get_config(self):
    ...
    base_config = super(TransformerNPU, self).get_config()
    return dict(list(base_config.items()) + list(config.items()))
  ...
  def call(self, inputs):
    ...
    if attention_mask is not None:
      attention_inputs.append(attention_mask)
      attention_mask_shape = tf_utils.get_shape_list(attention_mask, name='attention_mask_tensor')
      seq_length = attention_mask_shape[-1]
      attention_inputs.append(seq_length)
