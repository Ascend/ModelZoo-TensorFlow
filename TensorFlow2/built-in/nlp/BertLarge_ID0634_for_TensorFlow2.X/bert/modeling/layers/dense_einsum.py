...
class DenseEinsum(tf.keras.layers.Layer):
  ...
  def build(self, input_shape):
    ...
    if not self._output_layer:
        ...
        self._kernel_shape = [self._num_heads, self._head_size, self._output_shape[0]]
  ...
  def call(self, inputs):
    ...
    if self._output_layer:
        kernel_dim1, kernel_dim2, kernel_dim3 = self._kernel.shape
        # c,d,e->c,d*e
        reshape_kernel = tf.reshape(self._kernel, [kernel_dim1 * kernel_dim2, kernel_dim3])
        ret = tf.matmul(inputs, reshape_kernel)
    elif self._einsum_string == "ab,bcd->acd":
        kernel_dim1, kernel_dim2, kernel_dim3 = self._kernel.shape
        # b,c,d->b,c*d
        reshape_kernel = tf.reshape(self._kernel, [kernel_dim1, kernel_dim2 * kernel_dim3])
        ret = tf.matmul(inputs, reshape_kernel)
    elif self._einsum_string == "ab,bc->ac":
        ret = tf.matmul(inputs, self._kernel)
    else:
        raise RuntimeError("please check einsum equations")
    if self._use_bias:
      # add by zz
      reshape_bias = tf.reshape(self._bias, [-1, ])
      ret += reshape_bias
