...
@tf.keras.utils.register_keras_serializable(package='Text')
class MaskedLMBucket(tf.keras.Model):
  """Masked language model network head for BERT modeling.
  ...
  def __init__(self,
    ...
    lm_data = (
        tf.keras.layers.Dense(
            ...
            dtype=tf.float32)(masked_lm_input))
    # lm_data = tf.keras.layers.LayerNormalization(
    lm_data = bert_layernorm.LayerNormalization(
        axis=-1,
        epsilon=util.LAYER_NORM_EPSILON,
        name='cls/predictions/transform/LayerNorm',
        dtype=tf.float32)(
    ...
    super(MaskedLMBucket, self).__init__(
        inputs=[sequence_data, masked_lm_positions],
        outputs=output_tensors,
        **kwargs)

  def get_config(self):
    raise NotImplementedError('MaskedLMBucket cannot be directly serialized at this '
                              'time. Please use it only in Layers or '
                              'functionally subclassed Models/Networks.')
