...
def run_customized_training_loop(
  ...
  with distribution_utils.get_strategy_scope(strategy):
    ...
    batch_size_metric = tf.keras.metrics.Mean(
        'train_batch_size', dtype=tf.float32)
    if eval_input_fn:
    ...
    #add npu distribute code
    npu.distribute.broadcast(training_vars,root_rank=0)

    #@tf.function(experimental_compile=True)
    def _compiled_local_step(inputs, labels, training_vars, accum_vars):
      """Replicated training step."""
      with tf.GradientTape() as tape:
        model_outputs, bs, metric_outputs = model(inputs, training=True)
      ...
      if accum_vars is None:
        return grads, loss, model_outputs, bs, metric_outputs
      else:
        new_accum_vars = []
        ...
        return new_accum_vars, loss, model_outputs, bs, metric_outputs
      ...
      if explicit_allreduce:
        ...
        if num_accumulation_steps > 1:
          ...
          for local_step in range(num_accumulation_steps):
            accum_vars, loss, model_outputs, bs, metric_outputs = _compiled_local_step(
          ...
          grads, loss, model_outputs, bs, metric_outputs = _compiled_local_step(
              inputs, labels, training_vars, None)
          optimizer.apply_gradients(zip(grads, training_vars))
      # For reporting, the metric takes the mean of losses.
      train_loss_metric.update_state(loss)
      batch_size_metric.update_state(bs)
    ...
    def test_step(iterator):
      ...
      def _test_step_fn(inputs):
        ...
        inputs, labels = inputs
        model_outputs, bs, metric_outputs = model(inputs, training=False)
    ...
    if eval_input_fn and steps_before_eval_start == 0:
    ...
    while current_step < total_training_steps:
      ...
      batch_size_metric.reset_states()
      ...
      avg_bs_status="Train avg_bs is %s " % (train_batch_size)
      logging.info(avg_bs_status)
