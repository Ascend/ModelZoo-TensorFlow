...
def get_transformer_encoder_bucket(bert_config, sequence_length):
  """Gets a 'TransformerEncoder' object.
  ...
  kwargs = dict(
      ...
      initializer=tf.keras.initializers.TruncatedNormal(
          stddev=bert_config.initializer_range))
  assert isinstance(bert_config, configs.BertConfig)
  return networks.TransformerEncoderBucket(**kwargs)
...
def pretrain_model_bucket(bert_config,
                   seq_length,
                   max_predictions_per_seq,
                   initializer=None):
  """Returns model to be used for pre-training.
  ...
  """
  input_word_ids = tf.keras.layers.Input(
      shape=(None,), name='input_word_ids', dtype=tf.int32)
  input_mask = tf.keras.layers.Input(
      shape=(None,), name='input_mask', dtype=tf.int32)
  input_type_ids = tf.keras.layers.Input(
      shape=(None,), name='input_type_ids', dtype=tf.int32)
  masked_lm_positions = tf.keras.layers.Input(
      shape=(max_predictions_per_seq,),
      name='masked_lm_positions',
      dtype=tf.int32)
  ...
  transformer_encoder = get_transformer_encoder_bucket(bert_config, seq_length)
  if initializer is None:
    initializer = tf.keras.initializers.TruncatedNormal(
        stddev=bert_config.initializer_range)
  pretrainer_model = bert_pretrainer.BertPretrainerBucket(
      network=transformer_encoder,
      num_classes=2,  # The next sentence prediction label has two classes.
      num_token_predictions=max_predictions_per_seq,
      activation=tf_utils.get_activation(bert_config.hidden_act),
