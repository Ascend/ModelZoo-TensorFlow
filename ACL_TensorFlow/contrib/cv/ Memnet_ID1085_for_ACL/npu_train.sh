...
######训练执行######
###此处每个网络执行命令不同，需要修改
python3.7 ${code_dir}/main.py --output_dir=${output_path} --phase=train --training_set=${dataset_path}/VOC0712.tfrecords --batch_size=1 --training_steps=100000 --summary_steps=50 --checkpoint_steps=1000 --save_steps=500
if [ $? -eq 0 ];
then
