...
aclError InitInputDatasetbuf()
{
    size_t inputTensorDescNum = aclmdlGetNumInputs(modelDesc);
    if(inputTensorDescNum==0){
          LOG("InitTwoInput failed, inputTensorDescNum=%u,not expect[0]",inputTensorDescNum);
    }
	
    LOG("init InputDataset&buf start\n");
    ret = aclrtSetCurrentContext(context);
    if (ret != ACL_ERROR_NONE)
    {
        LOG("Set context failed, ret[%d]\n", ret);
	...
	model_input = aclmdlCreateDataset();
    if(model_input == NULL) {
        LOG("aclmdlCreateDataset Create failed");        
		return ret;
    }	
	...
	for(size_t i = 0; i < inputTensorDescNum; ++i) {
		...
		ret = aclrtMalloc(&inputBuffer, buffer_size, ACL_MEM_MALLOC_HUGE_FIRST);

		if (ret != ACL_ERROR_NONE) {
			LOG("aclrtMalloc failed, ret[%d] \n", ret);
		}
		LOG("aclrtMalloc,inputBuffer:%p,size:%lu \n", inputBuffer, buffer_size);
		...
		ret = aclmdlAddDatasetBuffer(model_input, inputData);
		if(ret != ACL_ERROR_NONE) {
			LOG("ACL_ModelInputDataAdd failed, ret[%d] \n", ret);
	...
	LOG("aclmdlAddDatasetBuffer:%p \n", model_input);	
	return ACL_ERROR_NONE;
}


aclError InitTwoInput(std::vector<std::string> files1,std::vector<std::string> files2)
{
    size_t inputTensorDescNum = aclmdlGetNumInputs(modelDesc);
    if(inputTensorDescNum==0){
          LOG("InitTwoInput failed, inputTensorDescNum=%u,not expect[0]",inputTensorDescNum);
	...
	inputDataframe.fileNames.clear();	
    for(size_t index = 0; index < inputTensorDescNum; ++index) {
		
	   	size_t modelInputSize = cfg.inputInfo[index].size;
	    size_t imgSize = modelInputSize / cfg.batchSize;
	
		void* dabuf =NULL;
		aclDataBuffer* dataBuffer = aclmdlGetDatasetBuffer(model_input, index);
		dabuf = aclGetDataBufferAddr(dataBuffer);
		size_t buffer_size =(size_t) aclGetDataBufferSize(dataBuffer);	
		buffer_size = buffer_size/cfg.batchSize;
	    ...
	    {	
		    LOG("dst = %p, cfg size = %ld, buf size = %ld\n", dabuf, modelInputSize,buffer_size);
			LOG("file size %d,%d\n", files1.size(),files2.size());
			std::string fileLocation1 = cfg.dataDir + "/" + files1[i];
			FILE *pFile1 = fopen(fileLocation1.c_str(), "r");
			LOG("==---open file1 %s \n", fileLocation1.c_str());
			if (pFile1 == nullptr)
			{
				ret = ACL_ERROR_OTHERS;
			...
			size_t fileSize1 = ftell(pFile1);			

			std::string fileLocation2 = cfg.dataDir + "/" + files2[i];
			FILE *pFile2 = fopen(fileLocation2.c_str(), "r");
			LOG("==-==open file2 %s \n", fileLocation2.c_str());
			if (pFile2 == nullptr)
			{
				ret = ACL_ERROR_OTHERS;
				LOG("open file %s failed, pFile2 is :%p \n", fileLocation2.c_str(),pFile2);
				return ret;
			}
			fseek(pFile2, 0, SEEK_END);
			size_t fileSize2 = ftell(pFile2);	
	

			if (fileSize1 == buffer_size)
	        {	
				LOG("filesize1 == dataBuffer  %d,%d \n", fileSize1,buffer_size);        

		        void *buff = nullptr;
		        ret = aclrtMallocHost(&buff, fileSize1);
		        if (ret != ACL_ERROR_NONE)
		        {
		            LOG("Malloc host buff failed[%d]\n", ret);
		            return ret;
		        ...
		        rewind(pFile1);
		        fread(buff, sizeof(char), fileSize1, pFile1);
				LOG("==---fread %s \n", fileLocation1.c_str());
		        fclose(pFile1);
				fclose(pFile2);
				

		        ret = aclrtMemcpy((uint8_t *)dabuf + pos1, fileSize1, buff, fileSize1, ACL_MEMCPY_HOST_TO_DEVICE);
		        pos1 += fileSize1;
		        LOG("input addr %p, len %ld\n", dabuf, fileSize1);
		        if (ret != ACL_ERROR_NONE)
		        {
		        ...
		        aclrtFreeHost(buff);
			...
			else if(fileSize2 == buffer_size){
					LOG("filesize2 == buffer_size  %d,%d \n", fileSize2,buffer_size);
				    void *buff = nullptr;
			        ret = aclrtMallocHost(&buff, fileSize2);
			        if (ret != ACL_ERROR_NONE)
			        {
			            LOG("Malloc host buff failed[%d]\n", ret);
			            return ret;
			        ...
			        rewind(pFile2);
			        fread(buff, sizeof(char), fileSize2, pFile2);
					LOG("==---fread %s \n", fileLocation2.c_str());
			        fclose(pFile2);
					fclose(pFile1);

			        ret = aclrtMemcpy((uint8_t *)dabuf + pos2, fileSize2, buff, fileSize2, ACL_MEMCPY_HOST_TO_DEVICE);
			        pos2 += fileSize2;
			        LOG("input addr %p, len %ld\n", dabuf, fileSize2);
				...
				ret = ACL_ERROR_OTHERS;
	            LOG("%s fileSize %lu * batch %lu don't match with model inputSize %lu, %lu\n", fileLocation1.c_str(), fileSize1, cfg.batchSize, modelInputSize,buffer_size);
				LOG("%s fileSize %lu * batch %lu don't match with model inputSize %lu, %lu\n", fileLocation2.c_str(), fileSize2, cfg.batchSize, modelInputSize,buffer_size);
	            return ret;
				}
		...
		LOG("inputDataframe.fileNames size %d\n", inputDataframe.fileNames.size());
	...
	for (int i = 0; i < files1.size(); i++)
	{
		...
		inputDataframe.fileNames.push_back(files1[i]);
	    cout << inputDataframe.fileNames[i] << endl;
	}	

	inputDataframe.dataset = model_input;
...
aclError Inference()
{
    ...
    for (size_t i = 0; i < cfg.outputNum; ++i)
    {
		...
		LOG("~~~~~outputDataframe.dataset   output,outputData:%p,~outputData:%p \n", output, outputData);
	...
	LOG("~~~~~outputDataframe.dataset  %p\n", outputDataframe.dataset);
