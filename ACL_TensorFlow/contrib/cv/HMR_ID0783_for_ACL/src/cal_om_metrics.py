...
def main(args):
    ...
    preds = sorted(os.listdir(args.pred_dir))
    raw_errors, raw_errors_pa = [], []
    for i in range(len(preds)):
        gt = gts[i:i+1, :, :]
        pred = np.fromfile(os.path.join(
            args.pred_dir, preds[i]
        ), dtype=np.float32)
        pred = pred.reshape(1, -1 ,3)
        pred = pred[:, :14, :]
        # Convert to mm!
        errors, errors_pa = compute_errors(gt * 1000., pred * 1000.)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pred_dir', type=str, default='./20210912_150914', help='where to load pred data')
    parser.add_argument('--gt_path', type=str, default='./test_label.npy', help='where to load gt data')
