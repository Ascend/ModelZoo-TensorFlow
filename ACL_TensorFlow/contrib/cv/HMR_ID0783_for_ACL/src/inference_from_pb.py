...
def test(config, sess):
    ...
    for n, record in enumerate(all_tfrecords):
        ...
        results = run_model(images, config, sess, input, output, n)
        # Evaluate!
...
def run_model(images, config, sess, input, output, n):
    """
    ...
    for b in tqdm(range(num_total_batches), desc='Record %d' % n):
