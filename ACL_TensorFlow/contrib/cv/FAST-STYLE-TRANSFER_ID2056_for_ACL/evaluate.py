...
def ffwd_video(path_in, path_out, checkpoint_dir, batch_size=4):
...
# get img_shape
def ffwd(data_in, paths_out, checkpoint_dir, batch_size=4):
    ...
    if len(remaining_in) > 0:
        ffwd(remaining_in, remaining_out, checkpoint_dir, batch_size=1)

def ffwd_to_img(in_path, out_path, checkpoint_dir):
    paths_in, paths_out = [in_path], [out_path]
    ffwd(paths_in, paths_out, checkpoint_dir, batch_size=1)

def ffwd_different_dimensions(in_path, out_path, checkpoint_dir, batch_size=4):
...
def main():
    ...
    if not os.path.isdir(opts.in_path):
        ...
        if opts.allow_different_dimensions:
            ffwd_different_dimensions(full_in, full_out, opts.checkpoint_dir, batch_size=opts.batch_size)
        else :
            ffwd(full_in, full_out, opts.checkpoint_dir,
