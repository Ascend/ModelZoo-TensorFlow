...
def main(argv):
    ...
    for file in files_predict:
        if file.endswith(".txt"):
            num += 1
            tmp = np.loadtxt(PREDICT_FLODER+'/'+file, dtype='int32')
            tmp = tmp if tmp.ndim is not 0 else [tmp]

            if(tmp[0] == int(label_map[file.split("_")[0]][0])):
              check_num += 1
...
if __name__ == '__main__':
    ...
    flags.DEFINE_string('PREDICT_FLODER',"./inference_out/20220414_152403/",'The floder of inference result')
    app.run(main)