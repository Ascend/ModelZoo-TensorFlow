...
flags.DEFINE_integer(
    ...
    ('If only sequences from the class=filter_label are used for training.'
     'if -1, no filter.'))
flags.DEFINE_string('in_tr_data_dir', '/home/test_user04/dataset/before_2011_in_tr',
                    'data directory of in-distribution training')
flags.DEFINE_string('in_val_data_dir', '/home/test_user04/dataset/between_2011-2016_in_val',
                    'data directory of in-distribution validation')
flags.DEFINE_string('ood_val_data_dir', '/home/test_user04/dataset/between_2011-2016_ood_val',
...
class SeqModel(object):
  ...
  def __init__(self, params,x,y0):
    ...
    self.x = x#tf.placeholder(tf.int32,shape=[None,250])
    self.y0 =y0#tf.placeholder(tf.int32,shape=[None])
    #self._make_dataset()
  ...
  def reset(self):
    ...
    #custom_op.parameter_map["mix_compile_mode"].b = False  #关闭混合计算
    custom_op.parameter_map["precision_mode"].s = tf.compat.as_bytes("allow_mix_precision") #开启混合精度
    #custom_op.parameter_map["auto_tune_mode"].s = tf.compat.as_bytes("RL,GA") #开启Auto tune 自动调优
  ...
  def print_log(self, str, print_time=True):
      ...
      with open("work_dir/oodback.log", 'a') as f:
          print(str, file=f)
  ...
  def pred_from_ckpt(self, x_in,y_in,x_out,y_out, num_samples):
    ...
    #self.in_val_handle = self.sess.run(in_val_iterator.string_handle())
    #self.ood_val_handle = self.sess.run(ood_val_iterator.string_handle())
    x_in = x_in.eval(session=self.sess)#np.array(x_in)
    y_in = y_in.eval(session=self.sess)#np.array(y_in)
    x_out = x_out.eval(session=self.sess)#np.array(x_out)
    y_out = y_out.eval(session=self.sess)#np.array(y_out)
    for _ in range(num_samples // self._params.batch_size):
        ...
        auc = roc_auc_score([0] * neg.shape[0] + [1] * pos.shape[0],
                            np.concatenate((neg, pos), axis=0))
        self.print_log(' in_val_loss={:.4f}, ood_val_loss={:.4f}\n'
                       ' in_val_acc={:.4f}, ood_val_acc={:.4f}\n'
                       'auc={:.4f}'.format( in_val_loss, ood_val_loss, in_val_acc, ood_val_acc, auc))
