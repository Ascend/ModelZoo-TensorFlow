...
class Gan(object):
    ...
    def sample(self, opts, num=100):
        ...
        with self._session.as_default(), self._session.graph.as_default():
            # print("=============self._sample_internal(opts, num)==========")
            # print(self._sample_internal(opts, num))
            # print(self._sample_internal(opts, num).shape)
    ...
    def _run_batch(self, opts,  placeholder, feed,
        ...
        """
        f = gfile.FastGFile("models/frozen_model.pb", "rb")
        ...
        with open("models/frozen_model.pb", "rb") as f:
        ...
        with tf.Session() as sess:
            ...
            assert len(data.shape) > 0, 'Empry feed.'
                ...
                if idx == batches_num - 1:
                    if feed2 is None:
                        res = self._session.run(
                            output_tensor_name,
                            feed_dict={placeholder: data[idx * batch_size:]})
                    else:
                        res = self._session.run(
                            output_tensor_name,
                            feed_dict={placeholder: data[idx * batch_size:],
...
class ToyGan(Gan):
    ...
    def _train_internal(self, opts):
        ...
        for _epoch in range(opts["gan_epoch_num"]):
    ...
    def _sample_internal(self, opts, num):
        ...
        # print(noise)
        # print(noise.shape)
        # noise = np.load(r"./noise.npy")
        # np.save("noise.npy", noise)

        sample = self._run_batch(opts, self._noise_ph, noise)
    ...
    def _train_mixture_discriminator_internal(self, opts, fake_images):
        ...
        for epoch in range(opts["mixture_c_epoch_num"]):
            for idx in range(int(batches_num)):
...
class ToyUnrolledGan(ToyGan):
    ...
    def generator(self, opts, noise, reuse=False):
        ...
        with tf.variable_scope("GENERATOR", reuse=reuse):
            ...
            # print(h2)
            h2 = tf.reshape(h2, [-1] + list(output_shape))
            temp = tf.zeros_like(h2,dtype=tf.float32)
            # print(tf.shape(h2))
            h2 = tf.add(h2,temp,name='output')
            # h2 = tf.nn.tanh(h2,name='output')
            print("===========h2==============")
            print(h2)
    ...
    def _train_internal(self, opts):
        ...
        for _epoch in range(opts["gan_epoch_num"]):
            # print("============我在训练==========")
            for _idx in TQDM(opts, range(int(batches_num)),
                             desc='Epoch %2d/%2d' %\
                ...
                batch_noise = utils.generate_noise(opts, opts['batch_size'])
                # batch_images = np.load(r"./real.npy")
                # batch_noise = np.load(r"./noise.npy")


                # np.save("real.npy", batch_images)
                # np.save("noise.npy", batch_noise)
                ...
                # Unrolling steps
                for _iter in range(opts['unrolling_steps']):
                    self._session.run(
                        self._d_optim_cp,
                        feed_dict={self._real_points_ph: batch_images,
                                   self._noise_ph: batch_noise})
                ...
                if opts['verbose'] and counter % opts['plot_every'] == 0:
                    ...
                    points_to_plot = self._run_batch(
                        opts, self._G, self._noise_ph,
                        self._noise_for_plots[0:320])
                    data_ids = np.random.choice(train_size, 64,
                                                replace=False,
                                                p=self._data_weights)
                    metrics.make_plots(
                        opts, counter,
                        self._data.data[data_ids].astype(np.float),
                        points_to_plot,
                        prefix='sample_e%04d_mb%05d_' % (_epoch, _idx))
                    # print("++++++++++real++++++++++")
                    # print(self._data.data[data_ids])
                    # print(self._data.data[data_ids].shape)
                    # np.save("real1.npy", self._data.data[data_ids])

        print("===================")
        saver.save(self._session,"./models/gan.ckpt")
        print("=====保存了=========")
...
class ImageUnrolledGan(ImageGan):
    ...
    def _train_internal(self, opts):
        ...
        logging.debug('Training GAN')
        for _epoch in range(opts["gan_epoch_num"]):
            for _idx in TQDM(opts, range(batches_num),
                             desc='Epoch %2d/%2d' %\
                             (_epoch + 1, opts["gan_epoch_num"])):
                # logging.debug('Step %d of %d' % (_idx, batches_num ) )
                data_ids = np.random.choice(train_size, opts['batch_size'],
