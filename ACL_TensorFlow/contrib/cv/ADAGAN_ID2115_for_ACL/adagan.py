...
class AdaGan(object):
    ...
    def make_step(self, opts, data):
        ...
        with self._gan_class(opts, data, self._data_weights) as gan:
            ...
            # sample = np.load(r"./out.npy")
            sample = np.loadtxt("frozen_model_output_0.txt")
            sample.shape = 50,2,1,1
            ...
            if opts["inverse_metric"]:
                ...
                images_hat, z, err_per_point, norms = gan.invert_points(
                    opts, data.data[ids])
                plot_pics = []
                for _id in range(min(16 * 8, len(ids))):
    ...
    def sample_mixture(self, num=100):
        ...
        points_per_component = [component_ids.count(i)
                                for i in range(self.steps_made)]

        # print("======points_per_component======")
        # print(points_per_component)
        # print("======p======")
        ...
        sample = []
        for comp_id  in range(self.steps_made):
