...
#        print('load_X_Y_train_next_batch')
    ...
    def load_X_Y_test_next_batch_sq(self, test_mv):
        #print('the number of test_files',len(self.X_rgb_test_files))
        X_rgb_files = self.X_rgb_test_files[self.batch_size*self.test_batch_index_sq*test_mv:self.batch_size*(self.test_batch_index_sq+1)*test_mv]
        Y_vox_files = self.Y_vox_test_files[self.batch_size*self.test_batch_index_sq*test_mv:self.batch_size*(self.test_batch_index_sq+1)*test_mv]
        self.test_batch_index_sq +=1

        X_rgb, Y_vox = self.load_X_Y_rgb_vox(X_rgb_files, Y_vox_files, train=True)
        X = []
        Y = []
        for b in range(self.batch_size):
            X.append(X_rgb[b * test_mv:(b + 1) * test_mv, :])
            Y.append(Y_vox[b * test_mv, :, :, :])
        X = np.asarray(X)
...
#        print('shuffle_test_files')
        ...
        self.total_test_batch_num = int(len(self.X_rgb_test_files)/(self.batch_size*test_mv))
