...
tf.app.flags.DEFINE_boolean("freeze_pb", False, "Set to True for freeze pb")
    ...
    with tf.compat.v1.Session(
        ...
        for key2d in test_set_2d.keys():
            ...
            for bidx in range(len(enc_in)):
                ...
                # if bidx == 0:
                #     poses3d.tofile(os.path.join(train_dir, "raw_output1.bin"))
                ...
                poses3d = data_utils.unNormalizeData(poses3d, data_mean_3d, data_std_3d, dim_to_ignore_3d)
                # if bidx == 0:
                #     poses3d.tofile(os.path.join(train_dir, "raw_output2.bin"))
...
def freeze_pb():
    ...
    with tf.compat.v1.Session(
        ...
        dp = 1.0
        _, _, poses3d = model.step(sess, encoder_inputs[0], decoder_outputs[0], dp, isTraining=False)
        poses3d.tofile(os.path.join(train_dir, "raw_output.bin"))
