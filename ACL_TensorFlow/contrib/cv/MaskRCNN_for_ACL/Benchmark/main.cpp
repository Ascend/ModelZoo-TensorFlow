...
void getCommandLineParam(int argc, char** argv, Config& config)
{
    while (1)
    {
        ...
        int c;
        c = getopt_long(argc, argv, "a:b:c:d:e:f:g:h:i:j:k:l:m:", long_options, &option_index);
        if (c == -1)
        {
            break;
        }
        ...
        switch (c)
        {
            ...
            case 'j':
                ...
                break; 
            case 'k':
                config.innum = atoi(optarg);
                printf("[INFO]innum = %d\n", config.innum);
                break; 		
            case 'l':
                config.suffix1 = std::string(optarg);
                printf("[INFO]suffix1 = %s\n", config.suffix1.c_str());
                break; 
            case 'm':
                config.suffix2 = std::string(optarg);
                printf("[INFO]suffix2 = %s\n", config.suffix2.c_str());
                break; 				
            default:
                break;
        }
...
{	
    ...
    {
        LOG("loopCnt %d, loopNum %d ,inum %d, fileNum %d\n", loopCnt, cfg.loopNum, cfg.innum,fileNum);
        for(size_t i = 0; i< fileNum/(cfg.batchSize*cfg.innum); i++)
        {
            gettimeofday(&startTmp, NULL);
            std::vector<std::string> batchFileNames1;
			std::vector<std::string> batchFileNames2;			
            for (int j = 0; j < cfg.batchSize*cfg.innum; j++) {
				if(cfg.innum != 1){
					if(strstr(fileNames[i*cfg.batchSize*cfg.innum+j].c_str(), cfg.suffix1.c_str())!=NULL){
 						batchFileNames1.push_back(fileNames[i*cfg.batchSize*cfg.innum+j]);
					}
    	            else{
						batchFileNames2.push_back(fileNames[i*cfg.batchSize*cfg.innum+j]);
					}
				}
				else{
					batchFileNames1.push_back(fileNames[i*cfg.batchSize+j]);
					}                
            ...
            if(cfg.useDvpp){
                ret = DvppInitInput(batchFileNames1);
            }	
            else if(cfg.innum == 2){
                ret = InitTwoInput(batchFileNames1,batchFileNames2);
            }
            else{
                ret = InitInput(batchFileNames1);
            }			
            gettimeofday(&endTmp, NULL);
            dataProcTime += (endTmp.tv_sec-startTmp.tv_sec)*1000000+(endTmp.tv_usec-startTmp.tv_usec);
            CHECK_RET(ret);

			LOG("Inference time ----------- %d\n", i);			
            ret = Inference();
            CHECK_RET(ret);
        ...
        //last img
        if (0 != fileNum % (cfg.batchSize*cfg.innum))
        {
            std::vector<std::string> batchFileNames1;
			std::vector<std::string> batchFileNames2;

            for(size_t i = (fileNum - fileNum % (cfg.batchSize*cfg.innum)); i< fileNum; i++)
            {
                
				if(cfg.innum != 1){
					LOG("===fileNames %s\n", fileNames[i].c_str());
					LOG("filesuffix1 %s\n", cfg.suffix1.c_str(),i);
					LOG("filesuffix2 %s\n", cfg.suffix2.c_str(),fileNum);
					if(strstr(fileNames[i].c_str(), cfg.suffix1.c_str())!=NULL){
						batchFileNames1.push_back(fileNames[i]);
					}
					else{
						batchFileNames2.push_back(fileNames[i]);
					}
				}
				else{
					batchFileNames1.push_back(fileNames[i]);
