        ...
        for i in range(len(datanames)):
            try:
                if "images" in datanames[i]:
                    ...
                    output5 = np.fromfile(os.path.join("{}".format(path), "{}_{}_image_info_output4.bin".format('davinci', datanames[i][0])),dtype=np.float32)
                    sourceid = np.fromfile(os.path.join("{}".format(soureidpath), "{}_source_ids.bin".format(datanames[i][0])),dtype=np.float32)
                    imginfo = np.fromfile(os.path.join("{}".format(testdatapath), "{}_image_info.bin".format(datanames[i][0])),dtype=np.float32)
                    output2 = output2.reshape(1, 100, 4)
                    output3 = output3.reshape(1, 100)
                    output4 = output4.reshape(1, 100)
                    output5 = output5.reshape(1, 100,28,28)
