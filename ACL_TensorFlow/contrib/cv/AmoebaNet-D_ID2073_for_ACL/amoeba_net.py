...
flags.DEFINE_string(
    'model_dir', '/home/test_user03/ckpt_allow_3',
    'Directory where model output is stored')

flags.DEFINE_string(
    'export_dir', '/home/test_user03/result',
...
# flags.DEFINE_integer(
#     'num_train_images', 10000, 'The number of images in the training set.')
# flags.DEFINE_integer(
#     'num_eval_images', 1000, 'The number of images in the evaluation set.')
flags.DEFINE_integer(
...
#     'Known inference batch sizes used to warm up for each core.') 修改为如下
flags.DEFINE_multi_integer(
...
def build_image_serving_input_receiver_fn(shape,
  ...
  def _preprocess_image(encoded_image):
    ...
    hh = tf.cast(image, dtype, name="inputx")
    print("==========inputx_shape=================")
    print(hh.shape)
  ...
  def serving_input_receiver_fn():
    ...
    print("==========inputy_shape=================")
    print(image_bytes_list.shape)
    images = tf.map_fn(
        _preprocess_image, image_bytes_list, back_prop=False, dtype=dtype, name="inputz")
    print("==========inputz_shape=================")
    print(images.shape)
...
def serving_input_fn():
  ...
  input_ids = tf.placeholder(tf.float32, shape=[None,299,299,3], name='input_ids')
  input_fn = tf.estimator.export.TensorServingInputReceiver(
...
def main(_):
  ...
  if hparams.use_tpu:
    ...
    # dump_config = DumpConfig(enable_dump_debug=True, dump_path="/home/test_user03/tpu-master/models/official/amoeba_net/output/output",dump_step="0|5|10", dump_debug_mode="all")
    
    # dump_config = npu_tf_config.estimator_dump_config(action='dump')
    # # dump_config = DumpConfig(enable_dump_debug=True,
    # #                          dump_path="/home/test_user03/tpu-master/models/official/amoeba_net/output/output",
    # #                          dump_step="0", dump_debug_mode="all")
    ...
    run_config = tf.estimator.RunConfig(
        model_dir=FLAGS.model_dir,
        save_checkpoints_steps=save_checkpoints_steps, save_summary_steps=0)
    '''
    image_classifier = NPUEstimator(
        model_fn=model.model_fn,
        config=npu_config,
        params=estimator_parmas)
  ...
  if FLAGS.export_dir is not None:
    ...
    image_classifier.export_savedmodel(FLAGS.export_dir, serving_input_fn)
