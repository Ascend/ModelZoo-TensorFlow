...
class DCGAN(object):
    ...
    def train(self, config):
        ...
        for epoch in xrange(config.epoch):#add =2
            if config.dataset == 'mnist':
                ...
                print("------------------",batch_idxs)
            for idx in xrange(0, int(batch_idxs)): #add (20018)-20000
                ...
                if epoch  > 0: #去掉第一轮不稳定的数据
                    ...
                    print("[%8d Epoch:[%2d/%2d]  [%4d/%4d]  time: %4.4f fps: %4.4f  d_loss: %.8f  g_loss: %.8f" \
                        % (counter, epoch, config.epoch, idx, batch_idxs,
                            avg_perf, avg_fps,errD_fake + errD_real, errG))

                if np.mod(counter, config.sample_freq) == 0:#add =20
                    if config.dataset == 'mnist':
                        ...
                        mox.file.copy_parallel(src_url='./{}/train_{:08d}.png'.format(config.sample_dir, counter),
                                               dst_url="obs://bupt.dcgan/result/{}_sample_{}/train_{:08d}.png".format(
                ...
                if np.mod(counter, config.ckpt_freq) == 0:#add =20
