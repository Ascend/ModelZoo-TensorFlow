...
def load_test_data(image_path, fine_size=256, is_grayscale=True):
    ...
    img = img / 127.5 - 1 # Transfrom from int image ([0,255]) to float tensor ([-1.,1.])
    return img
...
def load_train_data(image_path, load_size=256, fine_size=256, is_testing=False, is_grayscale=True):
    ...
    img_A = img_A / 127.5 - 1.  #Transfrom from int image ([0,255]) to float tensor
    img_B = img_B / 127.5 - 1.
    if is_grayscale:
...
def save_images(images, size, image_path):
    ...
    # print(images.shape) # (1, 256, 256, 1)
...
def merge(images, size):
    h, w = images.shape[1], images.shape[2]  # (batch, 256, 256, 3)
    img = np.zeros((h * size[0], w * size[1], 3))
    for idx, image in enumerate(images):
        i = idx % size[1]
        j = idx // size[1]
        img[j * h:j * h + h, i * w:i * w + w, :] = image  # (256, 256, 3)
