...
class DualNet(object):
    ...
    def sample_shotcut(self, sample_dir, epoch_idx, batch_idx):
        ...
        fp=open("mylog.txt", 'a')
        fp.write("[Sample] A_loss: {:.8f}, B_loss: {:.8f}".format(Ag, Bg) + '\n')
        fp.close()
        ...
        for epoch_idx in range(args.epoch):
            ...
            for batch_idx in range(0, epoch_size):
                ...
                if step % self.log_freq == 0:
                    ...
                    fp = open("mylog.txt", 'a')
                    fp.write("Epoch: [%2d] [%4d/%4d]"%(epoch_idx, batch_idx, epoch_size) + '\n')
                    fp.close()
                step = step + 1
    ...
    def run_optim(self,batch_A_imgs, batch_B_imgs,  counter, start_time, batch_idx):
        ...
        if batch_idx % self.log_freq == 0:
            ...
            fp = open("mylog.txt", 'a')
            fp.write("time: %4.4f, Ad: %.2f, Ag: %.2f, Bd: %.2f, Bg: %.2f,  U_diff: %.5f, V_diff: %.5f" \
                    % (time.time() - start_time, Ad,Ag,Bd,Bg, Aloss, Bloss) + '\n')
