...
from test_model import ModelInpaint
...
class Solver(object):
    ...
    def test(self):
        ...
        if self.flags.is_test:
            self.saver.save(self.sess,"model/model")
            exit(0)
        for num_try in range(self.flags.num_try):
            ...
            for iter_time in range(self.flags.iters):
                loss, img_outs = self.model(imgs, iter_time)  # inference
            ...
            print(type(blend_results),end='')
            print(blend_results.shape)
