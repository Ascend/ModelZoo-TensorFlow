...
aclError LoadModel()
{
    ...
    CHECK_ACL_RET("Error:ACL load model from file failed! model file:"+modelPath, ret);
...
aclError InitInput(std::vector<std::string> files)
{
    ...
    for (int i = 0; i < files.size(); i++) {
        ...
        ptr += fileSize;
        LOG("input file: %s, memory addr: %p, file size: %ld\n",files[i].c_str(), dstTmp, fileSize);
        if (ret != ACL_ERROR_NONE) {
...
aclError Inference()
{
    ...
    LOG("inference time cost: %lld us\n", timeUse);
    inferTime += timeUse;
