...
flags.DEFINE_string(
    'resnet50_path', '', 'path to resnet50'#./resnet_v2_50_checkpoint/resnet_v2_50.ckpt
                                                                    'checkpoint')
...
FLAGS = flags.FLAGS
TRAIN_WORLDS = [
    'Home_001_1', 'Home_001_2', 'Home_002_1', 'Home_003_1', 'Home_003_2', 'Home_005_1', 'Home_005_2', 'Home_006_1',
...
def create_task_io_config(
    ...
    shapes = {
        ...
        task_env.ModalityTypes.SEMANTIC_SEGMENTATION: shape_prefix + [1,],
        task_env.ModalityTypes.OBJECT_DETECTION: shape_prefix + [90,]
    }
    types = {k: tf.float32 for k in shapes}
    types[task_env.ModalityTypes.IMAGE] = tf.uint8
    inputs = collections.OrderedDict([[mtype, (types[mtype], shapes[mtype])] for mtype in modality_types])
...
def create_train_and_init_ops(policy, task):
  ...
  """
    ...
    mtypes=[]
    for mtype in list(inputs):  # 提取出字典inputs中的键
        mtypes.append(mtype)
    with tf.Session() as sess:
        a = inputs[mtypes[0]].eval()  # taskdata:0的 tensor2numpy
        b = inputs[mtypes[1]].eval()  # taskdata:1的 tensor2numpy
        c = inputs[mtypes[2]].eval()  # taskdata:2的 tensor2numpy
        logging.info('10+++++++++++++\n+++++++++++++\n')
        logging.info(a.shape)
        logging.info(b.shape)
        logging.info(c.shape)
        a.tofile('taskdata_0.bin', )
        b.tofile('taskdata_1.bin')
        c.tofile('taskdata_2.bin')

    # logging.info(input_palceholder[0].shape)
...
def create_eval_ops(policy, config, possible_targets):
    ...
    inputs_feed = collections.OrderedDict([[
        mtype, tf.placeholder(config.inputs[mtype].type, [1] + config.inputs[mtype].shape)] for mtype in config.inputs])
    ...
    logging.info('8----------------\n---------------\n------------\n---------------')
    logging.info(inputs_feed)
    logging.info('9----------------\n---------------\n------------\n---------------')
    logging.info(prev_state_feed)
    logging.info('10----------------\n---------------\n------------\n---------------')
    logging.info(policy_outputs)
    summary_feed = {}
...
def init(sequence_length, eval_init_points_file_name, worlds):  # 第一个被调用 构建网络

    """Initializes the common operations between train and test."""
    modality_types = create_modality_types()  # 返回'image', 'sseg', 'det', 'depth'中的一种
    ...
    env = active_vision_dataset_env.ActiveVisionDatasetEnv(   # ActiveVisionDatasetEnv的属性定义
    ...
    with sv.managed_session(
        ...
        while not sv.should_stop():
            ...
            for test_iter in range(FLAGS.test_iters):
                ...
                print("123456")
                print(prev_state_feed)
                print("1234567")
                print(distance_to_goal)
                print('goal = {}'.format(env.goal_string))
                eval_stats[env.goal_string].append(float(distance_to_goal[-1] <= 7))
            eval_stats = {k: np.mean(v) for k, v in eval_stats.items()}
            eval_stats['mean'] = np.mean(list(eval_stats.values()))
            print(eval_stats)
