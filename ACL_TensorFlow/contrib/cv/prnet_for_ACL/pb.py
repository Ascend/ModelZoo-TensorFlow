...
def main():
    ...
    # 调用网络模型生成推理图
    # logits = {"output": net(inputs, is_training=False)}
    logits = net(inputs, is_training=False)

    # flow = tf.cast(logits, tf.float32, name="output")
    # 定义网络的输出节点
    ...
    with tf.Session() as sess:
        ...
        freeze_graph.freeze_graph(
            input_graph='./model/model.pb',  # 传入write_graph生成的模型文件
            input_saver='',
            input_binary=False,
            input_checkpoint=ckpt_path,  # 传入训练生成的checkpoint文件
            output_node_names='resfcn256/Conv2d_transpose_16/Sigmoid',  # 与定义的推理网络输出节点保持一致
            restore_op_name='save/restore_all',
            filename_tensor_name='save/Const:0',
            output_graph='./model/prnet.pb',  # 改为需要生成的推理网络的名称
            clear_devices=False,
            initializer_nodes='')
    print("done")
