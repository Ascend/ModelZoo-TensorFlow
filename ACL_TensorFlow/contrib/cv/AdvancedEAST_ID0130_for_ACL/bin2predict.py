...
parser.add_argument('--input', default=r'../image_test_output/image_test_output', type=str, help='input file path.')
parser.add_argument('--input_image', default=r'../image_test1', type=str, help='input image path.')
args = parser.parse_args()
img_path = args.input_image

files_bin = os.listdir(args.input)
files_bin.sort()

for file_bin in files_bin:
    y = np.fromfile('../image_test_output/image_test_output/'+file_bin, dtype=np.float32).reshape((184, 184, 7))
    print(y.shape)
    ...
    # print('quad_scores:',quad_scores,'quad_after_nms', quad_after_nms)

    img_path = args.input_image + '/'+ file_bin[:-13]
