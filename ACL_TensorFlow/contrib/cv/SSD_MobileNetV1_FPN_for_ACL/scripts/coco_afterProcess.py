...
def data_process(img_info_dict,json_out):
    ...
    for test_idx in range(test_len):
        try:
            ...
            out_boxes = np.fromfile(('{}/{}_output_0.bin').format(args.result_file_path, img_temp), dtype='float16').reshape(1,100,4)
            out_scores = np.fromfile(('{}/{}_output_1.bin').format(args.result_file_path, img_temp), dtype='float16').reshape(1,100)
            out_classes = np.fromfile(('{}/{}_output_3.bin').format(args.result_file_path, img_temp), dtype='float16').reshape(1,100)
            out_num_detections = np.fromfile(('{}/{}_output_2.bin').format(args.result_file_path, img_temp),dtype='float16').reshape(1)
