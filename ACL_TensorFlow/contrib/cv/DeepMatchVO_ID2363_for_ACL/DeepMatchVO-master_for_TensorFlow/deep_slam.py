...
class DeepSlam(object):
    ...
    def train(self, opt):
        ...
        custom_op.parameter_map["profiling_options"].s = tf.compat.as_bytes('{"output":"/home/DeepMatchVo_ID2363_for_TensorFlow/profilingoutput","task_trace":"on","training_trace":"on","aicpu":"on","fp_point":"","bp_point":""}')
        ...
        #关闭融合规则
        custom_op.parameter_map["fusion_switch_file"].s = tf.compat.as_bytes("/home/DeepMatchVo_ID2363_for_TensorFlow/DeepMatchVO-master_for_TensorFlow/fusion_switch.cfg")

        # 溢出检测
        #custom_op.parameter_map["dump_path"].s = tf.compat.as_bytes("/home/DeepMatchVo_ID2363_for_TensorFlow/overflow")  # npu 192.168.88.119
    ...
    def select_tensor_or_placeholder_input(self, input_uint8):
        if input_uint8 == None:
            print("select_tensor_or_placeholder_input_______________input_uint8 == None")
    ...
    def build_pose_test_graph(self, input_uint8):
        input_mc = self.select_tensor_or_placeholder_input(input_uint8)#input_mc:Tensor("sub:0", shape=(4, 128, 1248, 3), dtype=float32)
        loader = DataLoader()#<data_loader.DataLoader object at 0xffff8ba7b6d0>
        print("def build_pose_test_graph________________________________________")
        #print("input_mc-----------------------------------------------")
        #print(input_mc)
        #print("loader-----------------------------------------------")
        #print(loader)
        ...
        '''
        with open('/home/DeepMatchVo_ID2363_for_TensorFlow/bin/tgt/bin0', "wb") as f:
            f.write(tgt_image)
        with open('/home/DeepMatchVo_ID2363_for_TensorFlow/bin/src/bin0', "wb") as f:
            f.write(src_image_stack)
        '''
        ########
