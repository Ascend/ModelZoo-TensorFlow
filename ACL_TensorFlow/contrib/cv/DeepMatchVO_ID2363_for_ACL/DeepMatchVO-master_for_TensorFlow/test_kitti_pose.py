...
def main():
    ...
    with tf.Session(config=npu_config_proto()) as sess:
        ...
        print(len(tgt_inds))
        batch_sample = loader.load_test_batch(image_sequence_names)#迭代器<tensorflow.python.data.ops.iterator_ops.Iterator object at 0xffff28044bd0>
        #print("batch_sample-----------------------------------------------")
        #print(batch_sample)
        sess.run(batch_sample.initializer) #<tensorflow.python.data.ops.iterator_ops.Iterator object at 0xffff28044bd0>
        #print("batch_sample aftersessrun-------------------------")
        #print(batch_sample)
        input_batch = batch_sample.get_next()#Tensor("IteratorGetNext:0", shape=(4, ?, ?, ?), dtype=uint8)
        #print("input_batch-----------------------------------------------")
        #print(input_batch)
        input_batch.set_shape([FLAGS.batch_size, FLAGS.img_height, FLAGS.img_width * FLAGS.seq_length, 3])#Tensor("IteratorGetNext:0", shape=(4, 128, 1248, 3), dtype=uint8)
        #print("input_batch after shape-----------------------------------------------")
        #print(input_batch)
        # init system
        system = DeepSlam()
