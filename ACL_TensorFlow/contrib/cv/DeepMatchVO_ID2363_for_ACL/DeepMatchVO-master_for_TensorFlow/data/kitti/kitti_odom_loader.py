...
sys.path.append(os.path.abspath(os.path.join(CURDIR, '...')))
sys.path.append('/root/DeepMatchVO-master')###wty
##from geo_utils import scale_intrinsics ###original
from geo_utils import scale_intrinsics ###wty
...
class kitti_odom_loader(object):
    ...
    def collect_test_frames(self):
        ...
        for seq in self.test_seqs:
            ...
            img_dir = os.path.join(seq_dir, 'image_1')##############wty original:image_2
    ...
    def load_image(self, drive, frame_id):
        img_file = os.path.join(self.dataset_dir, 'sequences', '%s/image_1/%s.png' % (drive, frame_id))##############wty original:image_2
