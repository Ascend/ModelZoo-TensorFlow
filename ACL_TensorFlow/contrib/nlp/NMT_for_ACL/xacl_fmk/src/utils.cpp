...
int g_THOUSAND = 1000;
int g_MILLION = 1000000;
size_t g_MALLOC_ONE = 1;
...
void Help() {
    printf("%s - I - [XACL]: Usage: ./xacl_fmk [input parameters]\n"
           ...
           "                         Relative and absolute paths are supported\n"
           "\n"
           "-i=inputFiles            Optional, input bin files or input file directories, use commas (,) to separate
                                     multiple inputs\n"
           "                         Relative and absolute paths are supported, set inputs to all zeros if not
                                     specified\n"
           "-g=mergeInput            Optional, whether merge input by batch size, only take effect in directories
                                     input\n"
           "                         The default value is false, in this case, each input must be saved in N batches\n"
           "                         Otherwise, each input must be saved in 1 batch and will be merged to N batches
                                     automatically\n"
           "-d=dumpJson              Optional, Configuration file used to save operator input and output data\n"
           "                         The default value is NULL, indicating that operator input and output data is not
                                     saved\n"
...
string GetTime() {
    ...
    int milliSecond = timeEval.tv_usec / g_THOUSAND;
...
Result DestroyResource(uint32_t modelId, aclmdlDesc *modelDesc, vector<aclrtContext> &v_context,
...
Result CreateInputDesc(aclmdlDataset *input, vector<string> v_inputFiles, aclTensorDesc *inputDesc[],
       map<char, string> &params) {
...
Result CreateInput(aclmdlDesc *modelDesc, aclmdlDataset *&input, string &outputPrefix, vector<string> v_inputFiles,
       vector<vector<string>> saveOriginInput, map<char, string> &params) {
    ...
    if (v_inputFiles.empty()) {
        ...
        for (size_t i = 0; i < inputSize; i++) {
            ...
            if (!atoi(params['r'].c_str())) {
                ...
                if (ret != ACL_ERROR_NONE) {
                    printf("%s - E - [XACL]: Interface of aclrtMalloc return failed, size is %zu,
                           error message is:\n%s\n",
...
Result Execute(aclmdlDataset *input, aclmdlDataset *output, aclmdlDesc *modelDesc, uint32_t modelId,
       vector<aclrtContext> &v_context, float &totalSamplesTime, string outputPrefix,
       vector<vector<string>> saveOriginInput, uint32_t fileIdx, map<char, string> &params) {
    // 拆分 imageRank
    ...
    for (uint32_t loopIdx = 0; loopIdx < atoi(params['l'].c_str()); ++loopIdx) {
        ...
        costTime = (1.0 * (endTimeStamp.tv_sec - startTimeStamp.tv_sec) * g_MILLION +
                      (endTimeStamp.tv_usec - startTimeStamp.tv_usec)) / g_THOUSAND;
        startTime = (1.0 * startTimeStamp.tv_sec * g_MILLION + startTimeStamp.tv_usec) / g_THOUSAND;
        endTime = (1.0 * endTimeStamp.tv_sec * g_MILLION + endTimeStamp.tv_usec) / g_THOUSAND;
        totalTime += costTime;
        ...
        for (size_t outIndex = 0; outIndex < aclmdlGetDatasetNumBuffers(output); ++outIndex) {
            ...
            if ((params['g'] != "0") && (!saveOriginInput.empty())) {
                ...
                for(int sampleIdx = 0; sampleIdx < atoi(params['b'].c_str()); sampleIdx++){
                    ...
                    string outputOriginName = outputPrefix + GetInputName(saveOriginInput[fileIdx][sampleIdx])
                    + "_output_";
                    string outputOriginNameIdx = outputOriginName + FormatInt(to_string(outIndex), 2) + loopName
                    + ".bin";
    ...
    printf("%s - I - [XACL]: Single step average NPU inference time of %d loops: %f ms %4.2f fps\n",
           ...
           (g_THOUSAND * atoi(params['l'].c_str()) * atoi(params['b'].c_str()) / totalTime));
...
Result WriteResult(uint32_t stepNumber, float totalSamplesTime, map<char, string> &params) {
    ...
    float totalAverageFPS = g_THOUSAND * samplesNumber / totalSamplesTime;
