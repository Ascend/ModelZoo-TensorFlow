...
Result DestroyResource(uint32_t modelId, aclmdlDesc *modelDesc,
 vector<aclrtContext> &v_context, map<char, string> &params);
...
vector<aclrtContext> &v_context, float &totalSamplesTime, string outputPrefix, vector<vector<string>> saveOriginInput,
 uint32_t fileIdx, map<char, string> &params);
