...
{   
    ...
    map<string, string> dym_config_map;
    ...
    if(params.count('c')){
        INFO_LOG("Dynamic input config file path: %s",params['c'].c_str());
        Utils::ParserConfig(params['c'],dym_config_map);
        g_is_dymConfig = true;
    }
    ...
    if ((input_files.empty() != 1) && (input_files[0].find(".bin") == string::npos)){
        ...
        for (size_t i = 0; i < fileName_vec.size(); ++i)
        {
            ...
            if (g_is_dymdims){
            ...
            else if (g_is_dymConfig){
                if (dym_config_map.count(fileName_vec[i]) > 0)
                {
                    Utils::SplitStringWithPunctuation(dym_config_map[fileName_vec[i]], dym_shape_tmp, ';');
                    ret = processModel.CheckDynamicShape(dym_shape_tmp, dym_shape_map, dims_num);
                    if(ret != SUCCESS){
                        ERROR_LOG("check dynamic shape failed");
                        return FAILED;
                ...
                {
                    ERROR_LOG("Can not find %s in the config file:[%s], Please check!", fileName_vec[i].c_str(),params['c'].c_str());
            ...
            processModel.OutputModelResult(times, modelName, g_dymbatch_size, dym_sign);
            processModel.DestroyInput();
