...
void InitAndCheckParams(int argc, char* argv[], map<char, string>& params, vector<string>& inputs)
{
    ...
    struct option opts[] = { { "model", required_argument, NULL, 'm' },
        ...
        { "dymConfig", required_argument, NULL, 'c' },
    ...
    while ((c = getopt_long(argc, argv, optstring, opts, &index)) != -1) {
        ...
        case 'c':
            check = optarg;
            if (check.find(".txt") != string::npos){
                params['c'] = optarg;
                break;
            }else{
                ERROR_LOG("Dynamic config file type is not .txt,please check your config file type!\n");
                exit(0);
            }
        case 'i':
            check = optarg;
        ...
        case 'l':
            ...
            if (g_loop > 10000 || g_loop < 1) {
                ERROR_LOG("loop must in 1 to 10000\n");
                exit(0);
            }
            break;
        case 'y':
...
int main(int argc, char* argv[])
{
    ...
    if (params.count('h') + params.count('y') + params.count('w') + params.count('s') + params.count('c')> 1){
        ERROR_LOG("dymBatch dymDims dymHW dymShape dymConfig cannot be set at the same time");
        return FAILED;
    }
