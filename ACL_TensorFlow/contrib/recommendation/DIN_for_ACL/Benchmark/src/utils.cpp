...
void Utils::printHelpLetter()
{
    ...
    cout << "  --dymShape     dynamic shape param, such as --dymShape \"data:1,600;img_info:1,600\"" << endl << endl << endl;
    cout << "  --dymConfig     dynamic config param, such as --dymShape \"data:1,600;img_info:1,600\"" << endl << endl << endl;
}
...
void Utils::ParserConfig(string str, map<string, string> &out)
{
    ...
    {
        if(single_line.find("name:") == string::npos || single_line.find("shape:") == string::npos){
            printf("[FORMAT ERROR] [%s], It should be:\'name:xxxx shape:xxxx\' \n",single_line.c_str());
            break;
        }
        else
        {
            ...
            string name;
            string shape;
            Utils::SplitStringWithPunctuation(single_line, temp, ' ');
            name = temp[0].replace(temp[0].find("name:"),5, "");
            shape = temp[1].replace(temp[1].find("shape:"),6, "");
            out[name] = shape;
        }
    }
}