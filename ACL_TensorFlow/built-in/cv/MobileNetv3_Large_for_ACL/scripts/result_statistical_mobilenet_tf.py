...
def read_file(file_path, csv_file_path):
    with open(file_path, 'r') as f:
        res = f.readlines()
        r = enumerate(res)
        list = sorted(r, key=lambda x: x[1], reverse=True)[0:5]
        print(list)
    ...
    p = 0
    for t in list:
        if( t[0] -1 ) == int(num) and list.index(t) == 0 and float(t[1]) != 0:
            #print("<%s>----------top1,top5---%s" % (img_name, t[1]))
            res = "top1,top5"
            if float(t[1]) >= 0.9:
                p = 1
            break
        elif( t[0] -1 )== int(num) and list.index(t) != 0 and float(t[1]) != 0:
            #print("<%s>----------top5--------%s" % (img_name, t[1]))
            res = "top5"
            if float(t[1]) >= 0.9:
                p = 1
            break
        elif (t[0] -1 )!= int(num) and list.index(t) == 4:
