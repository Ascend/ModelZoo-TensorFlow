...
aclError DVPP_SSD_ResNet34(std::string fileLocation, char *&ptr)
{
    /**
     * The preprocessing is divided into two steps:
     * 1. Decode (acldvppJpegDecodeAsync)
     * 2. resize (acldvppVpcCropAsync)
    **/

    /**************************Decode**************************/
    uint32_t W, H, W_Aligned, H_Aligned;
...
#endif
    acldvppPicDesc *resizeOutputDesc = nullptr;
    acldvppResizeConfig *resizeConfig = acldvppCreateResizeConfig();
    acldvppRoiConfig *cropconfig = InitCropRoiConfig(W, H);

    void *dstTmp = (void *)ptr;
    resizeOutputDesc = createDvppPicDesc(dstTmp, PIXEL_FORMAT_YUV_SEMIPLANAR_420, 300, 300, resizedWidthAligned, resizedHeightAligned, resizedOutputBufferSize);
    if( resizeOutputDesc == nullptr)
    {
        LOG("create dvpp pic desc failed");
        return -1;
    }
	...
	ret = acldvppVpcCropResizeAsync(dvpp_channel_desc, decodeOutputDesc, resizeOutputDesc, cropconfig, resizeConfig, stream);
#else
    ret = acldvppVpcCropAsync(dvpp_channel_desc, decodeOutputDesc, resizeOutputDesc, cropconfig, stream);
#endif
    if( ret != ACL_ERROR_NONE)
    {
        LOG("crop failed\n");
    ...
    acldvppDestroyPicDesc(resizeOutputDesc);
...
aclError DvppInitInput(std::vector<std::string> files)
{
    ...
    {
        std::string fileLocation = cfg.dataDir + "/" + files[i];
        ret = DVPP_SSD_ResNet34(fileLocation, ptr);
        inputDataframe.fileNames.push_back(files[i]);
    }
