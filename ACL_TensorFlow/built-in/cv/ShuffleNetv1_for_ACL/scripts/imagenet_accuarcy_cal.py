...
if __name__=='__main__':
    ...
    parser.add_argument("--label", type=str, default="./ILSVRC2012_validation_ground_truth.txt")
    ...
    if args.label.endswith(".csv"):
        ...
        with open(args.label, 'r') as cs:
            ...
            index = 1
            for line in rs_list:
                image_name = "ILSVRC2012_val_{}".format(str(index).zfill(8))
    ...
    for i in sorted(ground_truth):
        try:
            ...
            #查看输出的文件
            if os.path.exists(os.path.join(args.infer_result,'davinci_{}_output.bin'.format(image_name))):
                bin_path = os.path.join(args.infer_result,'davinci_{}_output.bin'.format(image_name))
                pred = np.fromfile(bin_path, dtype=args.dtype)
            elif os.path.exists(os.path.join(args.infer_result,'davinci_{}_output{}.bin'.format(image_name, args.output_index))):
