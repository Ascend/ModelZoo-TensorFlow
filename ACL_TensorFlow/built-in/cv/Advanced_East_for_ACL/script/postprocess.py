...
def cut_text_line(geo,scale_ratio_w, scale_ratio_h, imarry, img_path, s):
...
def predict(east_detect, img_path, pixel_threshold, quiet=False):
    ...
    d_weight, d_height = resize_image(img, cfg.max_predict_img_size)
    img = img.resize((d_weight, d_height), Image.NEAREST).convert('RGB')
...
def predict_txt(east_detect, img_path, txt_path, pixel_threshold, quiet=False):
    img = image.load_img(img_path)
    d_weight, d_height = cfg.max_predict_img_size, cfg.max_predict_img_size
    scale_ratio_w = d_weight / img.width
    ...
    for score, geo in zip(quad_scores, quad_after_nms):
        if np.amin(score) > 0:
            rescaled_geo = geo / [scale_ratio_w, scale_ratio_h]
            rescaled_geo = np.round(rescaled_geo,decimals=0).astype(np.int32)
            rescaled_geo = order_points_tuple(rescaled_geo)
            rescaled_geo_list = np.reshape(rescaled_geo, (8,)).tolist()
            txt_item = ','.join(map(str, rescaled_geo_list))
            txt_items.append(txt_item + '\n')
        elif not quiet:
...
if __name__=='__main__':
    ...
    east_detect.save("saved_model/AdvancedEast_model.h5")
