...
# AlexNet Tensorflow离线推理

此链接提供AlexNet TensorFlow模型在NPU上离线推理的脚本和方法
...
cd Modelzoo-TensorFlow/ACL_TensorFlow/built-in/cv/AlexNet_for_ACL
```
...
- Pb模型转换为om模型
  ...
  ```
  atc --model=alexnet_tf.pb --framework=3 --output=alexnet_tf_aipp --output_type=FP32 --soc_version=Ascend310P3 --input_shape="input:1,224,224,3" --log=info --insert_op_conf=alexnet_tf_aipp.cfg
  ```
...
| :---------------:|:-------:|:-------: | :-------------: |
| offline Inference| Ascend310     | 50K images | 59.08 %/ 81.37% |
| offline Inference| Ascend310P3     | 50K images | 59.9 %/ 82.0% |
