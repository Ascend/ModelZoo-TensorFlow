...
def define_ascend_distribute_flags():
    ...
    parser.add_argument(
        "--env",
        default=None,
        help="Necessary, environment information, please input with '--env {ip}:{device num}:{device ip}' format, when training with MultiWorker, please use ',' to separate different workers",
    )
    ...
    parser.add_argument(
        ...
        default=None,
        type=str,
        help="Necessary, training command, input like --train_command 'bash train_1p.sh' or --train_command 'python3 train.py'",
    )
    ...
    parser.add_argument(
        ...
        default=False,
        type=bool,
        help="Optional, if or not training with custom tune bank file witch generated by AOE, default is False, use --use_library=True to enable",
    )
