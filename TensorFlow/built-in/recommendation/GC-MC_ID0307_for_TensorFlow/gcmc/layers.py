...
class Dense(Layer):
    ...
    def _call(self, inputs):
        x_u = inputs[0]
        x_u = npu_ops.dropout(x_u, 1 - self.dropout)
        x_u = tf.matmul(x_u, self.vars['weights_u'])

        x_v = inputs[1]
        x_v = npu_ops.dropout(x_v, 1 - self.dropout)
...
class OrdinalMixtureGCN(Layer):
    ...
    def _call(self, inputs):
        ...
        if self.sparse_inputs:
            ...
            x_u = npu_ops.dropout(inputs[0], 1 - self.dropout)
            x_v = npu_ops.dropout(inputs[1], 1 - self.dropout)
...
class BilinearMixture(Layer):
    ...
    def _call(self, inputs):
        ...
        u_inputs = npu_ops.dropout(inputs[0], 1 - self.dropout)
        v_inputs = npu_ops.dropout(inputs[1], 1 - self.dropout)
