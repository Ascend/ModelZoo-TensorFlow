...
#         writer.writerow([v for v in values])
    ...
    if epoch % 100 == 0 and epoch > 1000 and not TESTING and False:
        ...
        save_path = saver.save(sess, "ckpt/%s_seed%d.ckpt" % (model.name, DATASEED), global_step=model.global_step)
...
saver = tf.train.Saver()
save_path = saver.save(sess, "ckpt/%s.ckpt" % model.name, global_step=model.global_step)
file_write.close()
