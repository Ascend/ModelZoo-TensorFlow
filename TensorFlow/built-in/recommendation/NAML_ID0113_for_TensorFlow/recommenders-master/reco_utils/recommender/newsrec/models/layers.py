...
class SliceLayer(layers.Layer):
    def __init__(self, sindex=None, eindex=None, **kwargs):
        self.sindex = sindex
        self.eindex = eindex
        super(SliceLayer, self).__init__(**kwargs)

    def call(self, inputs, mask=None, **kwargs):
        x = inputs[:, self.sindex : self.eindex]
        return x

    # input_shape=(None, 82)
    # (None, 30) (None, 50) (None, 1) (None, 1)
    def compute_output_shape(self, input_shape):
        if self.sindex == None:
            start_shape = 0
        else:
            start_shape = self.sindex

        if self.eindex == None:
            end_shape = input_shape[-1]
        else:
            end_shape = self.eindex

        return None, end_shape-start_shape

    def get_config(self):
        config = {'sindex':self.sindex, 'eindex':self.eindex}
...
class AttLayer2(layers.Layer):
    ...
    def get_config(self):
        """ add dim, seed into layer config.

        Returns:
            dict: config of AttLayer2 layer.
        """
        config = {'dim':self.dim}
