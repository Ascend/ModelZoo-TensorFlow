...
class BaseModel:
    ...
    def __init__(
        ...
        #if self.hparams.rank_size > 1:
        #    self.train_optimizer = NPULossScaleOptimizer(self.train_optimizer, loss_scale_manager, is_distributed=True)
        #    self.train_optimizer = NPUDistributedOptimizer(self.train_optimizer)
        #else:
        #    self.train_optimizer = NPULossScaleOptimizer(self.train_optimizer, loss_scale_manager)
    ...
    def _get_opt(self):
        ...
        if optimizer == "adam":
            ...
            train_opt = tf.compat.v1.train.AdamOptimizer(learning_rate=lr)
    ...
    def fit(
        ...
        for epoch in range(1, self.hparams.epochs + 1):
            ...
            for batch_data_input in tqdm_util:
                ...
                if step % self.hparams.show_step == 0:
                    ...
                    tqdm_util.set_description(
                        "step {0:d}, step_time: {1:.4f}, total_loss: {2:.4f}, data_loss: {3:.4f}".format(
                            step, step_time, epoch_loss / step, step_data_loss
                        )
                    )

                if self.hparams.max_steps and step >= self.hparams.max_steps:
                    break
            ...
            if self.hparams.max_steps:
                logging.warning(
                    "at epoch {0:d} max_step {1:d}, train time: {2:.1f}".format(epoch, self.hparams.max_steps, train_time)
                    + "\ntrain info: "
                    + train_info
                )
            else:
            ...
            self.model.save(os.path.join(self.hparams.model_path, "naml_epoch{0:d}.h5".format(epoch)))
