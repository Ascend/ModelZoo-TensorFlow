...
from reco_utils.recommender.newsrec.models.layers import AttLayer2,SliceLayer
...
class NAMLModel(BaseModel):
    ...
    def _build_newsencoder(self):
        ...
        sequences_input_title = SliceLayer(sindex=None, eindex=hparams.title_size)(
            input_title_body_verts
        )

        sequences_input_body = SliceLayer(sindex=hparams.title_size,
                                          eindex=hparams.title_size + hparams.body_size)(
            input_title_body_verts
        )
        input_vert = SliceLayer(sindex=hparams.title_size + hparams.body_size,
                                eindex=hparams.title_size + hparams.body_size + 1)(
            input_title_body_verts
        )
        input_subvert = SliceLayer(sindex=hparams.title_size + hparams.body_size + 1,
                                   eindex=None)(
            input_title_body_verts
        )

        embedding_layer_title = layers.Embedding(
            self.word2vec_embedding.shape[0],
            hparams.word_emb_dim,
            weights=[self.word2vec_embedding],
            trainable=True,
            name="titleencoder_embedding"
        )

        embedding_layer_body = layers.Embedding(
            self.word2vec_embedding.shape[0],
            hparams.word_emb_dim,
            weights=[self.word2vec_embedding],
            trainable=True,
            name="bodyencoder_embedding"
        )

        title_repr = self._build_titleencoder(embedding_layer_title)(sequences_input_title)
        body_repr = self._build_bodyencoder(embedding_layer_body)(sequences_input_body)
