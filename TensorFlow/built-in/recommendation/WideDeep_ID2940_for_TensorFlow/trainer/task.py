...
def main(FLAGS):
    ...
    if not FLAGS.hvd or get_npu_local_rank_id() == 0:
    ...
    if FLAGS.gpu:
        if FLAGS.amp:
            ...
            session_config = tf.compat.v1.ConfigProto(
                graph_options=npu_graph_options(graph_options=tf.compat.v1.GraphOptions(rewrite_options=rewrite_options)),
    ...
    def wide_optimizer():
        ...
        if FLAGS.amp:
            loss_scale = ExponentialUpdateLossScaleManager(init_loss_scale=2**15, incr_every_n_steps=2000, incr_ratio=2)
            opt = NPULossScaleOptimizer(opt, loss_scale)
        return opt
    ...
    def deep_optimizer():
        ...
        if FLAGS.Adam:
            opt = tf.train.AdamOptimizer(learning_rate=learning_rate_fn, epsilon=9e-8)
        else:
