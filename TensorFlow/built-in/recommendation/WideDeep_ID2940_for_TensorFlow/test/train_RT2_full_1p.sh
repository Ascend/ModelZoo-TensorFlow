...
batch_size=131072
#网络名称，同目录名称
Network="WideDeep_TF_ID2940_for_TensorFlow"
#Device数量，单卡默认为1
RankSize=1
#训练epoch，可选
train_epochs=120
...
cd $cur_path/../
start=$(date +%s)
nohup python3  -m trainer.task --gpu \
 --model_type=wide \
...
#输出训练精度,需要模型审视修改
train_accuracy=`grep 'INFO:tensorflow:Saving dict for global step' $cur_path/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log| tail -n 1 |awk -F' = ' '{print $3}' |awk -F',' '{print $1}'`
#打印，不需要修改
