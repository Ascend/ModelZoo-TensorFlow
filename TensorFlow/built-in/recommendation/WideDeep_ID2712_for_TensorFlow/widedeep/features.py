...
def get_feature_inputs(input_data=None, force_subset=None):
    ...
    for column_name in training_columns:
        if column_name in HASH_BUCKET_SIZES or column_name in IDENTITY_NUM_BUCKETS:
            if column_name in HASH_BUCKET_SIZES:
                vocab_size = HASH_BUCKET_SIZES[column_name]
                embedding_dim = 64 \
                                if column_name not in EMBEDDING_DIMENSIONS \
                                else EMBEDDING_DIMENSIONS[column_name]
            else:
                vocab_size = IDENTITY_NUM_BUCKETS[column_name]
                embedding_dim = 64

            feature_ids = tf.math.floormod(input_data[column_name], vocab_size)
            feature_ids = tf.to_int32(feature_ids,name="FloorModInt32")
            with tf.variable_scope("deep_embeddings", reuse=tf.AUTO_REUSE):
                deep_emb = tf.get_variable('deep_' + column_name, shape=[vocab_size, embedding_dim],
                                           initializer=tf.random_uniform_initializer(-0.01, 0.01),
                                           dtype=tf.float32,
                                           collections=[tf.GraphKeys.GLOBAL_VARIABLES, "deep", "deep_emb"])
            deep_inputs.append(tf.reshape(tf.gather(deep_emb, feature_ids),
                                          [-1, input_data[column_name].get_shape()[1] * embedding_dim]))
            with tf.variable_scope("wide_embeddings", reuse=tf.AUTO_REUSE):
                wide_emb = tf.get_variable('wide_' + column_name, shape=[vocab_size, 1],
                                           initializer=tf.random_uniform_initializer(-0.01, 0.01),
                                           dtype=tf.float32,
                                           collections=[tf.GraphKeys.GLOBAL_VARIABLES, "wide", "wide_emb"])
            wide_inputs.append(tf.reshape(tf.gather(wide_emb, feature_ids),
                                          [-1, input_data[column_name].get_shape()[1]]))
        elif column_name in FLOAT_COLUMNS_SIMPLE_BIN_TRANSFORM:
            deep_inputs.append(input_data[column_name])
            with tf.variable_scope("wide_embeddings", reuse=tf.AUTO_REUSE):
                wide_emb = tf.get_variable('wide_' + column_name, shape=[1],
                                           initializer=tf.random_uniform_initializer(-0.01, 0.01),
                                           dtype=tf.float32,
                                           collections=[tf.GraphKeys.GLOBAL_VARIABLES, "wide", "wide_emb"])
            wide_inputs.append(tf.ones_like(input_data[column_name], dtype=tf.float32) * wide_emb)
        elif column_name in FLOAT_COLUMNS_LOG_BIN_TRANSFORM or column_name in INT_COLUMNS:
            deep_inputs.append(input_data[column_name + '_log_01scaled'])
            with tf.variable_scope("wide_embeddings", reuse=tf.AUTO_REUSE):
                wide_emb = tf.get_variable('wide_' + column_name, shape=[1],
                                           initializer=tf.random_uniform_initializer(-0.01, 0.01),
                                           dtype=tf.float32,
                                           collections=[tf.GraphKeys.GLOBAL_VARIABLES, "wide", "wide_emb"])
            wide_inputs.append(tf.ones_like(input_data[column_name + '_log_01scaled'], dtype=tf.float32) * wide_emb)
