...
class WideDeep:
    ...
    def init_graph(self):
        ...
        self.wts_mask = tf.expand_dims(self.wt_hldr, axis=2)

        self.wts_emb_mask = tf.multiply(self.wts_mask, self.val_emb_table) 
        self.id_emb = tf.gather(self.unique_emb, self.global_unique_id_inver) # [bs, 39, 80]
        all_emb = tf.concat([self.wts_emb_mask, self.id_emb], axis=1) 

        self.deep_out = self.deep_forward(all_emb, self.keep_prob, training=True)

        self.wts_weight_mask = tf.multiply(self.wt_hldr, self.val_weight_table) # [bs, 13]
        
        self.id_weight = tf.gather(self.unique_weight_table, self.global_unique_id_inver)
        self.id_weight = tf.squeeze(self.id_weight, axis=2)
        all_weight = tf.concat([self.wts_weight_mask, self.id_weight], axis=1)
        ...
        self.log_loss = tf.nn.sigmoid_cross_entropy_with_logits(logits=self.logits, labels=self.lbl_hldr)
    ...
    def init_inputs(self):
        ...
        self.wt_hldr = self.input_data["global_wts"]
        self.lbl_hldr = self.input_data["global_labels"]
        
        self.rank_size = get_rank_size()
        self.rank_id = get_rank_id()
        self.batch_size = self.lbl_hldr.shape[0] // self.rank_size
    ...
    def get_global_unique_id_params(self, global_unique_id):
        ...
        # all reduce操作,从而每个卡可以获取到global_unique_id对应全量host params
    ...
    def host_params_split(self, unique_host_params):
        # self.host_params_split_index = [80, 80, 80, 1, 1, 1, 13] # emb, emb_moment, emb_volecity, weight, z_w, n_w, pad
    ...
    def get_host_params(self, global_unique_id, mode="train"):
        ...
        # 每个卡根据mask的index将拉取到的host params去填充到固定shape,方便后续做all reduce操作,从而每个卡可以获取到全量global_unique_id的数据
    ...
    def init_optimizer(self, deep_loss, wide_loss):
        ...
        self.wide_op = self.wide_ptmzr.minimize(wide_loss, var_list=[self.val_weight_table])
        ...
        self.train_op = [self.deep_update, self.remote_write_op, self.wide_op]
    # 
    ...
    def adam_update_embedding(self, loss):
        ...
        self.unique_id_emb_grad = tf.math.unsorted_segment_sum(tf.reshape(self.id_emb_grad, [-1, self.embed_size]), 
                                                          segment_ids=tf.reshape(self.global_unique_id_inver, [-1]), 
...
def input_fn_tfrecord(data_path, file_pattern, batch_size=16000,
    ...
    def extract_fn(data_record):
        ...
        return sample
    # 
    def reformat_unique_fn(sample): #  去重减少通信(h2d)   
    ...
    dataset = dataset.map(extract_fn, num_parallel_calls=num_parallel).batch(int(batch_size/line_per_sample), drop_remainder=True).map(reformat_unique_fn)
    # 
    ...
    padded_shapes = {
        ...
        'global_unique_id' : tf.TensorShape([48000]),
    ...
    dataset = dataset.prefetch(buffer_size=tf.contrib.data.AUTOTUNE).map(reshape_fn)
...
def init_host_params_table(sess, config, chief_num, rank_id, host_params_shape=[200000, 256]): 
    ...
    # Graph 1
    # 首先，host侧创建了一个host变量用于存储embedding,其中每一行表示每个featureid的embedding，然后host_params_address会返回这个host变量的首地址。
    # 要拉取对应featureid的embedding时，首先传入host_params_address和featureid给embeddingrankid这个算子，这个算子会计算出每个featureid对应embedding的偏移地址，
    # 这样就可以把算出来的地址传给hccl算子，把对应的embedding从host上拉取下来了
    # 初始化共享内存,用来保存embedding;多p场景下,rankid=0的进程负责初始化hostembedding而其他进程只需要申请统一变量名的共享内存用于获取同一块地址
    host_params_address_table = npu_plugin.malloc_shared_memory("host_param", host_params_shape, tf_adapter.DT_FLOAT)
    if 0 == rank_id:
        with npu_scope.npu_variable_scope(placement=NpuExecutePlacement.HOST):
            ...
            emb_opt_params = np.zeros([host_params_shape[0], 80 *2]) # adam的两个优化器参数：moment, velocity
            ...
            weight_opt_params = np.zeros([host_params_shape[0], 2]) # ftrl的两个优化器参数：linear, accumulator
            weight_params = np.concatenate([weight_table, weight_opt_params], axis=1)

            pad_params = np.zeros([host_params_shape[0], host_params_shape[1] - 80*3 - 3]) # 32位对齐，影响性能

            host_params = np.concatenate([emb_params, weight_params, pad_params], axis=1)
            # 
            host_params_table = tf.get_variable(name='host_param',
                                                shape=host_params_shape,  # [200000, 256]
                                                initializer=tf.constant_initializer(host_params),
                                                dtype=tf.float32)
        # 
        sess.run(util.variable_initializer_in_host([host_params_table ]))
        #host_params_address_table = npu_plugin.get_var_addr_and_size("host_param")
    #else:
        #host_params_address_table = npu_plugin.malloc_shared_memory("host_param", host_params_shape, tf_adapter.DT_FLOAT)
    ...
    #申请了一块device内存,用于保存拉取下来的embedding数据
