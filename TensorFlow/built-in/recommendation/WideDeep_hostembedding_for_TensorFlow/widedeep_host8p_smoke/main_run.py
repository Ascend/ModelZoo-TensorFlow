...
os.environ["TF_CPP_MIN_LOG_LEVEL"]='3' # 鍙樉绀 warning 鍜 Error 
...
class WideDeep:
    ...
    def get_host_params(self, global_unique_id, mode="train"):
        """  paddding -1  瀵 global_unique_id 娑夊強鍒扮殑 璁＄畻鐨勫奖鍝  """
...
def parse_args():
    ...
    # dataset
    parser.add_argument('--data_path', default='/autotest/data',
...
if __name__ == '__main__':
    ...
    if 'autotune' in os.environ:
        autotune = str(os.environ['autotune']).lower()
        print('Environment variables \'autotune\' detected: ' + autotune)
        if autotune == 'true':
            print("Detected autotune module as " + autotune)
            custom_op.parameter_map["auto_tune_mode"].s = tf.compat.as_bytes("RL,GA")
        elif autotune == 'false':
            print("Detected autotune module as " + autotune)
        else:
            print(autotune + " is not under law, plz check")
    else:
        print('Environment variables \'autotune\' detected failed! pass.')
