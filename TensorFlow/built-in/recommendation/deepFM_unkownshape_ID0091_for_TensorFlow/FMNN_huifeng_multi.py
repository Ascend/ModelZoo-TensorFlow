...
class FMNN_v2:
    ...
    def __init__(self,  dataset_argv, architect_argv, init_argv, ptmzr_argv,
        ...
        # batch_indices ... >????
        # [B,S]
        self.wt_hldr_uniq = tf.placeholder(tf.float32, shape=[None, self.dim_unique], name='wt_unique')# 褰撳墠鑺傜偣鐨勬暟鎹畍alue  dim_unique 闇瑕佽鍒嗗壊鐨勬暟閲忥紝sparse
        # [N*B,S]
        self.id_hldr_uniq_all = tf.placeholder(tf.int32, shape=[None, self.dim_unique], name='id_unique_all')# 鎵鏈夎妭鐐归渶瑕佽鍒嗗壊鐨刬d
        # [B]
        self.batch_indices = tf.placeholder(tf.int32, shape=[None], name='batch_indices')# np.range锟?list 褰撳墠鑺傜偣鐨勬暟鎹湪all閲岄潰鐨勮**   锟?weight 琛屼竴锟?        # 褰撳墠鑺傜偣瑕佺敤鍒扮殑鏍锋湰鏄湪 锛屽摢浜涜鏄嚜宸辩殑锟?
        # 鍥犱负鍓嶉潰鏄痙ense 鍚庨潰鏄痵parse 锛屾墍浠ヨ璁瞫parse鐨刬d 浠庨浂寮锟?# 鐩墠鍓嶉潰鏄痙ense 鍚庨潰鏄痵parse 锛屾媺锟?        # [B,S]->[B*S]  ;  [?] [B*S]
        ...
        # general_dim ... >   [N*B*S] ; [N*B锛孲]
        self.id_uniq_inver_all = tf.reshape(self.id_uniq_inver_all_, [-1, config.general_dim]) # *** global unique
        # batch_indices ... >   [N*B锛孲] ; [B锛孲]
        self.id_uniq_inver_ori = tf.gather(self.id_uniq_inver_all, indices=self.batch_indices)
        # batch_indices ...   [B锛孲] ; [B锛孲] ***鏀硅繃
        self.id_uniq_inver = tf.reshape(self.id_uniq_inver_ori, [-1, self.dim_unique])
        self.id_uniq_local_mask = tf.where(tf.equal(tf.cast(get_rank_id(), dtype=tf.int32), self.id_uniq_u%get_rank_size())) #鍔ㄦ佺殑****鐪嬫槸鍚︽槸鏈湴
        #sen

        self.id_uniq_local = tf.gather_nd(self.id_uniq_u, self.id_uniq_local_mask) #鍔ㄦ佺殑 锛屽彲浠ョ敤gather V2 LOCAL mask 琛ㄧず id_)uniq 鐨勫摢涓綅锟?        
        self.id_uniq_local_ = tf.reshape(self.id_uniq_local, [-1, 1])# 鍔ㄦ佺殑 杞崲local鐨刬d锟?        
        self.id_uniq_owner = self.id_uniq_local // get_rank_size() # 鍔ㄦ佺殑 杞崲local鐨刬d锟?    
        #sen

        self.index_all = tf.reshape(tf.range(0,tf.shape(self.id_uniq_u)[0]), [-1,1])
        # self.uniq_index = self.id_uniq_local_mask
        self.uniq_index = tf.gather_nd(self.index_all, self.id_uniq_local_mask)

        mask_uniq = self.wt_hldr_uniq
        # uniquq lookup
        #[local?] [voc,E]锟?[local?,E]
        ...
        self.eval_uniq_index = tf.gather_nd(self.eval_index_all, self.eval_id_uniq_local_mask)
        # uniquq lookup
