...
do
    ...
    nohup python3 2unique_deepfm_main_record_uid_fp32.py  \
	    --data_dir=$data_path \
		--max_epochs=$train_epochs \
		--batch_size=$batch_size \
		--max_steps=$max_steps > ${cur_path}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log 2>&1 &
done 
wait
...
#参数改回
sed -i 's|"device_\id"\:"'$ASCEND_DEVICE_ID'"|"device\_id"\:"0"|g' $cur_path/../configs/hccl.json
wait

#结果打印，不需要修改
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
FPS=`grep "epoch:" ${cur_path}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|awk -F 'fps:' '{print $2}' |awk 'NR>1'| awk '{sum+=$1} END {print  sum/NR}'`
#打印，不需要修改
