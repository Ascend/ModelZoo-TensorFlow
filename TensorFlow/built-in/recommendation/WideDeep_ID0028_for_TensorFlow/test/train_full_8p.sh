...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
FPS=`grep 'fps' ${cur_path}/output/0/train_0.log| awk -F " " '{print $25}'| awk 'END {print $1}'`
#打印，不需要修改
echo "Final Performance images/sec : $FPS"

#输出训练精度,需要模型审视修改
train_accuracy=`grep 'best auc:' ${cur_path}/output/0/train_0.log| awk -F " " '{print $4}'|awk 'END {print $1}'`
#打印，不需要修改
...
grep 'loss =' ${cur_path}/output/0/train_0.log|awk -F " " '{print $9}' >> $cur_path/output/${ASCEND_DEVICE_ID}/train_${CaseName}_loss.txt
