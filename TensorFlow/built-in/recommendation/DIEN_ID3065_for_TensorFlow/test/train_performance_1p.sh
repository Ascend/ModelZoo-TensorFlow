...
do
    ...
    nohup python3 script/train.py train DIEN 3 train > ${cur_path}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log 2>&1 &
done 
wait
...
echo "------------------ Final result ------------------"
# #输出性能FPS，需要模型审视修改
FPS=`grep  avg_examples_per_second $cur_path/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log | awk -F ":" 'END{print $2}' |sed s/[[:space:]]//g | tail -n +1 | awk '{sum+=$1} END {print sum/NR}'`
#打印，不需要修改
