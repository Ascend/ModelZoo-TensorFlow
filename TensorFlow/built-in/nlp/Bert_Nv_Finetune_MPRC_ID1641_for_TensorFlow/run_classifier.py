...
def get_frozen_tftrt_model(bert_config, shape, num_labels, use_one_hot_embeddings, init_checkpoint):
  ...
  with tf.Session(config=npu_config_proto(config_proto=tf_config)) as tf_sess:
...
def main(_):
  ...
  if FLAGS.horovod:
      ...
      tf.compat.v1.logging.info("hvd.size() = %d hvd.rank() = %d", get_npu_rank_size(), get_npu_rank_id())
      global_batch_size = FLAGS.train_batch_size * FLAGS.num_accumulation_steps * get_npu_rank_size()
      master_process = (get_npu_rank_id() == 0)
      hvd_rank = get_npu_rank_id()
      config_proto.gpu_options.visible_device_list = str(get_npu_local_rank_id())
      #set_affinity(get_npu_local_rank_id())
      if get_npu_rank_size() > 1:
  ...
  if FLAGS.enable_exception_dump:
  ...
  if FLAGS.do_train:
    ...
    if FLAGS.horovod:
      tmp_filenames = [os.path.join(FLAGS.output_dir, "train.tf_record{}".format(i)) for i in range(get_npu_rank_size())]
      num_examples_per_rank = len(train_examples) // get_npu_rank_size()
      remainder = len(train_examples) % get_npu_rank_size()
      if get_npu_rank_id() < remainder:
        start_index = get_npu_rank_id() * (num_examples_per_rank+1)
        end_index = start_index + num_examples_per_rank + 1
      else:
        start_index = get_npu_rank_id() * num_examples_per_rank + remainder
  ...
  if FLAGS.do_train:
    ...
    if master_process:
        ...
        tf.compat.v1.logging.info("Total Training Time W/O Overhead = %0.2f for Sentences = %d", train_time_wo_overhead,
                        (training_hooks[-2].count - training_hooks[-2].skipped) * global_batch_size)
