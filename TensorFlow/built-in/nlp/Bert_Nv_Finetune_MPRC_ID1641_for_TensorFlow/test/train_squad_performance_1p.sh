...
nohup python run_squad.py \
  ...
  --do_train=True \
  --train_file=${data_path}/$task_name/${squad_version}/$train-v${squad_version}.json \
  --do_predict=True \
  --predict_file=${data_path}/$task_name/${squad_version}/dev-v${squad_version}.json \
  --eval_script=evaluate-v${squad_version}.py \
  ...
  $use_xla_tag --version_2_with_negative=${version_2_with_negative}   > $cur_path/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log 2>&1 &
