  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    if FLAGS.verbose_logging:
        ...
        for var in tvars:
          ...
          tf.compat.v1.logging.info("  %d :: name = %s, shape = %s%s", 0 if hvd is None else get_npu_rank_id(), var.name, var.shape,
                          init_string)
...
def input_fn_builder(input_files,
  ...
  def input_fn():
    ...
    if is_training:
      ...
      if hvd is not None: d = d.shard(get_rank_size(), get_rank_id())
      d = d.repeat()
...
def main(_):
  ...
  if FLAGS.horovod and len(input_files) < get_npu_rank_size():
...
#    config.gpu_options.per_process_gpu_memory_fraction = 0.7
  ...
  run_config = tf.estimator.RunConfig(
      ...
      save_checkpoints_steps=FLAGS.save_checkpoints_steps if not FLAGS.horovod or get_npu_rank_id() == 0 else None,
      save_summary_steps=FLAGS.save_checkpoints_steps if not FLAGS.horovod or get_npu_rank_id() == 0 else None,
  ...
  model_fn = model_fn_builder(
      ...
      learning_rate=FLAGS.learning_rate if not FLAGS.horovod else FLAGS.learning_rate*get_npu_rank_size(),
  ...
  if FLAGS.do_train:
    ...
    training_hooks = []
    if FLAGS.horovod and get_npu_rank_size() > 1:
      training_hooks.append(NpuEmptyHook())
    if (not FLAGS.horovod or get_npu_rank_id() == 0):
      global_batch_size = FLAGS.train_batch_size * FLAGS.num_accumulation_steps if not FLAGS.horovod else FLAGS.train_batch_size * FLAGS.num_accumulation_steps * get_npu_rank_size()
    ...
    estimator.train(input_fn=train_input_fn, hooks=npu_hooks_append(hooks_list=training_hooks), max_steps=FLAGS.num_train_steps)
  ...
  if FLAGS.do_eval and (not FLAGS.horovod or get_npu_rank_id() == 0):
