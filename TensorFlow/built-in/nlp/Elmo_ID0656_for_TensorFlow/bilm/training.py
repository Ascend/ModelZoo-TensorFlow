    ...
    def _build(self):
        ...
        for lstm_num, lstm_input in enumerate(lstm_inputs):
            ...
            if self.is_training:
                ...
                lstm_output_flat = npu_ops.dropout(lstm_output_flat,
                    keep_prob)
...
def train(options, data, n_gpus, tf_save_dir, tf_log_dir,
    ...
    with tf.device('/cpu:0'):
        ...
        opt = npu_tf_optimizer(tf.train.AdagradOptimizer(learning_rate=lr,
    ...
    custom_op.parameter_map["modify_mixlist"].s = tf.compat.as_bytes("./test/ops_info.json")
    ...
    ########################npu modify###########################
        ...
        n_batch_stop = options['n_batch_stop']
        print("[Training] total {} epochs.".format(options['n_epochs']))
        print("[Training] {} batches per epoch.".format(n_batches_per_epoch))
        print("[Training] total {} batches.".format(n_batches_total))
        print("[Training] will stop after min({},{}) batches.".format(n_batch_stop,n_batches_total))
        ...
        for batch_no, batch in enumerate(data_gen, start=1):
            if batch_no > n_batch_stop:
                ...
                if batch_no % 1250 == 0:
                    # run the histogram summaries
                    ret = sess.run(
                        [train_op, summary_op, train_perplexity, hist_summary_op] + 
                        final_state_tensors,
                        feed_dict=feed_dict)
                    n_sec_per_batch = time.time() - begin_time
                ...
                print("Batch {}".format(batch_no),flush=True)
                print("all_gpu_loss= {} , n_sec_per_batch= {}".format(ret[2], n_sec_per_batch),flush=True)
