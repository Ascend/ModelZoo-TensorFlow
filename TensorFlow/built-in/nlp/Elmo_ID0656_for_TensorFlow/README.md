...
**描述（Description）：基于TensorFlow框架的Elmo网络训练代码**
...
1. 模型使用自制数据集，参考 vocab/cn_vocab 以及 pre_data/code/pre.py 和 pre_data/code/vocab.py，即可处理出词典。

2. 自制数据集后，将生成的数据集放入模型目录下，在训练脚本中指定数据集路径，可正常使用。示例如下：
```
    ├── elmo_dataset
    ├── ├── data
    ├── ├── ├── cn_train_data.txt
...
- 开始训练
    ...
    2. 单卡训练
        ...
        2.1 设置单卡训练参数（脚本位于于./Elmo_ID0656_for_TensorFlow/test/train_full_1p.sh）。示例如下。请确保下面例子中的“data_path”修改为用户数据集的路径(即如data_path="./elmo_dataset")。

        ```
        `python3 train_elmo.py --train_prefix=${data_path} `
        ```
        

        2.2 单卡训练指令（脚本位于于./Elmo_ID0656_for_TensorFlow/test/train_full_1p.sh）
...
## 脚本和示例代码<a name="section08421615141513"></a>
    ...
    └── train_elmo.py 							// 训练入口脚本


## 脚本参数<a name="section6669162441511"></a>

```
    --save_dir                        Location of checkpoint files
    --vocab_file                      Vocabulary file
    --train_prefix                    Prefix for train files
    --batch_size                      batch size
    --n_epochs                        total number of epochs to train
    --n_batch_stop                    how many batchs to stop train
```
