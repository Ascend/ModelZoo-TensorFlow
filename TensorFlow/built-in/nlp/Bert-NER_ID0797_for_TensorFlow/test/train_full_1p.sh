...
#规避不同环境日志问题
FPS1=`grep "examples/sec:" $cur_path/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log | grep "tpu_estimator.py"|awk 'END{printf $6}'`
if [ x"$FPS" = x ];
then
FPS=$FPS1
fi
...
#输出训练精度,需要模型审视修改
train_accuracy=`grep 'accuracy' $cur_path/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|awk '{print $2}'|awk -F ';' '{print $1}'|sed 's/%//g'`
#打印，不需要修改
