...
class InputHelper(object):
    ...
    def data_shard(self, device_size, device_id, x_train, y_train,batch_size=None):
        if device_size!=1:
            sample_one_device = int(len(x_train) / device_size) 
            x_train = x_train[device_id * sample_one_device : (device_id+1)*sample_one_device]
            if batch_size:
                res_len = len(x_train) % int(batch_size) *(-1)
                x_train = x_train[:res_len]
        return x_train

    def batch_iter(self, data, batch_size, num_epochs, shuffle=True,device_size=1,device_id=0):
        """
        ...
        ########################
        if device_size>1:
            data = self.data_shard(device_size,device_id,data,None,None)
        #########################
