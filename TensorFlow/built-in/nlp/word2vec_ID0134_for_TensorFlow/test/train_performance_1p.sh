...
#基础参数，需要模型审视修改
#网络名称，同目录名称
Network="word2vec_ID0134_for_TensorFlow"
#训练epoch
train_epochs=1
#训练batch_size
batch_size=128
...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
train_time=`grep -a 'time cost' $cur_path/output/$ASCEND_DEVICE_ID/train_${ASCEND_DEVICE_ID}.log |awk 'END {print $10}'`
FPS=`awk 'BEGIN{printf "%.2f\n",'2000'*'${batch_size}'/'${train_time}'}'`
#打印，不需要修改
echo "Final Performance images/sec : $train_time"
#输出训练精度,需要模型审视修改
#train_accuracy=`grep -a 'loss' $cur_path/output/$ASCEND_DEVICE_ID/train_${ASCEND_DEVICE_ID}.log |awk 'END {print $6}'`
train_accuracy="None"
#打印，不需要修改
