...
-   描述（Description）：使用tensorflow实现word2vec中文词向量的训练
...
- 参考实现：
    ...
    https://github.com/Deermini/word2vec-tensorflow

- 适配昇腾 AI 处理器的实现：

    https://gitee.com/ascend/modelzoo/tree/master/built-in/TensorFlow/Research/nlp/word2vec_ID0134_for_TensorFlow
...
脚本已默认开启混合精度，设置precision_mode参数的脚本参考如下。
    ...
    #    custom_op.parameter_map["iterations_per_loop"].i = 2000
...
- 单卡训练 
  ...
  2. 启动训练。
     ...
     启动单卡性能训练 （脚本为word2vec_ID0134_for_TensorFlow/test/train_performance_1p.sh） 

     ```
     bash train_performance_1p.sh --dataset_dir=/cnews
     ```
     启动单卡精度训练 （脚本为word2vec_ID0134_for_TensorFlow/test/train_full_1p.sh） 

     ```
     bash train_full_1p.sh --dataset_dir=/cnews
     ```
