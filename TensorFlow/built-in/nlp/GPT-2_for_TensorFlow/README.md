...
**描述（Description）：基于TensorFlow框架的GPT-2代码**

<h2 id="概述.md">概述</h2>
 通常在任务特定的数据集上通过监督学习来处理自然语言处理任务，例如问题解答，机器翻译，阅读理解和摘要。我们证明，当在称为WebText的数百万个网页的新数据集上进行训练时，语言模型将开始在没有任何明确监督的情况下学习这些任务。当以文档加问题为条件时，由语言模型生成的答案在CoQA数据集上达到55 F1-在不使用127,000+培训示例的情况下，匹配或超过了4个基准系统中3个的性能。语言模型的功能对于零镜头任务传输的成功至关重要，而提高语言能力则可以跨任务以对数线性方式提高性能。我们最大的模型GPT-2是一个1.5B参数的Transformer，它可以在零镜头设置下在8个经过测试的语言建模数据集中的7个数据集中获得7个的最新结果，但仍不适合WebText。该模型的样本反映了这些改进，并包含了连贯的文本段落。这些发现为建立语言处理系统提供了一条有希望的途径，该系统将从自然发生的演示中学习执行任务。  
