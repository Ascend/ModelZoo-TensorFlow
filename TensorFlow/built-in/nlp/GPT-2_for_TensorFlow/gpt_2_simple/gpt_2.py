...
def start_tf_sess(threads=-1, server=None):
    ...
    """
    config = npu_config_proto(config_proto=tf.compat.v1.ConfigProto())
    ...
    if server is not None:
        return tf.compat.v1.Session(config=npu_config_proto(config_proto=config), target=server.target)
    
    return tf.compat.v1.Session(config=npu_config_proto(config_proto=config))
...
def finetune(sess,
    ...
    if optimizer == 'adam':
        opt = npu_tf_optimizer(tf.compat.v1.train.AdamOptimizer(learning_rate=learning_rate))
    elif optimizer == 'sgd':
        opt = npu_tf_optimizer(tf.compat.v1.train.GradientDescentOptimizer(learning_rate=learning_rate))
