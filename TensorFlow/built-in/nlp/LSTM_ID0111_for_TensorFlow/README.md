...
**描述（Description）：基于TensorFlow框架的LSTM训练代码**

<h2 id="概述.md">概述</h2>

    LSTM是一个基于TensorFlow框架，使用CNN和长短记忆进行情感分析的网络。这是一个二进制分类任务。
...
1. 用户需自行准备IMDB训练数据集，可参考github源获取。
...
- 开始训练    
    ...
    2. 单卡训练 
        ...
        2.1 配置train_full_1p.sh脚本中`data_path`（脚本路径LSTM_ID0111_for_TensorFlow/test/train_full_1p.sh）,请用户根据实际路径配置，数据集参数如下所示：

            --data_path=${data_path}
            
        2.2 1p指令如下:

            bash train_full_1p.sh --data_path=IMDB
