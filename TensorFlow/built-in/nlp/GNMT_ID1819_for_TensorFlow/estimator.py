...
class ModelFnFactory(object):
  ...
  def get_optimizer(self, hparams, learning_rate):
    ...
    if hparams.optimizer == "sgd":
      ...
      if hparams.npu_loss_scale < 1:
          ...
          elif hparams.npu_loss_scale > 1:
              loss_scale_manager = FixedLossScaleManager(loss_scale=hparams.npu_loss_scale)
  ...
  def build_graph_npu(self, features, labels, mode, params):
      """docstring."""
      del labels, params
      ...
      if mode == tf.contrib.learn.ModeKeys.INFER:
          ...
          tower_features = self._shard_inputs(features, rank_size)
          model = gnmt_model.GNMTModel(
              hparams, mode=mode, features=tower_features[rank_id])
          # sync training.
          ...
          #opt = tf.train.GradientDescentOptimizer(learning_rate)
          #opt = NPUDistributedOptimizer(opt)
          grads_and_vars = opt.compute_gradients(tower_loss)
                                                 #colocate_gradients_with_ops=self.hparams.colocate_gradients_with_ops)
          ...
          saveable_vars = savable_variables()
          ...
          print("vars_saver=================", saveable_vars + saveable_objects)
          # local_init_op = tf.local_variables_initializer()
          ...
          # return loss, vars, grads, predictions, train_op, scaffold
          return tower_loss, None, None, None, train_op, scaffold
  ...
  def build_graph(self, features, labels, mode, params):
    ...
    if mode == tf.contrib.learn.ModeKeys.INFER:
      ...
      num_towers = int(os.getenv('RANK_SIZE'))
      # Shard inputs
...
def make_model_fn(hparams):
  ...
  if hparams.use_dist_strategy:
    ...
    build_fn = factory.build_graph_npu
    def fn(features, labels, mode, params):
      """docstring."""
...
def train_fn(hparams):
  ...
  #for npu start
  if hparams.data_dump_flag:
    dump_config = DumpConfig(enable_dump=True,
                             dump_path=hparams.data_dump_path,
                             dump_step=hparams.data_dump_step,
                             dump_mode="all")
  elif hparams.over_dump:
    dump_config = DumpConfig(enable_dump_debug=True,
                            dump_path=hparams.over_dump_path,
                            dump_debug_mode="all")
  else:
    dump_config = None
  if hparams.profiling:
    profiling_options = '{"output":hparams.profiling_dump_path, "training_trace":"on", "task_trace":"on", "aicpu":"on", "fp_point":"", "bp_point":""}'
  ...
  config = NPURunConfig(dump_config=dump_config,
                        ...
                        precision_mode=hparams.precision_mode,
