...
do
    ...
    nohup python3 nmt.py \
        --mode=${mode} \
        --data_dir=${data_path} \
        --max_train_epochs=${train_epochs} \
        --debug_num_train_steps=${train_steps} \
...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
time=`grep -r "] loss" $cur_path/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log | awk 'END {print $11}' | cut -d '(' -f 2`
train_time=`echo "$time"|awk '{print $1/10}'`
FPS=`echo "$batch_size $train_time"|awk '{print $1/$2}'`
#打印，不需要修改
echo "Final Performance images/sec : $FPS"

#输出训练精度,需要模型审视修改
#train_accuracy=`grep "bleu is" $cur_path/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|awk '{print $3}'|tail -1`
train_accuracy=`grep "INFO:tensorflow:loss =" $cur_path/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|awk -F "," 'END {print $1}' | awk -F "=" '{print $2}'`
#打印，不需要修改
