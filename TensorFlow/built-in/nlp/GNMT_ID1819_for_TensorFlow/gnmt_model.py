...
class GNMTModel(model.BaseModel):
  ...
  def _build_encoder(self, hparams):
    ...
    with tf.variable_scope("encoder"):
      dtype = self.dtype
      #pt_op = tf.Print(self.embedding_encoder, [self.embedding_encoder], message="embedding_encoder_printing", name="print_encoder")
      pt_op = tf.print("self.embedding_encoder", self.embedding_encoder, output_stream=sys.stderr)

      with tf.control_dependencies([pt_op]):
        encoder_emb_inp = self.encoder_emb_lookup_fn(self.embedding_encoder, source)
      ...
      bi_encoder_outputs, bi_encoder_state = self._build_encoder_layers_bidi(
          ...
          dtype)
      #pt_op2 = tf.Print(encoder_emb_inp, [encoder_emb_inp], message="encoder_emb_inp_printing")

      #with tf.control_dependencies([pt_op2]):
                # Build 1st bidi layer.
      #    bi_encoder_outputs, bi_encoder_state = self._build_encoder_layers_bidi(encoder_emb_inp, self.features["source_sequence_length"], hparams, dtype)
  ...
  def _build_bidi_rnn_fused(self, inputs, sequence_length, hparams, dtype):
    ...
    #replace with DynamicRNN begin
    if not hparams.use_DynamicRNN:
      ...
      fwd_cell = DynamicRNN(hidden_size=hparams.num_units, dtype=dtype)
      fwd_encoder_outputs, output_h, output_c, i, j, f, o, tanh = fwd_cell(inputs)
    ...
    if not hparams.use_DynamicRNN:
      ...
      bak_cell = DynamicRNN(hidden_size=hparams.num_units, dtype=dtype)
      bak_encoder_outputs, output_h2, output_c2, i, j, f, o, tanh = bak_cell(inputs_r)
    ...
    if not hparams.use_DynamicRNN:
      ...
      fwd_state = tf.nn.rnn_cell.LSTMStateTuple(output_c[-1,:,:], output_h[-1,:,:])
      bak_state = tf.nn.rnn_cell.LSTMStateTuple(output_c2[-1,:,:], output_h2[-1,:,:])
    #replace with DynamicRNN end
  ...
  def _build_unidi_rnn_fused(self, inputs, state,
    ...
    if not hparams.use_DynamicRNN:
      ...
      cell = DynamicRNN(hidden_size=hparams.num_units, dtype=dtype)
      outputs, output_h, output_c, i, j, f, o, tanh = cell(inputs)
      return outputs, tf.nn.rnn_cell.LSTMStateTuple(output_c[-1, :, :], output_h[-1, :, :])
  ...
  def _build_encoder_layers_unidi(self, inputs, sequence_length,
    ...
    if hparams.use_fused_lstm:
      for num_i in range(num_uni_layers):
        if (not np.isclose(hparams.dropout, 0.) and
            self.mode == tf.contrib.learn.ModeKeys.TRAIN):
        ...
        if not hparams.use_DynamicRNN:
          ...
          cell = DynamicRNN(hidden_size=hparams.num_units, dtype=dtype)
          encoder_outputs, output_h, output_c, i, j, f, o, tanh = cell(cell_inputs)
          encoder_state += (tf.nn.rnn_cell.LSTMStateTuple(output_c[-1,:,:], output_h[-1,:,:]),)
        #end
        if num_i >= num_uni_layers - self.num_encoder_residual_layers:
  ...
  def _build_decoder_fused_for_training(self, encoder_outputs, initial_state,
    ...
    # The 1st LSTM layer
    if self.time_major:
      #tf.shape返回的是tensor,执行报错
      #batch = tf.shape(encoder_outputs)[1]
      #tgt_max_len = tf.shape(decoder_emb_inp)[0]
      batch = (encoder_outputs.get_shape())[1]
      tgt_max_len = (decoder_emb_inp.get_shape())[0]
      # [batch_size] -> scalar
