...
#基础参数，需要模型审视修改
#网络名称，同目录名称
Network="Roberta_ID2366_for_TensorFlow"
batch_size=32
num_train_epochs=1
...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
FPS_1p=`grep examples/sec ${cur_path}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log | awk -F'examples/sec: ' 'END{print $2}'`
FPS=`awk 'BEGIN{printf "%.1f\n",'${FPS_1p}'*'${RANK_SIZE}'}'`
#打印，不需要修改
