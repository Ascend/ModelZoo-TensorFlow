...
def file_based_input_fn_builder_predict(input_file, seq_length, is_training, drop_remainder):
    name_to_features = {
        "input_ids": tf.FixedLenFeature([seq_length], tf.int64),
        "input_mask": tf.FixedLenFeature([seq_length], tf.int64),
        "segment_ids": tf.FixedLenFeature([seq_length], tf.int64),
    ...
    if args.do_train and args.do_eval:
        ...
        class MyHook(tf.train.SessionRunHook):
            def begin(self):
                ...
                self._loss_scale = tf.get_default_graph().get_tensor_by_name('loss_scale:0')
            ...
            def before_run(self, run_context):
                ...
                self._step += 1
                return tf.train.SessionRunArgs({'loss': self._loss_tensor,'loss_scale': self._loss_scale})

            def after_run(self, run_context, run_values):
                self._end = time.time()
                cost = self._end - self._begin
                loss = run_values.results['loss']
                ls = run_values.results['loss_scale']
                print('step: %d, cost time: %.3fs, losses: %.3f, loss scale: %d' % (self._step, cost, loss, ls))
        ...
        train_spec = tf.estimator.TrainSpec(input_fn=train_input_fn, max_steps=num_train_steps,
                                            hooks=npu_hooks_append(hooks_list=[myHook, early_stopping_hook]))
        eval_spec = tf.estimator.EvalSpec(input_fn=eval_input_fn, hooks=npu_hooks_append())
