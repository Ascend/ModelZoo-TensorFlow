...
# modify for npu op overflow dump start
import argparse
def parse_args():
    parser = argparse.ArgumentParser(formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    parser.add_argument('--over_dump', default=False,
                        help="""Whether to enable op overflow dump""")
    parser.add_argument('--over_dump_path', default="./overflow_dump",
                        help="""Path to save overdump files.""")
    parser.add_argument('--data_dir', default="file:///data/tfrecord_dataset",
    ...
    return args
# modify for npu op overflow dump end
...
def get_estimator_model_func(features, labels, mode, params):
    ...
    with tf.control_dependencies(update_ops):
        with tf.name_scope('loss_scale'):
            ...
            loss_scale_div = tf.constant(1024.0, tf.float16) if params['optimizer'] == 'smdp_adamw' else tf.constant(1024.0, tf.float32)
            scaled_grads_and_vars = opt.compute_gradients(total_loss * loss_scale)

      #      if params['optimizer'] == 'smdp_adamw':#smdp_opt保存的var都是fp16的，计算出来的grad也是fp16的,allreduce之前cast成fp32
      #          scaled_grads_and_vars = [(tf.cast(g,tf.float32), v) for g,v in scaled_grads_and_vars]
                
            grad_var_list = [ (g/loss_scale_div, v)  for g,v in scaled_grads_and_vars ]
            if get_data_parallel_world_size() > 1:
                #手动实现数据并行维度的梯度汇聚
                with tf.name_scope('Grad_Allreduce'):
                    allreduce_num = len(grad_var_list)
                    split_num = allreduce_num // 8 +1   #allreduce融合成8份
                    averaged_grad_var_list = []
                    for index, (g,v) in enumerate(grad_var_list):
                        averaged_grad_var_list.append((average_grad(g,fusion_id=index//split_num),v))
#                    averaged_grad_var_list = [ (average_grad(g), v)  for g,v in grad_var_list ]
            ...
            if params['optimizer'] == 'smdp_adamw':#smdp_opt保存的var都是fp16的，计算出来的grad也是fp16的，allreduce之后cast成fp32
                averaged_grad_var_list = [(tf.cast(g,tf.float32), v) for g,v in averaged_grad_var_list]
...
def average_grad(grad, fusion_id):
    return hccl_ops.allreduce(grad, "sum", fusion=2, fusion_id=fusion_id, group=get_data_parallel_group())/float(get_data_parallel_world_size())
...
def main():
    ...
    #modify for npu op overflow dump start
    args = parse_args()
    if args.data_dir == "file:///data/tfrecord_dataset":
        pass
    else:
        f1 = open("megatron_config.py", "r")
        lines = f1.readlines()
        f2 = open("megatron_config.py", "w")
        for line in lines:
    ...
    graph_mem = 1024 * 1024 * 1024 * 16
    var_mem = 1024 * 1024 * 1024 * 15
    # modify for npu op overflow dump start
    print("args.over_dump----------------->", args.over_dump)
    print("args.over_dump_path------------>", args.over_dump_path)
    if args.over_dump is True:
        print("OP overflow dump is enabled.")
        from npu_bridge.npu_init import DumpConfig
        dump_config = DumpConfig(enable_dump_debug=True, dump_path=args.over_dump_path, dump_debug_mode="all")
        ...
        if "autotune" in os.environ:
            ...
            if os.environ["autotune"] == "True":
                ...
                print ("autotune is set !")
                run_config = NPURunConfig(dump_config=dump_config,      
            hcom_parallel=False,
        ...
        print("OP overflow dump is disabled.")
        if "autotune" in os.environ:
            print ("autotune value is " + os.environ["autotune"])
