...
def gradients(ys, xs, grad_ys=None, checkpoints='collection', **kwargs):
    ...
    if type(checkpoints) is not list:
        if checkpoints == 'collection':
            ...
            print ('----Speed mode checkpoints:', len(checkpoints), checkpoints)


        elif checkpoints == 'self_define':
            # checkpoint all expensive ops to maximize running speed
            checkpoints = ge.filter_ts_from_regex(fwd_ops, 'Checkpoints')
            print ('----Self defined checkpoints:', len(checkpoints), checkpoints)
    ...
    for ts in checkpoints_sorted_lists[::-1]:
        ...
        def _unsparsify(x):
            ...
            print ('---dense_shape:', x.dense_shape)
#            return tf.scatter_nd( tf.cast( indices, tf.int64), x.values, tf.cast(x.dense_shape, tf.int64))
            return tf.scatter_nd( indices, x.values, x.dense_shape) #910上aicore不支持int的cast,会放到aicpu上执行，导致耗时超长，故此处取消int32 cast到64
