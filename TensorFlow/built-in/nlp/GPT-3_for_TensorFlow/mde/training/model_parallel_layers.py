...
def ColumnParallelLinear(args, scope, x, output_size, gather_output=False, init_scale_by_depth=False):
    ...
    with tf.variable_scope(scope):
     ...
     #   x = tf.layers.dense( inputs=x, units=output_size_partial, kernel_initializer = tf.random_uniform_initializer() )
        ...
        #分布式初始化
#        w = tf.get_variable('w', [shape_list(x)[-1], output_size_partial], initializer=tf.random_normal_initializer(stddev=w_init_stdev,seed=MODEL_PARALLEL_RANK), trainable=True)
#        b = tf.get_variable('b', [output_size//get_model_parallel_world_size()], initializer=tf.constant_initializer(0))
#        x = tf.matmul(x, w) + b

     #   *start, nx = shape_list(x)
     #   nf = output_size_partial
     #   w = tf.get_variable('w', [1, nx, nf], initializer=tf.random_normal_initializer(stddev=w_init_stdev,seed=MODEL_PARALLEL_RANK))
     #   b = tf.get_variable('b', [nf], initializer=tf.constant_initializer(0))
     #   x = tf.reshape(tf.matmul(tf.reshape(x, [-1, nx]), tf.reshape(w, [-1, nf])) + b, start + [nf])
...
def RowParallelLinear(args, scope, x, output_size, input_is_parallel=True, init_scale_by_depth=False):
 ...
 #       x = tf.layers.dense( inputs=x, units=output_size, kernel_initializer = tf.random_uniform_initializer() )
        ...
        #分布式初始化
#        input_size=shape_list(x)[-1]
#        w = tf.get_variable('w', [input_size, output_size], initializer=tf.random_normal_initializer(stddev=w_init_stdev,seed=MODEL_PARALLEL_RANK), trainable=True)
