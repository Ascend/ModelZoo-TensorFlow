...
class BackoffScaler(object):
  def __init__(self, params):
    ...
    self.scale_min = params.get('scale_min', 128.0)
    self.scale_max = params.get('scale_max', 512.0)
    self.step_factor = params.get('step_factor', 2.0)
    self.step_window = params.get('step_window', 20)
  ...
  def update_op(self, has_nan, amax):
    ...
    def scale_case():
      ...
      with tf.device('/cpu:0'): #tf.mod is not supported by GPU, so it should be calculated on CPU, which will slow down the training
...
class LogMaxScaler(object):
  def __init__(self, params):
    ...
    self.scale_min = params.get('scale_min', 128.0)
    self.scale_max = params.get('scale_max', 1024.0)
