...
# 帮助信息，不需要修改
if [[ $1 == --help || $1 == -h ]];then
    echo"usage:./train_full_4p.sh <args>"
    echo " "
...
nohup python3 ./run_classifier.py \
        ...
        --num_core_per_host=1 \
        --eval_all_ckpt=True \
        --is_regression=True >> $cur_path/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log 2>&1 &
...
#输出训练精度,需要模型审视修改
train_accuracy=`grep 'eval_pearsonr' $cur_path/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log|tail -n 2|head -n 1|awk '{print $8}'`
#打印，不需要修改
