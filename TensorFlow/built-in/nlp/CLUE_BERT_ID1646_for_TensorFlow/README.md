...
---------------------------------------------------------------------
    使用方式： 
    1.参照“数据集介绍与下载”章节下载数据集
    2.下载预训练模型 https://github.com/google-research/bert
    3.执行finetune
      分类任务  
           例如：
           cd test
           bash train_performance_1p.sh --task_name=afqmc --data_path=${CLUE_data} --train_batch_size=16  
...
数据集介绍Introduction of datasets 
--------------------------------------------------------------------

用户需要自行准备好对应的数据集，数据集的下载及处理可以参考GPU开源链接
