...
flags.DEFINE_integer("print_loss_step", 1, "Indicates the frequency of printing loss")

#flags.DEFINE_integer("check_loss_scale", 0 ,"Whether check loss scale")
flags.DEFINE_integer("init_loss_scale_value", 2**32, "Initial loss scale value for loss scale optimizer")
flags.DEFINE_integer("npu_bert_loss_scale", 0, "Whether to use loss scale, -1 is disable, 0 is dynamic loss scale, >=1 is statis loss scale")
##################NPU_modify end###############################
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      logging_hook = tf.train.LoggingTensorHook(
        {"loss":total_loss,"global_step":tf.train.get_global_step()},every_n_iter=FLAGS.print_loss_step)
      ##################NPU_modify end#############################
