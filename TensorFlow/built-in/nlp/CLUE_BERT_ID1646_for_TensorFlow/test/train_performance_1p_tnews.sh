...
#基础参数，需要模型审视修改
#网络名称，同目录名称
Network="Bert_Clue_Finetune_TNEWS_ID1647_for_TensorFlow"
#训练batch_size
train_batch_size=16
#训练ephch
...
step_sec=`grep -a 'INFO:tensorflow:global_step/sec: ' $cur_path/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log| awk '{print $2}'|tail -n +2|awk '{sum+=$1} END {print"",sum/NR}'|sed s/[[:space:]]//g`
