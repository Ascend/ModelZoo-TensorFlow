...
nohup python3 -u run_pretraining.py \
        ...
        --do_train=$do_train   \
        --bert_config_file=$ckpt_path/albert_config_xlarge.json \
...
grep "INFO:tensorflow:global_step " $cur_path/test/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log|awk '{print $6}' > $cur_path/test/output/$ASCEND_DEVICE_ID/train_${CaseName}_loss.txt
#最后一个迭代loss值，不需要修改' 
