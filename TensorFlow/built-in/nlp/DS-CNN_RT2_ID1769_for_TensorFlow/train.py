...
def main(_):
  ...
  sess = tf.InteractiveSession(config=global_config)
  ...
  audio_processor = input_data.AudioProcessor(
      ...
      FLAGS.testing_percentage, model_settings, sess)
  ...
  ####################
  #cross_entropy_mean = util.set_graph_exec_config(cross_entropy_mean,dynamic_input=True,
                                                  #dynamic_graph_execute_mode='dynamic_execute',
                                                  #dynamic_inputs_shape_range='data:[100,490],[100,12],[]')
  ####################
...
#        learning_rate_input).minimize(cross_entropy_mean)
  ...
  ###################
  #confusion_matrix = util.set_graph_exec_config(confusion_matrix,dynamic_input=True,
                                                  #dynamic_graph_execute_mode='dynamic_execute',
                                                  #dynamic_inputs_shape_range='data:[100,490],[100,12]')
  ###################
  ...
  tf.logging.info('**** start create dataset')
  dataset_start = time.time()
  dataset = audio_processor.create_dataset(training_steps_max,
                                         FLAGS.batch_size, 0, model_settings, FLAGS.background_frequency,
                                         FLAGS.background_volume, time_shift_samples, 'training', sess)
  iterator = dataset.make_initializable_iterator()
  audio_processor.train_fingerprints, audio_processor.train_ground_truth = iterator.get_next()
  sess.run(iterator.initializer)
  dataset_end = time.time()

  tf.logging.info('**** create dataset finished, duration: %.6f' % (dataset_end-dataset_start))
  ...
  for training_step in xrange(start_step, training_steps_max + 1):
    ...
    train_fingerprints, train_ground_truth = audio_processor.get_data(
        ...
        FLAGS.background_volume, time_shift_samples, 'training', sess)
    '''
    train_fingerprints, train_ground_truth = sess.run([audio_processor.train_fingerprints, audio_processor.train_ground_truth])
    ...
    print('**** data prepare duration: {:.6f} train duration: {:.6f}'.format(start-data_start, end-start))
