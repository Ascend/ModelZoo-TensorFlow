...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
FPS=`grep throughput_train $cur_path/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|awk -F 'throughput_train' '{print $2}'|awk -F ':' '{print $2}'|awk '{print $1}'`
#打印，不需要修改
