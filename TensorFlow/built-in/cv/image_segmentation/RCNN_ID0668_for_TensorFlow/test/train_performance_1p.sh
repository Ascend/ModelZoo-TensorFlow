...
#基础参数，需要模型审视修改
#网络名称，同目录名称
Network=RCNN_ID0668_for_TensorFlow
#训练step
train_steps=3000
#训练batch_size
batch_size=64
...
if [[ $1 == --help || $1 == -h ]];then
    ...
    --log_interval           print log interval
...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
step_sec=(`grep -r perf $cur_path/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log | awk 'END {print $NF}'`)
step_sec=$(awk 'BEGIN{printf "%.4f\n",('$step_sec'/'$log_interval'*'1000')}')
FPS=`awk 'BEGIN {printf "%.2f\n", '1000'*'${batch_size}'/'${step_sec}'}'`
#打印，不需要修改
echo "Final Performance images/sec : $FPS"

#输出训练精度,需要模型审视修改
train_accuracy=(`grep -r accuracy $cur_path/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log | awk -F, '{print $2}' | awk 'END {print $NF}'`)
#打印，不需要修改
...
grep "loss" $cur_path/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log | awk -F, '{print $1}' | awk '{print $NF}'  > $cur_path/output/$ASCEND_DEVICE_ID/${CaseName}_loss.txt
#最后一个迭代loss值，不需要修改
