...
- 参考实现：
        ...
        https://github.com/JimLee4530/RCNN/tree/master/TensorFlow-RCNN
...
-   训练数据集预处理：
    -   获取数据集格式为.mat,对行进行调换位置，随机获取64个元素，然后按照64,32,32,3的图像进行训练
...
- 开始训练。
    ...
    2. 单卡训练
        ...
        2.1 设置单卡训练参数（脚本位于RCNN_ID0668_For_Tensorflow/test/train_performance_1p.sh），示例如下。
        ...
        2.2 单卡训练指令（脚本位于RCNN_ID0668_For_Tensorflow/test/train_performance_1p.sh） 
