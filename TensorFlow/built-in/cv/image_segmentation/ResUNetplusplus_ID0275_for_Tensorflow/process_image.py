...
def center_crop(image, mask, crop_size, size):
    """
    :param :image mask crop_size size
    :return:image mask
    """
    h, w, _ = image.shape
...
def scale_augmentation(image, mask, scale_range, crop_size, size):
    """
    :param :image, mask, scale_range, crop_size, size
    :return:image, mask
    """
...
def random_rotation(image, mask, size, angle_range=(0, 90)):
    """
    :param :image, mask, size, angle_range
    :return:image, mask
    """
    h1, w1, _ = image.shape
    h2, w2, _ = mask.shape
...
def cutout(image_origin, mask_origin, mask_size, mask_value='mean'):
    """
    :param :image_origin, mask_origin, mask_size, mask_value
    :return:image, mask
    """
