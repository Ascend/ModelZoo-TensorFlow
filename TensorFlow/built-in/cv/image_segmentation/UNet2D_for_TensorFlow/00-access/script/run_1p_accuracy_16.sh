...
then
    ...
    do
        ...
        ${currentDir}/exec_1p_16.sh ${i}
        echo "[`date +%Y%m%d-%H:%M:%S`] [INFO] fold${i} train end"
    done

    python3.7 ${currentDir}/../utils/parse_results.py --model_dir ${currentDir}/../results --exec_mode convergence --env TF-AMP_1GPU
else
    echo "[`date +%Y%m%d-%H:%M:%S`] [INFO] fold$fold train start"
    ${currentDir}/exec_1p_16.sh ${fold}
