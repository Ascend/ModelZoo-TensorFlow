...
#基础参数，需要模型审视修改
#网络名称，同目录名称
Network=TCN_ID0022_for_TensorFlow
#训练step
train_steps=1000
#训练batch_size
batch_size=2
...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
step_sec=(`grep -r sec/step $cur_path/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log | awk 'END {print $NF}'`)
...
grep "training loss" $cur_path/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log | awk '{print $NF}' > $cur_path/output/$ASCEND_DEVICE_ID/${CaseName}_loss.txt
#最后一个迭代loss值，不需要修改
