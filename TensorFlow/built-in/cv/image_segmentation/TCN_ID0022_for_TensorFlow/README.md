...
**描述（Description）： 像素转置卷积网络的Tensorflow实现 

<h2 id="概述.md">概述</h2>
-    反卷积层已广泛用于各种用于上采样的深度模型，包括用于语义分割的编码器- 解码器网络和用于无监督学习的深度生成模型。反卷积操作的一个关键限制是它们会导致所谓的棋盘问题。这是因为输出要素图上的相邻像素之间不存在直接关系。为了解决这个问题，我们建议使用像素反卷积层（PixelDCL）在上采样特征图上的相邻像素之间建立直接关系。我们的方法基于对常规反卷积运算的全新解释。生成的PixelDCL可用于以即插即用的方式替换任何反卷积层，而不会影响原始模型的完全可训练功能。所提出的PixelDCL可能会导致效率略有下降，但这可以通过实现技巧来克服。语义分割的实验结果表明，PixelDCL可以考虑边缘和形状等空间特征，并且比反卷积层产生更准确的分割输出。当用于图像生成任务时，我们的PixelDCL可以在很大程度上克服常规反卷积操作所遭受的棋盘问题。
...
- 参考实现：
        ...
        https://github.com/HongyangGao/PixelTCN
...
1. 数据集下载路径（https://github.com/HongyangGao/PixelTCN/tree/master/dataset）

   ```
   
   ```

3. 放入代码目录下，在训练脚本中指定数据集路径，可正常使用。
...
- 开始训练。
    ...
    2. 单卡训练
        ...
        2.1 设置单卡训练参数（脚本位于TCN_ID0022_For_Tensorflow/run_1p.sh），示例如下。
        ...
        2.2 单卡训练指令（脚本位于TCN_ID0022_For_Tensorflow/run_1p.sh） 
...
3. 8卡训练
   ...
   请确保下面例子中的“--data_path”修改为用户生成的tfrecord的实际路径。
   ```
   ` --data_path=/data/slimImagenet`
   ```
...
```
    ...
    --max_step                        total number of epochs to train the model:default: 1000
...
将训练脚本（train_alexnet_1p.sh,train_alexnet_8p.sh）中的data_path设置为训练数据集的路径。具体的流程参见“模型训练”的示例。
