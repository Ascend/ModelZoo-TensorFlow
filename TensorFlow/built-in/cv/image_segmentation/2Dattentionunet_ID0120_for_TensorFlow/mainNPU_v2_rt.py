...
parser.add_argument('--class_balancing', type=str2bool, default=False,
...
parser.add_argument('--brightness', type=float, default=None,
                    ...
                    Specifies the max bightness change.')
parser.add_argument('--rotation', type=float, default=None,
                    help='Whether to randomly rotate the image for data augmentation. Specifies the max rotation angle.')
parser.add_argument('--model', type=str, default='dunet',
                    help='The model you are using. Currently supports:\
                    encoder-decoder, deepUNet,attentionNet, deep, UNet')
######Modify for NPU begin#######
parser.add_argument('--precision_mode', type=str, default='allow_fp32_to_fp16',
                   help='precision mode，allow_fp32_to_fp16/force_fp16/must_keep_origin_dtype/allow_mix_precision, default is allow_fp32_to_fp16')
parser.add_argument('--loss_scale', type=int, default=0,
                    help='The loss scale you are usring. 0: dynamic loss scale; >= 1: fix loss scale.')
parser.add_argument('--data_dir', type=str, default="/home/data", help='Directory to save dataset')
parser.add_argument('--over_dump', type=str2bool, default=False, help='Whether to enable overflow, default is False')
parser.add_argument('--over_dump_path', type=str, default='/home/output/overflow', help='Directory to save overflow files')
parser.add_argument('--data_dump_flag', type=str2bool, default=False, help='Whether to enable op data dump, default is False')
parser.add_argument('--data_dump_step', type=int, default=10, help='step number to dump data')
parser.add_argument('--data_dump_path', type=str, default='/home/output/opdata', help='Directory to save op data dump files')
parser.add_argument('--profiling', type=str2bool, default=False, help='Whether to enable profiling data dump, default is False')
parser.add_argument('--profiling_dump_path', type=str, default='/home/output/profiliSng', help='Directory to save profiling dump files')
parser.add_argument('--autotune', type=str2bool, default=False, help='Whether to enable autotune')
######Modify for NPU end#######
...
def prepare_data(dataset_dir=args.data_dir, type=args.load_data):
    if type:
        (train_input_names, train_output_names, val_input_names, val_output_names, test_input_names, test_output_names)\
            = load_divided(dataset_dir)
    else:
        (train_input_names, train_output_names, val_input_names, val_output_names, test_input_names, test_output_names)\
            = load_nondivided(dataset_dir)
    print('Training data length : {} and validation data length: {}'.format(len(train_input_names), len(val_input_names)))
    (train_input_names.sort(),
     train_output_names.sort(),
     val_input_names.sort(),
     val_output_names.sort(),
     test_input_names.sort(),
     test_output_names.sort())
    return (train_input_names, train_output_names, val_input_names, val_output_names, test_input_names, test_output_names)
...
(class_names_list, label_values) = helpers.get_label_info(os.path.join(args.data_dir, 'class_dict.csv'))
class_names_string = ''
...
if args.class_balancing:
    print('Computing class weights for', args.data_dir, '...')
    class_weights = utils.compute_class_weights(labels_dir=(args.data_dir + '/train_labels'), label_values=label_values)
    if act:
...
if (not os.path.isdir(check)):
    os.makedirs(check)
model_checkpoint_name = ((((check + '/latest_model_') + args.model) + '_') + 'lanshan' + '.ckpt')
...
if (args.mode == 'train'):
    ...
    for epoch in range(0, args.num_epochs):
        ...
        #saver.save(sess, model_checkpoint_name)
        if ((val_indices != 0) and ((epoch % 100) == 0)):
    ...
    print('Final accuracy MAP: %.4f' % avg_scores_per_epoch[-1])
