...
class DualNet(object):
    ...
    def load_random_samples(self):
        #np.random.choice(
        sample_files =np.random.choice(glob('{}/val/A/*.*[g|G]'.format(self.dataset_name)),self.batch_size)
        sample_A_imgs = [load_data(f, image_size =self.image_size, flip = False) for f in sample_files]
        
        sample_files = np.random.choice(glob('{}/val/B/*.*[g|G]'.format(self.dataset_name)),self.batch_size)
    ...
    def read_data(self, a, b):
        imgA = process_data(a, image_size=self.image_size, flip=self.flip)
        imgB = process_data(b, image_size=self.image_size, flip=self.flip)
        return imgA, imgB
    ...
    def train(self, args):
        ...
        self.g_optim = npu_tf_optimizer(tf.train.RMSPropOptimizer(args.lr, decay=decay)) \
    ...
    def test_domain(self, args, test_log, type = 'A'):
        test_files = glob('{}/val/{}/*.*[g|G]'.format(self.dataset_name,type))
        # load testing input
