...
#输出性能FPS，需要模型审视修改
#FPS=`grep -a 'FPS:'  $cur_path/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|awk -F '[:| ]' '{print $13}'|awk 'END {print}'`
...
DeviceType=`uname -m`
CaseName=${Network}${name_bind}_bs${BatchSize}_${RANK_SIZE}'p'_'pref'
