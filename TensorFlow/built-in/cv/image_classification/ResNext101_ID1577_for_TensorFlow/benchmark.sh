...
function exec_train() {
    ...
    FPS=`grep "hooks.py:141"  $currentDir/output/1/train_1.log |awk -F "," 'END {print $4}' |awk -F ":" '{print $2}' | awk -F "." '{print $1}'`
    train_accuracy=`grep "accuracy =" $currentDir/output/1/train_1.log|awk -F "," 'END {print $1}'|awk -F " " 'END {print $NF}'`
