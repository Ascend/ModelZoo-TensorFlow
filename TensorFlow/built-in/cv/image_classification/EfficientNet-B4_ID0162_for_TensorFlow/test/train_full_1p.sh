...
#基础参数，需要模型审视修改
#网络名称，同目录名称
Network="EfficientNetB4_ID0162_for_TensorFlow"
#训练epoch
train_epochs=350
#训练batch_size
batch_size=32
...
do
    ...
    nohup python3 ${cur_path}/../main_npu.py \
         ...
         --model_name=efficientnet-b4  > $cur_path/output/$ASCEND_DEVICE_ID/eval_$ASCEND_DEVICE_ID.log 2>&1 &
...
#输出训练精度,需要模型审视修改
train_accuracy=`grep -a 'top_1_accuracy ' ${cur_path}/output/${ASCEND_DEVICE_ID}/eval_${ASCEND_DEVICE_ID}.log|awk 'END {print $19}'|cut -d "," -f 1`
#打印，不需要修改
...
TrainingTime=`awk -v x="1" -v y="$FPS" 'BEGIN{printf "%.2f\n",x/y}'`
...
do
    ...
    #创建DeviceID输出目录，不需要修改
    if [ -d ${cur_path}/output/${ASCEND_DEVICE_ID} ];then
        cd ${cur_path}/output/$ASCEND_DEVICE_ID/ckpt
        rm -rf * 
    fi
