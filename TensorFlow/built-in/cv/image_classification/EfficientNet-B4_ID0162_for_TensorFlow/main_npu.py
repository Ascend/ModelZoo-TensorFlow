...
def model_fn(features, labels, mode, params):
  ...
  if is_training:
    ...
    if FLAGS.use_tpu:
      ...
      # user, this should look like regular synchronous training. 0
      optimizer = tf.contrib.tpu.CrossShardOptimizer(optimizer)

    # for NPU
    tf.identity(learning_rate, name='learning_rate')
    
    if int(os.getenv('RANK_SIZE')) == 8:
      optimizer = NPUDistributedOptimizer(optimizer)

    #loss_scale_manager = FixedLossScaleManager(loss_scale=1) #enable_overflow_check=False
    loss_scale_manager = ExponentialUpdateLossScaleManager(init_loss_scale=2**8,incr_every_n_steps=1000,
  ...
  return NPUEstimatorSpec(
    ...
    scaffold=tf.train.Scaffold(saver=tf.train.Saver(restore_vars_dict)) if not is_training else None)
  # for NPU
...
def main(unused_argv):
  ...
  # for NPU
  dump_dir = FLAGS.model_dir
  dump_config = DumpConfig(enable_dump_debug=True,dump_path=dump_dir,dump_debug_mode="all")
  #dump_config = DumpConfig(enable_dump=True,dump_path=dump_dir,dump_step="0|5|8",dump_mode="all")

  estimator_config = tf.ConfigProto()
  #npu modify start
  custom_op =  estimator_config.graph_options.rewrite_options.custom_optimizers.add()
  custom_op.name =  "NpuOptimizer"
  custom_op.parameter_map["use_off_line"].b = True
  custom_op.parameter_map["enable_data_pre_proc"].b = True
  custom_op.parameter_map["precision_mode"].s = tf.compat.as_bytes("allow_fp32_to_fp16")
  estimator_config.graph_options.rewrite_options.remapping = RewriterConfig.OFF
  #npu modify end
  gpu_thread_count = 1
  ...
  # for NPU hcom_parallel=True,dump_config=dump_config,modify_mixlist="ops/ops_info.json",fusion_switch_file="./fusion_switch.json"
  ...
  # for NPU
  params = dict(
    steps_per_epoch=FLAGS.num_train_images // (FLAGS.train_batch_size * int(os.getenv('RANK_SIZE'))),
    batch_size=FLAGS.eval_batch_size if FLAGS.mode == 'eval' else FLAGS.train_batch_size,
    use_bfloat16=False)
  est = NPUEstimator(
    model_fn=model_fn,
    config=config,
  ...
  def build_imagenet_input(is_training):
    ...
    print("looooooooooooooooooooooook at me", FLAGS.data_dir)
  ...
  if FLAGS.mode == 'eval':
    ...
    # for NPU
    # evaluate the latest checkpoint in model_dir
    ...
    config_logger = {
        'num_training_samples': FLAGS.num_train_images,
        # for 1p, just per loop print, for 8p, print each epoch params['steps_per_epoch']
        'display_every': 100,
        'log_name': 'train_log.log',
        'log_dir': FLAGS.model_dir,
        'global_batch_size': FLAGS.train_batch_size * int(os.getenv('RANK_SIZE')),
        'iterations_per_loop': FLAGS.iterations_per_loop
    }

    hooks.append(LogSessionRunHook(config_logger))
    # for logger
    ...
    if FLAGS.mode == 'train':
      ...
      while current_step < FLAGS.train_steps:
        ...
        # for NPU
        # we use the same batch size for evaluate in train_and_eval mode for simplicity
        FLAGS.eval_batch_size = FLAGS.train_batch_size
        est.train(input_fn=imagenet_train.input_fn, max_steps=next_checkpoint, hooks=hooks)
        # for NPU
