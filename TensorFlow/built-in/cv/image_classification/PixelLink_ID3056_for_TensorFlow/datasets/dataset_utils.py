...
def convert_to_example(image_data, filename, labels, labels_text, bboxes, oriented_bboxes, shape):
    ...
    example = tf.train.Example(features=tf.train.Features(feature={
            ...
            'image/object/bbox/label': int64_feature(labels),
            'image/object/bbox/label_text': bytes_feature([item.encode() for item in labels_text]),      #edit 2022/7/14
            'image/format': bytes_feature(image_format),
            'image/filename': bytes_feature([item.encode() for item in [filename]]),               #edit 2022/7/14
            'image/encoded': bytes_feature(image_data)}))
    return example
...
def my_get_split(split_name, dataset_dir, file_pattern, num_samples, reader=None):         #edit 2022/7/18 for dataset
    dataset_dir = util.io.get_absolute_path(dataset_dir)
    ...
    dataset = collections.namedtuple(
        'dataset', ['data_sources', 'keys_to_features', 'items_to_handlers', 'num_samples',
                    'items_to_descriptions', 'num_classes', 'labels_to_names'
                    ])
    return dataset(
        data_sources=file_pattern,
