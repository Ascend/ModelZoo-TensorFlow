...
def load_zip_file(file,fileNameRegExp='',allEntries=False):
    ...
    try:
        #print("-----------Zip file", file)
        archive=zipfile.ZipFile(file, mode='r', allowZip64=True)
        #print("-----------archiv file", archive, archive.namelist())
    except :
        raise Exception('Error loading the ZIP archive')    
    ...
    for name in archive.namelist():
        ...
        keyName = name
        if fileNameRegExp!="":
            #print("---------------fileNameRegExp", fileNameRegExp, name)
            m = re.match(fileNameRegExp,name)
            if m == None:
                addFile = False
            else:
...
def main_evaluation(p,default_evaluation_params_fn,validate_data_fn,evaluate_method_fn,show_result=True,per_sample=True,para=None):
    """
    ...
    """
    sys.argv[1:] = ['-g=gt.zip', '-s=submit.zip', '-o=./', '-p=', '-encodedCommand', 'XAAiAEkATwBVAF8AQwBPAE4AUwBUAFIAQQBJAE4AVABcACIAOgAwAC4AOAA=', '-inputFormat', 'xml', '-outputFormat', 'text']
    sys.argv[1:] = ['–g=gt.zip', '–s=submit-2s.zip', '–o=./', '-p={\"IOU_CONSTRAINT\":0.8}']
    sys.argv[1:] = para
    if (p == None):
