...
cd $cur_path/../official/r1/resnet
...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
FPS=`grep "tensorflow:steps" $cur_path/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|awk -F 'FPS:' '{print $2}'|cut -c 1-7|awk 'NR>2'|awk '{sum+=$1} END {print  sum/NR}'`
#打印，不需要修改
...
grep 'loss =' $cur_path/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|awk -F 'loss =' '{print $2}'|tr -d ',' |awk '{print $1}' > $cur_path/output/$ASCEND_DEVICE_ID/train_${CaseName}_loss.txt
#最后一个迭代loss值，不需要修改
