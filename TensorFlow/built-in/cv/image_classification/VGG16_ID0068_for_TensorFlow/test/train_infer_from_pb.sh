...
#sed -i "s|JPEG|jpeg|g" ${data_path}/valb_lable.txt
#训练开始时间，不需要修改
start_time=$(date +%s)
cd $cur_path/../
#进入训练脚本目录，需要模型审视修改
...
do
    ...
    nohup python3.7 $cur_path/../infer_from_pb.py \
    --batchsize=${batch_size} \
    --model_path=${ckpt_path}/dataset/vgg16_tf_910.pb \
    --image_path=${ckpt_path}/dataset/photo \
    --label_file=${ckpt_path}/dataset/val_lable.txt > ${cur_path}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log 2>&1 &
...
#获取精度
accuracy=`grep accuracy $cur_path/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log|head -n+1|awk -F " " '{print $3}'`
echo "top1 accurancy : $accurancy"
#输出性能FPS，需要模型审视修改
