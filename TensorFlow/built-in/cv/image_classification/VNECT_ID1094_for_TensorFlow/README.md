...
**描述（Description）：基于TensorFlow框架的VNECT网络训练代码**

<h2 id="概述.md">概述</h2>

-   VNECT网络使用单RGB摄像机进行实时三维人体姿态估计，基于骨架模型，利用卷积神经网络进行学习，可估算出骨架姿态与动作。 
...
-   网络结构
    -   优化器：AdadeltaOptimizer
    -   单卡batchsize：8
    -   总step数：1000

-   训练数据集预处理：
    -   模型使用 MPI-INF-3DHP 数据集，请用户自行下载
...
1. 模型训练使用 MPI-INF-3DHP 数据集，数据集请用户自行获取。
2. 获取数据集后，放入模型目录下，在训练脚本中指定数据集路径，可正常使用。数据集文件结构示例如下：
```
    ├── VNect_dataset
    │   ├── mpi_inf_3dhp   // MPI-INF-3DHP数据集文件夹
    则 data_path=./VNect_dataset 即可
```
...
- 开始训练
    ...
    2. 单卡训练
        ...
        2.1 设置单卡训练参数（脚本位于./VNECT_ID1094_for_TensorFlow/test/train_full_1p.sh），示例如下。
```
            # 训练epoch
            train_epochs=1
            # 训练batch_size
            ...
            train_steps=1000
            # 数据集路径（示例）
            data_path=./VNect_dataset
```
        2.2 单卡训练指令（脚本位于./VNECT_ID1094_for_TensorFlow/test/train_full_1p.sh）示例如下。    
```
            bash train_full_1p.sh --data_path=./VNect_dataset
```
