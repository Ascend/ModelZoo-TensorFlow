...
do
    ...
    nohup python3 ${cur_path}/../train_8p_rev.py \
        --log_dir=${cur_path}/output/$ASCEND_DEVICE_ID/log \
        --num_point=2048 \
        --batch_size=32 \
        --learning_rate=0.009 \
        --max_epoch=41 > ${cur_path}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log 2>&1 &     
...
do
  ...
  ${bind_core} python3 ${cur_path}/../evaluate.py \
        --num_point=2048 \
        --model_path=${cur_path}/output/$ASCEND_DEVICE_ID/log/model.ckpt > ${cur_path}/output/$ASCEND_DEVICE_ID/eval.log 2>&1
done
wait
...
shapes=`awk -v x=40 -v y=2048 'BEGIN{printf "%.2f\n",x*y}'`
FPS=`awk -v x="$shapes" -v y="$FPS" 'BEGIN{printf "%.2f\n",x*y}'`
#打印，不需要修改
echo "Final Performance images/sec : $FPS"

#输出训练精度,需要模型审视修改
train_accuracy=`grep -a 'eval accuracy: ' ${cur_path}/output/${ASCEND_DEVICE_ID}/eval.log|awk 'END {print $3}'`
#打印，不需要修改
