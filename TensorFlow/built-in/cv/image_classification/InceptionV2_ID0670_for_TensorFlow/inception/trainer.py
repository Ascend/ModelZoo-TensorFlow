...
from npu_bridge.estimator.npu import npu_hook
...
class Trainer(object):
    ...
    def get_npu_classifier(self):
        ...
        custom_op.parameter_map["iterations_per_loop"].i = self.args.iterations_per_loop
        custom_op.parameter_map["hcom_parallel"].b =True
        sess_config.graph_options.rewrite_options.remapping= RewriterConfig.OFF
        sess_config.graph_options.rewrite_options.memory_optimization= RewriterConfig.OFF
        #modify NPU custom_op end
        ...
        run_config = tf.estimator.RunConfig(model_dir=self.args.log_dir, session_config=sess_config,
                            save_checkpoints_steps=self.args.nsteps_per_epoch * 10,
                            keep_checkpoint_max=5)
        ...
        classifier = tf.estimator.Estimator(model_fn=self.model.get_estimator_model_func, config=npu_run_config_init(run_config=run_config))
      
        training_hooks = []
        # npu set iterations_per_loop
        training_hooks.append(npu_hook.SetIterationsVarHook(self.args.iterations_per_loop))
