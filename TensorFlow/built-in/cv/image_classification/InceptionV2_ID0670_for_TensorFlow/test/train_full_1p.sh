...
if [[ $1 == --help || $1 == --h ]];then
   echo "usage:./train_full_1p.sh --data_path=../VCTK-Corpus"
   exit 1
fi
for para in $*
do
...
train_accuracy=`grep -A 1 top1 $cur_path/test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|awk 'END {print $3}'`
##获取性能数据，不需要修改
#吞吐量
ActualFPS=${FPS}
#单迭代训练时长
