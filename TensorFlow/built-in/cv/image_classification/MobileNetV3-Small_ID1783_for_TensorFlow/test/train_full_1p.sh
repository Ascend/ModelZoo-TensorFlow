...
#基础参数，需要模型审视修改
#网络名称，同目录名称
Network="MobileNetV3-Small_ID1783_for_TensorFlow"
#训练epoch
train_epochs=90
#训练batch_size
batch_size=256
...
do
    ...
    # echo `ls -l $cur_path/../`
    echo `ls -l $cur_path/../ | awk '{print $NF}' `
    echo "======================================$ckpt_path     ls -l $cur_path/../results | awk '{print $NF}' | tail -2 | head -1 | awk -F '.' '{print $2}'"
...
python3.7 $cur_path/../eval_image_classifier_mobilenet.py \
    ...
    --model_name="mobilenet_v3_small"  > ${cur_path}/output/${ASCEND_DEVICE_ID}/eval_${ASCEND_DEVICE_ID}.log 2>&1 
...
echo `ls -l $cur_path/../ | awk '{print $NF}' `
echo `ls -l $cur_path/../ | awk '{print $NF}' ` >> $cur_path/output/$ASCEND_DEVICE_ID/${CaseName}.log
