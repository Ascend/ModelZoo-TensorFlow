...
**描述（Description）：基于TensorFlow框架的NASNet网络训练代码**

<h2 id="概述.md">概述</h2>
-     Nasnet 是 ImageNet 数据集上最先进的图像分类架构（ArXiv 发布日期为 2017 年 7 月 21 日），据报道，Nasnet 大型模型的单一作物精度为 82.7。
...
2. 创建目录data，下载cifar-10-python.tar.gz到data目录；

3. 解压后，将得到的目录重命名为cifar-10-batches-py；

4. 退出data目录，并执行：

   ```
   python3 convert_customized_data.py --dataset_name cifar10 --dataset_dir data
   ```
