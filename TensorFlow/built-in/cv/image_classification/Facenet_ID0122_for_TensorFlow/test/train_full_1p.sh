...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
TrainingTime=`grep  RegLoss $cur_path/test/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log| tail -n +2 | head -4999 | awk '{print $4}' | tr -d s | awk '{sum+=$1} END {print sum/NR}'`
wait
...
bst_e=${bst_e%%]*}
bst_e=${bst_e#*[}
cp ${cur_path}/src/models/${ASCEND_DEVICE_ID}/*/*ckpt-${bst_e}* ${saved_model_path}
