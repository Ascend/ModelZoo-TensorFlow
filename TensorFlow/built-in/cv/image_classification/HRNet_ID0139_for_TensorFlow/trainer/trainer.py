...
class Trainer():
    ...
    def __init__(self, data_path, netcfg):
        ...
        ####################NPU modify start#####################
        #self.model_path = '/autotest/CI_daily/CarPeting_HRNet/00-access/models'
        self.model_path = FLAGS.model_path
        ####################NPU modify end#####################
        #self.model_path = './models'
        ####################NPU modify start#####################
        #self.log_path = '/autotest/CI_daily/CarPeting_HRNet/00-access/logs'
        self.log_path = FLAGS.model_path
        ####################NPU modify end#####################
        #self.log_path = './logs'
    ...
    def build_graph(self, is_train):
        with tf.Graph().as_default():
            ...
            ####################NPU modify start#####################
            #custom_op.parameter_map["dump_path"].s = tf.compat.as_bytes("/autotest/CI_daily/CarPeting_HRNet/00-access/dump_output")
            #custom_op.parameter_map["enable_dump_debug"].b = True
            #custom_op.parameter_map["dump_debug_mode"].s = tf.compat.as_bytes("all")
            if FLAGS.over_dump:
                custom_op.parameter_map["dump_debug_mode"].s = tf.compat.as_bytes(FLAGS.over_dump_path)
                custom_op.parameter_map["enable_dump_debug"].b = True
            ...
            ####################NPU modify end#####################
            custom_op.parameter_map["use_off_line"].b = True # 必须显示开启
            config.graph_options.rewrite_options.remapping = RewriterConfig.OFF  # 必须显示关闭
            ...
            with tf.variable_scope(self.model_scope):
                ...
                if is_train:
                    ...
                    #loss_scale_manager = FixedLossScaleManager(loss_scale=100) #static
            ...
            if is_train:
                ...
                if FLAGS.enbl_multi_gpu:
                    ...
                    self.bcast_op = hccl_ops.broadcast(input, 0)
    ...
    def setup_lrn_rate(self, global_step):
        ...
        batch_size = FLAGS.batch_size * (1 if not FLAGS.enbl_multi_gpu else get_rank_size())
        ####################NPU modify end#####################
        lrn_rate = setup_lrn_rate_piecewise_constant(global_step, self.lr_init, batch_size, idxs_epoch, decay_rates)
        #nb_iters = 100 #int(FLAGS.nb_smpls_train * nb_epochs * nb_epochs_rat / batch_size)
