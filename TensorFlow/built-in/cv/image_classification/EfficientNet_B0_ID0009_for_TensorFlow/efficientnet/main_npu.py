...
flags.DEFINE_string('restore_path', default='',
                    help='restore path')

flags.DEFINE_list('restore_exclude', default=['efficientnet-b0/model/head/dense/'],
...
def model_fn(features, labels, mode, params):
    ...
    if FLAGS.restore_path:
        print("restore ckpt for finetune. num_label_classes: {}, restore_path: {}".format(
            FLAGS.num_label_classes, FLAGS.restore_path))
        variables_to_restore = tf.contrib.slim.get_variables_to_restore(exclude=FLAGS.restore_exclude)
        tf.train.init_from_checkpoint(FLAGS.restore_path,
...
def main(unused_argv):
    ...
    config = NPURunConfig(
        ...
        modify_mixlist='ReduceMeanD.json',
