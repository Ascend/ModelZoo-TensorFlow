...
def model_fn(features, labels, mode, params):
    ...
    is_training_ = tf.constant(is_training, dtype=tf.bool)
    logits = shufflenet(
        features['images'], is_training_,
        num_classes=params['num_classes'],
        groups=params['group'],
        dropout=params['dropout'],
        complexity_scale_factor=params['complexity_scale_factor']
    )
    ...
    if mode == tf.estimator.ModeKeys.EVAL:
        eval_metric_ops = {
            'accuracy': tf.metrics.accuracy(tf.cast(labels['labels'], tf.float32), predictions['classes']),
...
def add_weight_decay(weight_decay):
    ...
    weights = [
        v for v in tf.trainable_variables() if 'kernel' in v.name and 'depthwise_kernel' not in v.name
        ]
    for w in weights:
...
class RestoreMovingAverageHook(tf.train.SessionRunHook):
    ...
    def begin(self):
        if tf.train.latest_checkpoint(self.model_dir) is not None:
            tf.logging.info('________________________________:LOAD PRETRAINED_MODEL Successfully!!!')
    ...
    def after_create_session(self, sess, coord):
        ...
        if tf.train.latest_checkpoint(self.model_dir) is not None:
            tf.logging.info(tf.train.latest_checkpoint(self.model_dir))
            self.load_ema(sess)
