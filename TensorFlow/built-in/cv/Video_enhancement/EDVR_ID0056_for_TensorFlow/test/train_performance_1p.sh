...
do
    ...
    nohup python3.7 $cur_path/../tools/main.py --config-file $cur_path/../configs/edvr.yaml solver.checkpoint_interval 5000 solver.print_interval 100 rank_size 1 > ${cur_path}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log 2>&1 &
...
#参数还原
sed -i "s|${data_path}/|data/|g" $cur_path/../ascendvsr/config/defaults.py
sed -i "s|cfg.solver.print_interval = 100|cfg.solver.print_interval = 20|g" $cur_path/../ascendvsr/config/defaults.py
sed -i "s|${train_steps}|2000|g" $cur_path/../configs/edvr.yaml

#结果打印，不需要修改
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
FPS=`grep 'fps:' $cur_path/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log| awk -F "fps:" '{print $2}' | awk -F "," '{print $1}' | tail -n +2| awk '{sum+=$1} END {print sum/NR}'`
#打印，不需要修改
