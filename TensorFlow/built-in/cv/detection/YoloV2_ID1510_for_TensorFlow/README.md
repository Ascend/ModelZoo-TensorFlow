...
**描述（Description）：基于TensorFlow框架的yolov2目标检测算法训练代码**

<h2 id="概述.md">概述</h2>

-    yolov2是传统的one-stage检测算法，它在yolov1的基础上通过提高分辨率，引入anchorbox，使用batchnorm等一系列手段，有效提升了检测的准确率。
...
- 参考实现：
    ...
    [https://github.com/shishichang/yolov2-tensorflow](https://github.com/shishichang/yolov2-tensorflow)
...
-   网络结构
    ...
    - 初始学习率为0.0001，使用exponential_decay
...
1. 模型训练使用PASCAL_VOC2007数据集，数据集请用户自行获取。
...
- 开始训练。
    ...
    2. 单卡训练
        ...
        2.1 设置单卡训练参数（脚本位于YoloV2_ID1510_for_TensorFlow/test/train_full_1p.sh），示例如下。
        ...
        2.2 单卡训练指令（脚本位于YoloV2_ID1510_for_TensorFlow/test） 

        ```
        于终端中运行export ASCEND_DEVICE_ID=0 (0~7)以指定单卡训练时使用的卡
        bash train_full_1p.sh --data_path=xx
        数据集应有如下结构，配置data_path时需指定为voc这一层，例：--data_path=/home/voc
        ├─voc
           ├─Annotations
           ├─ImageSets
           ├─JPEGImages
