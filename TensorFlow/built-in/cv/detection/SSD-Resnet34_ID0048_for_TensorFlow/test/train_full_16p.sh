...
# export RANK_TABLE_FILE=${cur_path}/../npu_config/8p.json
...
do
    ...
    # fi
    python3.7 ${cur_path}/../ssd_main.py --mode=train_and_eval \
    --train_batch_size=${batch_size} \
    --training_file_pattern=${data_path}/coco_official_2017/tfrecord/train2017* \
    --resnet_checkpoint=${data_path}/resnet34_pretrain/model.ckpt-28152 \
    --validation_file_pattern=${data_path}/coco_official_2017/tfrecord/val2017* \
...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
FPS=`cat ${cur_path}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log | grep "\] FPS:" | awk -F "FPS: " '{print $2}' | awk -F "," '{print $1}' | tail -n +2 | awk '{sum+=$1} END {print sum/NR}'`
#打印，不需要修改
echo "Final Performance images/sec : $FPS"

#输出训练精度,需要模型审视修改
train_accuracy=`grep Current ${cur_path}/output/0/train_0.log|awk 'END {print $3}'`
#打印，不需要修改
...
grep "\] FPS:" $cur_path/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log | awk -F "loss: " '{print $2}' >> $cur_path/output/$ASCEND_DEVICE_ID/train_${CaseName}_loss.txt
