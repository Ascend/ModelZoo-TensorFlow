...
#基础参数 需要模型审视修改
#网络名称，同目录名称
Network="YoloV3_ID0076_for_TensorFlow"
#训练epoch
train_epochs=200
...
#sed -i "s/total_epoches = 200/total_epoches = 10/g" ${cur_path}/../args_single.py
sed -i "s|cocoGt_file = '/opt/npu/dataset/coco/coco2014/annotations/instances_val2014.json'|cocoGt_file = '$data_path/coco/coco2014/annotations/instances_val2014.json'|g" ${cur_path}/../eval_coco.py
...
do
    ...
    #cp -r $data_path/darknet53* ${cur_path}/../data/darknet_weights/
    mkdir -p ${cur_path}/../training/t1/D$ASCEND_DEVICE_ID/training/
    ...
    ${bind_core} python3 train.py \
        ...
        --save_dir ${cur_path}/../training/t1/D$ASCEND_DEVICE_ID/training/ > ${cur_path}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log 2>&1 &
...
RESTORE_PATH=./training/t1/D0/training/
python3.7 eval.py \
--save_json True \
--score_thresh 0.0001 \
--nms_thresh 0.55 \
...
--max_test 10000 \
--save_json_path eval_res_D0.json > test/eval.log 2>&1
wait

#sed -i "s/total_epoches = 10/total_epoches = 200/g" ${cur_path}/../args_single.py
sed -i "s|cocoGt_file = '$data_path/coco/coco2014/annotations/instances_val2014.json'|cocoGt_file = '/opt/npu/dataset/coco/coco2014/annotations/instances_val2014.json'|g" ${cur_path}/../eval_coco.py
...
grep "loss" FPS.log | awk 'END {print $12}' |tr -d , >> $cur_path/output/$ASCEND_DEVICE_ID/train_${CaseName}_loss.txt

#最后一个迭代loss值，不需要修改
ActualLoss=`awk 'END {print}' $cur_path/output/$ASCEND_DEVICE_ID/train_${CaseName}_loss.txt`

train_accuracy=`tail -n15 test/eval.log |grep AP|awk '{print $13}'|awk 'NR==1'`
