...
class MultiWorkerExecuter(DistributedExecuter):
  ...
  def build_strategy_configuration(self):
    ...
    if self._flags.over_dump == 'True':
        print("NPU overflow dump is enabled")
        dump_config = DumpConfig(
                enable_dump_debug=True, dump_path=self._flags.over_dump_path, dump_debug_mode="all")
  ...
  def build_mask_rcnn_estimator(self, params, run_config, mode):
    ...
    cpu_run_config = NPURunConfig(jit_compile=False, model_dir=self._flags.model_dir, precision_mode="allow_mix_precision")
