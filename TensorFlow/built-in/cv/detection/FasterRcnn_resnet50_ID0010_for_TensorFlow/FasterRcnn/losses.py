...
def _rpn_score_loss_origin(score_outputs, score_targets, normalizer=1.0):
  """Computes score loss."""
...
def _rpn_score_loss(score_outputs, score_targets, normalizer=1.0):
  ...
  # the anchor is don't care (ignore).
  with tf.name_scope('rpn_score_loss'):
    weights = tf.greater_equal(score_targets, 0)
    weights = tf.cast(weights, tf.float32)
    score_targets = tf.maximum(score_targets, tf.zeros_like(score_targets))
    score_loss = _sigmoid_cross_entropy(
          score_targets, score_outputs, weights)
    score_loss /= normalizer
    return score_loss
...
def _huber_loss(y_true, y_pred, weights, delta):
  num_non_zeros = tf.math.reduce_sum(weights)
  y_true = tf.expand_dims(y_true, axis=-1)
  y_pred = tf.expand_dims(y_pred, axis=-1)

  huber_keras_loss = tf.keras.losses.Huber(delta=delta, name='huber_loss')
  huber_loss = huber_keras_loss(y_true, y_pred)
  huber_loss = tf.math.multiply(huber_loss, weights)
  huber_loss = tf.math.reduce_sum(input_tensor=huber_loss)
