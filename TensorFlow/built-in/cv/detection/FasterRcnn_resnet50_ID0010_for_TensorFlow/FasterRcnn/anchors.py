...
class AnchorLabeler(object):
  ...
  def _get_rpn_samples(self, match_results):
    ...
    positive_labels = tf.where(
        tf.logical_and(samples, labels),
        tf.constant(2, dtype=tf.float32, shape=match_results.shape),
        tf.constant(0, dtype=tf.float32, shape=match_results.shape))
    negative_labels = tf.where(
        tf.logical_and(samples, tf.logical_not(labels)),
        tf.constant(1, dtype=tf.float32, shape=match_results.shape),
        tf.constant(0, dtype=tf.float32, shape=match_results.shape))
    ignore_labels = tf.fill(match_results.shape, -1.0)
