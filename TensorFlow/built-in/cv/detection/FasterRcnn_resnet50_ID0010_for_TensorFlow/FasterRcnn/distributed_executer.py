...
class DistributedExecuter(object):
  ...
  def train(self,
    ...
    train_estimator = self.build_mask_rcnn_estimator(params, run_config,
                                                     'train')
    if True: #self._model_params.use_tpu:
...
class TPUEstimatorExecuter(DistributedExecuter):
  ...
  def build_strategy_configuration(self):
    ...
    run_config = tf.estimator.tpu.RunConfig(
        ...
        model_dir=self._flags.model_dir,
        log_step_count_steps=self._model_params.log_step_count_steps,
        keep_checkpoint_max=self._model_params.keep_checkpoint_max,
        save_checkpoints_steps=self._model_params.save_checkpoints_steps,
...
class MultiWorkerExecuter(DistributedExecuter):
  ...
  def build_strategy_configuration(self):
    ...
    #dump_config = DumpConfig(enable_dump=True, dump_path=os.path.join('/npu/dump/', timestamp), dump_mode="all")
    dump_config = DumpConfig(enable_dump=True, dump_path=timestamp, dump_mode="all")
    # 溢出检测    #dump_config = DumpConfig(enable_dump_debug=True, dump_path=timestamp, dump_debug_mode="all")
    #run_config = NPURunConfig(model_dir=self._flags.model_dir, precision_mode="force_fp16", dump_config=dump_config)
    #run_config = NPURunConfig(model_dir=self._flags.model_dir, precision_mode="allow_mix_precision", dump_config=dump_config)
    run_config = NPURunConfig(
        hcom_parallel=True,
        model_dir=self._flags.model_dir,
        precision_mode=self._model_params.npu_precision_mode,
        iterations_per_loop=self._model_params.iterations_per_loop,
        # tf_random_seed=0,
    ...
    #     tf_random_seed=0,
    # )
    #run_config = NPURunConfig(model_dir=self._flags.model_dir, precision_mode="force_fp16")
    return run_config
  ...
  def build_mask_rcnn_estimator(self, params, run_config, mode):
    ...
    timestamp = time.strftime('%Y%m%d%H%M%S', time.gmtime())
    dump_config = DumpConfig(enable_dump=True, dump_path=timestamp, dump_mode="all")
    #cpu_run_config = NPURunConfig(model_dir=self._flags.model_dir, precision_mode="force_fp16", dump_config=dump_config)
    #cpu_run_config = NPURunConfig(model_dir=self._flags.model_dir, precision_mode="allow_mix_precision", dump_config=dump_config)
    #cpu_run_config = NPURunConfig(model_dir=self._flags.model_dir, precision_mode="allow_mix_precision")
    #cpu_run_config = NPURunConfig(model_dir=self._flags.model_dir, precision_mode="force_fp16")
    cpu_run_config = NPURunConfig(model_dir=self._flags.model_dir, precision_mode=self._model_params.npu_precision_mode)
    return NPUEstimator(
        model_fn=self._model_fn,
        model_dir=self._flags.model_dir,
