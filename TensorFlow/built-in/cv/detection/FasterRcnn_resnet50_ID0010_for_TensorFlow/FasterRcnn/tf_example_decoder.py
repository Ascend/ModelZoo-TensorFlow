...
def _get_source_id_from_encoded_image(parsed_tensors, ext=''):
  return tf.strings.as_string(
      tf.strings.to_hash_bucket_fast(parsed_tensors['image{}/encoded'.format(ext)],
                                     2**63 - 1))
...
class TfExampleDecoder(object):
  ...
  def _decode_image(self, parsed_tensors, ext=''):
    """Decodes the image and set its static shape."""
    image = tf.io.decode_image(parsed_tensors['image{}/encoded'.format(ext)], channels=3)
    image.set_shape([None, None, 3])
    return image

  def _decode_boxes(self, parsed_tensors, ext=''):
    """Concat box coordinates in the format of [ymin, xmin, ymax, xmax]."""
    xmin = parsed_tensors['image{}/object/bbox/xmin'.format(ext)]
    xmax = parsed_tensors['image{}/object/bbox/xmax'.format(ext)]
    ymin = parsed_tensors['image{}/object/bbox/ymin'.format(ext)]
    ymax = parsed_tensors['image{}/object/bbox/ymax'.format(ext)]
    return tf.stack([ymin, xmin, ymax, xmax], axis=-1)

  def _decode_masks(self, parsed_tensors, ext=''):
    ...
    height = parsed_tensors['image{}/height'.format(ext)]
    width = parsed_tensors['image{}/width'.format(ext)]
    masks = parsed_tensors['image{}/object/mask'.format(ext)]
    return tf.cond(
  ...
  def _decode_areas(self, parsed_tensors, ext=''):
    xmin = parsed_tensors['image{}/object/bbox/xmin'.format(ext)]
    xmax = parsed_tensors['image{}/object/bbox/xmax'.format(ext)]
    ymin = parsed_tensors['image{}/object/bbox/ymin'.format(ext)]
    ymax = parsed_tensors['image{}/object/bbox/ymax'.format(ext)]
    height = tf.cast(parsed_tensors['image{}/height'.format(ext)], dtype=tf.float32)
    width = tf.cast(parsed_tensors['image{}/width'.format(ext)], dtype=tf.float32)
    return tf.cond(
        tf.greater(tf.shape(parsed_tensors['image{}/object/area'.format(ext)])[0], 0),
        lambda: parsed_tensors['image{}/object/area'.format(ext)],
