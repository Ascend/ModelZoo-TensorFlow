...
def main(argv=None):
    ...
    with tf.device('/gpu:%d' % gpu_id):
        with tf.name_scope('model_%d' % gpu_id) as scope:
            ...
            #total_loss, model_loss, rpn_cross_entropy = model.loss(bbox_pred, cls_pred, input_bbox,
                                                                                 input_im_info)
    ...
    #    batch_norm_updates_op = tf.group(*tf.get_collection(tf.GraphKeys.UPDATE_OPS, scope))
    #    grads = opt.compute_gradients(total_loss)
    labels_logits = tf.get_collection('label_logits')
    ...
    with tf.Session(config=config) as sess:
        ...
        writer = tf.summary.FileWriter("output.pbtxt", sess.graph)
        writer.close()
        ...
        for step in range(restore_step, FLAGS.max_steps):
            ...
            ml, tl, _, summary_str,labels_logits_val = sess.run([model_loss, total_loss, train_op, summary_op,labels_logits],
                                              feed_dict={input_image: data[0],
                                                         input_bbox: data[1],
                                                         input_im_info: data[2]},
                                              options=options,
                                              run_metadata=run_metadata)
            np.save('input_image.npy',data[0])
            np.save('input_bbox.npy',data[1])
            np.save('input_im_info.npy',data[2])
        ...
        #labels_logits_val = sess.run(labels_logits)
        fetched_timeline = timeline.Timeline(run_metadata.step_stats)
        chrome_trace = fetched_timeline.generate_chrome_trace_format()
        with open('timeline_01.json', 'w') as f:
             f.write(chrome_trace)

        print('labels shape',len(labels_logits_val[0][0]))   
        print('labels shape',labels_logits_val[0][0].shape)   
        print('logits shape', len(labels_logits_val[0][1])) 
        print('logits shape', labels_logits_val[0][1].shape) 
        np.save('labels.npy',np.array(labels_logits_val[0][0]))
        np.save('logits.npy',np.array(labels_logits_val[0][1]))  
