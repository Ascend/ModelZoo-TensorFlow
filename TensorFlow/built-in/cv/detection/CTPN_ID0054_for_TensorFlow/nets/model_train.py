...
from utils.rpn_msr.tf_anchor_target_layer import anchor_target_layer as anchor_target_layer_tf
...
def Bilstm(net, input_channel, hidden_unit_num, output_channel, scope_name):
    ...
    with tf.variable_scope(scope_name) as scope:
        ...
        #inputdata = tf.cast(net, tf.float16)
        inputdata = tf.transpose(net, [1, 0, 2], name='transpose_inputdata')
        fw1_cell = DynamicRNN(128, tf.float32, time_major=True, forget_bias=1.0)
        bw1_cell = DynamicRNN(128, tf.float32, time_major=True, forget_bias=1.0)
        ...
        stack_lstm_layer = tf.concat((y, tf.reverse(y2, axis=[0])), axis=2)
        lstm_out = tf.transpose(stack_lstm_layer, [1, 0, 2], name='lstm_out')
        #lstm_out = tf.cast(lstm_out, tf.float32)
...
def loss(bbox_pred, cls_pred, bbox, im_info=[[1216,1216,3]]):
    ...
    tf.add_to_collection('label_logits',[rpn_label,rpn_cls_score])
    # box loss
...
def loss_v2(bbox_pred, cls_pred, bbox):
    rpn_data = anchor_target_layer_tf(cls_pred, bbox,bbox_pred)
    rpn_label = rpn_data[0]
    rpn_bbox_targets = rpn_data[1]
    rpn_cls_score = rpn_data[2]
    rpn_bbox_pred = rpn_data[3]
    rpn_bbox_inside_weights = rpn_data[4]
    rpn_bbox_outside_weights = rpn_data[5]
    

    #tf.add_to_collection("rpn_bbox", )
    fg_keep = tf.equal(rpn_label, 1)
    num_fg = tf.reduce_sum(tf.cast(fg_keep, tf.float32))
    rpn_label = tf.reshape(rpn_label, [-1,1])
    rpn_cls_score = tf.reshape(rpn_cls_score, [-1,1,2])
...
def loss_v6(bbox_pred, cls_pred, bbox, num_bbox):
    rpn_data = anchor_target_layer_tf_v5(cls_pred, bbox,bbox_pred,num_bbox)
    rpn_label = rpn_data[0]
    rpn_bbox_targets = rpn_data[1]
    rpn_cls_score = rpn_data[2]
    rpn_bbox_pred = rpn_data[3]
    ...
    tf.summary.scalar('rpn_cross_entropy', rpn_cross_entropy)
    tf.summary.scalar('rpn_loss_box', rpn_loss_box)
    tf.add_to_collection("num_fg", num_fg)
    return total_loss, model_loss, rpn_cross_entropy, rpn_loss_box
