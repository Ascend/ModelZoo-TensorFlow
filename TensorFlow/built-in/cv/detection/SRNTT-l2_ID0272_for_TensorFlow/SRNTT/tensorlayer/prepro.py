...
def rotation(x, rg=20, is_random=False, row_index=0, col_index=1, channel_index=2,
    ...
    fill_mode : string
        Method to fill missing pixel, default 鈥榥earest鈥, more options 鈥榗onstant鈥, 鈥榬eflect鈥 or 鈥榳rap鈥
...
def swirl(x, center=None, strength=1, radius=100, rotation=0, output_shape=None, order=1, mode='constant', cval=0, clip=True, preserve_range=False, is_random=False):
    ...
    mode : {鈥榗onstant鈥, 鈥榚dge鈥, 鈥榮ymmetric鈥, 鈥榬eflect鈥, 鈥榳rap鈥檥, optional
        Points outside the boundaries of the input are filled according to the given mode, with 鈥榗onstant鈥 used as the default. Modes match the behaviour of numpy.pad.
    cval : float, optional
        Used in conjunction with mode 鈥榗onstant鈥, the value outside the image boundaries.
    clip : bool, optional
...
def elastic_transform(x, alpha, sigma, mode="constant", cval=0, is_random=False):
    ...
    cval : float, optional. Used in conjunction with mode 鈥榗onstant鈥, the value outside the image boundaries.
    is_random : boolean, default False
...
def imresize(x, size=[100, 100], interp='bilinear', mode=None):
    ...
    interp : str, optional
        Interpolation to use for re-sizing (鈥榥earest鈥, 鈥榣anczos鈥, 鈥榖ilinear鈥, 鈥榖icubic鈥 or 鈥榗ubic鈥).
    mode : str, optional
        The PIL image mode (鈥楶鈥, 鈥楲鈥, etc.) to convert arr before resizing.
...
def find_contours(x, level=0.8, fully_connected='low', positive_orientation='low'):
    ...
    fully_connected : str, {鈥榣ow鈥, 鈥榟igh鈥檥.  Indicates whether array elements below the given level value are to be considered fully-connected (and hence elements above the value will only be face connected), or vice-versa. (See notes below for details.)
    positive_orientation : either 鈥榣ow鈥 or 鈥榟igh鈥. Indicates whether the output contours will produce positively-oriented polygons around islands of low- or high-valued elements. If 鈥榣ow鈥 then contours will wind counter-clockwise around elements below the iso-value. Alternately, this means that low-valued elements are always on the left of the contour.
    """
