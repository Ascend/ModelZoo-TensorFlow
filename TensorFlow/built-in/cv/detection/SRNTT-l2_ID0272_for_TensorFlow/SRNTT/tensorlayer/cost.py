...
def dice_coe(output, target, epsilon=1e-10):
    """S酶rensen鈥揇ice coefficient for comparing the similarity of two distributions,
    ...
    References
    -----------
    - `wiki-dice <https://en.wikipedia.org/wiki/S酶rensen鈥揇ice_coefficient>`_
    """
...
def dice_hard_coe(output, target, epsilon=1e-10):
    """Non-differentiable S酶rensen鈥揇ice coefficient for comparing the similarity of two distributions,
    usually be used for binary image segmentation i.e. labels are binary.
    The coefficient = [0, 1], 1 if totally match.
