...
def load_cifar10_dataset(shape=(-1, 32, 32, 3), path='data/cifar10/', plotable=False, second=3):
    ...
    >>>         plt.gca().xaxis.set_major_locator(plt.NullLocator())    # 涓嶆樉绀哄埢搴(tick)
...
def load_ptb_dataset(path='data/ptb/'):
    ...
    uniformly in [鈭0.05, 0.05]. They apply 50% dropout on the non-recurrent
    ...
    uniformly in [鈭0.04, 0.04]. We apply 65% dropout on the non-recurrent
