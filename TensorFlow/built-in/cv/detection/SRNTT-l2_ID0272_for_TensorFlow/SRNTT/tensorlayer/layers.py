...
class ReconLayer(DenseLayer):
    ...
    def __init__(
        ...
        self.train_op = npu_tf_optimizer(tf.train.AdamOptimizer(learning_rate, beta1=0.9, beta2=0.999,
...
class DropoutLayer(Layer):
    ...
    def __init__(
        ...
        if is_train is False:
            ...
            # of the Layer.
            if is_fix:
                self.outputs = npu_ops.dropout(self.inputs, keep, seed=seed, name=name)
            else:
                set_keep[name] = tf.placeholder(tf.float32)
                self.outputs = npu_ops.dropout(self.inputs, set_keep[name], seed=seed, name=name) # 1.2
...
class DropconnectDenseLayer(Layer):
    ...
    def __init__(
        ...
        set_keep[name] = tf.placeholder(tf.float32)
        W_dropcon = npu_ops.dropout(W,  set_keep[name])
...
class RNNLayer(Layer):
    ...
    def __init__(
        ...
        #     If input is锛籦atch_size, n_steps, n_features], we do not need to reshape it.\n
  ...
  def __init__(self,
    ...
    with tf.variable_scope(name) as vs:
        ...
        if not forward_only:
          self.gradient_norms = []
          self.updates = []
          opt = npu_tf_optimizer(tf.train.GradientDescentOptimizer(self.learning_rate))
          for b in xrange(len(buckets)):
