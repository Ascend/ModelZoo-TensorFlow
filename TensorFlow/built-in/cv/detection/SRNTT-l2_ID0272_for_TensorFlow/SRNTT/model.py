...
def npu_tf_optimizer(opt):
    ...
    if RANK_SIZE == 1:
        ...
        npu_opt = npu_distributed_optimizer_wrapper(opt)
...
class SRNTT(object):
    ...
    def model(
        ...
        with tf.variable_scope("content_extractor", reuse=reuse):
            ...
            for i in range(16):  # residual blocks
                ...
                net_ = ElementwiseLayer(
                    layer=[net, net_], combine_fn=tf.add, name='b_residual_add/%s' % i)
                net = net_
            ...
            net = BatchNormLayer(layer=net, is_train=is_train,
                                 gamma_init=g_init, name='n64s1/b/m')
            content_feature = ElementwiseLayer(
                layer=[net, temp], combine_fn=tf.add, name='add3')
            ...
            net = SubpixelConv2d(
                net=net, scale=2, n_out_channel=None, act=tf.nn.relu, name='pixelshufflerx2/1')
            net = Conv2d(net=net, n_filter=256, filter_size=(3, 3), strides=(1, 1), act=None,
                         padding='SAME', W_init=w_init, name='n256s1/2')
            net = SubpixelConv2d(
                net=net, scale=2, n_out_channel=None, act=tf.nn.relu, name='pixelshufflerx2/2')
        ...
        with tf.variable_scope("texture_transfer", reuse=reuse):
            ...
            map_in = InputLayer(inputs=content_feature.outputs,
                                name='content_feature_maps')
            if weights is not None and concat:
                self.a1 = tf.get_variable(
                    dtype=tf.float32, name='small/a', initializer=1.)
                self.b1 = tf.get_variable(
                    dtype=tf.float32, name='small/b', initializer=0.)
                map_ref = maps[0] * tf.nn.sigmoid(self.a1 * weights + self.b1)
            else:
                map_ref = maps[0]
            map_ref = InputLayer(
                inputs=map_ref, name='reference_feature_maps1')
            net = ConcatLayer(layer=[map_in, map_ref],
                              concat_dim=-1, name='concatenation1')
            net = Conv2d(net=net, n_filter=64, filter_size=(3, 3), strides=(1, 1), act=tf.nn.relu,
            ...
            for i in range(self.num_res_blocks):  # residual blocks
                ...
                net_ = ElementwiseLayer(
                    layer=[net, net_], combine_fn=tf.add, name='small/resblock_%d/add' % i)
                net = net_
            net = Conv2d(net=net, n_filter=64, filter_size=(3, 3), strides=(1, 1), act=None,
                         padding='SAME', W_init=w_init, b_init=b_init, name='small/conv2')
            net = BatchNormLayer(layer=net, is_train=is_train,
                                 gamma_init=g_init, name='small/bn2')
            net = ElementwiseLayer(
                layer=[net, map_in], combine_fn=tf.add, name='small/add2')
            # upscaling (2x)
            net = Conv2d(net=net, n_filter=256, filter_size=(3, 3), strides=(1, 1), act=None,
                         padding='SAME', W_init=w_init, name='small/conv3')
            net = SubpixelConv2d(
                net=net, scale=2, n_out_channel=None, act=tf.nn.relu, name='small/subpixel')

            # fusion content and texture maps at the medium scale
            # print('\tfusion content and texture maps at MEDIUM scale')
            map_in = net
            if weights is not None and concat:
                self.a2 = tf.get_variable(
                    dtype=tf.float32, name='medium/a', initializer=1.)
                self.b2 = tf.get_variable(
                    dtype=tf.float32, name='medium/b', initializer=0.)
                map_ref = maps[1] * tf.nn.sigmoid(self.a2 * tf.image.resize_bicubic(
                    weights, [weights.get_shape()[1] * 2, weights.get_shape()[2] * 2]) + self.b2)
            else:
                map_ref = maps[1]
            map_ref = InputLayer(
                inputs=map_ref, name='reference_feature_maps2')
            net = ConcatLayer(layer=[map_in, map_ref],
                              concat_dim=-1, name='concatenation2')
            net = Conv2d(net=net, n_filter=64, filter_size=(3, 3), strides=(1, 1), act=tf.nn.relu,
            ...
            for i in range(int(self.num_res_blocks / 2)):  # residual blocks
                ...
                net_ = ElementwiseLayer(
                    layer=[net, net_], combine_fn=tf.add, name='medium/resblock_%d/add' % i)
                net = net_
            net = Conv2d(net=net, n_filter=64, filter_size=(3, 3), strides=(1, 1), act=None,
                         padding='SAME', W_init=w_init, b_init=b_init, name='medium/conv2')
            net = BatchNormLayer(layer=net, is_train=is_train,
                                 gamma_init=g_init, name='medium/bn2')
            net = ElementwiseLayer(
                layer=[net, map_in], combine_fn=tf.add, name='medium/add2')
            # upscaling (2x)
            net = Conv2d(net=net, n_filter=256, filter_size=(3, 3), strides=(1, 1), act=None,
                         padding='SAME', W_init=w_init, name='medium/conv3')
            net = SubpixelConv2d(
                net=net, scale=2, n_out_channel=None, act=tf.nn.relu, name='medium/subpixel')

            # fusion content and texture maps at the large scale
            # print('\tfusion content and texture maps at LARGE scale')
            map_in = net
            if weights is not None and concat:
                self.a3 = tf.get_variable(
                    dtype=tf.float32, name='large/a', initializer=1.)
                self.b3 = tf.get_variable(
                    dtype=tf.float32, name='large/b', initializer=0.)
                map_ref = maps[2] * tf.nn.sigmoid(self.a3 * tf.image.resize_bicubic(
                    weights, [weights.get_shape()[1] * 4, weights.get_shape()[2] * 4]) + self.b3)
            else:
                map_ref = maps[2]
            map_ref = InputLayer(
                inputs=map_ref, name='reference_feature_maps3')
            net = ConcatLayer(layer=[map_in, map_ref],
                              concat_dim=-1, name='concatenation3')
            net = Conv2d(net=net, n_filter=64, filter_size=(3, 3), strides=(1, 1), act=tf.nn.relu,
            ...
            for i in range(int(self.num_res_blocks / 4)):  # residual blocks
                ...
                net_ = ElementwiseLayer(
                    layer=[net, net_], combine_fn=tf.add, name='large/resblock_%d/add' % i)
                net = net_
            net = Conv2d(net=net, n_filter=64, filter_size=(3, 3), strides=(1, 1), act=None,
                         padding='SAME', W_init=w_init, b_init=b_init, name='large/conv2')
            net = BatchNormLayer(layer=net, is_train=is_train,
                                 gamma_init=g_init, name='large/bn2')
            net = ElementwiseLayer(
                layer=[net, map_in], combine_fn=tf.add, name='large/add2')
    ...
    def discriminator(self, input_image, is_train=True, reuse=False):
        ...
        b_init = None
        g_init = tf.random_normal_initializer(1., 0.02)
        def lrelu(x): return act.lrelu(x, 0.2)
        df_dim = 32
        with tf.variable_scope('discriminator', reuse=reuse):
            ...
            for i in range(5):
                ...
                net = BatchNormLayer(layer=net, act=lrelu, is_train=is_train,
                                     gamma_init=g_init, name='n%ds1/b' % n_channels)
    ...
    def tf_gram_matrix(self, x):
        x = tf.reshape(x, tf.stack(
            [-1, tf.reduce_prod(x.get_shape()[1:-1]), x.get_shape()[-1]]))
    ...
    def train(
            ...
            # the init model is trained only with the reconstruction loss
            ...
            # the layer name to compute perceptrual loss
        ...
        if not use_pretrained_model and exists(join(self.save_dir, MODEL_FOLDER)):
            logging.warning('The existing model dir %s is removed!' %
                            join(self.save_dir, MODEL_FOLDER))
        ...
        self.ground_truth = tf.placeholder(
            dtype=tf.float32, shape=[batch_size, input_size * 4, input_size * 4, 3])
        ...
        # weight maps
        self.weights = tf.placeholder(dtype=tf.float32, shape=[
                                      batch_size, input_size, input_size])

        # reference images, ranges[-1, 1]
        self.ref = tf.placeholder(dtype=tf.float32, shape=[
                                  batch_size, input_size, input_size, 3])

        # SRNTT network
        if use_weight_map:
            self.net_upscale, self.net_srntt = self.model(
                self.input, self.maps, weights=tf.expand_dims(self.weights, axis=-1))
        else:
            self.net_upscale, self.net_srntt = self.model(
                self.input, self.maps)

        # VGG19 network, input range [0, 255]
        self.net_vgg_sr = VGG19(
            (self.net_srntt.outputs + 1) * 127.5, model_path=self.vgg19_model_path)
        self.net_vgg_hr = VGG19((self.ground_truth + 1)
                                * 127.5, model_path=self.vgg19_model_path)

        # discriminator network
        self.net_d, d_real_logits = self.discriminator(self.ground_truth)
        _, d_fake_logits = self.discriminator(
            self.net_srntt.outputs, reuse=True)
        ...
        if is_L1_loss:
            loss_reconst = tf.reduce_mean(
                tf.abs(self.net_srntt.outputs - self.ground_truth))
        else:
            loss_reconst = cost.mean_squared_error(
                self.net_srntt.outputs, self.ground_truth, is_mean=True)
        ...
        try:
            ...
            if use_lower_layers_in_per_loss:
                loss_percep = tf.reduce_mean(
                    [loss_percep] + loss_percep_lower_layers)
        except Exception:
        ...
        if use_weight_map:
            ...
            loss_texture = tf.reduce_mean(tf.squared_difference(
                ...
                self.tf_gram_matrix(self.net_vgg_sr.layers['relu1_1'] * tf.nn.sigmoid(tf.image.resize_bicubic(
                    tf.expand_dims(self.weights, axis=-1), [input_size * 4] * 2) * self.a3 + self.b3))
        ...
        if is_WGAN_GP:
            # WGAN losses
            loss_d = tf.reduce_mean(d_fake_logits) - \
                tf.reduce_mean(d_real_logits)
            loss_g = -tf.reduce_mean(d_fake_logits)
            # GP: gradient penalty
            alpha = tf.random_uniform(
                shape=[batch_size, 1, 1, 1], minval=0., maxval=1.)
            interpolates = alpha * self.ground_truth + \
                ((1 - alpha) * self.net_srntt.outputs)
            ...
            loss_g = cost.sigmoid_cross_entropy(
                d_fake_logits, tf.ones_like(d_fake_logits))
            loss_d_fake = cost.sigmoid_cross_entropy(
                d_fake_logits, tf.zeros_like(d_fake_logits))
            loss_d_real = cost.sigmoid_cross_entropy(
                d_real_logits, tf.ones_like(d_real_logits))
            loss_d = loss_d_fake + loss_d_real

        # back projection loss
        loss_bp = back_projection_loss(
            tf_input=self.input, tf_output=self.net_srntt.outputs)
        ...
        if RANK_SIZE > 1:
            num_batches = int(num_files / (batch_size * RANK_SIZE))
        else:
        ...
        # optimizer
        optimizer_init = npu_tf_optimizer(tf.train.AdamOptimizer(
        ...
        optimizer_d = npu_tf_optimizer(tf.train.AdamOptimizer(
        ...
        samples_texture_map_tmp = [np.load(files_map[i], allow_pickle=True)[
            'target_map'] for i in idx]
        samples_texture_map = [[]
                               for _ in range(len(samples_texture_map_tmp[0]))]
        ...
        if use_weight_map:
            samples_weight_map = [np.pad(np.load(files_map[i], allow_pickle=True)[
                                         'weights'], ((1, 1), (1, 1)), 'edge') for i in idx]
        else:
            samples_weight_map = np.zeros(
                shape=(batch_size, input_size, input_size))
        frame_size = int(np.sqrt(batch_size))
        vis.save_images(np.array(samples_in), [frame_size, frame_size], join(
            self.save_dir, SAMPLE_FOLDER, 'HR.png'))
        ...
        vis.save_images(np.array(samples_ref), [frame_size, frame_size], join(
            self.save_dir, SAMPLE_FOLDER, 'Ref.png'))
        ...
        with tf.Session(config=npu_config_proto(config_proto=config)) as sess:
            ...
            model_path = join(self.srntt_model_path,
                              SRNTT_MODEL_NAMES['content_extractor'])
            if files.load_and_assign_npz(
                ...
                exit(0)
            vis.save_images(
                np.round((self.net_upscale.outputs.eval(
                    {self.input: samples_input}) + 1) * 127.5).astype(np.uint8),
            ...
            if use_init_model_only:
                ...
                model_path = join(self.save_dir, MODEL_FOLDER,
                                  SRNTT_MODEL_NAMES['discriminator'])
                if files.load_and_assign_npz(
                        sess=sess,
                        name=model_path,
                        network=self.net_d):
            ...
            if not is_load_success:
                ...
                if use_init_model_only:
                    ...
                    model_path = join(
                        self.srntt_model_path, SRNTT_MODEL_NAMES['conditional_texture_transfer'])
                    if files.load_and_assign_npz(
                            sess=sess,
                            name=model_path,
                            network=self.net_srntt):
            ...
            print("num_init_epochs:",num_init_epochs)
            for epoch in xrange(num_init_epochs):
                ...
                for n_batch in xrange(num_batches):
                    step_time = time.time()
                    if RANK_SIZE > 1:
                        start_idx = n_batch * batch_size * RANK_SIZE + RANK_ID * batch_size
                        sub_idx = idx[start_idx:start_idx + batch_size]
                    else:
                        sub_idx = idx[n_batch * batch_size:n_batch *
                                      batch_size + batch_size]
                    batch_imgs = [imread(files_input[i], mode='RGB')
                                  for i in sub_idx]
                    batch_truth = [img.astype(
                        np.float32) / 127.5 - 1 for img in batch_imgs]
                    batch_input = [imresize(img, .25, interp='bicubic').astype(
                        np.float32)/127.5-1 for img in batch_imgs]
                    batch_maps_tmp = [np.load(files_map[i], allow_pickle=True)[
                        'target_map'] for i in sub_idx]
                    ...
                    n_iter_remain = (num_init_epochs - epoch - 1) * \
                        num_batches + num_batches - n_batch
                    eta_str, eta_ = self.eta(
                        time_per_iter, n_iter_remain, current_eta)
                    current_eta = eta_
                    logging.info('Pre-train: Epoch [%02d/%02d] Batch [%03d/%03d]\tETA: %s\n'
                                 '\tperf: %.4f\n'
                                 '\tloss:l_rec = %.4f \t l_bp = %.4f' %
            ...
            for epoch in xrange(num_epochs):
                ...
                for n_batch in xrange(num_batches):
                    ...
                    _, _, l_rec, l_per, l_tex, l_adv, l_dis, l_bp = sess.run(
                        fetches=[optimizer, optimizer_d, loss_reconst,
                    ...
                    # print
                    time_per_iter = time.time() - step_time
                    n_iter_remain = (num_epochs - epoch - 1) * \
                        num_batches + num_batches - n_batch
                    eta_str, eta_ = self.eta(
                        time_per_iter, n_iter_remain, current_eta)
                    current_eta = eta_
    ...
    def test(
        ...
        if h * w * 16 > SRNTT.MAX_IMAGE_SIZE:  # avoid OOM
            ...
            for ind_row in range(0, h - (patch_size - stride), stride):
                for ind_col in range(0, w - (patch_size - stride), stride):
                    patch = img_input[ind_row:ind_row +
                                      patch_size, ind_col:ind_col + patch_size, :]
                    if patch.shape != (patch_size, patch_size, 3):
                        patch = np.pad(patch,
                                       ((0, patch_size - patch.shape[0]), (0,
                                        patch_size - patch.shape[1]), (0, 0)),
                                       'reflect')
        ...
        if ref_scale <= 0:  # keep the same scale as HR image
            img_ref = [imresize(img, (h * 4, w * 4), interp='bicubic')
                       for img in img_ref]
        elif ref_scale != 1:
            img_ref = [imresize(img, float(ref_scale), interp='bicubic')
                       for img in img_ref]
        ...
        if not self.is_model_built:
            ...
            if use_weight_map:
                ...
                self.net_upscale, self.net_srntt = self.model(
                    self.input_srntt, self.maps, is_train=False)
            ...
            if self.save_dir is None:
                ...
                if use_init_model_only:
                    model_path = join(
                        self.save_dir, MODEL_FOLDER, SRNTT_MODEL_NAMES['init'])
                    if files.load_and_assign_npz(
                            sess=self.sess,
                            name=model_path,
                            network=self.net_srntt):
        ...
        for idx, patch in enumerate(img_input):
            ...
            logging.info('Saved to %s' %
                         join(result_dir, 'tmp', 'srntt_%05d.png' % idx))
        t_end = time.time()
        logging.info('Reconstruct SR image')
        out_srntt_files = sorted(glob(join(result_dir, 'tmp', 'srntt_*.png')))
        out_upscale_files = sorted(
            glob(join(result_dir, 'tmp', 'upscale_*.png')))
        ...
        # save bicubic
        imsave(join(result_dir, 'Bicubic.png'), imresize(
            img_input_copy, 4., interp='bicubic'))
        # save SR images
        imsave(join(result_dir, 'Upscale.png'), np.array(
            out_upscale).squeeze().round().clip(0, 255).astype(np.uint8))
        imsave(join(result_dir, 'SRNTT.png'), np.array(
            out_srntt).squeeze().round().clip(0, 255).astype(np.uint8))
    ...
    def test_without_ref(
        ...
        if is_ref:
            for i in img_ref:
                img_ref_downscale = imresize(i, .25, interp='bicubic')
                img_ref_upscale = self.net_upscale.outputs.eval(
                    {self.input_srntt: [img_ref_downscale / 127.5 - 1]}, session=self.sess)
        ...
        for idx, patch in enumerate(img_input):
            ...
            if 'Urban' in input_dir:
                img_input_upscale = imread(
                    join('../EDSR-PyTorch/test_Urban100_MDSR',
                         split(input_dir)[-1], 'SRNTT.png'),
                    mode='RGB').astype(np.float32)
            elif 'CUFED5' in input_dir and False:
                img_input_upscale = imread(
                    join('../EDSR-PyTorch/test_CUFED5_MDSR',
                         split(input_dir)[-1], 'SRNTT.png'),
                    mode='RGB').astype(np.float32)
            elif 'Sun80' in input_dir or 'sun80' in input_dir:
                img_input_upscale = imread(
                    join('../EDSR-PyTorch/test_Sun100_MDSR',
                         split(input_dir)[-1].split('.')[0], 'SRNTT.png'),
                    mode='RGB').astype(np.float32)
            else:
                img_input_upscale = self.net_upscale.outputs.eval(
                    {self.input_srntt: [patch / 127.5 - 1]}, session=self.sess)
