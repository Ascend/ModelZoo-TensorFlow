...
def mish(inputs):
    with tf.variable_scope("mish"):
        ...
        ### tf.exp may cause overflow on float16
        # MISH_THRESH = 20.0
        # tmp = inputs
...
def conv(input_data, filters_shape, trainable, name, downsample=False, activate=True, bn=True, act_fun='leaky_relu'):
    with tf.variable_scope(name):
        ...
        conv = tf.nn.conv2d(input=input_data, filter=weight, strides=strides, padding=padding, name="conv")
        ...
        if bn:
            with tf.compat.forward_compatibility_horizon(2019, 5, 1):
                conv = tf.layers.batch_normalization(conv, beta_initializer=tf.zeros_initializer(), gamma_initializer=tf.ones_initializer(),
                                                     ...
                                                     training=trainable[0], name="bn")
        else:
            bias = tf.get_variable(name='bias', shape=filters_shape[-1], trainable=True,
                                   dtype=tf.float32, initializer=tf.constant_initializer(0.0))
            conv = tf.nn.bias_add(conv, bias, name="badd")
        ...
        if activate == True:
            if act_fun == 'mish':
                ...
                conv = tf.nn.leaky_relu(conv, alpha=0.1, name="lrelu")
    return conv
...
class YOLOV5(object):
    ...
    def bbox_iou(self, boxes1, boxes2):
        ...
        len_scale = 0.01  # to avoid overflow when calculating area on float16

        boxes1_area = (boxes1[..., 2]*len_scale) * (boxes1[..., 3]*len_scale)
        boxes2_area = (boxes2[..., 2]*len_scale) * (boxes2[..., 3]*len_scale)
        ...
        inter_area = (inter_section[..., 0]*len_scale) * (inter_section[..., 1]*len_scale)
        union_area = boxes1_area + boxes2_area - inter_area
        
        iou = 1.0 * inter_area / (union_area + self.iou_epsilon)
        return iou
    ...
    def bbox_giou(self, boxes1, boxes2):
        ...
        boxes1_area = ((boxes1[..., 2] - boxes1[..., 0])*len_scale) * ((boxes1[..., 3] - boxes1[..., 1])*len_scale)
        boxes2_area = ((boxes2[..., 2] - boxes2[..., 0])*len_scale) * ((boxes2[..., 3] - boxes2[..., 1])*len_scale)
        ...
        inter_area = (inter_section[..., 0]*len_scale) * (inter_section[..., 1]*len_scale)
        union_area = boxes1_area + boxes2_area - inter_area
        iou = inter_area / (union_area + self.iou_epsilon)

        enclose_left_up = tf.minimum(boxes1[..., :2], boxes2[..., :2])
        enclose_right_down = tf.maximum(boxes1[..., 2:], boxes2[..., 2:])
        enclose = tf.maximum(enclose_right_down - enclose_left_up, 0.0)
        enclose_area = (enclose[..., 0]*len_scale) * (enclose[..., 1]*len_scale)
        
        giou = iou - 1.0 * (enclose_area - union_area) / (enclose_area + self.iou_epsilon)
        return giou
    ...
    def bbox_diou(self, boxes1, boxes2):
        ...
        center_distance = tf.reduce_sum(tf.square((boxes1_center -boxes2_center)*len_scale), axis=-1)
        enclose_left_up = tf.minimum(boxes1[..., :2], boxes2[..., :2])
        enclose_right_down = tf.maximum(boxes1[..., 2:], boxes2[..., 2:])
        enclose_wh = tf.maximum(enclose_right_down - enclose_left_up, 0.0)
        enclose_diagonal = tf.reduce_sum(tf.square(enclose_wh*len_scale), axis=-1)

        diou = iou - 1.0 * center_distance / (enclose_diagonal + self.iou_epsilon)
        return diou
    ...
    def bbox_ciou(self, boxes1, boxes2):
        ...
        alp = v / (1.0 - iou + v + self.iou_epsilon)
        alp = tf.stop_gradient(alp)
    ...
    def yolov5_loss(self, conv, pred, label, bboxes, stride, iou_use=1, focal_use=False, label_smoothing=0):
        ...
        # to avoid overflow when calculating area on float16
        bbox_loss_scale = 2.0 - (1.0 * label_xywh[:, :, :, :, 2:3] / input_size) * (1.0 * label_xywh[:, :, :, :, 3:4] / input_size)
        ...
        if focal_use:
            ...
            conf_loss = focal * (0.75 * respond_bbox * tf.nn.sigmoid_cross_entropy_with_logits(labels=respond_bbox, logits=conv_raw_conf) + \
                                 0.25 * respond_backgd * tf.nn.sigmoid_cross_entropy_with_logits(labels=respond_bbox, logits=conv_raw_conf))
