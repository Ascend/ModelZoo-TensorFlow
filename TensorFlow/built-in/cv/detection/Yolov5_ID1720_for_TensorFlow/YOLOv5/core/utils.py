...
def draw_bbox(image, bboxes, classes=read_class_names(cfg.YOLO.CLASSES), show_label=True):
    ...
    for i, bbox in enumerate(bboxes):
        ...
        cv2.rectangle(mask, c1, c2, bbox_color, bbox_thick)

        if show_label:
            bbox_mess = '%s: %.2f' % (classes[class_ind], score)
            t_size = cv2.getTextSize(bbox_mess, 0, fontScale, thickness=bbox_thick // 2)[0]
            
            cv2.rectangle(mask, c1, (c1[0] + t_size[0], c1[1] + t_size[1]), bbox_color, -1)  # filled
            cv2.putText(mask, bbox_mess, (c1[0], c1[1] + t_size[1]), cv2.FONT_HERSHEY_SIMPLEX,
                        fontScale, (0, 0, 0), bbox_thick // 2, lineType=cv2.LINE_AA)
        
        image = (image + mask*0.8).astype(np.int32)
