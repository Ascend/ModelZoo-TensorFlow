...
class HourglassModel():
	...
	"""
	def __init__(self, nFeat = 512, nStack = 4, nModules = 1, nLow = 4, outputDim = 16, batch_size = 16, drop_rate = 0.2, lear_rate = 2.5e-4, decay = 0.96, decay_step = 2000, dataset = None, training = True, w_summary = True, logdir_train = None, logdir_test = None,tiny = True, attention = False,modif = True,w_loss = False, name = 'tiny_hourglass',  joints = ['r_anckle', 'r_knee', 'r_hip', 'l_hip', 'l_knee', 'l_anckle', 'pelvis', 'thorax', 'neck', 'head', 'r_wrist', 'r_elbow', 'r_shoulder', 'l_shoulder', 'l_elbow', 'l_wrist'], precision_mode="allow_fp32_to_fp16"):
		""" Initializer
		Args:
	...
	def generate_model(self, nEpochs = 10):
		""" Create the complete graph
		"""
		self.generator = self.dataset.make_dataset(self.batchSize, epoch=nEpochs)
		# self.valid_gen = self.dataset.make_dataset(self.batchSize, epoch=1)
		self.iterator_generator = self.generator.make_initializable_iterator()
		img_train, gt_train, weight_train = self.iterator_generator.get_next()
		...
		with tf.device('/cpu:0'):
			with tf.name_scope('rmsprop'):
				if(self.precision_mode=="allow_mix_precision"):
					...
					self.rmsprop=npu_tf_optimizer(npu_opt)
				else:
					self.rmsprop = npu_tf_optimizer(tf.train.RMSPropOptimizer(learning_rate=self.lr))
				# loss_scale=loss_scale_manager.get_loss_scale()
				# print(self.Session.run(loss_scale))
			optimTime = time.time()
	...
	def _train(self, nEpochs = 10, epochSize = 1000, saveStep = 500, validIter = 10):
		...
		with tf.name_scope('Train'):
			...
			self.Session.run(self.iterator_generator.initializer)
			...
			self.resume['loss'] = []
			self.resume['err'] = []
			iteration_loop_op = util.set_iteration_per_loop(self.Session, self.train_rmsprop, 10)
			for epoch in range(nEpochs):
				...
				for i in range(epochSize):
					...
					# img_train, gt_train, weight_train=self.Session.run(nextelement)


					# if (self.precision_mode == "allow_mix_precision"):
					# 	lossScale = tf.get_default_graph().get_tensor_by_name("rmsprop/loss_scale:0")
					# 	overflow_status_reduce_all = tf.get_default_graph().get_tensor_by_name("overflow_status_reduce_all:0")
					if i % saveStep == 0:
						if self.w_loss:
							_, c, summary = self.Session.run([iteration_loop_op, self.loss, self.train_op])
						else:
							if (False):
								l_s, overflow_status_reduce_all, global_step, _, c, summary = self.Session.run([lossScale, overflow_status_reduce_all, global_step, iteration_loop_op, self.loss,self.train_op])
								printstr = "global_step:{},lossscale:{},overflow_status_reduce_all:{}".format(global_step, lossScale, overflow_status_reduce_all)
								print(printstr)
							else:
								_, c, summary = self.Session.run([iteration_loop_op, self.loss, self.train_op])
						...
						if self.w_loss:
							_, c, = self.Session.run([iteration_loop_op, self.loss])
						else:
							if (False):
								l_s, overflow_status_reduce_all, global_step, _, c, = self.Session.run([lossScale, overflow_status_reduce_all, global_step, iteration_loop_op, self.loss])
								printstr = "global_step:{},lossscale:{},overflow_status_reduce_all:{}".format(global_step,lossScale,overflow_status_reduce_all)
								print(printstr)
							else:
								_, c, = self.Session.run([iteration_loop_op, self.loss])
	...
	def _init_weight(self):
		...
		custom_op.parameter_map["precision_mode"].s = tf.compat.as_bytes(self.precision_mode)
		#################### 循环迭代下沉 ##########################
	...
	def _init_session(self):
		...
		print('Session initialization')
		t_start = time.time()
		self.Session = tf.Session(config=npu_config_proto())
		print('Sess initialized in ' + str(int(time.time() - t_start)) + ' sec.')
			...
			if self.tiny:
				with tf.name_scope('stacks'):
					with tf.name_scope('stage_0'):
						...
						drop[0] = npu_ops.dropout(hg[0], keep_prob=self.dropout_rate, name='dropout')
					...
					for i in range(1, self.nStack - 1):
						with tf.name_scope('stage_' + str(i)):
							...
							drop[i] = npu_ops.dropout(hg[i], keep_prob=self.dropout_rate, name='dropout')
					...
					with tf.name_scope('stage_' + str(self.nStack - 1)):
						...
						drop[self.nStack - 1] = npu_ops.dropout(hg[self.nStack - 1], keep_prob=self.dropout_rate, name='dropout')
		...
		with tf.name_scope('stacks'):
			for i in range(self.nStack):
				...
				if i == self.nStack - 1:
					...
					drop = npu_ops.dropout(ll2, keep_prob=0.1)
