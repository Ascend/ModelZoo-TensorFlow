...
function exec_train() {
    ...
    FPS=`grep -a 'INFO:tensorflow:global_step/sec: ' $currentDir/test/output/1/train.log|awk 'END {print $2}'`
    FPS=`awk 'BEGIN{printf "%.2f\n",'32'*'${FPS}'*'8'}'`
    train_accuracy=`grep Precision $currentDir/test/output/1/train.log|grep Average |awk 'NR==1 {print $13}'`
