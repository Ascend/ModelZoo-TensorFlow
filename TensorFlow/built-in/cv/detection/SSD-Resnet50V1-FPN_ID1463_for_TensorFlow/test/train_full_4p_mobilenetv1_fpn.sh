...
#训练参数，需要根据模型修改
Network="SSD-MobilenetV1-FPN_for_TensorFlow"
num_train_steps=25000
batch_size=32
ckpt_path=/checkpoints
pipeline_config=$cur_path/../models/research/configs/ssd_mobilenet_v1_fpn_shared_box_predictor_640x640_coco14_sync_4gpus.config
...
##获取错误信息
#系统错误信息
error_code="E19999: Inner Error!"
error_msg="node:Postprocessor/BatchMultiClassNonMaxSuppression/MultiClassNonMaxSuppression/non_max_suppression/NonMaxSuppressionV3,output: 0"
#判断错误信息是否和历史状态一致，此处无需修改
Status1=`grep "${error_code}" $cur_path/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log | wc -l`
Status2=`grep "${error_msg}" $cur_path/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log | wc -l`
...
ModelStatus="图执行FAIL"
#DTS单号或者issue链接
DTS_Number="TuscanyV1R1C78B200_IR_0602_SF_100_SR_001_SR_002"
...
echo "error_code = ${error_code}" >> $cur_path/output/$ASCEND_DEVICE_ID/${CaseName}.log
