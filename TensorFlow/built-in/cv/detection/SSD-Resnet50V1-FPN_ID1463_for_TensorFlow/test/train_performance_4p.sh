...
#训练参数，需要根据模型修改
Network="SSD-Resnet50V1-FPN_for_TensorFlow"
num_train_steps=200
batch_size=32
ckpt_path=/checkpoints
pipeline_config=$cur_path/../models/research/configs/ssd320_full_4gpus.config
...
#训练执行脚本，需要根据网络修改
  nohup python3 -u ./object_detection/model_main.py \
       ...
       "${@:1}"  > $cur_path/output/$ASCEND_DEVICE_ID/train.log 2>&1 &
done
wait
...
echo "-----------------------Final result------------------------"
#性能FPS计算，需要根据网络修改
FPS=`grep -a 'INFO:tensorflow:global_step/sec: ' $cur_path/output/$ASCEND_DEVICE_ID_START/train.log|awk 'END {print $2}'` 

FPS=`awk 'BEGIN{printf "%.2f\n",'${batch_size}'*'${FPS}'*'${RANK_SIZE}'}'`
echo "Final Performance images/sec : $FPS"
################################精度结果处理#########################
#精度计算，需要根据网络修改
train_accuracy=`grep Precision $cur_path/output/$ASCEND_DEVICE_ID_START/train.log|grep Average |awk 'NR==1 {print $13}'`
...
grep INFO:tensorflow:loss $cur_path/output/$ASCEND_DEVICE_ID_START/train.log|awk '{print $3}'|sed 's/,//g'|sed '/^$/d' >> $cur_path/output/$ASCEND_DEVICE_ID_START/train_${CaseName}_loss.txt

ActualLoss=`awk 'END {print}' $cur_path/output/$ASCEND_DEVICE_ID_START/train_${CaseName}_loss.txt`
...
echo "ActualLoss = ${ActualLoss}" >> $cur_path/output/$ASCEND_DEVICE_ID_START/${CaseName}.log
...
#判断错误信息是否和历史版本一致
Status=`grep "${error_msg}" $cur_path/output/$ASCEND_DEVICE_ID_START/train.log | wc -l`
...
echo "ModelStatus = ${ModelStatus}" >> $cur_path/output/$ASCEND_DEVICE_ID_START/${CaseName}.log
echo "DTS_Number = ${DTS_Number}" >> $cur_path/output/$ASCEND_DEVICE_ID_START/${CaseName}.log
echo "Status = ${Status}" >> $cur_path/output/$ASCEND_DEVICE_ID_START/${CaseName}.log
echo "error_msg = ${error_msg}" >> $cur_path/output/$ASCEND_DEVICE_ID_START/${CaseName}.log
