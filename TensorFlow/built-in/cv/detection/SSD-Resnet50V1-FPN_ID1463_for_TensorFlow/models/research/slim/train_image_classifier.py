...
def _configure_optimizer(learning_rate):
    ...
    if (FLAGS.optimizer == 'adadelta'):
        optimizer = npu_tf_optimizer(tf.train.AdadeltaOptimizer(learning_rate, rho=FLAGS.adadelta_rho, epsilon=FLAGS.opt_epsilon))
    elif (FLAGS.optimizer == 'adagrad'):
        optimizer = npu_tf_optimizer(tf.train.AdagradOptimizer(learning_rate, initial_accumulator_value=FLAGS.adagrad_initial_accumulator_value))
    elif (FLAGS.optimizer == 'adam'):
        optimizer = npu_tf_optimizer(tf.train.AdamOptimizer(learning_rate, beta1=FLAGS.adam_beta1, beta2=FLAGS.adam_beta2, epsilon=FLAGS.opt_epsilon))
    elif (FLAGS.optimizer == 'ftrl'):
        optimizer = npu_tf_optimizer(tf.train.FtrlOptimizer(learning_rate, learning_rate_power=FLAGS.ftrl_learning_rate_power, initial_accumulator_value=FLAGS.ftrl_initial_accumulator_value, l1_regularization_strength=FLAGS.ftrl_l1, l2_regularization_strength=FLAGS.ftrl_l2))
    elif (FLAGS.optimizer == 'momentum'):
        optimizer = npu_tf_optimizer(tf.train.MomentumOptimizer(learning_rate, momentum=FLAGS.momentum, name='Momentum'))
    elif (FLAGS.optimizer == 'rmsprop'):
        optimizer = npu_tf_optimizer(tf.train.RMSPropOptimizer(learning_rate, decay=FLAGS.rmsprop_decay, momentum=FLAGS.rmsprop_momentum, epsilon=FLAGS.opt_epsilon))
    elif (FLAGS.optimizer == 'sgd'):
        optimizer = npu_tf_optimizer(tf.train.GradientDescentOptimizer(learning_rate))
    else:
...
def main(_):
    ...
    with tf.Graph().as_default():
        ...
        if FLAGS.sync_replicas:
            optimizer = npu_tf_optimizer(tf.train.SyncReplicasOptimizer(opt=optimizer, replicas_to_aggregate=FLAGS.replicas_to_aggregate, total_num_replicas=FLAGS.worker_replicas, variable_averages=variable_averages, variables_to_average=moving_average_variables))
