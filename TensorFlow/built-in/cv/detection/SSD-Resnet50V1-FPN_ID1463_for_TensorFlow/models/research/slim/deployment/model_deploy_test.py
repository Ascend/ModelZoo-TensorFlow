...
class OptimizeclonesTest(tf.test.TestCase):
    ...
    def testCreateLogisticClassifier(self):
        ...
        with g.as_default():
            ...
            update_ops = tf.get_collection(tf.GraphKeys.UPDATE_OPS)
            self.assertEqual(update_ops, [])
            optimizer = npu_tf_optimizer(tf.train.GradientDescentOptimizer(learning_rate=1.0))
...
class DeployTest(tf.test.TestCase):
    ...
    def testNoSummariesOnGPU(self):
        with tf.Graph().as_default():
            ...
            model = model_deploy.deploy(deploy_config, ModelFn, optimizer=npu_tf_optimizer(tf.train.GradientDescentOptimizer(1.0)))
