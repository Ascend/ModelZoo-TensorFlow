...
def clip_boxes(boxes, height, width):
  ...
  with tf.name_scope('clip_box'):
    ...
    # clip_by_value does not support broadcasting
    #height_tile = tf.tile(height, [1, y_min.shape[1], 1])
    #width_tile = tf.tile(width, [1, y_min.shape[1], 1])
    #clipped_y_min = tf.maximum(tf.minimum(y_min, height_tile - 1.0), 0.0)
    #clipped_y_max = tf.maximum(tf.minimum(y_max, height_tile - 1.0), 0.0)
    #clipped_x_min = tf.maximum(tf.minimum(x_min, width_tile - 1.0), 0.0)
    #clipped_x_max = tf.maximum(tf.minimum(x_max, width_tile - 1.0), 0.0)
