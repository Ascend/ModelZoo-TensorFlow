  ...
  with tf.variable_scope('rpn_head', reuse=tf.AUTO_REUSE):
    ...
    def shared_rpn_heads(features, num_anchors):
      ...
      features = tf.layers.conv2d(
          ...
          kernel_initializer=tf.random_normal_initializer(stddev=0.01, seed=rnd_seed),
          padding='same',
          name='rpn')
...
def box_head(roi_features, num_classes=91, mlp_head_dim=1024):
  ...
  with tf.variable_scope('box_head', reuse=tf.AUTO_REUSE):
    ...
    box_outputs = tf.layers.dense(
        net, num_classes * 4,
        kernel_initializer=tf.random_normal_initializer(stddev=0.001, seed=rnd_seed),
        bias_initializer=tf.zeros_initializer(),
        name='box-predict')
  ...
  with tf.variable_scope('mask_head'):
    ...
    for i in range(4):
      ...
      net = tf.layers.conv2d(
          ...
          dilation_rate=(1, 1),
          activation=tf.nn.relu,
          kernel_initializer=tf.random_normal_initializer(stddev=init_stddev, seed=rnd_seed),
          bias_initializer=tf.zeros_initializer(),
