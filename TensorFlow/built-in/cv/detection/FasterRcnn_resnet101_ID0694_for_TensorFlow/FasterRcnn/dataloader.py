...
class InputReader(object):
  ...
  def __call__(self, params, input_context=None):
    ...
    if self._mode == tf.estimator.ModeKeys.TRAIN:
      ...
      dataset_p = dataset_p.map(pre_parser_fn, num_parallel_calls=tf.data.experimental.AUTOTUNE)
    ...
    dataset = dataset.map(dataset_parser_fn, num_parallel_calls=64)
