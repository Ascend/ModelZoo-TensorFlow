...
grep "Time taken"  $cur_path/test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log > traintime.log
sed -i '1,2d' traintime.log
TrainingTime=`cat traintime.log |grep "Time taken" |awk '{sum+=$3} END {print sum/NR}'`
