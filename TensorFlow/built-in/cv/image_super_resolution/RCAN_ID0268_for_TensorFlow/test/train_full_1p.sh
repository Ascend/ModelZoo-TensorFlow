...
#基础参数，需要模型审视修改
#网络名称，同目录名称
Network="RCAN_ID0268_for_TensorFlow"
#训练epoch
train_epochs=6
#训练step
train_steps=1000
...
do
    ...
    nohup python3 -u main.py --train_GT_path ${data_path}/DIV2K_train_HR --train_LR_path ${data_path}/DIV2K_train_LR_bicubic/X2 --max_step 102 --scale 2 --log_freq 1 > ${cur_path}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log 2>&1 &
if [ $? -ne 0 ];then
  exit 1
fi
done
...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
TrainingTime=`grep "Elpased time" $cur_path/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log | awk 'END{print $4}'`
...
grep "Loss :" $cur_path/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log | awk '{print $3}' > $cur_path/output/$ASCEND_DEVICE_ID/${CaseName}_loss.txt
#最后一个迭代loss值，不需要修改
ActualLoss=`tail -n 5 $cur_path/output/$ASCEND_DEVICE_ID/${CaseName}_loss.txt|awk -F " " '{number[$1]=$1}END{for(i in number){print number[i]|"sort -n|head -n 1"}}'`
