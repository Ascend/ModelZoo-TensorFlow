...
**描述（Description）：基于TensorFlow框架的vae算法训练代码**

<h2 id="概述.md">概述</h2>

-    VAE是两大生成模型之一，它开创性地使用两个encoder对每个样本拟合均值与方差以构造专属的正态分布，而后对其采样并重构原始分布。其特点为在encode过程中与decode过程中蕴含了对抗思想，并且二者对抗过程不同于gan，是同步的。

- 参考论文：

    [http://arxiv.org/abs/1312.6114v10](http://arxiv.org/abs/1312.6114v10)

- 参考实现：

    [https://github.com/ikostrikov/Tensorflow-VAE-GAN-DRAW](https://github.com/ikostrikov/Tensorflow-VAE-GAN-DRAW)
...
- 开始训练。
    ...
    2. 单卡训练
        ...
        2.1 设置单卡训练参数（脚本位于VAE-GAN_ID1800_for_TensorFlow/test/train_full_1p.sh），示例如下。
        ...
        2.2 单卡训练指令（脚本位于VAE-GAN_ID1800_for_TensorFlow/test） 

        ```
        于终端中运行export ASCEND_DEVICE_ID=0 (0~7)以指定单卡训练时使用的卡
        bash train_full_1p.sh --data_path=xx
        数据集应有如下结构，配置data_path时需指定为MNIST这一层，例：--data_path=/home/MNIST
        ├─MNIST
           ├─mnist.npz
