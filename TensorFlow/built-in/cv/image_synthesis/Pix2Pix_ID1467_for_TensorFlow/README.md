...
**描述（Description）：基于TensorFlow框架pix2pix模型的动漫图片自动上色**

<h2 id="概述.md">概述</h2>

-    基于TensorFlow框架pix2pix模型的动漫图片自动上色
...
3. 注意：当前datasets中的训练集过少，可增加训练集进行训练，防止过拟合
...
- 开始训练。
    ...
    2. 单卡训练
        ...
        2.1 设置单卡训练参数（脚本位于Pix2Pix_ID1467_for_TensorFlow/test/train_performance_1p.sh），示例如下。
        ...
        2.2 单卡训练指令（脚本位于Pix2Pix_ID1467_for_TensorFlow/test/train_performance_1p.sh） 
