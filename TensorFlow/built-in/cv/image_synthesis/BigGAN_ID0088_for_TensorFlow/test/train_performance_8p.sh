...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
#FPS=`grep "FPS:" $cur_path/output/0/train_0.log| awk '{print $10}' | awk 'END {print $1}' |tr -d ,`
...
ActualLoss=`grep "d_loss:" $cur_path/output/0/train_0.log| awk '{print $12}' | awk 'END {print $1}' |tr -d ,`
