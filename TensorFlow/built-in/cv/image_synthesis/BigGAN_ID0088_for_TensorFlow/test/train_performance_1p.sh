...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
#FPS=`grep "FPS:" $cur_path/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log| awk '{print $10}' | awk 'END {print $1}' |tr -d ,`
#$temp1=`echo "1000 * ${batch_size}"|bc`
#TrainingTime=`echo "scale=2;${temp1} / ${FPS}"|bc`
avg_fps=`cat $cur_path/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log |grep "Epoch:" | grep -v "\[    0\/" | grep -v "\[   10\/" | awk -F "FPS: " '{print $2}' | awk -F ',' '{print $1}' | awk '{sum+=$1} END {print sum/NR}'`
...
ActualLoss=`grep "d_loss:" $cur_path/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log| awk '{print $12}' | awk 'END {print $1}' |tr -d ,`
