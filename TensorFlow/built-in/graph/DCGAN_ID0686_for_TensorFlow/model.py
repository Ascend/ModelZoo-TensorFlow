...
class DCGAN(object):
  ...
  def train(self, config):
    ...
    d_optimizer = tf.train.AdamOptimizer(learning_rate=config.learning_rate, beta1=config.beta1)
    if config.precision_mode == 'allow_mix_precision':
      print("config.precision_mode==================>", config.precision_mode)
      loss_scale_manager = ExponentialUpdateLossScaleManager(init_loss_scale=2 ** 16,
                                                             incr_every_n_steps=1000, decr_every_n_nan_or_inf=2,
                                                             decr_ratio=0.5)
      # loss_scale_manager = FixedLossScaleManager(loss_scale=128, enable_overflow_check=True)
    ...
    g_optimizer = tf.train.AdamOptimizer(learning_rate=config.learning_rate, beta1=config.beta1)
    if config.precision_mode == 'allow_mix_precision':
      loss_scale_manager = ExponentialUpdateLossScaleManager(init_loss_scale=2 ** 16,
                                                             incr_every_n_steps=1000, decr_every_n_nan_or_inf=2,
                                                             decr_ratio=0.5)
    ...
    for epoch in xrange(config.epoch):
      ...
      for idx in xrange(0, int(batch_idxs)):
        ...
        if config.dataset == 'mnist':
          ...
          # modify for NPU start
          # 迭代循环下沉需要
          # d_optim = util.set_iteration_per_loop(self.sess, d_optim, 100)
          # modify for NPU end
          ...
          # 迭代循环下沉需要
          # g_optim = util.set_iteration_per_loop(self.sess, d_optim, 100)
        ...
        counter += 1
    print("Final accuracy d_loss", errD_fake+errD_real)
    # print("Average duration pre step %4.4f" % ( (time.time() - start_time)/config.epoch/batch_idxs))
    # print("FPS %4.4f" % ( config.epoch*70000 / (time.time() - start_time)))
