...
#输出CompileTime
CompileTime=`grep "perf" $cur_path/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log |head -n 1|awk '{sum+=$(NF)} END {print sum}'`
...
if [[ $dynamic_bs == "" || $dynamic_bs == "False" ]];then
    ...
    CaseName=${Network}_dynamic_bs${BatchSize}_${RANK_SIZE}'p'_'RT2'_'perf'
fi
