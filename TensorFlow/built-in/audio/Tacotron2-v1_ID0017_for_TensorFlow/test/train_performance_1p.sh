...
#基础参数，需要模型审视修改
#网络名称，同目录名称
Network="Tacotron2-v1_ID0017_for_TensorFlow"
#训练epoch
train_epochs=2
#训练step
train_steps=100
...
do
    ...
    nohup python3 train.py \
		...
		--log_name="test" \
		--data_paths=${data_path}/ljspeech/LJSpeech-1.1 \
		--epoch=${train_epochs} \
...
grep "Loss" $cur_path/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log | awk -F "=" '{print $2}' | awk '{print $1}' > $cur_path/output/$ASCEND_DEVICE_ID/train_${CaseName}_loss.txt
#最后一个迭代loss值，不需要修改
...
sed -i "s/ModuleNotFoundError/ModuleNotFound/g" `grep ModuleNotFoundError -rl $cur_path/output/$ASCEND_DEVICE_ID/train_*.log`