...
#基础参数，需要模型审视修改
#网络名称，同目录名称
Network="vqave_ID0236_for_TensorFlow"
#训练epoch
train_epochs=42
#训练batch_size
batch_size=32
...
do
    ...
    nohup python3 ../mnist.py \
        --data_path=${data_path} \
        --TRAIN_NUM=20000 > ${cur_path}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log 2>&1 &
done 
wait
...
grep Loss: $cur_path/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log|awk '{print $4}' >> $cur_path/output/$ASCEND_DEVICE_ID/train_${CaseName}_loss.txt
