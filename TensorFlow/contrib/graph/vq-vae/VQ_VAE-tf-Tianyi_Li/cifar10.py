...
def get_image(train=True,num_epochs=None):
    ...
    #filename_queue = tf.train.string_input_producer(filenames,num_epochs=num_epochs) # back up4Li
...
def main(config,
    ...
    # >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> Run!
    #config = tf.ConfigProto() #original code
    #config.gpu_options.allow_growth = True # original code
...
def get_default_param():
    ...
    return {
        ...
        'TRAIN_NUM' : 10, #250000, #Size corresponds to one epoch
        'BATCH_SIZE': 20, #128,
...
if __name__ == "__main__":
    ...
    extract_z(**config)
    #config['TRAIN_NUM'] = 300000  #backup code4Li
    config['TRAIN_NUM'] = 10
    config['LEARNING_RATE'] = 0.001
    config['DECAY_VAL'] = 0.5
    #config['DECAY_STEPS'] = 100000 # backup code4Li
    config['DECAY_STEPS'] = 10
