...
# config.data_urls3://ngnn/dataset/
# config.train_urls3://ngnn/jobname/output/V0001/
print("config.data_url "+args.data_url)
print("config.train_url "+args.train_url)

# ModelArts results file
data_dir = "./data"
#os.makedirs(data_dir)
# create data file in config
files=os.listdir(data_dir)
print("===>>>1 File number:", files)
#  copy data from OBS to ModelArts
#mox.file.copy_parallel(args.data_url, data_dir)
files=os.listdir(data_dir)
print("===>>>2 Files number:", files)
print("===>>>over read data")

# unzip.
# if have already unzipped,skip it
zf1 = zipfile.ZipFile(data_dir+"/polyvore_text_onehot_vectors.zip")
zf1.extractall(data_dir)
zf1.close()
print("===>>>over unzip polyvore_text_onehot_vectors.zip")
zf2 = zipfile.ZipFile(data_dir+"/polyvore_image_vectors.zip")
zf2.extractall(data_dir)
zf2.close()
print("===>>>over unzip polyvore_image_vectors.zip")

# create ModelArts results file
model_dir = "./result"
os.makedirs(model_dir)
print("===>>>over create ./result file")

fic = open(data_dir+'/category_summarize_100.json', 'r')
category_item = json.load(fic)
num_category = len(category_item)
print('more than 100 category: %d' % num_category)
fr = open(data_dir+'/cid2rcid_100.json', 'r')
cid2rcid = json.load(fr)
#print (cid2rcid)
image_feature_path = data_dir+'/polyvore_image_vectors/'
text_feature_path = data_dir+'/polyvore_text_onehot_vectors/'
...
def load_fitb_data1(index, batch_size, outfit_list, image_feature_path, text_feature_path):

    time = int(batch_size / 4)      # 16/4=4
    num_category = load_num_category()

    image = np.zeros((batch_size, num_category, 2048), dtype=np.float32)
    text = np.zeros((batch_size, num_category, 2757), dtype=np.float32)
    graph = np.zeros((batch_size, num_category, num_category), dtype=np.float32)   # 初始化子图
    outfit_list_ = outfit_list[index*time: (index + 1)*time]    # [4*index:(4+1)*index]
    for i in range(len(outfit_list_)):   # len(outfit_list_)=4
        outfit = outfit_list_[i]
        ii = outfit['items_index']       # items id
        ci = outfit['items_category']    # items category id
        sid = outfit['set_id']           # items outfit id
        rcid_list = []
        length = len(ii)      # outfit中item数量
        blank_index = random.randint(0, length - 1)       # 随机挖空
        # get the items image_feature and text_feature of this outfit
        for j in range(len(ii)):
            cid = ci[j]    # 种类id
            #print("cid: "+str(cid))
            iid = ii[j]
            rcid = int(cid2rcid[str(cid)])    # 筛选后的种类新排序索引
            #print("rcid: " + str(rcid))
            image_feature = json.load(open(image_feature_path + str(sid) + '_' + str(iid) + '.json'))
            text_feature = json.load(open(text_feature_path + str(sid) + '_' + str(iid) + '.json'))
            # 挖空处
            if j == blank_index:    # the blank
                # 首先放上真值
                rcid_pos = rcid
                image[i * 4][rcid] = image_feature
                text[i * 4][rcid] = text_feature
                ...
                # category_y[i * 4 + 3] = rcid
                # 获取3个随机选项
                r1 = random.choice(category_item)   # 随机在category_item中选择一个category
                rcid_w1 = cid2rcid[str(r1['id'])]   # 根据category id得到category的str，再根据str获得排序后的种类索引
                r11 = random.choice(r1['items'])    # 随机获取这个category的一个item
                image[i * 4 + 1][rcid_w1] = json.load(open(image_feature_path + r11 + '.json'))
                text[i * 4 + 1][rcid_w1] = json.load(open(text_feature_path + r11 + '.json'))
                r2 = random.choice(category_item)
                rcid_w2 = cid2rcid[str(r2['id'])]
                ...
                rcid_list.append(rcid)     # 将不是空白处的 种类新排序索引（rcid） 放入 list
                image[i * 4][rcid] = image_feature
                image[i * 4 + 1][rcid] = image_feature
                image[i * 4 + 2][rcid] = image_feature
                image[i * 4 + 3][rcid] = image_feature
        ...
        for a in rl_pos:
            for b in rl_pos:
                if b != a:
                    g1[a][b] = 1.
        graph[i * 4] = g1    # 得到graph有向图联系

        # 后三个随机选项 有向图（双向都置为1）中记录种类联系（如果有出现这个种类置为1）
        rl_w1 = rcid_list[:]
        rl_w1.append(rcid_w1)
        g2 = np.zeros((num_category, num_category), dtype=np.float32)
        for a in rl_w1:
...
def load_auc_data1(index, batch_size, outfit_list,image_feature_path, text_feature_path):

    time = int(batch_size / 2)    # 16/2=8

    num_category = load_num_category()    # 120

    image = np.zeros((batch_size, num_category, 2048), dtype=np.float32)    # [16,120,2048]
    text = np.zeros((batch_size, num_category, 2757), dtype=np.float32)     # [16,120,2757]
    graph = np.zeros((batch_size, num_category, num_category), dtype=np.float32)   # [16,120,120]
    outfit_list_ = outfit_list[index*time: (index + 1)*time]    # [8*index :8(index+1)]
    for i in range(len(outfit_list_)):   # len(outfit_list_)=8  0<=i<8
        outfit = outfit_list_[i]
        ii = outfit['items_index']
        ci = outfit['items_category']
        sid = outfit['set_id']
        ...
        graph_pos = []
        graph_neg = []
        # 得到s的image_feature和text_feature, 放进偶数项
        for j in range(length):
            cid = ci[j]
            iid = ii[j]
            rcid = int(cid2rcid[str(cid)])
            ...
            text[i * 2][rcid] = text_feature
            graph_pos.append(rcid)
        # 得到s-的image_feature和text_feature, 放进奇数项
        for j in range(length):
            r = random.choice(category_item)
            rcid_w = cid2rcid[str(r['id'])]
            rr = random.choice(r['items'])
        ...
        for a in graph_pos:
            for b in graph_pos:
                if b != a:
                    g[a][b] = 1.   # 两种类别出现搭配
        graph[i * 2] = g           # 放进偶数项 0<=i*2<=14  [i*2,120,120]
