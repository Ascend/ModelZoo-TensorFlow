...
def cm_ggnn(batch_size, image_hidden_size, text_hidden_size, n_steps, learning_rate, G, num_category, opt, i, beta):
    ...
    s_pos = tf.reshape(s_pos, [batch_size, 1], name="s_pos_output")
    s_neg = tf.reshape(s_neg, [batch_size, 1], name="s_neg_output")

    s_pos_mean = tf.reduce_mean(s_pos, name="s_pos_mean_output")
    s_neg_mean = tf.reduce_mean(s_neg, name="s_neg_mean_output")
