...
fic = open('/root/ngnn-lcj/data/category_summarize_100.json', 'r')
category_item = json.load(fic)
num_category = len(category_item)
print('more than 100 category: %d' % num_category)
fr = open('/root/ngnn-lcj/data/cid2rcid_100.json', 'r')
cid2rcid = json.load(fr)
#print (cid2rcid)
image_feature_path = '/polyvore_image_vectors/'
#image_feature_path = '/home/cuizeyu/polyvore_image_vectors/'
text_feature_path = '/polyvore_text_onehot_vectors/'
    ...
    for outfit in outfit_list[i : i + time]:
        ...
        for j in j_list:
            ...
            for k in range(len(ii)):
                ...
                image_feature = json.load(open('/root/ngnn-lcj/data'+image_feature_path + str(sid)+ '_' + str(iid)+ '.json'))
                text_feature = json.load(open('/root/ngnn-lcj/data'+text_feature_path + str(sid) + '_' + str(iid) + '.json'))
                if k == j:  #if k==j k is y,else k is x
                    ...
                    image_neg[now_size][rcid_neg] = json.load(open('/root/ngnn-lcj/data'+image_feature_path + ri + '.json'))
                    text_neg[now_size][rcid_neg] = json.load(open('/root/ngnn-lcj/data'+text_feature_path + ri + '.json'))
...
def load_train_size():
    ftrain = open('/root/ngnn-lcj/data/train_no_dup_new_100.json', 'r')
    ...
    for i in range(len(outfit_list_)):
        ...
        for j in range(len(ii)):
            ...
            if j == blank_index:
                ...
                image[i * 4 + 1][rcid_w1] = json.load(open('/root/ngnn-lcj/data'+image_feature_path + r11 + '.json'))
                text[i * 4 + 1][rcid_w1] = json.load(open('/root/ngnn-lcj/data'+text_feature_path + r11 + '.json'))
                r2 = random.choice(category_item)
                rcid_w2 = cid2rcid[str(r2['id'])]
                r22 = random.choice(r2['items'])
                image[i * 4 + 2][rcid_w2] = json.load(open('/root/ngnn-lcj/data'+image_feature_path + r22 + '.json'))
                text[i * 4 + 2][rcid_w2] = json.load(open('/root/ngnn-lcj/data'+text_feature_path + r22 + '.json'))
                r3 = random.choice(category_item)
                rcid_w3 = cid2rcid[str(r3['id'])]
                r33 = random.choice(r3['items'])
                image[i * 4 + 3][rcid_w3] = json.load(open('/root/ngnn-lcj/data'+image_feature_path + r33 + '.json'))
                text[i * 4 + 3][rcid_w3] = json.load(open('/root/ngnn-lcj/data'+text_feature_path + r33 + '.json'))
...
def load_test_size():
    ftest = open('/root/ngnn-lcj/data/test_no_dup_new_100.json', 'r')
...
def load_auc_data(index, batch_size, outfit_list):
    ...
    for i in range(len(outfit_list_)):
        ...
        for j in range(length):
            ...
            image[i * 2 + 1][rcid_w] = json.load(open('/root/ngnn-lcj/data'+image_feature_path + rr + '.json'))
            text[i * 2 + 1][rcid_w] = json.load(open('/root/ngnn-lcj/data'+text_feature_path + rr + '.json'))
