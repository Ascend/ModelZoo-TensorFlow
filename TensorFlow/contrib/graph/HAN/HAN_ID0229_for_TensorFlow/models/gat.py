...
class HeteGAT_multi(BaseGAttN):
    def inference(inputs_list, nb_classes, nb_nodes,
                  bias_mat_list, hid_units, n_heads, activation=tf.nn.elu, training=True, attn_drop=0, ffd_drop=0, residual=False,
                  mp_att_size=128):
        embed_list = []
        print('###########################################################################')
        print(inputs_list)
        ...
        logits = tf.expand_dims(logits, axis=0, name='OutputNode')
