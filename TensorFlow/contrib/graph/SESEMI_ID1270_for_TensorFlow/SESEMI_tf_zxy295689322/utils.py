...
def jitter(image, row_axis=0, col_axis=1, channel_axis=2,
    ...
    for image_channel in image:
        channel_images.append(ndimage.interpolation.affine_transform(image_channel,\
        final_affine_matrix, final_offset, order=order, mode=fill_mode, cval=cval))
    #channel_images = [ndimage.interpolation.affine_transform(image_channel,final_affine_matrix,final_offset,order=order,mode=fill_mode,cval=cval) for image_channel in image]
...
def compile_sesemi(network, input_shape, nb_classes,
    ...
    sesemi_model.compile(optimizer=npu_distributed_optimizer_wrapper(optimizer=sgd),
...
class DenseEvaluator(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        data = open("result.txt", "w", encoding="utf-8")
