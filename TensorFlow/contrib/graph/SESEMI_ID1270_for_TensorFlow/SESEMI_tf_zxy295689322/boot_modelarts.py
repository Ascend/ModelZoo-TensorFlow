...
if __name__ == '__main__':
    ...
    parser.add_argument("--SESEMI_data_dir", type=str, default="/cache/dataset")
    parser.add_argument("--SESEMI_result_dir", type=str, default="/cache/result")
    ...
    if not os.path.exists(config.SESEMI_result_dir):
        os.makedirs(config.SESEMI_result_dir)
    bash_header = os.path.join(code_dir, 'run_1p.sh')
    arg_url = '%s %s %s %s' % (code_dir, config.SESEMI_data_dir, config.SESEMI_result_dir, config.train_url)
