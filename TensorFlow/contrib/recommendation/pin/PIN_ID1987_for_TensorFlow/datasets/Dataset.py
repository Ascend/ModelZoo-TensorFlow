    ...
    def load_data(self, gen_type='train', num_workers=1, task_index=0):
        ...
        for hdf_in, hdf_out in self._files_iter_(gen_type, False):
            ...
            with pd.HDFStore(hdf_in, mode='r') as hdf_in, pd.HDFStore(hdf_out, mode='r') as hdf_out:
                ...
                X_block = pd.read_hdf(hdf_in, mode='r', start=start, stop=stop).values # as_matrix()
                y_block = pd.read_hdf(hdf_out, mode='r', start=start, stop=stop).values # as_matrix()
            ...
            if on_disk:
                ...
                for hdf_in, hdf_out in self._files_iter_(gen_type=gen_type, shuffle_block=shuffle_block):
                    with pd.HDFStore(hdf_in, mode='r') as hdf_in, pd.HDFStore(hdf_out, mode='r') as hdf_out:
                        ...
                        X_all = pd.read_hdf(hdf_in, mode='r', start=start, stop=stop).values # as_matrix()
                        y_all = pd.read_hdf(hdf_out, mode='r', start=start, stop=stop).values # as_matrix()
