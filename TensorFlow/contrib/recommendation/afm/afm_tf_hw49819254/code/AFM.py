...
def parse_args():
    ...
    parser.add_argument('--data_url', nargs='?', default= 's3://afm-h-y/attentional_factorization_machine/data/',     #../data/',
                        help='dataset  path type: train, evaluate.')
    parser.add_argument('--train_url', nargs='?', default='s3://afm-h-y/attentional_factorization_machine/pretrain/',
                        help='train model save path')
    ...
    # æ‹†åˆ†hidden_factorå’ŒKeep
    parser.add_argument('--hidden_factor0',  nargs='?', default='16',
                        help='Number of hidden factors.')
    parser.add_argument('--hidden_factor1',  nargs='?', default='16',
                        help='Number of hidden factors.')
    parser.add_argument('--hidden_factor', nargs='?', default='[16,16]',
                        help='Number of hidden factors.')
    parser.add_argument('--keep0',  nargs='?', default= '1.0',
    ...
    parser.add_argument('--keep1',  nargs='?', default= '0.5',
                        help='Keep probility 1-dropout of each layer. 1 no dropout. The first index is for the attention-aware pairwise interaction layer.')
...
class AFM(BaseEstimator, TransformerMixin):
    ...
    def _init_graph(self):
        ...
        with self.graph.as_default():  # , tf.device('/cpu:0'):
            ...
            # Input data.
            self.train_features = tf.placeholder(tf.int32, shape=[None, 3], name="train_features_afm")  # None * features_M
    ...
    def train(self, Train_data, Validation_data, Test_data):  # fit a dataset
        ...
        for epoch in range(self.epoch):
            ...
            self.history['train_RMSE'].append(train_result)
            self.history['validation_RMSE'].append(valid_result)
    ...
    def plot(self, history=None):
        ...
        # print(self.max_waiting_time)
        x = history['epoch']
        r = history['train_RMSE']
        l = history['validation_RMSE']
        plt.plot(x, r, label="train_RMSE",color = 'blue')
        plt.plot(x, l, label='validation_RMSE', color = 'yellowgreen')
        plt.savefig(r'rmse_of_train_and_valid.png')
        plt.show()
...
def make_save_file(args):
    pretrain_path = args.train_url+'afm_%s_%d' %(args.dataset, eval(args.hidden_factor)[1])
    if args.mla:
        pretrain_path += '_mla'
    if not os.path.exists(pretrain_path):
        os.makedirs(pretrain_path)
    save_file = pretrain_path+'/%s_%d' %(args.dataset, eval(args.hidden_factor)[1]) #ml-tag_256
    return save_file

def train(args):
    # Data loading
    data = DATA.LoadData(args.data_url, args.dataset)
    if args.verbose > 0:
...
def evaluate(args):
    # load test data
    data = DATA.LoadData(args.data_url, args.dataset).Test_data
    save_file = make_save_file(args)
    ...
    if args.mla:
        ...
        feed_dict = {train_features_afm: data['X'], train_labels_afm: [[y] for y in data['Y']], dropout_keep_afm: [1.0,1.0], train_phase_afm: False}
        #print("train_features_afm=",data['X'])
        #print("train_labels_afm=",[[y] for y in data['Y']])
        predictions = sess.run((out_of_afm), feed_dict=feed_dict)
    print("predictions dtype :", predictions.dtype, "  shape:", predictions.shape)
    #predictions.tofile("predictions.bin")
    # calculate rmse
    y_pred_afm = np.reshape(predictions, (num_example,))
    y_true = np.reshape(data['Y'], (num_example,))
    #y_true.tofile("y_true.bin")
    predictions_bounded = np.maximum(y_pred_afm, np.ones(num_example) * min(y_true))  # bound the lower values
    predictions_bounded = np.minimum(predictions_bounded, np.ones(num_example) * max(y_true))  # bound the higher values
    RMSE = math.sqrt(mean_squared_error(y_true, predictions_bounded))
    print("predictions_bounded 1~5: ",predictions_bounded[0:5]," shape: ",predictions_bounded.shape ,"  dtype:",predictions_bounded.dtype)
    print("y_true 1~5: ",y_true[0:5],"  shape:",y_true.shape,"  dtype:",y_true.dtype)
    #predictions_bounded.tofile("test_input.bin")
...
if __name__ == '__main__':
    args = parse_args()
    args.keep= '['+args.keep0+','+args.keep1+']'
    args.hidden_factor = '['+args.hidden_factor0+','+args.hidden_factor1+']'
    # if args.mla:
    #     args.lr = 0.1
    #     args.keep = '[1.0,1.0]'
