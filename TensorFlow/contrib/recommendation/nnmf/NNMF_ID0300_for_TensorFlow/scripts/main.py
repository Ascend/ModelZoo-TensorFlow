...
def train(model, sess, saver, train_data, valid_data, batch_size, max_epochs, use_early_stop, early_stop_max_epoch):
    ...
    for epoch in range(max_epochs):
        ...
        if use_early_stop:
            ...
            saver.save(sess, model.model_filename)
        tf.io.write_graph(sess.graph, './model', 'graph.pbtxt', as_text=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train', metavar='TRAIN_INPUT_FILE', type=str, default='/npu/traindata/NNMF/data/ml-100k/split/u.data.train',
                        help='the location of the training set\'s input file')
    parser.add_argument('--valid', metavar='VALID_INPUT_FILE', type=str, default='/npu/traindata/NNMF/data/ml-100k/split/u.data.valid',
                        help='the location of the validation set\'s input file')
    parser.add_argument('--test', metavar='TEST_INPUT_FILE', type=str, default='/npu/traindata/NNMF/data/ml-100k/split/u.data.test',
    ...
    parser.add_argument('--max-epochs', metavar='MAX_EPOCHS', type=int, default=1,
