...
def multihead_attention(queries,
    ...
    # Dropouts
    weights = npu_ops.dropout(weights,dropout_keep_prob)
...
class AutoInt():
    ...
    def _init_graph(self):
        ...
        with self.graph.as_default():
            ...
            self.feat_index = tf.placeholder(tf.int32, shape=[None, self.field_size-1],
                                                 name="feat_index")  # None * M-1
            self.feat_value = tf.placeholder(tf.float32, shape=[None, self.field_size-1],
                                                 name="feat_value")  # None * M-1

            # placeholder for multi-value field. (movielens dataset genre field)
            self.genre_index = tf.placeholder(tf.int32, shape=[None, self.field_size-1],
                                                 name="genre_index") # None * 6
            self.genre_value = tf.placeholder(tf.float32, shape=[None, self.field_size-1],
                                                 name="genre_value") # None * 6

            self.label = tf.placeholder(tf.float32, shape=[None, 1], name="label")  # None * 1

            # In our implementation, the shape of dropout_keep_prob is [3], used in 3 different places.
            self.dropout_keep_prob = tf.placeholder(tf.float32, shape=[2], name="dropout_keep_prob")
            ...
            self.embeddings = npu_ops.dropout(self.embeddings, self.dropout_keep_prob[1]) # None * M * d
            ...
            if self.deep_layers != None:
                ...
                for i in range(0, len(self.deep_layers)):
                    ...
                    self.y_dense = npu_ops.dropout(self.y_dense, self.dropout_keep_prob[2])
