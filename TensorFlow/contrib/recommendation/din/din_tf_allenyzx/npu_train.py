...
tf.app.flags.DEFINE_integer('save_checkpoint_steps', 100000, '')
...
def main(argv=None):
    ...
    # ========================== Create dataset =======================
    feature_columns, behavior_list, train, val = create_amazon_electronic_dataset(file, embed_dim, maxlen, cache=True)
    ...
    din = DIN(feature_columns, behavior_list, att_hidden_units, ffn_hidden_units, att_activation,
              ffn_activation, maxlen, dnn_dropout, device='npu')([dense_inputs, sparse_inputs, seq_inputs, item_inputs])
    ...
    with tf.Session(config=config) as sess:
        ...
        for step in range(max_steps + 1):
            ...
            if step != 0 and step % FLAGS.save_checkpoint_steps == 0:
                ...
                logging.info('save: checkpoint to step {}'.format(step))

            if step != 0 and step % 1000 == 0:
                val_auc, val_acc = eval(sess=sess, val_X=val_X, val_y=val_y, model=din)
                logging.info("[step:{}] model_loss {:.5} , val_auc {:.5}, val_acc {:.5}".format(step,
                                                                                     float(np.mean(loss_list)),
                                                                                     val_auc, val_acc))
            else:
                if step != 0 and step % 200 == 0:
                    loss_list.append(ploss)
                    logging.info("[step:{}] model_loss {:.5}".format(step, float(np.mean(loss_list))))


        val_auc, val_acc = eval(sess=sess, val_X=val_X, val_y=val_y, model=din)
        logging.info("[step:{}] total_loss {:.5} , val_auc {:.5}, val_acc {:.5}".format(step,
                                                                                        float(np.mean(loss_list)),
                                                                                        val_auc, val_acc))
