...
#parser.add_argument('--envs_num', default=1, type=int, help='envs num')
...
class ConvNet():
    def __init__(self, model_name, batch_size):
        ...
        print("------choose_op：{}".format(self.choose_op))
    ...
    def model(self):
        with tf.variable_scope(self.model_name):
            ...
            print('------self.input_tensor:{}'.format(self.input_tensor))
    ...
    for step in range(restart_step + 1, int(STEP_NUM // N_ENVS) + 1):
        ...
        if step % SAVE_FREQ == 0:
            ...
            # save model
                ...
                #print(pickle.dump(np.array(result), pkl_file))
                #pkl_file.close()
                # mox.file.copy_parallel(RESULT_PATH, OBS_RESULT_PATH)
                #print("save done")
            if  step * N_ENVS > 416000:#416000
                break
        s = s_
...
def eval(model):
    ...
    while True:
        ...
        for info in infos:
            maybeepinfo = info.get('episode')
            #print('------maybeepinfo:{}'.format(maybeepinfo))
            if maybeepinfo:
