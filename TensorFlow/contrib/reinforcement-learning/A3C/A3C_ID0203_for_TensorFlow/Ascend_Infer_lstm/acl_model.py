...
class Model(object):
    ...
    def _gen_output_dataset(self, size):
        ...
        for i in range(size):
            ...
            #print("temp_buffer_size: ",temp_buffer_size)
    ...
    def _gen_input_dataset_from_array(self, input):
        
        self.input_dataset = acl.mdl.create_dataset()
        
        for i in range(3):
            in_data = input[i]
            ptr = acl.util.numpy_to_ptr(in_data)
            size = in_data.size * in_data.itemsize
            
            if self.data[i] == None:
                self.data[i] = copy_data_device_to_device(ptr, size)
            else:
                ret = acl.rt.memcpy(self.data[i], size, ptr, size, ACL_MEMCPY_DEVICE_TO_DEVICE)
            ...
            dataset_buffer = acl.create_data_buffer(self.data[i], size)
