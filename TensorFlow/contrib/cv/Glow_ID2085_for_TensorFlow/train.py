...
def init_visualizations(hps, model, logdir):
    ...
    def draw_samples(epoch):
        ...
        for i in range(len(x_samples)):
            ...
            graphics.save_raster(x_sample, os.path.join(logdir, 'epoch_{}_sample_{}.png'.format(epoch, i) )) #logdir +
...
def get_data(hps, sess):
    ...
    if hps.n_test == -1:
        hps.n_test = {'mnist': 10000, 'cifar10': 10000, 'imagenet-oord': 50000, 'imagenet': 50000,
                      'celeba': 3000, 'lsun_realnvp': 300*get_npu_rank_size(), 'lsun': 300*get_npu_rank_size()}[hps.problem]
    ...
    hps.local_batch_test = hps.local_batch_train #{64: 50, 32: 25, 16: 10, 8: 5, 4: 2, 2: 2, 1: 1}[
    ...
    print("Rank {} Batch sizes Train {} Test {} Init {}".format(
        get_npu_rank_id(), hps.local_batch_train, hps.local_batch_test, hps.local_batch_init))
    ...
    if hps.problem in ['imagenet-oord', 'imagenet', 'celeba', 'lsun_realnvp', 'lsun']:
        ...
        train_iterator, test_iterator, data_init = \
            v.get_data(sess, hps.data_dir, get_npu_rank_size(), get_npu_rank_id(), hps.pmap, hps.fmap, hps.local_batch_train,
        ...
        train_iterator, test_iterator, data_init = \
            v.get_data(hps.problem, get_npu_rank_size(), get_npu_rank_id(), hps.dal, hps.local_batch_train,
...
def main(hps):
    ...
    tf.set_random_seed(get_npu_rank_id() + get_npu_rank_size() * hps.seed)
    np.random.seed(get_npu_rank_id() + get_npu_rank_size() * hps.seed)
    ...
    logdir = os.path.join(code_dir, hps.logdir)
    # logdir = "/home/ma-user/modelarts/user-job-dir/code/logs"
    print(logdir)
...
def train(sess, model, hps, logdir, visualise):
    ...
    for epoch in range(1, hps.epochs):
        ...
        for it in range(hps.train_its):
            ...
            if hps.verbose and get_npu_rank_id() == 0:
                _print(n_processed, time.time()-_t, train_results[-1])
                sys.stdout.flush()

            # Images seen wrt anchor resolution
            n_processed += get_npu_rank_size() * hps.n_batch_train
            # Actual images seen at current resolution
            n_images += get_npu_rank_size() * hps.local_batch_train

        train_results = np.mean(np.asarray(train_results), axis=0)

        dtrain = time.time() - t
        ips = (hps.train_its * get_npu_rank_size() * hps.local_batch_train) / dtrain
        train_time += dtrain
...
def get_its(hps):
    ...
    train_its = int(np.ceil(hps.n_train / (hps.n_batch_train * get_npu_rank_size())))
    test_its = int(np.ceil(hps.n_test / (hps.n_batch_train * get_npu_rank_size())))
    train_epoch = train_its * hps.n_batch_train * get_npu_rank_size()

    # Do a full validation run
    if get_npu_rank_id() == 0:
        print(hps.n_test, hps.local_batch_test, get_npu_rank_size())
    assert hps.n_test % (hps.local_batch_test * get_npu_rank_size()) == 0
    full_test_its = hps.n_test // (hps.local_batch_test * get_npu_rank_size())
...
def tensorflow_session():
    ...
    config.gpu_options.visible_device_list = str(get_npu_local_rank_id())
...
if __name__ == "__main__":
    ...
    parser.add_argument("--learntop", action="store_true",
                        help="Learn spatial prior")#default=True,
