...
do
    ...
    nohup python3 ${cur_path}/../deeplab/train_8p.py \
        ...
        --tf_initial_checkpoint=${ckpt_path}/resnet_v1_101_beta_imagenet/model.ckpt \
...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
TrainingTime=`grep "] global step " ${cur_path}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|tail -n +10|awk '{print $11}'|awk -F'(' '{print $2}'|awk '{sum+=$1} END {print sum/NR}'`
...
DeviceType=`uname -m`
CaseName=${Network}_bs${BatchSize}_${RANK_SIZE}'p'_'resnet_v1_101_beta_imagenet_perf'
