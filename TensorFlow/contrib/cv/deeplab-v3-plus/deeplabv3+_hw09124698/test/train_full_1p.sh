...
python3 ${cur_path}/../deeplab/eval.py \
        --logtostderr \
        --eval_split="val" \
        --model_variant="xception_65" \
        --atrous_rates=6 \
        ...
        --checkpoint_dir=log_1p \
        --eval_logdir=log_eval \
        --dataset_dir=${data_path}/tfrecord \
...
#输出训练精度,需要模型审视修改
train_accuracy=`grep "miou_1.0_overall" ${cur_path}/output/${ASCEND_DEVICE_ID}/eval_${ASCEND_DEVICE_ID}.log | awk 'END {print $1}' | awk -F "[" '{print $2}'|awk -F"]" '{print $1}'`
#train_accuracy="null"
#打印，不需要修改
