...
export LD_LIBRARY_PATH=${install_path}/dirver/lib64/common/:${install_path}/driver/lib64/driver:$LD_LIBRARY_PATH
...
#基础参数，需要模型审视修改
#网络名称，同目录名称
Network="RetinaNet_for_TensorFlow"
#训练epoch
train_epochs=0
#训练batch_size
batch_size=7
...
do
    ...
    nohup python3 ${cur_path}/deeplab/train_8p.py \
        ...
        --train_logdir=log_8p \
        --dataset_dir=${cur_path}/deeplab/datasets/pascal_voc_seg/tfrecord \
        --tf_initial_checkpoint=${cur_path}/ckpt/xception_65_coco_pretrained/x65-b2uls2p-d48-2-3x256-sc-cr300k_init.ckpt \
...
#最后一个迭代loss值，不需要修改
ActualLoss=`awk '{print}' $cur_path/output/$ASCEND_DEVICE_ID/train_${CaseName}_loss.txt|tail -n 3`
