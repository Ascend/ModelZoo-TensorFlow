...
def zero_pad_and_crop(img, amount=4):
  """Zero pads by `amount` zero pixels on each side then take a random crop.
...
def create_cutout_mask(img_height, img_width, num_channels, size):
  ...
  if img_height != img_width:
    raise ValueError('Image height and width are not equal')
...
def pil_unwrap(pil_img):
  ...
  pic_array = (
      np.array(pil_img.getdata()).reshape((IMAGE_SIZE, IMAGE_SIZE, 4)) / 255.0)
...
def apply_policy(policy, img):
  """Applies the `policy` to the numpy `img`.
...
def _shear_x_impl(pil_img, level):
  ...
  if random.random() > 0.5:
    level = -level
  return pil_img.transform((IMAGE_SIZE, IMAGE_SIZE), Image.AFFINE,
                           (1, level, 0, 0, 1, 0))
