...
class Model(object):
  ...
  def _custom_dtype_getter(
      ...
      getter,
      name,
      shape=None,  # pylint: disable=keyword-arg-before-vararg
      dtype=DEFAULT_DTYPE,
      *args,
      **kwargs):
...
class ImagenetModelv2(Model, StrategyNetBase):
  ...
  def __init__(self,
    ...
    Args:
      ...
      weight_decay_rate: l2 weight decay weight.
      resnet_version: Integer representing which version of the ResNet network
        to use. See README for details. Valid values: [1, 2]
  ...
  def get_partial_variables(self, level=-1):
    ...
    if level == 0:
      ...
      for v in self.trainable_variables:
        # TODO(zizhaoz): Some dense layer api may have different name perfix.
        if 'dense' in v.name:
          vs.append(v)
    elif level < 0:
  ...
  def __call__(self, inputs, name, training, reuse=True, custom_getter=None):
    ...
    with tf.variable_scope(
      ...
      if not isinstance(reuse, bool) or not reuse:
        ...
        self.init(name, with_name='batch_normalization', outputs=logits)
