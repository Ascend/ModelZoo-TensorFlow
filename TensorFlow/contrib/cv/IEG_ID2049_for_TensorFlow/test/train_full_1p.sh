...
mkdir -p ./test/output/${ASCEND_DEVICE_ID}/graphcheckp

# 修改参数
sed -i "s#/home/TestUser06/IEG_YYW/code/output#${output_path}#" ./code/main.py
sed -i "s#/home/TestUser06/IEG_YYW/data/cifar-10-python/cifar-10-batches-py#${data_path}/cifar-10-python/cifar-10-batches-py#" ./code/dataset_utils/datasets.py
sed -i 's/200/172/' ./code/main.py
sed -i "s#/home/TestUser06/IEG_YYW/code/output#${output_path}#" ./code/models/model.py
sed -i "s#/home/TestUser06/IEG_YYW/code/output#${output_path}#" ./code/models/basemodel.py
...
if [ x"${modelarts_flag}" != x ];
then
    python3.7 ./code/main.py > ${print_log} 2>&1
else
    python3.7 ./code/main.py > ${print_log} 2>&1
    python3.7 ./code/main.py --mode=evaluation >> ${print_log} 2>&1
fi

# 性能相关数据计算
StepTime=`grep "this step use time" ${print_log} | sed '/4990/d' | awk -F ":" '{print $4}' | sed 's/^0//' | tail -n 10 | awk '{sum+=$1} END {print sum/NR}'`
FPS=`awk 'BEGIN{printf "%.2f\n", '${batch_size}'/'${StepTime}'}'`

# 精度相关数据计算
train_accuracy=`grep "Evaluation" ${print_log} | tail -n 1 | awk '{print $10}'`

# 提取所有loss打印信息
grep "loss:" ${print_log} | awk -F "loss:" '{print $3}' | tr -d " mc_" | sed '/^$/d' >  ./test/output/${ASCEND_DEVICE_ID}/my_output_loss.txt
