...
class ImagePot(Pot):
    ...
    def dcgan_like_arch(self, opts, noise, is_training, reuse, keep_prob):
        ...
        h0 = tf.reshape(h0, [-1, int(height), int(width), num_units])
        h0 = tf.nn.relu(h0)
        layer_x = h0
...
#         emb_c_loss = emb_c_loss / tf.stop_gradient(emb_c_loss)
    ...
    def add_least_gaussian2d_ops(self, opts):
        ...
        print(tf.eye(npoints))
        print("############")
        print(tf.ones(npoints))
        print("############")
        print(tf.cast(npoints, tf.float32))
        a = tf.eye(npoints) - tf.ones(npoints) / tf.cast(npoints, tf.float32)
    ...
    def _train_internal(self, opts):
        ...
        for _epoch in range(opts["gan_epoch_num"]):
            ...
            for _idx in range(int(batches_num)):
