...
# 修改参数
sed -i 's/10009/500/' ./configs/mnasnet_config.py
...
if [ x"${modelarts_flag}" != x ];
then
    python3.7 ./mnasnet_main.py --data_url=${data_path}/imagenet_tfrecord --train_url=${output_path}
else
    python3.7 ./mnasnet_main.py --data_url=${data_path}/imagenet_tfrecord --train_url=${output_path}  1>${print_log} 2>&1
fi

# 性能相关数据计算
step_per_sec=`grep "global_step/sec" ${print_log} | grep -v "basic_session_run_hooks.py" | awk '{print $NF}' | tail -n 10 | awk '{sum+=$1} END {print"",sum/NR}' | sed s/[[:space:]]//g`
StepTime=`awk 'BEGIN{printf "%.4f\n", '1'/'${step_per_sec}'}'`
FPS=`awk 'BEGIN{printf "%.2f\n", '${batch_size}'/'${StepTime}'}'`

# 精度相关数据计算
train_accuracy=`grep "top_1_accuracy" ${print_log} | tail -n 2 | head -n 1 | awk '{print $9}' | tr -d ","`
# 提取所有loss打印信息
grep "tensorflow:loss =" ${print_log} | awk '{print $3}' | tr -d "," > ./test/output/${ASCEND_DEVICE_ID}/my_output_loss.txt
