...
# from tensorflow15 import tensorflow as tf
...
flags.DEFINE_string('data_url', default='obs://id0728--mnasnet/data/imagenet-tfrecord/imagenet_tfrecord/', help=('data path'))
flags.DEFINE_string('train_url', default='obs://id0728--mnasnet/res/', help=('out path'))
...
def build_model_fn(features, labels, mode, params):
  ...
  if mode == tf.estimator.ModeKeys.PREDICT:
    ...
    if FLAGS.export_moving_average:
      ...
      restore_checkpoint = tf.train.latest_checkpoint(model_dir)
  ...
  if is_training:
    ...
    with tf.control_dependencies(update_ops + tf.contrib.summary.all_summary_ops()):
...
def main(unused_argv):
  ...
  params.validate()
  params.lock()
  print('Flags tpu:', FLAGS.tpu, params.use_tpu)
  ...
  # # Initializes model parameters.初始化模型参数
 ...
 # tf.logging.info("######################\nFlags.mode = {}\n###################".format(FLAGS.mode))
  ...
  if FLAGS.mode == 'eval':
    ...
    for ckpt in tf.train.checkpoints_iterator(
        model_dir, timeout=FLAGS.eval_timeout):
