...
def obs_data2modelarts(config):
    ...
    print("===>>>Copy files from obs:{} to modelarts dir:{}".format(config.data_url, config.SESEMI_data_dir))
    mox.file.copy_parallel(src_url=config.data_url, dst_url=config.SESEMI_data_dir)
    end = datetime.datetime.now()
    print("===>>>Copy from obs to modelarts, time use:{}(s)".format((end - start).seconds))
    files = os.listdir(config.SESEMI_data_dir)
    print("===>>>Files:", files)
