...
def ExtractLocalFeatures(image, image_scales, max_feature_num, abs_thres, iou,
  ...
  ## TODO: rf, stride, padding = [291.0, 16.0 * stride_factor, 145.0]
  rf, stride, padding = [291.0 / 8, 16.0 * stride_factor / 8, 145.0 / 8]
  ## feature_depth = 1024
  feature_depth = 128
  ...
  def _ProcessSingleScale(scale_index, boxes, features, scales, scores):
    ...
    rf_boxes = tf.divide(rf_boxes, scale, name="rf_boxes")
    attention_prob = tf.reshape(attention_prob, [-1], name="attention_prob")
    feature_map = tf.reshape(feature_map, [-1, feature_depth], name="feature_map")

    # Use attention score to select feature vectors.
    indices = tf.reshape(tf.where(attention_prob >= abs_thres), [-1], name="indices")
  ...
  nms_max_boxes= tf.squeeze(nms_max_boxes)
