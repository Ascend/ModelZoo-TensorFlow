...
from model import pooling as pooling_layers
...
class _IdentityBlock(tf.keras.Model):
  ...
  def call(self, input_tensor, training=False):
    ...
    tf.add_to_collection(tf.GraphKeys.UPDATE_OPS, self.bn2a.updates)

    x = tf.nn.relu(x)

    x = self.conv2b(x)
    x = self.bn2b(x, training=training)
    tf.add_to_collection(tf.GraphKeys.UPDATE_OPS, self.bn2b.updates)

    x = tf.nn.relu(x)

    x = self.conv2c(x)
    x = self.bn2c(x, training=training)
    tf.add_to_collection(tf.GraphKeys.UPDATE_OPS, self.bn2c.updates)
...
class _ConvBlock(tf.keras.Model):
  ...
  def call(self, input_tensor, training=False):
    ...
    tf.add_to_collection(tf.GraphKeys.UPDATE_OPS, self.bn_shortcut.updates)
...
class ResNet50(tf.keras.Model):
  ...
  def build_call(self, inputs, training=True, intermediates_dict=None):
    ...
    tf.add_to_collection(tf.GraphKeys.UPDATE_OPS, self.bn_conv1.updates)
    x = tf.nn.relu(x)
