...
if [[ $1 == --help || $1 == -h ]];then
    ...
    --train_steps            # max_step for trainin
...
then
    python3.7 train.py \
      ...
      --data_path ${data_path}/train_inputs \
      --dataset_version gld_v2_clean > ${print_log} 2>&1
else
    python3.7 train.py \
      --output_path ${output_path} \
      --data_path ${data_path}/train_inputs \
      --dataset_version gld_v2_clean \
      --save_interval 50000 > ${print_log} 2>&1      
fi

# 性能相关数据计算
StepsTime=`grep "Train global steps" ${print_log} | awk -F "cost=" '{print $2}' | awk '{print $1}' | tail -n 10 | awk '{sum+=$1} END {print"",sum/NR}' | awk '{print $1}'`
StepTime=`awk 'BEGIN{printf "%.2f\n", '${StepsTime}'/'500'}'`
FPS=`awk 'BEGIN{printf "%.2f\n", '${batch_size}'/'${StepTime}'}'`

# 提取所有loss打印信息
grep "Eval end in global" ${print_log} | awk -F "desc_loss=" '{print $2}' | awk '{print $1}' | tr -d "," > ./test/output/${ASCEND_DEVICE_ID}/my_output_loss.txt
