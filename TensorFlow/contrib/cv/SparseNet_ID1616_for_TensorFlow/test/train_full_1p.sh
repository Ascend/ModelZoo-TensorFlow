...
#基础参数，需要模型审视修改
#网络名称，同目录名称
Network="SparseNet_ID1616_for_TensorFlow"
#训练epoch
train_epochs=10
#训练batch_size
batch_size=128
...
do
	...
	--drop_1 150 \
	--drop_2 225 \
	--max_epoch 3 >  $cur_path/test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log 2>&1
...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
grep "global_step "  $cur_path/test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log |awk -F'global_step ' '{print $2}'| tail -n +2 > $cur_path/test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}_time.txt
...
#最后一个迭代loss值，不需要修改
grep 'cross_entropy_loss:' $cur_path/test/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log |awk -F'cross_entropy_loss:' '{print $2}' > $cur_path/test/output/$ASCEND_DEVICE_ID/train_${CaseName}_loss.txt
