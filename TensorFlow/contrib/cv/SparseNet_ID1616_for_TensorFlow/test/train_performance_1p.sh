...
do
	...
	--drop_1 150 \
	--drop_2 225 \
	--max_epoch 5 >  $cur_path/test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log 2>&1
...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
grep "global_step "  $cur_path/test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log |awk -F'global_step ' '{print $2}'| tail -n +3 > $cur_path/test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}_time.txt
