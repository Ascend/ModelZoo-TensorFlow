...
def resnet_v2_m(inputs,
                ...
                reuse=None,
                scope=None):
    # print("begin resnet_v2", flush=True)
    with tf.variable_scope(scope, 'resnet_v2', [inputs], reuse=reuse) as sc:
        ...
        with slim.arg_scope([slim.conv2d, bottleneck, utils.stack_blocks_dense],
            ...
            with slim.arg_scope([slim.batch_norm], is_training=is_training):
                ...
                # print("begin unit", flush=True)
                net = utils.stack_blocks_dense(net, blocks, output_stride)
                # print("finish unit", flush=True)
...
def resnet_v2_m_100(inputs,
                    ...
                    reuse=None,
                    scope='resnet_v2_100'):
    """ResNet-100 model of [1]. See resnet_v2() for arg and return description."""
    blocks = [
        resnet_v2_block('block1', base_depth=16, num_units=3, stride=2),
        resnet_v2_block('block2', base_depth=32, num_units=13, stride=2),
        resnet_v2_block('block3', base_depth=64, num_units=30, stride=2),
        resnet_v2_block('block4', base_depth=128, num_units=3, stride=2),
    ]
    # print("finish blocks", flush=True)
    return resnet_v2_m(inputs, blocks, num_classes, is_training=is_training, return_raw=return_raw,
                       global_pool=global_pool, output_stride=output_stride, include_root_block=True,
                       spatial_squeeze=spatial_squeeze, reuse=reuse, scope=scope)
