...
then
    ...
    python3.7 ./train.py --data_path=${data_path} --output_path=${output_path} --train_epochs=${train_epochs} &> ${print_log}
fi

# 性能相关数据计算
StepTime=`grep "sec/step :" ${print_log} | tail -n 10 | awk -F':' '{print $5}' |cut -b -15 | awk '{sum+=$1} END {print sum/NR}'`
