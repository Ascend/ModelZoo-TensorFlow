...
#   Testing                                                                   #
###############################################################################
def load_sess(modelpath=None, sess=None, modpath=None, mods=None, var_list=None, init=False, init_dict=None):
    #load session if there exist any models, and initialize the sess if not
    assert modpath == None or mods == None
    assert (not modelpath == None) or (
        not modpath == None) or (not modpath == None)
    if sess == None:
        sess = tf.Session()
    if init:
    ...
    if modpath != None:
        ...
        if ckpt:
            mod = tf.train.latest_checkpoint(modelpath)
            print('[*]loading from model:', mod)
            saver.restore(sess, mod)
        else:
