...
def _crop(img, labels, max_loop=250):
    ...
    def crop_loop_body(i, img, labels):
        ...
        valid_crop = tf.cond(tf.math.reduce_any(value >= 1), lambda: valid_crop, lambda: 0)
        centers = (labels[:, :2] + labels[:, 2:4]) / 2
        mask_a = tf.reduce_all(tf.math.logical_and(roi[:2] < centers, centers < roi[2:]),axis=1)
        labels_t = tf.boolean_mask(labels, mask_a)
        valid_crop = tf.cond(tf.reduce_any(mask_a), lambda: valid_crop, lambda: 0)
        ...
        '''
        return tf.cond(tf.constant(valid_crop == 1, dtype=tf.bool),
                       lambda: (max_loop, img_t, labels_t),
                       lambda: (i + 1, img, labels))
...
def _resize(img, labels, img_dim):
    ...
    def resize(method):
        def _resize():
            ...
            return tf.image.resize_images(
                img, [img_dim, img_dim], method=method, align_corners=True)
        return _resize
    '''
    ...
    img = tf.case([(tf.equal(resize_case, 0), resize(2)),
                   (tf.equal(resize_case, 1), resize(3)),
                   (tf.equal(resize_case, 2), resize(1))],
                  default=resize(0))
