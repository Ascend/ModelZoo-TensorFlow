...
def MultiBoxLoss(num_class=2, neg_pos_ratio=3):
    ...
    def multi_box_loss(y_true, y_pred):
        ...
        loss_landm = _smooth_l1_loss(tf.ragged.boolean_mask(landm_true, mask_landm_b).values,
                                     tf.ragged.boolean_mask(landm_pred, mask_landm_b).values)
        loss_landm = tf.reduce_mean(loss_landm)

        # localization loss (smooth L1)
        mask_pos_b = tf.broadcast_to(mask_pos, tf.shape(loc_true))
        loss_loc = _smooth_l1_loss(tf.ragged.boolean_mask(loc_true, mask_pos_b).values,
                                   tf.ragged.boolean_mask(loc_pred, mask_pos_b).values)
        loss_loc = tf.reduce_mean(loss_loc)

        # classification loss (crossentropy)
        # 1. compute max conf across batch for hard negative mining

        # loss_class = tf.where_v2(mask_neg, 1 - class_pred[:, 0][..., tf.newaxis], 0)
        ...
        loss_class_mask_b = tf.broadcast_to(loss_class_mask,
                                            tf.shape(class_pred))
        filter_class_true = tf.ragged.boolean_mask(tf.cast(mask_pos, tf.float32),
                                            loss_class_mask).values
        filter_class_pred = tf.ragged.boolean_mask(class_pred, loss_class_mask_b).values
