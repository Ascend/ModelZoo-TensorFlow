...
from modules.models import RetinaFaceModel, RetinaFaceModelFunc
from modules.utils import (set_memory_growth, load_yaml, draw_bbox_landm,
                           pad_input_image, recover_pad_output)


# flags.DEFINE_string('model', './configs/retinaface_res50.yaml',
#                     'config file path')
flags.DEFINE_string('model', 'res50', 'model type')
flags.DEFINE_string('gpu', '4', 'which gpu to use')
...
def main(_argv):
    ...
    model = RetinaFaceModelFunc(cfg, training=False, iou_th=FLAGS.iou_th,
                            score_th=FLAGS.score_th)


    # load checkpoint
    checkpoint_dir = os.path.join(root_path, 'checkpoints', cfg['sub_name']+'_backup')
    ...
    testset_list = os.path.join(root_path, testset_folder, 'label.txt')
    ...
    for img_index, img_path in enumerate(img_paths):
        ...
        save_name = os.path.join(
            root_path, FLAGS.save_folder, sub_dir, img_name.replace('.jpg', '.txt'))
