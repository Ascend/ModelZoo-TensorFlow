...
class FixMatch(CTAReMixMatch):
        ...
        with tf.train.MonitoredTrainingSession(
            ...
            while self.tmp.step < train_nimg:
                loop = trange(self.tmp.step % report_nimg, report_nimg, batch,
                              ...
                              desc='Epoch %d/%d' % (1 + (self.tmp.step // report_nimg), train_nimg // report_nimg))#65536,64
                for _ in loop:
                    self.train_step(train_session, gen_labeled, gen_unlabeled)
                    while self.tmp.print_queue:
                        loop.write(self.tmp.print_queue.pop(0))
                if(self.tmp.step//report_nimg  >= FLAGS.less_step):
                    break
            while self.tmp.print_queue:
    ...
    def model(self, batch, lr, wd, wu, confidence, uratio, ema=0.999, **kwargs):
        ...
        loss_scale_manager = ExponentialUpdateLossScaleManager(init_loss_scale=2 ** 32, incr_every_n_steps=10000,
                                                               decr_every_n_nan_or_inf=2, decr_ratio=0.5)
        optimizer = NPULossScaleOptimizer(npu_tf_optimizer(tf.train.MomentumOptimizer(lr, 0.9, use_nesterov=True)), loss_scale_manager)
        train_op = optimizer.minimize(loss_xe + wu * loss_xeu + wd * loss_wd, colocate_gradients_with_ops=True)
        with tf.control_dependencies([train_op]):
            train_op = tf.group(*post_ops)

        return utils.EasyDict(
            xt=xt_in, x=x_in, y=y_in, label=l_in, train_op=train_op,loss_sum =loss_xe + wu * loss_xeu + wd * loss_wd, #test loss
...
if __name__ == '__main__':
    ...
    flags.DEFINE_integer('less_step', 1<<10,"Less_step num.")
