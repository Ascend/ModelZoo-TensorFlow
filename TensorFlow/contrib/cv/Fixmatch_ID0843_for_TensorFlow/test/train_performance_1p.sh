...
if [[ $1 == --help || $1 == -h ]];then
    ...
    --less_step	             the number of step, the number 1 represents 1 epoch, default is 1024
...
python fixmatch.py \
        ...
        --dataset=${dataset} \
        --train_dir=${train_dir} \
        --less_step=${less_step} > ${cur_path}/test/output/${ASCEND_DEVICE_ID}/loss+pref.log
wait
end=$(date +%s)
...
#打印，不需要修改
echo "less_step Train Accuracy : ${train_accuracy}"
