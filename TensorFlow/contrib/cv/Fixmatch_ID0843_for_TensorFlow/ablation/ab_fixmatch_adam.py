...
class AB_FixMatch_Adam(FixMatch):
    def model(self, batch, lr, wd, wu, confidence, uratio, beta1, beta2, ema=0.999, **kwargs):
        ...
        if beta2 == 0:
            train_op = npu_tf_optimizer(tf.train.AdamOptimizer(lr, beta1, beta2, 1e-4)).minimize(
                loss_xe + wu * loss_xeu + wd * loss_wd, colocate_gradients_with_ops=True)
        else:
            train_op = npu_tf_optimizer(tf.train.AdamOptimizer(lr, beta1, beta2)).minimize(
                loss_xe + wu * loss_xeu + wd * loss_wd, colocate_gradients_with_ops=True)
...
def main(argv):
    ...
    model = AB_FixMatch_Adam(
        os.path.join(FLAGS.train_dir, dataset.name, AB_FixMatch_Adam.cta_name()),
        dataset,
        lr=FLAGS.lr,
        wd=FLAGS.wd,
        arch=FLAGS.arch,
