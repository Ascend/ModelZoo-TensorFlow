...
class AB_FixMatch_SharpenConditionalConsistency(FixMatch):
    def model(self, batch, lr, wd, wu, uratio, T, ema=0.999, confidence=0.95, **kwargs):
        ...
        sharp_labels = tf.stop_gradient(tf.nn.softmax(logits_weak / T))
        loss_xeu = tf.nn.softmax_cross_entropy_with_logits_v2(labels=sharp_labels, logits=logits_strong)
...
def main(argv):
    ...
    model = AB_FixMatch_SharpenConditionalConsistency(
        os.path.join(FLAGS.train_dir, dataset.name, AB_FixMatch_SharpenConditionalConsistency.cta_name()),
        dataset,
        lr=FLAGS.lr,
        wd=FLAGS.wd,
        arch=FLAGS.arch,
...
if __name__ == '__main__':
    ...
    flags.DEFINE_float('T', 1, 'Temperature for sharpening.')
