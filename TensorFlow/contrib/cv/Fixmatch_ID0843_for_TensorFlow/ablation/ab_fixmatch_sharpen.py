...
class AB_FixMatch_Sharpen(FixMatch):
    def model(self, batch, lr, wd, wu, uratio, T, ema=0.999, confidence=0.95, **kwargs):
        ...
        pseudo_labels = tf.stop_gradient(tf.nn.softmax(logits_weak / T))
...
def main(argv):
    ...
    model = AB_FixMatch_Sharpen(
        os.path.join(FLAGS.train_dir, dataset.name, AB_FixMatch_Sharpen.cta_name()),
        dataset,
        lr=FLAGS.lr,
        wd=FLAGS.wd,
        arch=FLAGS.arch,
