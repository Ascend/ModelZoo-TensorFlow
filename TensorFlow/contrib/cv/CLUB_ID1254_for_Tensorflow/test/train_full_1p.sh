...
if [ x"${modelarts_flag}" != x ];
then
    python3 ./MI_DA/main_DANN.py --data_path=${data_path}/ --save_path=${output_path}/ --train_url=${output_path} 1>${print_log} 2>&1
else
    python3 ./MI_DA/main_DANN.py --data_path=${data_path}/ --save_path=${output_path}/ --train_url=${output_path} 1>${print_log} 2>&1
fi
...
# 精度相关数据计算
train_accuracy=`grep "p_acc" ${print_log} | awk '{print $NF}' | awk 'NR==1{max=$1;next}{max=max>$1?max:$1}END{print max}'`

# 提取所有loss打印信息
grep "mi_loss" ${print_log} | awk -F "mi_loss" '{print $2}' | awk '{print $2}' > ./test/output/${ASCEND_DEVICE_ID}/my_output_loss.txt 
