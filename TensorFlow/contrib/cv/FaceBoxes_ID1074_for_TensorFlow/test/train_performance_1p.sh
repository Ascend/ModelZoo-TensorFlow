...
#基础参数，需要模型审视修改
#网络名称，同目录名称
Network="FaceBoxes_ID1074_for_TensorFlow"
#训练epoch
train_epochs=1
#训练batch_size
batch_size=16
...
do
	...
	--step=8  >  $cur_path/test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log 2>&1
...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
TrainingTime_step=`grep 'global_step/sec' $cur_path/test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log |awk '{print $2}' |tail -n +2|awk '{sum+=$1} END {print sum/NR}'`
