...
def batch_non_max_suppression(
    ...
    # Replace tf.map_fn with for loop
    out_boxes=[]
    out_scores=[]
    out_numboxes=[]
    batch_size=16 if self.is_training else 1
    for i in range(batch_size):
        each_input=boxes[i],scores[i]
        each_out_box,each_out_score,each_out_numboxes=fn(each_input)
        out_boxes.append(each_out_box)
        out_scores.append(each_out_score)
        out_numboxes.append(each_out_numboxes)
    boxes=tf.stack(out_boxes,axis=0)
    scores=tf.stack(out_scores,axis=0)
    num_detections=tf.stack(out_numboxes,axis=0)
