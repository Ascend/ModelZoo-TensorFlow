...
def obs_data2modelarts_data(config):
    """
    Copy train_gpu data from obs to modelarts by using moxing api.
    """
...
def obs_data2modelarts_result(config):
    """
    Copy train_gpu data from obs to modelarts by using moxing api.
    """
    start = datetime.datetime.now()
    print("===>>>Copy files from obs:{} to modelarts dir:{}".format(config.obs_checkpoint_dir,
                                                                    config.modelarts_result_dir))
    mox.file.copy_parallel(src_url=config.obs_checkpoint_dir, dst_url=config.modelarts_result_dir)
    end = datetime.datetime.now()
    print("===>>>Copy from obs to modelarts, time use:{}(s)".format((end - start).seconds))
    files = os.listdir(config.modelarts_data_dir)
    print("===>>>Files:", files)
...
def modelarts_result2obs(FLAGS):
    ...
    # obs_result_dir = os.path.join(FLAGS.obs_dir, 'modelatrs_result')
    obs_result_dir = os.path.join(FLAGS.obs_dir, 'modelarts_result_mixpre')
