...
#基础参数，需要模型审视修改
#网络名称，同目录名称
Network="DDCGAN_ID2123_for_TensorFlow"
#训练epoch
train_epochs=
#训练batch_size
batch_size=24
...
do
    ...
    python3 main.py --data_path=${data_path}/data/ --output_path=./output --n_batches=${train_steps} > ${cur_path}test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log 2>&1 &
...
grep 'G_loss:' $cur_path/test/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log|awk '{print $2}'|sed 's/,//g' > $cur_path/test/output/$ASCEND_DEVICE_ID/train_${CaseName}_loss.txt
