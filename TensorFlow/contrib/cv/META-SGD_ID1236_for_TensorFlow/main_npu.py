...
flags.DEFINE_string("data_path", "./miniImagenet", "dataset path")
flags.DEFINE_integer('num_total_batches_train', 200000, 'num total batches train')
flags.DEFINE_integer('num_total_batches_test', 600, 'num total batches test')
flags.DEFINE_integer('summary_interval', 100, 'summary interval')
flags.DEFINE_integer('print_interval', 100, 'print interval')
...
def train(model, saver, sess, exp_string, data_generator, resume_itr=0):
    ...
    for itr in range(resume_itr, FLAGS.pretrain_iterations + FLAGS.metatrain_iterations):
        ...
        if (itr!=0) and itr % PRINT_INTERVAL == 0:
            ...
            print_str += ': ' + str(np.mean(prelosses)) + ', ' + str(np.mean(postlosses))  + '  time: ' + str((time.time()-time_start)/PRINT_INTERVAL)
            print(print_str)
            prelosses, postlosses = [], []
...
def main():
    ...
    if FLAGS.datasource == 'sinusoid':
        data_generator = DataGenerator(sess, FLAGS.update_batch_size*2, FLAGS.meta_batch_size)
    else:
        if FLAGS.metatrain_iterations == 0 and FLAGS.datasource == 'miniimagenet':
            assert FLAGS.meta_batch_size == 1
            assert FLAGS.update_batch_size == 1
            data_generator = DataGenerator(sess, 1, FLAGS.meta_batch_size)  # only use one datapoint,
        else:
            if FLAGS.datasource == 'miniimagenet': # TODO - use 15 val examples for imagenet?
                if FLAGS.train:
                    data_generator = DataGenerator(sess, FLAGS.update_batch_size+15, FLAGS.meta_batch_size)  # only use one datapoint for testing to save memory
                else:
                    data_generator = DataGenerator(sess, FLAGS.update_batch_size*2, FLAGS.meta_batch_size)  # only use one datapoint for testing to save memory
            else:
                data_generator = DataGenerator(sess, FLAGS.update_batch_size*2, FLAGS.meta_batch_size)  # only use one datapoint for testing to save memory
