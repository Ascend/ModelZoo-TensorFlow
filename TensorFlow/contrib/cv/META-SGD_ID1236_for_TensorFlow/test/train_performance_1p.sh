...
#基础参数，需要模型审视修改
#网络名称，同目录名称
Network="META-SGD_ID1236_for_TensorFlow"
#训练epoch
train_epochs=
#训练batch_size
batch_size=4
...
do
    ...
    python3 main_npu.py \
        --data_path=${data_path}/miniImagenet/ \
        --num_total_batches_train=1 \
        --num_total_batches_test=1 \
        --summary_interval=1 \
        --print_interval=1 \
        --datasource=miniimagenet \
        --metatrain_iterations=${train_steps} \
        --meta_batch_size=4 \
        --update_batch_size=1 \
        --update_lr=0.01 \
        --num_updates=5 \
        ...
        --num_filters=32 \
        --max_pool=True > ${cur_path}test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log 2>&1
...
grep 'Iteration' $cur_path/test/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log|awk '{print $3}'|sed 's/,//g' > $cur_path/test/output/$ASCEND_DEVICE_ID/train_${CaseName}_loss.txt
