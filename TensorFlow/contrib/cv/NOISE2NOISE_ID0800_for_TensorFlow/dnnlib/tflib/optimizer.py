...
class Optimizer:
    ...
    def register_gradients(self, loss: TfExpression, trainable_vars: Union[List, dict]) -> None:
        ...
        print("var.device:{}".format(trainable_vars[0].device))
        assert all(var.device == dev for var in trainable_vars)
        dev = '/device:NPU:0'
        # Register device and compute gradients.
        with tf.name_scope(self.id + "_grad"), tf.device(None):
            ...
            grads = self._dev_opt[dev].compute_gradients(loss, trainable_vars,
    ...
    def apply_updates(self) -> (tf.Operation, str, TfExpression, TfExpression):
        ...
        with tfutil.absolute_name_scope(self.scope):
            ...
            for dev_idx, dev in enumerate(devices):
                with tf.name_scope("ProcessGrads%d" % dev_idx), tf.device(None):
                    sums = []
            ...
            for dev_idx, (dev, grads) in enumerate(dev_grads.items()):
                with tf.name_scope("ApplyGrads%d" % dev_idx), tf.device(None):
            ...
            return tf.group(*ops, name="TrainingOp"), self.id, grad_ok, ls_var
