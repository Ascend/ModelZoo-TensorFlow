...
class BaseNetwork(object):
            ...
            with tf.variable_scope(name) as scope:
                for i, size in enumerate(hidden_layers):
                    ...
                    if self.dropout is not None:
                        input_feat = slim.dropout(input_feat, keep_prob=self.dropout, is_training=is_training,
                                                  scope='dropout_%d' % i)

                output_feat = slim.fully_connected(input_feat, output_dim,
                                                   trainable=is_training, activation_fn=None, reuse=tf.AUTO_REUSE,
                                                   scope="fc_out")
    ...
    def cross_entropy_with_labelvec(self, prob, label_vecs, target=1, weight=None, sample_weight=None, neg_loss=0,
                                    mask=None):
    ...
    def triplet_margin_loss(self, anchor, positive, negative):
        dist = self.distance_metric(anchor, positive) - self.distance_metric(anchor,
                                                                             negative) + self.args.triplet_margin
    ...
    def test_step(self, sess, blobs, score_op):
        ...
        for key in score_op.keys():
            score[key][0] = {
                ...
                # (a, o): tf.convert_to_tensor(score[key][0][:, i])
                ##################################################
