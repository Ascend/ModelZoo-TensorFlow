...
then
    ...
    python3.7 ${code_dir}/run_symnet.py --name UT_best --data UT --epoch train_epochs --obj_pred UT_obj_lr1e-3_test_ep260.pkl --trained_weight snapshot_epoch_599.ckpt --batchnorm  --wordvec onehot  --lr 1e-4 --bz batch_size --lambda_cls_attr 1 --lambda_cls_obj 0.5 --lambda_trip 0.5 --lambda_sym 0.01 --lambda_axiom 0.03 --data_url=${data_path} --train_url=${output_path}
else
