...
#基础参数，需要模型审视修改
#网络名称，同目录名称
Network="MZSR_ID1072_for_TensorFlow"
#训练epoch
#train_epochs=1
#训练batch_size
batch_size=5
...
do
	...
	#sed -i "s|for x in range(37500)|for x in range(300)|g" train.py
    ...
    python3 main.py \
     ...
     --gpu 0 \
     --trial 0 \
     --dataset=${data_path} | tee ${cur_path}test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log   2>&1
    #sed -i "s|for x in range(300)|for x in range(37500)|g" train.py
...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
TrainingTime=`grep "Loss:"  $cur_path/test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log |awk 'END{print $9}'`
