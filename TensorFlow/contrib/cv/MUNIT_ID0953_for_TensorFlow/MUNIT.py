...
class MUNIT(object) :
    def __init__(self, sess, args):
        ...
        self.dataset_name = args.data_path
        ...
        # lossScale相关
        self.bert_loss_scale = args.bert_loss_scale
        self.mmgr = {}
    ...
    def build_model(self):
        ...
        # 开启loss_scale
        self.G_optim = self.open_loss_scale(self.G_optim, 'G')
        self.D_optim = self.open_loss_scale(self.D_optim, 'D')

        self.G_optim = self.G_optim.minimize(self.Generator_loss, var_list=G_vars)
        self.D_optim = self.D_optim.minimize(self.Discriminator_loss, var_list=D_vars)
    ...
    def train(self):
        ...
        for epoch in range(start_epoch, self.epoch):
            ...
            for idx in range(start_batch_id, self.iteration):
                ...
                batch_A_images, batch_B_images, fake_A, fake_B, _, g_loss, summary_str, g_scale = self.sess.run(
                    [self.real_A, self.real_B, self.fake_A, self.fake_B, self.G_optim, self.Generator_loss,
                     self.G_loss, self.mmgr['G'].get_loss_scale()], feed_dict=train_feed_dict)
                self.writer.add_summary(summary_str, counter)

                # display training status
                counter += 1
                print("Epoch: [%2d] [%6d/%6d] time: %4.4f d_loss: %.8f, g_loss: %.8f, d_scale: %d, g_scale: %d\n" \
                      % (epoch, idx, self.iteration, time.time() - start_time, d_loss, g_loss, d_scale, g_scale))
    ...
    def test(self):
        ...
        test_A_files = self.trainA_dataset
        test_B_files = self.trainB_dataset
    ...
    def open_loss_scale(self, opt, key):
        opt_tmp = opt
        if self.bert_loss_scale == 0:
            # loss_scale_manager = ExponentialUpdateLossScaleManager(init_loss_scale=2 ** 32, incr_every_n_steps=1000,
            #                                                        decr_every_n_nan_or_inf=2, decr_ratio=0.5)
            loss_scale_manager = ExponentialUpdateLossScaleManager(init_loss_scale = 2 ** 10, incr_every_n_steps = 100, decr_every_n_nan_or_inf = 2, decr_ratio = 0.8)
            print("lossScale type: exponential")
        elif self.bert_loss_scale >= 1:
            loss_scale_manager = FixedLossScaleManager(loss_scale=self.bert_loss_scale)
        else:
