...
parser.add_argument('--mix_precision', type=str, default='on', \
    help='Turn on mix precision [off/on]')
...
def train():
    with tf.Graph().as_default():
        ...
        # trainer = npu_tf_optimizer(tf.train.AdamOptimizer(learning_rate))
        # trainer = tf.train.AdamOptimizer(learning_rate)
        opt_tmp = tf.train.AdamOptimizer(learning_rate)
        # loss scale
        loss_scale_manager = ExponentialUpdateLossScaleManager(init_loss_scale=2 ** 32, incr_every_n_steps=1000, \
            decr_every_n_nan_or_inf=2, decr_ratio=0.5)
        trainer = NPULossScaleOptimizer(opt_tmp, loss_scale_manager)
        # loss scale, end
