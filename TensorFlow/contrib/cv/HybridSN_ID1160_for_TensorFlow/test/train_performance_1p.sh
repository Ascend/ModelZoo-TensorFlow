...
#基础参数，需要模型审视修改
#网络名称，同目录名称
Network="HybridSN_ID1160_for_TensorFlow"
#训练epoch
train_epochs=
#训练batch_size
batch_size=256
...
do
    ...
    python3 Hybrid-Spectral-Net.py --train_step=${train_steps} --datapath=${data_path} > ${cur_path}test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log 2>&1
...
grep "16238/16238" $cur_path/test/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log|awk -F '16238/16238' '{print $2}'|grep 'loss:'|awk '{print $7}' > $cur_path/test/output/$ASCEND_DEVICE_ID/train_${CaseName}_loss.txt
