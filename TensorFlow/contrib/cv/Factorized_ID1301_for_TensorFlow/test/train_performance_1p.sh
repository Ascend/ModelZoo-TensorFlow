...
cp -r ${data_path}/MA-new-code-04-22-09-36/code/utils ./
mkdir ./data
mkdir ./workplace
...
batch_size=32
epochs=1
python3.7 train.py --input_dir=${data_path}/data10w --output_dir=${output_path} --data_url=./data --train_url=./workplace --max_epochs=${epochs} > ${print_log} 2>&1

step=`grep "epoch 200" ${print_log} | awk '{print $5}' | tail -n 1`
# 性能相关数据计算
FPS=`grep "image/sec" ${print_log} | awk '{print $7}' | tail -n 1`

# 精度相关数据计算
loss_diversity=`grep "loss_diversity" ${print_log} | awk '{print $2}' | tail -n 1`
loss_decay=`grep "loss_decay" ${print_log} | awk '{print $2}' | tail -n 1`
# 提取所有loss打印信息
grep "loss_align" ${print_log} | awk '{print $2}' > ./test/output/${ASCEND_DEVICE_ID}/my_output_loss.txt
...
echo "progress epoch 200 step $step image/sec $FPS remaining 0m"
echo "loss_align $ActualLoss"
echo "loss_diversity $loss_diversity"
