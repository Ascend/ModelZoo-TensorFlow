...
if not ('data_url' in FLAGS._flags()):
    tf.app.flags.DEFINE_string('data_url', '../dataset/cifar10',
...
def load_cifar10():
    ...
    for i, data_fn in enumerate(
            sorted(glob.glob(FLAGS.data_url + '/cifar-10-batches-py/data_batch*'))):
    ...
    test = unpickle(FLAGS.data_url + '/cifar-10-batches-py/test_batch')
    ...
    np.save('{}/components'.format(FLAGS.data_url), components)
    np.save('{}/mean'.format(FLAGS.data_url), mean)
...
def prepare_dataset():
    ...
    dirpath = os.path.join(FLAGS.data_url, 'seed' + str(FLAGS.dataset_seed))
...
def inputs(batch_size=100,
    ...
    filename_queue = generate_filename_queue(filenames, FLAGS.data_url, num_epochs)
...
def unlabeled_inputs(batch_size=100,
    ...
    filenames = [os.path.join('seed' + str(FLAGS.dataset_seed), filename) for filename in filenames]
    filename_queue = generate_filename_queue(filenames, FLAGS.data_url)
    image, label = read(filename_queue)
