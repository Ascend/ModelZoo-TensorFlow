...
if not ('aug_trans' in FLAGS._flags()):
    tf.app.flags.DEFINE_bool('aug_trans', False, "")
if not ('aug_flip' in FLAGS._flags()):
...
def generate_filename_queue(filenames, data_url, num_epochs=None):
    print("filenames in queue:", filenames)
    for i in range(len(filenames)):
        filenames[i] = os.path.join(data_url, filenames[i])
    with npu_scope.without_npu_compile_scope():
