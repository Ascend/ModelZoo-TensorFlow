...
do
    ...
    nohup python3 -u train_forward_test.py "test_run" \
        --data_root $data_path/chess --preset synthesizer/presets/chess.json \
            --tacotron_train_steps 200000 > ${cur_path}/test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log 2>&1 &  #/home/jiayansuo/Lip2wav_train/Lip2Wav-master_npu_20210602003231/Dataset/chess/ \ 
done 
wait
...
grep "loss=" $cur_path/test/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log| awk '{print ($5)}'| grep -o '[0-9.]\+'> $cur_path/test/output/$ASCEND_DEVICE_ID/train_${CaseName}_loss.txt # #awk  '{print ($14+$16)}'
