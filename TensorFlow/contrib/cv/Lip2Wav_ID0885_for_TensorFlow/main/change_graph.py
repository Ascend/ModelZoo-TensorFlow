...
with open("./synthesizer/presets/chess.json") as f:
...
with tf.variable_scope("Tacotron_model", reuse=tf.AUTO_REUSE) as scope:
    trainer.initialize(inputs, input_lengths, speaker_embeddings, split_infos=split_infos)
...
#sess_npu = tf.Session(config = config)

with tf.Session(config=config) as sess_npu:

    load_model(sess_npu, model_path, weights_dict) #trainer.sess

    saver = tf.train.Saver()
    saver.save(sess_npu, "/home/jiayansuo/Lip2wav_train/Lip2Wav-master_npu_20210602003231/main/trial_checkpoint/npu_graph.ckpt")
