...
class Tacotron2:
    def __init__(self, checkpoint_path, hparams, gta=False, model_name="Tacotron"):
        ...
        '''
        inputs = tf.placeholder(tf.float32, shape=(1, hparams.T, hparams.img_size, 
                                    hparams.img_size, 3), name="inputs"),
        
        input_lengths = tf.placeholder(tf.int32, shape=(1,), name="input_lengths"),
        
        targets = tf.placeholder(tf.float32, shape=(1, hparams.mel_step_size, hparams.num_mels), 
               name="mel_targets"),
        ...
        # SV2TTS
        speaker_embeddings = tf.placeholder(tf.float32, shape=(1, 256), 
               name="speaker_embeddings")
        ...
        def split_func(x, split_pos):
            ...
            for i in range(split_pos.shape[0]):
                print ("x slicing : ", x[:, :]) #start:start + split_pos[i]])
                rst.append(x[:, :]) #x[:, start:start + split_pos[i]])
        ...
        if model_conv:
            # print(split_infos[0])
            test_val = split_func(inputs[0], split_infos[0][:,0])

            print ("test val : ", test_val) #shape: 1,?,48,48,3

            new_inputs = tf.placeholder(tf.float32, shape=(1, 90, 48, 48, 3), name ="new_inputs") #tf.float32, shape=(1, hparams.T, hparams.img_size, 
                                    #hparams.img_size, 3), name="inputs")
        ...
        with tf.variable_scope("Tacotron_model") as scope:
            ...
            self.model.initialize(new_inputs, input_lengths[0], speaker_embeddings[0], split_infos=split_infos[0])
            #self.model.initialize(inputs[0], input_lengths[0], speaker_embeddings[0], split_infos=split_infos[0])
#             print ("gta val : ", gta )
#             if gta:

#                 self.model.initialize(inputs, input_lengths, speaker_embeddings, targets, gta=False,
#                                       split_infos=split_infos)

#                 # self.model.initialize(inputs, input_lengths, speaker_embeddings, targets, gta=gta,
#                 #                       split_infos=split_infos)
#             else:
#                 n_inputs= list(inputs)#[0]

#                 print (n_inputs[0][0], type(n_inputs))

#                 n_inputs[0][0] = 1.0

#                 print ('n vals : ', n_inputs)

#                 #print ("n_inputs shape : ", tf.shape(inputs)[0], type(inputs))
#                 # n_inputs[0][0] = 1
#                 #n_input_lengths = list(input_lengths[0])

#                 #input_lengths[0] = 1
#                 #speaker_embeddings[0] = 1
#                 self.model.initialize(inputs[0], input_lengths[0], speaker_embeddings[0], split_infos=split_infos[0])
        ...
        self.session = tf.Session(config = config) #TODO: =npu_config_proto(config_proto=config))
    ...
    def my_synthesize(self, seqs):
        ...
        print ("feed dict input_seqs : ", len(input_seqs))
        print ("feed dict params input len : ", len(input_lengths))
        feed_dict = {
        ...
        print ("before sess run mels, aligments ", len(self.mel_outputs), len(self.alignments))
        #TODO: Org code
        # mels, alignments = self.session.run(
        #     [self.mel_outputs, self.alignments],
        #     feed_dict=feed_dict)

        #print ("after sess run mels, aligments")

        mels = self.session.run(
            self.mel_outputs,
            feed_dict=feed_dict)

        print ("after sess run mels, aligments")

        #mels, alignments= list(self.mel_outputs[0]), self.alignments[0]

        mels = list(mels[0])

        #alignments = self.alignments[0]

        #print ("after sess run mels, aligments", mels.T, alignments)
        
        #TODO:
        #mels, alignments= list(self.mel_outputs[0]), self.alignments[0]

        #mels, alignments= list(mels[0]), self.alignments[0]
        # Trim the output
        '''
        ...
        return [mel.T for mel in mels], None
