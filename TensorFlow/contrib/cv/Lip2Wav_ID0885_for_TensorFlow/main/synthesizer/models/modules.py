...
class ZoneoutLSTMCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        if self.is_training:
            ...
            c = (1 - self._zoneout_cell) * npu_ops.dropout(new_c - prev_c,
                                                         (1 - self._zoneout_cell)) + prev_c
            h = (1 - self._zoneout_outputs) * npu_ops.dropout(new_h - prev_h,
...
class EncoderConvolutions3D:
    ...
    def conv3d(self, inputs, kernel_size, channels, strides, 
        ...
        with tf.variable_scope(scope):
            ...
            # skip relu
            batched_shape = tf.shape(batched)
            #batched = tf.reshape(batched,[batched_shape[0],batched_shape[1],batched_shape[2],batched_shape[3] * batched_shape[4]] )
            #activated = activation(batched)
            
            #activated = tf.reshape(activated, batched_shape)
            
            #tf.add_to_collection("activated_node", activated)
            return batched
...
class EncoderRNN:
    ...
    def __call__(self, inputs, input_lengths):
        #print ("encoder RNN inputs : ", tf.shape(inputs))
        with tf.variable_scope(self.scope):
            ...
            # print ("inside EncoderRNN")
            # print ("concat cond : ", tf.concat(outputs, axis=2))
    ...
    def __call__(self, inputs):
        ...
        with tf.variable_scope(self.scope):
            for i, size in enumerate(self.layers_sizes):
                # skip relu
                dense = tf.layers.dense(x, units=size, activation=None, #self.activation,
    ...
    def __call__(self, inputs):
        with tf.variable_scope(self.scope):
            ...
            for i in range(self.postnet_num_layers - 1):
                ...
                x = conv1d(x, self.kernel_size, self.channels, self.activation,
                           self.is_training, self.drop_rate,
                           "conv_layer_{}_".format(i + 1) + self.scope, False)

            x = conv1d(x, self.kernel_size, self.channels, lambda _: _, self.is_training,
                       self.drop_rate,
                       "conv_layer_{}_".format(5) + self.scope, False)
          
        return x


def conv1d(inputs, kernel_size, channels, activation, is_training, drop_rate, scope, bn = True):
    with tf.variable_scope(scope):
        ...
        conv1d_output = tf.layers.conv1d(
            ...
            activation=None,
            padding="same")
        #print ("post conv1d : ", conv1d_output.shape)
        if bn:
