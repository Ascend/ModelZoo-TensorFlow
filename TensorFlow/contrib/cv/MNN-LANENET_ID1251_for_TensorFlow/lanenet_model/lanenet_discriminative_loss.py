...
def discriminative_loss(prediction, correct_label, feature_dim, image_shape,
    ...
    batch_size = prediction.get_shape()[0]
    ...
    for i in range(0, batch_size):
        ...
        out_disc_loss.append(disc_loss)
        out_l_var.append(disc_loss)
        out_l_dist.append(disc_loss)
        out_l_reg.append(disc_loss)

    disc_loss = tf.stack(out_disc_loss)
    disc_loss = tf.reduce_mean(disc_loss)

    l_var = tf.stack(out_l_var)
    l_var = tf.reduce_mean(l_var)

    l_dist = tf.stack(out_l_dist)
    l_dist = tf.reduce_mean(l_dist)

    l_reg = tf.stack(out_l_reg)
    ...
    #     dtype=tf.float32, size=0, dynamic_size=True)
    #
    # fla1, fla2, out_loss_op, out_var_op, out_dist_op, out_reg_op, fla3 = tf.while_loop(
    #     cond, body, [
