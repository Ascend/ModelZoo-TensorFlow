...
class LaneNetTusimpleTrainer(object):
    ...
    def __init__(self, cfg):
        ...
        with tf.variable_scope('learning_rate'):
            ...
            # self._learn_rate = self._init_learning_rate
        ...
        with tf.variable_scope(name_or_scope='train_step'):
            ...
            npu_opt = NPUDistributedOptimizer(optimizer)
            # lossscale
            self.loss_scale_manager = ExponentialUpdateLossScaleManager(init_loss_scale=2 ** 32,
                                                                        incr_every_n_steps=1000,
                                                                        decr_every_n_nan_or_inf=2, decr_ratio=0.5)
            npu_opt = NPULossScaleOptimizer(npu_opt, self.loss_scale_manager)
            optimizer = NPULossScaleOptimizer(npu_opt, self.loss_scale_manager, is_distributed=False)
        ...
        # custom_op.parameter_map["graph_memory_max_size"].s = tf.compat.as_bytes(str(22 * 1024 * 1024 * 1024))
        # custom_op.parameter_map["variable_memory_max_size"].s = tf.compat.as_bytes(str(9 * 1024 * 1024 * 1024))
        ...
        print('Initialize tusimple lanenet trainner complete')
    ...
    def train(self):
        ...
        # print('=> Starts to train LaneNet from scratch ...')
        # epoch_start_pt = 1
        for epoch in range(epoch_start_pt, self._train_epoch_nums):
            ...
            for i in traindataset_pbar:

                if self._enable_miou and epoch % self._record_miou_epoch == 0:
                    fla1, fla2, summary, train_step_loss, train_step_binary_loss, \
                    ...
                    train_epoch_losses = np.append(train_epoch_losses, train_step_loss)
                    train_epoch_mious = np.append(train_epoch_mious, train_step_miou)
                    ...
                    fla1, summary, train_step_loss, train_step_binary_loss, \
