...
#基础参数，需要模型审视修改
#网络名称，同目录名称
Network="realmix_ID1248_for_TensorFlow"
...
do
    ...
    export ML_DATA=${data_path}/dataset
    python3 read_fromobs.py
    for seed in 1; do
        for size in 250; do
            python3 scripts/create_split.py --seed=$seed --label_split_size=$size $ML_DATA/SSL/cifar10 $ML_DATA/cifar10-train.tfrecord &
        done
        wait
    done
    python3 realmix.py \
        --filters=32 \
	    --dataset=cifar10_aug50.1@250-500 \
        --w_match=75 \
        --beta=0.75 \
        --custom_dataset=True \
        --augment=cifar10 \
        --tsa=linear_schedule > ${cur_path}/test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log 2>&1
    wait
    python3 realmix.py \
        --filters=32 \
	    --dataset=cifar10_aug50.1@250-500 \
        ...
        --augment=cifar10 \
        --tsa=linear_schedule \
        --eval_ckpt=True > ${cur_path}/test/output/${ASCEND_DEVICE_ID}/test_${ASCEND_DEVICE_ID}.log 2>&1
...
FPS=`grep 'fps' $cur_path/test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|awk 'END {print $4}'`
#打印，不需要修改
echo "Final Performance TrainingTime : $TrainingTime"
echo "Final Performance images/sec : $FPS"

#输出训练精度,需要模型审视修改
train_accuracy=`grep tuned_raw $cur_path/test/output/${ASCEND_DEVICE_ID}/test_${ASCEND_DEVICE_ID}.log|awk '{print $4}'`
...
grep 'loss_scale' $cur_path/test/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log|awk '{print $3}' >> $cur_path/test/output/$ASCEND_DEVICE_ID/train_${CaseName}_loss.txt
#最后一个迭代loss值，不需要修改
