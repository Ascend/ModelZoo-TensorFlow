...
def memoize(dataset: tf.data.Dataset) -> tf.data.Dataset:
    ...
    with tf.Session(config=npu_config_proto(config_proto=utils.get_config())) as session:
...
def augment_cutout_func(x, prob=0.3):
...
def compute_mean_std(data: tf.data.Dataset):
    data = data.map(lambda x: x['image']).batch(1024, drop_remainder=True).prefetch(1)
...
class DataSet:
    ...
    def creator(cls, name, seed, label, valid, augment, parse_fn=default_parse, do_memoize=True, colors=3,
        ...
        root_train = os.path.join(DATA_DIR, 'SSL', name.split("_")[0] + fullname)
        ...
        def create():
            ...
            train_labeled = parse_fn(dataset([root_train + '-label.tfrecord']))
            ...
            return cls(name + name_suffix + fullname + '-' + str(valid),
                       ...
                       eval_labeled=parse_fn(dataset([root_train + '-label.tfrecord'])),
...
augment_identity = lambda x: dict(image=x['image'], label=x['label'])
...
augment_color = lambda x: dict(image=augment_shift(augment_color_func(augment_mirror(x['image'])), 4), label=x['label'])
augment_cutout = lambda x: dict(image=augment_cutout_func(x['image']), label=x['label'])
augment_custom_cutout = lambda x: dict(image=augment_shift(augment_color_func(augment_mirror(augment_cutout_func(x['image']))), 4), label=x['label'])
...
DATASETS.update([DataSet.creator('cifar10_aug25', seed, label, valid, augment_cifar10)
                 for seed, label, valid in
                 itertools.product(range(6), [250, 500, 1000, 2000, 4000, 8000], [1, 5000])])
