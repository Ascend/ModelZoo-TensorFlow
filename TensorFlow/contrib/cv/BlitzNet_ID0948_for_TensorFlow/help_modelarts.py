...
def modelarts_result2obs(args):
    ...
    obs_result_dir = os.path.join(args.obs_dir, 'result')
    if not mox.file.exists(obs_result_dir):
        mox.file.make_dirs(obs_result_dir)
    mox.file.copy_parallel(src_url=args.result_dir, dst_url=obs_result_dir)
    print("===>>>Copy Event or Checkpoint from modelarts dir:{} to obs:{}".format(args.result_dir, obs_result_dir))
...
def modelarts_dump2obs(dump_data_dir, dump_graph_dir, args):
    obs_dump_dir = os.path.join(args.obs_dir, 'dump')
    mox.file.copy_parallel(dump_data_dir, obs_dump_dir)      # 将算子的dump数据传输至OBS
    mox.file.copy_parallel(dump_graph_dir, obs_dump_dir)     # 将dump图文件传输至OBS
    print("===>>>Copy Event or Checkpoint from modelarts dir:{} to obs:{}".format(dump_data_dir, obs_dump_dir))