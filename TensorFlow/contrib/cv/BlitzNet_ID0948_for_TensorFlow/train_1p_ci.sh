...
cd $cur_path/
sed -i "s|"../Weights_imagenet/"|"$ckpt_path"|g"  Train/resnet.py
bash scripts/train_testcase.sh  $cur_path   $data_path  $cur_path/results | tee train_output.log
sed -i "s|"$ckpt_path"|"../Weights_imagenet/"|g"  Train/resnet.py
#####################
