...
class DualNet(object):
    ...
    def load_random_samples(self):
        #np.random.choice(
        # sample_files = np.random.choice(glob('{}/{}/val/A/*.*[g|G]'.format(self.data_dir, self.dataset_name)),
        #                                 self.batch_size)
        sample_files = np.random.choice(glob('{}/val/A/*.*[g|G]'.format(self.data_dir)),
                                      self.batch_size)
        sample_A_imgs = [load_data(f, image_size=self.image_size, flip=False) for f in sample_files]

        # sample_files = np.random.choice(glob('{}/{}/val/B/*.*[g|G]'.format(self.data_dir, self.dataset_name)),
        #                                 self.batch_size)
        sample_files = np.random.choice(glob('{}/val/B/*.*[g|G]'.format(self.data_dir)),
                                        self.batch_size)
    ...
    def sample_shotcut(self, sample_dir, epoch_idx, batch_idx):
        ...
        save_images(A2B_imgs, [self.batch_size, 1],
                    '{}/{}/{}/{:06d}_{:04d}_A2B.jpg'.format(self.result, sample_dir, self.dir_name, epoch_idx,
                                                            batch_idx))
        save_images(A2B2A_imgs, [self.batch_size, 1],
                    '{}/{}/{}/{:06d}_{:04d}_A2B2A.jpg'.format(self.result, sample_dir, self.dir_name, epoch_idx,
                                                              batch_idx))

        save_images(B2A_imgs, [self.batch_size, 1],
                    '{}/{}/{}/{:06d}_{:04d}_B2A.jpg'.format(self.result, sample_dir, self.dir_name, epoch_idx,
                                                            batch_idx))
        save_images(B2A2B_imgs, [self.batch_size, 1],
                    '{}/{}/{}/{:06d}_{:04d}_B2A2B.jpg'.format(self.result, sample_dir, self.dir_name, epoch_idx,
                                                              batch_idx))
    ...
    def train(self, args):
        ...
        self.writer = tf.summary.FileWriter("{}/logs/".format(self.result)+self.dir_name, self.sess.graph)
        ...
        for epoch_idx in range(args.epoch):
            # data_A = glob('{}/{}/train/A/*.*[g|G]'.format(self.data_dir, self.dataset_name))
            # data_B = glob('{}/{}/train/B/*.*[g|G]'.format(self.data_dir, self.dataset_name))
            data_A = glob('{}/train/A/*.*[g|G]'.format(self.data_dir))
            data_B = glob('{}/train/B/*.*[g|G]'.format(self.data_dir))
    ...
    def fcn(self, imgs, prefix=None, reuse = False):
        with tf.variable_scope(tf.get_variable_scope()) as scope:
            ...
            if self.dropout_rate <= 0.:
                ...
                d1 = npu_ops.dropout(batch_norm(self.d1, name = prefix+'bn_d1'), self.dropout_rate)
            ...
            if self.dropout_rate <= 0.:
                ...
                d2 = npu_ops.dropout(batch_norm(self.d2, name = prefix+'bn_d2'), self.dropout_rate)
            ...
            if self.dropout_rate <= 0.:
                ...
                d3 = npu_ops.dropout(batch_norm(self.d3, name = prefix+'bn_d3'), self.dropout_rate)
    ...
    def save(self, checkpoint_dir, step):
        ...
        checkpoint_dir = os.path.join(self.result,checkpoint_dir, model_dir)
        ...
        self.saver.save(self.sess,
                        ...
                        global_step=step)
        tf.train.write_graph(self.sess.graph, checkpoint_dir, "graph.pbtxt", as_text = True)
    ...
    def test(self, args):
        ...
        if self.load(self.checkpoint_dir):
            print(" [*] Load SUCCESS")
            test_dir = '{}/{}/{}'.format(self.result, args.test_dir, self.dir_name)
    ...
    def test_domain(self, args, test_log, type = 'A'):
        # test_files = glob('{}/{}/val/{}/*.*[g|G]'.format(self.data_dir,self.dataset_name,type))
        test_files = glob('{}/val/{}/*.*[g|G]'.format(self.data_dir,type))
        # load testing input
        ...
        test_path = '{}/{}/{}/'.format(self.result, args.test_dir, self.dir_name)
        # test input samples
        if type == 'A':
