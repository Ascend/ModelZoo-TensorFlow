...
if [ x"${etp_flag}" != x ];
then
    python3.7 ./neural_style.py --content=${data_path}/1-content.jpg --styles=${data_path}/1-style.jpg --output=${output_path}/11.jpg --network=${data_path}/imagenet-vgg-verydeep-19.mat --iterations 1000 --print-iterations 1 --progress-write --progress-plot &> ${print_log}
else
    python3.7 ./neural_style.py --content=${data_path}/1-content.jpg --styles=${data_path}/1-style.jpg --output=${output_path}/11.jpg --network=${data_path}/imagenet-vgg-verydeep-19.mat --iterations 1000 --print-iterations 1 --progress-write --progress-plot &> ${print_log}
fi

# 性能相关数据计算
StepTime=`grep "sec/step:" ${print_log} | tail -n +2 | awk '{print $NF}' | awk '{sum+=$1} END {print sum/NR}'`
FPS=`awk 'BEGIN{printf "%.2f\n", '${batch_size}'/'${StepTime}'}'`

# 精度相关数据计算（本模型没有明确的精度指标，但可以看最后一步的损失）
train_accuracy=`grep "total loss:" ${print_log} | awk -F ":" '{print $2}' | tail -n 1`
# 提取所有loss打印信息（py文件中已经存在保存loss的代码，以progess.txt文件储存）
grep "total loss:" ${print_log} | awk -F ":" '{print $2}' > ./test/output/${ASCEND_DEVICE_ID}/my_output_loss.txt
