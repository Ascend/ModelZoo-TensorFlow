...
if __name__ == '__main__':
    ...
    parser.add_argument('--trajectory_length', default=4)#此处在297已改好，不影响后面
...
# print('Restoring Entire Session from checkpoint : %s'%args.outputpath+"model200.meta")
# imported_meta = tf.train.import_meta_graph(args.outputpath + "model200.meta")
# print('Success')
# imported_meta.restore(sess, tf.train.latest_checkpoint(args.outputpath))#查找最新保存的ckpt文件名，并加载
...
#         fh = open("//root//convlstm//DeepVO//data//color//dataset//output//output_file", "a")#位姿真值存放文件
...
#         fh.close()
#         fh = open("//root//convlstm//DeepVO//data//color//dataset//output//estimated","a")#位姿预估值存放文件
#         fh.write("%f %f %f %f %f %f\n"%(output[i][0,0],
#                                         output[i][0,1],
#                                         output[i][0,2],
#                                         output[i][0,3],
