...
import warnings
import argparse
# import datetime 此为格式化输出时间，但为方便之后计算，使用以秒为单位的包如下
import time
# 加两句
# import sys
# sys.path.remove('/usr/local/python3.7.5/lib/python3.7/site-packages')
...
def train_model(data_loader, sess, merged, loss_op, train_op, input_data, labels_, i, test_writer, train_writer):
    print('Current epoch : %d----' % data_loader.current_epoch, end="")
    print('step : %d----' % i, end="")
    if i % 10 == 0:  # Record summaries and test-set accuracy
        ...
        print('Train_error : %s----' % train_loss, end="")


def train(args, datapath, outputpath, epoches, trajectory_length):
    # configuration
    ...
    train_writer = tf.summary.FileWriter(outputpath + 'train', sess.graph)
    ...
    def get_next_batch(self):
        ...
        for j in range(self.args.bsize):
            ...
            for i in range(self._current_initial_frame, self._current_initial_frame + self.args.time_steps):
                ...
                pose = self.get6DoFPose(poses[i + 1, :]) - self.get6DoFPose(poses[i, :])#位姿差
                labels_series.append(pose)
            img_batch.append(img_stacked_series)
            label_batch.append(labels_series)
            self._current_initial_frame += self.args.time_steps
        label_batch = np.array(label_batch)#创建数组
        img_batch = np.array(img_batch)
        return img_batch, label_batch#返回每个batch的拼接图片，位姿差真值
...
# Main_code
# args = MyArgs(datapath='/root/convlstm/DeepVO/data/color/dataset/',
...
#                      outputpath = '/root/convlstm/DeepVO/data/color/dataset/output/'
...
# args = MyArgs(datapath='/root/convlstm/DeepVO/data/color/dataset/',
#               outputpath='/root/convlstm/DeepVO/data/color/dataset/output4/',
#               bsize=32,
#               trajectory_length=4,
#               train_iter=1,
#               time_steps=TIME_STEPS)
...
# print('Restoring Entire Session from checkpoint : %s'%args.outputpath+"model80.meta")
# imported_meta = tf.train.import_meta_graph(args.outputpath + "model80.meta")
# print('Success')
...
#        for j in range(32):
#             fh = open("//root//convlstm//DeepVO//txtcsv//output_file.txt", "a")#位姿真值存放文件
...
#             fh.close()
#             fh = open("//root//convlstm//DeepVO//txtcsv//estimated.txt","a")#位姿预估值存放文件
#             fh.write("%f %f %f %f %f %f\n"%(output[i][j,0],
#                                             output[i][j,1],
#                                             output[i][j,2],
#                                             output[i][j,3],
...
#                                             output[i][j,5]))
#             fh.close()
# file_old = open("//root//convlstm//DeepVO//txtcsv//output_file.txt", 'rb+')
...
# file_old.close()
#
# file_oldd = open("//root//convlstm//DeepVO//txtcsv//estimated.txt", 'rb+')
