...
def generate_depth_map(calib_dir, velo_file_name, im_shape, cam=2, interp=False, vel_depth=False):
    ...
    # xr = np.array(list(map(float,velo2cam['R'.encode()].decode().split(' ')))).reshape(3,3)
    # xt = np.array(list(map(float,velo2cam['T'.encode()].decode().split(' '))))[..., np.newaxis]
    velo2cam = np.hstack((np.array(list(map(float,velo2cam['R'.encode()].decode().split(' ')))).reshape(3,3), np.array(list(map(float,velo2cam['T'.encode()].decode().split(' '))))[..., np.newaxis]))
    velo2cam = np.vstack((velo2cam, np.array([0, 0, 0, 1.0])))

    # compute projection matrix velodyne->image plane
    R_cam2rect = np.eye(4)
    R_cam2rect[:3,:3] = np.array(list(map(float,cam2cam['R_rect_00'.encode()].decode().split(' ')))).reshape(3,3)
    P_rect = np.array(list(map(float,cam2cam[('P_rect_0'+str(cam)).encode()].decode().split(' ')))).reshape(3,4)
