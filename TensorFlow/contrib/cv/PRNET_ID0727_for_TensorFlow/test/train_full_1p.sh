#!/bin/bash
echo "`date +%Y%m%d%H%M%S`-[INFO] start to run train_performance_1p.sh "
#当前路径,不需要修改
cur_path=`pwd`/../
...
#基础参数，需要模型审视修改
#网络名称，同目录名称
Network="PRNET_ID0727_for_TensorFlow"
#训练epoch
train_epochs=1
#训练batch_size
batch_size=32
...
do
	...
	cp -r ${data_path}/uv-data/weight_mask_final.jpg Data/uv-data/
	cp -r ${data_path}/Out face3d/examples/Data/BFM/
...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
grep "iter_time:"  $cur_path/test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log  |awk '{print $8}'  |awk -F':' '{print $2}' |tail -n +2 > $cur_path/test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}_traintime.txt
