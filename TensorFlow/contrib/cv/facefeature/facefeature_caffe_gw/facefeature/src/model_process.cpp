...
void ModelProcess::destroy_resource() {
    if (isReleased_) {
        return ;
    }
    unload();
...
Result ModelProcess::create_input(void *input1, size_t input1size)
{
...
{
    if(input_ == nullptr) {
        ERROR_LOG("execute model 0000 failed, modelId ");
    }
    if(output_ == nullptr) {
        ERROR_LOG("execute model 1111 failed, modelId");
    }
