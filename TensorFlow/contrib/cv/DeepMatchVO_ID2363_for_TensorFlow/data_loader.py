...
import numpy as np
import sys
sys.path.append('/root/DeepMatchVO-master')
class DataLoader(object):
    ...
    def load_train_batch(self):
        ...
        all_dataset = all_dataset.batch(self.batch_size, drop_remainder=True).repeat().prefetch(self.batch_size*4)
    ...
    def load_test_batch(self, image_sequence_names):
        ...
        def _parse_test_img(img_path):
            with tf.device('/cpu:0'):
                img_buffer = tf.read_file(img_path)                #####img_buffer = tf.read_file(str(img_path))##########wty img_buffer = tf.read_file(img_path)
                image_decoded = tf.image.decode_jpeg(img_buffer)
            return image_decoded

        image_dataset = tf.data.Dataset.from_tensor_slices(image_sequence_names).map(
            _parse_test_img).batch(self.batch_size, drop_remainder=True).prefetch(self.batch_size*4)
