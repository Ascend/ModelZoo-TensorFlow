...
class kitti_raw_loader(object):
    ...
    def get_train_example_with_idx(self, tgt_idx):
        if not self.is_valid_sample(self.train_frames, tgt_idx):
            return False
        example = self.load_example(self.train_frames, tgt_idx, load_matches=(self.match_num!=0))
        return example
    ...
    def load_example(self, frames, tgt_idx, load_matches=False):
        ...
        if load_matches:
            date = tgt_drive[:10]
            sift_folder = os.path.join(self.dataset_dir, date, tgt_drive, 'sift')
            match_folder = os.path.join(self.dataset_dir, date, tgt_drive, 'match')
