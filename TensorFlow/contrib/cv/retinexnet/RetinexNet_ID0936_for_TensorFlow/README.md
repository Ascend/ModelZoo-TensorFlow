...
**描述（Description）：基于TensorFlow框架对低光照图片进行光照增强的训练代码** 

<h2 id="概述.md">概述</h2>

Retinex-Net 的光照增强过程分为三个步骤：分解、调整和重构。 在分解步骤中，一个子网络 Decom-Net 将输入图像分解为reflectance 和 illuminati。 在调整步骤中，基于 Enhance-Net 的编码器-解码器照亮了图像的illuminati。最后，重构调整reflectance 和 illuminati以获得增强的结果。
...
Loss
- Loss on npu an on gpu is both around 0.13
