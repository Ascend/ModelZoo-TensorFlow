...
if __name__ == '__main__':
    ...
    parser.add_argument('--test_dir', dest='test_dir', default='./data/test/low',
    ...
    parser.add_argument('--model_path', dest='model_path', default='./model',
    ...
    parser.add_argument("--modelarts_data_dir", type=str, default="./data")
    parser.add_argument("--modelarts_result_dir", type=str, default="./result")
    ...
    custom_op = config.graph_options.rewrite_options.custom_optimizers.add()  #train 的时候需要放开，是跑到npu上面的，test 的时候跑不到npu上，npu报错，需要注释掉
    
    custom_op.name = "NpuOptimizer" #train 的时候需要放开，是跑到npu上面的 test 的时候跑不到npu上，npu报错，需要注释掉
    custom_op.parameter_map['use_off_line'].b = True  #train 的时候需要放开，是跑到npu上面的   test 的时候跑不到npu上，npu报错，需要注释掉
    
    #config 中增加以下2行配置，支持动态shape
    custom_op.parameter_map['dynamic_input'].b = True
    custom_op.parameter_map['dynamic_graph_execute_mode'].s =  tf.compat.as_bytes("dynamic_execute")
    
    config.graph_options.rewrite_options.remapping = RewriterConfig.OFF   #train 的时候需要放开，是跑到npu上面的     test 的时候跑不到npu上，npu报错，需要注释掉
