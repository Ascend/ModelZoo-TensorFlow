...
class LowlightEnhance(object):
    ...
    def __init__(self, sess):
        ...
        self.loss_Decom = self.recon_loss_low + self.recon_loss_high + 0.001 * self.recon_loss_mutal_low + 0.001 * \
                          self.recon_loss_mutal_high + 0.1 * self.Ismooth_loss_low + 0.1 * self.Ismooth_loss_high + \
                          0.01 * self.equal_R_loss
        self.loss_Relight = self.relight_loss + 3 * self.Ismooth_loss_delta

        self.lr = tf.placeholder(tf.float32, name='learning_rate')
        # optimizer = npu_tf_optimizer(tf.train.AdamOptimizer(self.lr, name='AdamOptimizer'))
    ...
    def train(self, train_low_data, train_high_data, eval_low_data, obs_result_dir, data_url, newest_model, save_data,
              batch_size, patch_size, epoch, lr, sample_dir,
              ckpt_dir, eval_every_epoch, train_phase):
        """

        :param train_low_data:
        :param train_high_data:
        :param eval_low_data:
        :param obs_result_dir:
        :param data_url:
        :param newest_model:
        :param save_data:
        :param batch_size:
        ...
        if load_model_status:
            ...
            start_epoch = int(int(global_step) // numBatch)
        ...
        for epoch in range(start_epoch, epoch):
            ...
            if (epoch + 1) % eval_every_epoch == 0:
                ...
                self.save(obs_result_dir, data_url, newest_model, save_data, saver, iter_num, ckpt_dir,
                          "RetinexNet-%s" % train_phase)

        print("[*] Finish training for phase %s." % train_phase)

    def save(self, obs_result_dir, data_url, newest_model, save_data, saver, iter_num, ckpt_dir, model_name):
        """
        ...
        tf.train.Saver().save(self.sess, os.path.join(ckpt_dir, model_name + '.ckpt'))
        tf.io.write_graph(self.sess.graph, ckpt_dir, 'graph.pbtxt', as_text=True)

        for index in range(2):
            a_path = save_data[index]
            #mox.file.copy_parallel(src_url=a_path, dst_url=obs_result_dir + a_path.replace('.', ''))
            #if index == 0:
                #mox.file.copy_parallel(src_url=a_path, dst_url=obs_result_dir[0:-20] + newest_model)
                #mox.file.copy_parallel(src_url=a_path, dst_url=data_url + newest_model)
    ...
    def load(self, saver, ckpt_dir):
        ...
        if ckpt and ckpt.model_checkpoint_path:
            ...
            try:
                print('///****')
                global_step = int(full_path.split('/')[-1].split('-')[-1])  #./model/Relight/RetinexNet-tensorflow
    ...
    def test(self, test_low_data, test_high_data, test_low_data_names, newest_res, save_dir, decom_flag, model_path,
             save_data, obs_result_dir):
        """

        :param test_low_data:
        :param test_high_data:
        :param test_low_data_names:
        :param newest_res:
        :param save_dir:
        :param decom_flag:
        :param model_path:
        ...
        #########
        load_model_status_Decom, _ = self.load(self.saver_Decom, model_path + '/Decom')
        load_model_status_Relight, _ = self.load(self.saver_Relight, model_path + '/Relight')
        ...
        a_path = save_dir
        print(a_path)
        print(obs_result_dir + a_path.replace('.', ''))
        #mox.file.copy_parallel(src_url=a_path, dst_url=obs_result_dir + a_path.replace('.', ''))
        #mox.file.copy_parallel(src_url=a_path, dst_url=obs_result_dir[0:-20] + newest_res)
