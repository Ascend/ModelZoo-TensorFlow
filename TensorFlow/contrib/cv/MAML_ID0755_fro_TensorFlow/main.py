...
# datasource=miniimagenet;metatrain_iterations=60000;meta_batch_size=4;update_batch_size=5;update_lr=0.01;num_updates=5;num_classes=5;logdir=/cache/result/logs/miniimagenet5shot/;num_filters=32;max_pool=True;train=False;test_set=True;
...
flags.DEFINE_float('meta_lr', 0.001, 'the base learning rate of the generator')#0.001
...
# flags.DEFINE_string('logdir', 'logs/test', 'directory for summaries and checkpoints.')
flags.DEFINE_string('logdir', '/cache/result', 'directory for summaries and checkpoints.')
...
# flags.DEFINE_string('data_url', './dataset', 'data path in obs')
# flags.DEFINE_string('train_url', './output', 'obs path')
...
# locals_dir = "/cache/dataset"
# start = datetime.datetime.now()
# os.makedirs(locals_dir)
# mox.file.copy_parallel(FLAGS.data_url, locals_dir)
# end = datetime.datetime.now()
# print('------copy over, time:{}(s)'.format((end - start).seconds))
...
def main():
    ...
    print('===FLAGS.datasource===:', FLAGS.datasource)
    if FLAGS.datasource == 'sinusoid':
    ...
    if FLAGS.datasource == 'miniimagenet' or FLAGS.datasource == 'omniglot':
        ...
        if FLAGS.train: # only construct training model if needed
            random.seed(5)
            image_tensor, label_tensor, iterator_train = data_generator.make_data_tensor()
        ...
        random.seed(6)
        image_tensor, label_tensor, iterator = data_generator.make_data_tensor(train=False)
    ...
    if FLAGS.train:
        iterator_train.initializer.run(session=sess)
    ...
    if FLAGS.resume or not FLAGS.train:
        ...
        if model_file:
            ...
            print(ind1)
            print(model_file)
            print(model_file[ind1+5:])
            if(model_file[ind1+5:] != ""):
    ...
    if FLAGS.train:
        ...
        # mox.file.copy_parallel(FLAGS.train_url, model_dir)
        # mox.file.copy_parallel(model_dir, FLAGS.train_url)
    else:
