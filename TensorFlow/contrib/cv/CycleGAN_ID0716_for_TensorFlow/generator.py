...
class Generator:
    ...
    def __call__(self, input_data):
        """
    Args:
      input_data: batch_size x width x height x 3
    Returns:
      output: same size as input
    """
        with tf.variable_scope(self.name):
            # conv layers
            c7s1_32 = ops.c7s1_k(input_data, self.ngf, is_training=self.is_training, norm=self.norm,
    ...
    def sample(self, input_data):
        image = utils.convert2int(self.__call__(input_data))
        image = tf.image.encode_jpeg(tf.squeeze(image, [0]))
        return image

    def sample_for_eval(self, input_data):
        image = utils.convert2int(self.__call__(input_data))
