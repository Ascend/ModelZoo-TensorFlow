...
def _get_detection_path(root, detection_folder_name, world):  # 读取到root/meta/detection/home_0xx_x.py
...
def read_cached_data(should_load_images, dataset_root, segmentation_file_name,
  ...
  """
    ...
    # with tf.gfile.Open(annotated_target_path, encoding='latin1') as f:
    with open(annotated_target_path, "rb") as f:
        result_data['targets'] = np.load(f, allow_pickle=True, encoding="latin1").item()
    ...
    with open(depth_image_path, "rb") as f:
        depth_data = np.load(f, allow_pickle=True, encoding="latin1").item()
    ...
    with open(sseg_path, "rb") as f:
        sseg_data = np.load(f, allow_pickle=True, encoding="latin1").item()
    ...
    if should_load_images:
        ...
        with open(image_path, "rb") as f:
            image_data = np.load(f, allow_pickle=True, encoding="latin1").item()

        result_data[task_env.ModalityTypes.IMAGE] = image_data

    with open(os.path.join(dataset_root, 'Meta/world_id_dict.npy'), "rb") as f:
        result_data['world_id_dict'] = np.load(f, allow_pickle=True, encoding="latin1").item()
...
class ActiveVisionDatasetEnv(task_env.TaskEnv):
    ...
    # 核对是否满足class的属性
        ...
        for world in self._worlds:
            with open(_get_json_path(self._dataset_root, world), 'r') as f:
                file_content = f.read()
        ...
        if eval_init_points_file_name is not None:
            ...
            with open(init_points_path, "rb") as points_file:
                data = np.load(points_file, allow_pickle=True, encoding="latin1").item()
            self._eval_init_points = []
            for world in self._worlds:
                for goal in self._targets:
                    if world in self._annotated_targets[goal]:
                        bytes=world.encode()
                        for image_id in data[bytes]:
    ...
    """
        ...
        if self._cur_world not in self._detection_cache:
            with open(
                    _get_detection_path(self._dataset_root, self._detection_folder_name,
                                        self._cur_world), "rb") as f:
                # Each file contains a dictionary with image ids as keys and detection
                # dicts as values.
                self._detection_cache[self._cur_world] = np.load(f, allow_pickle=True, encoding="latin1").item()
        ...
        if self._cur_world not in self._vertex_to_pose:
            ...
            self._vertex_to_pose[self._cur_world] = {
                index: (-index,) for index in list(self._cur_graph.target_indexes.values())
            }
            ...
            self._pose_to_vertex[self._cur_world] = {
                tuple(v): k
                for k, v in list(self._vertex_to_pose[self._cur_world].items())
            }
