  ...
  """
    ...
    """
        ...
        self._inputs = collections.OrderedDict(
            [(k, io_config(*value)) for k, value in list(inputs.items())])
...
class UnrolledTask(object, metaclass=abc.ABCMeta):
  ...
  """
    ...
    """
        ...
        batched_inputs = {
            k: np.concatenate(i, axis=0) for k, i in batched_inputs.items()
        }
        if batched_queries:
    ...
    """
        ...
        touts = []
        shapes = []
        for _, i in list(self._config.inputs.items()):
            touts.append(i.type)
        ...
        tf_episode_batch_dict = collections.OrderedDict([
            (mtype, episode)
            for mtype, episode in zip(list(self.config.inputs.keys()), tf_episode_batch)
        ])
        cur_index = len(list(self.config.inputs.keys()))
        tf_query = None
...
class RandomExplorationBasedTask(UnrolledTask):
    ...
    """
        ...
        if len(list(self.config.inputs.keys())) > 1:
    ...
    """
        in_seq_len = list(self._config.inputs.values())[0].shape[0]
        ...
        max_path_len = int(
            round(in_seq_len * float(len(path)) / float(len(list(obs.values())[0]))))
    ...
    def _obs_to_state(self, path, states):
        ...
        path_coordinates = list(map(self._env.vertex_to_pose, path))
...
class AreNearbyTask(RandomExplorationBasedTask):
  ...
  """
    ...
    """
        ...
        assert len(list(observations.values())[0]) == len(states)
        ...
        for i, curr_o in enumerate(list(observations.values())[0]):
            p = obs_to_path[i]
            ...
            nearby_visited = [
                ii for ii in path[low:i + 1] + list(g[p].keys()) if ii in obs_to_path[:i]
            ]
            nearby_visited = [ii for ii in index_groups[1] if ii in path_to_obs]
            # NOT Nearby visited indices, label 0.
            not_nearby_visited = [ii for ii in path[:low] if ii not in list(g[p].keys())]
            ...
            queries.append(list(o.values())[0])
            labels.append(label)
...
class NeighboringQueriesTask(RandomExplorationBasedTask):
  ...
  """
    ...
    """
        ...
        closest_indices = list(nx.single_source_shortest_path(
...
class GotoStaticXTask(RandomExplorationBasedTask):
  ...
  """
    ...
    def episode(self):
        ...
        query = list(self._env.observation(self._env.vertex_to_pose(goal)).values())[0]
  ...
  """
    ...
    def episode(self):
      ...
      """
        ...
        query = list(self._env.observation(location).values())[0]
...
class GotoStaticXNoExplorationTask(UnrolledTask):
  ...
  """
    ...
    def __init__(self, env, *args, **kwargs):
        ...
        for input_config in list(self._config.inputs.values()):
    ...
    """
        ...
        sequence_length = list(self._config.inputs.values())[0].shape[0]
        modality_types = list(self._config.inputs.keys())
        ...
        for modality_type, obs in observations.items():
