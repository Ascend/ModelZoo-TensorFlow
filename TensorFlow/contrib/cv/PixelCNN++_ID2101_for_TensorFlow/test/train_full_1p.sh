...
then
    ...
    python3.7 ./train.py --data_path=${data_path} --output_path=${output_path} --max_epochs=70 > ${print_log} 2>&1
fi
...
# 精度相关数据计算
train_accuracy=`grep "final test bits_per_dim" ${print_log}  | awk '{print $NF}'`
# 提取所有loss打印信息
