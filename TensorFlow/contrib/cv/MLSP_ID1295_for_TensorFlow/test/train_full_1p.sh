...
do
	...
	--data_path=${data_path} \
	--epoch=20 \
	--end_step=0 >  ${cur_path}test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log   2>&1
...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
grep "1748/1748"  $cur_path/test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log |awk '{print $5}' |awk -F'ms' '{print $1}' |tail -n +2 > $cur_path/test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}_traintime.txt
...
#输出训练精度,需要模型审视修改
train_acc=`grep "ACCURACY:" $cur_path/test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log |awk  '{print $2}'`
#打印，不需要修改
