...
def fc_layers(input_layer,
    ...
    for i in range(len(fc_sizes)):
        if i < len(fc_sizes)-1:
            ...
            act  = out_activation
            layer_type = 'out'
        x = Dense(int(fc_sizes[i]), activation=act,
...
def inception_block(x, size=768, name=''):
    ...
    branch3x3_1 = conv2d_bn(branch3x3, int(size/2), 1, 3, name=name+'3x3_1x3')
    branch3x3_2 = conv2d_bn(branch3x3, int(size/2), 3, 1, name=name+'3x3_3x1')
