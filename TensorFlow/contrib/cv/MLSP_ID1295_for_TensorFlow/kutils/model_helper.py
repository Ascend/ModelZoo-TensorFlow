...
class ModelHelper:
    ...
    def _callbacks(self):
       ...
       # log_dir = os.path.join(self.params.logs_root, self.model_name())
        if p.histogram_freq:
            ...
            tb_callback = TensorBoardWrapper(valid_gen,
        ...
        # /cache/models/data_url_0/
        print("打印best_model_path的路径："+best_model_path)
    ...
    def train(self, train_gen=None, valid_gen=None, lr=1e-4, epochs=1, end_step=0):
        """
        ...
        history = self.model.fit_generator(train_gen, epochs = epochs,
                                           steps_per_epoch   = len(train_gen)-end_step,
    ...
    def load_model(self, model_name='', best=True, 
        ...
        # model_name = data_url_0/bn2_bsz128_do[0.25,0.25,0.5,0]_ds[AVA_data_official_test.csv]_fc1[2048]_i1[5,5,16928]_im[orig]_lMSE_mon[val_plcc_tf]_o1[1]
        model_file_name = (model_name + ('_best' if best else '_final') + 
                          ('_weights' if from_weights else '') + '.h5')
        # model_file_name = data_url_0/bn2_bsz128_do[0.25,0.25,0.5,0]_ds[AVA_data_official_test.csv]_fc1[2048]_i1[5,5,16928]_im[orig]_lMSE_mon[val_plcc_tf]_o1[1]_best_weights.h5
        model_path = os.path.join(self.params.models_root, model_file_name)
        print('打印最终的模型存储路径：' + model_path)
