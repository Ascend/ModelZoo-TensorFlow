...
def modelarts_result2obs(FLAGS):
    ...
    mox.file.copy_parallel(src_url=FLAGS.model_dir, dst_url=obs_result_dir)
    print("===>>>Copy Event or Checkpoint from modelarts dir:{} to obs:{}".format(
        FLAGS.model_dir, obs_result_dir))
    ...
    if FLAGS.npu_dump_data:
        ...
        mox.file.copy_parallel(FLAGS.dump_dir, obs_dump_data_dir)
        print("===>>>Dumped graph:{} on OBS dir:{}".format(
            mox.file.list_directory(obs_dump_data_dir), obs_dump_data_dir))
    ...
    if FLAGS.npu_profiling:
        ...
        mox.file.copy_parallel(FLAGS.profiling_dir, obs_profiling_dir)
        print("===>>>Profiling data:{} on OBS dir:{}".format(
            mox.file.list_directory(obs_profiling_dir), obs_profiling_dir))
    ...
    if FLAGS.npu_dump_graph:
        ...
        print("===>>>Dumped data:{} on OBS dir:{}".format(
            mox.file.list_directory(obs_dump_graph_dir), obs_dump_graph_dir))
    ...
    if FLAGS.npu_auto_tune:
        ...
        print("===>>>Auto tune:{} on OBS dir:{}".format(
            mox.file.list_directory(obs_auto_tune_dir), obs_auto_tune_dir))
