...
#基础参数，需要模型审视修改
#网络名称，同目录名称
Network="efficientnet-condconv_ID2074_for_TensorFlow"
#训练epoch
train_epochs=1
#训练batch_size
batch_size=128
...
cd $cur_path/../
mkdir result
rm -rf /cache/dump_dir/
rm -rf /cache/profiling_dir/
...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
perf=`grep "tensorflow:global_step/sec"  $cur_path/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|awk '{print $2}'|awk 'NR>2{print p}{p=$0} END{print p}'| awk '{sum+=$1} END {print"",sum/NR}'|sed s/[[:space:]]//g`
