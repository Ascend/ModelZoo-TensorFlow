...
class CrowdCount {
public:
    CrowdCount(const char* modelPath,
                 uint32_t modelWidth, uint32_t modelHeight);
    ~CrowdCount();

    Result init();
    Result pre_process(ImageData& resizedImage, ImageData& srcImage);
    Result inference(aclmdlDataset*& inferenceOutput, ImageData& resizedImage);
    Result post_process(ImageData& image, aclmdlDataset* modelOutput,
