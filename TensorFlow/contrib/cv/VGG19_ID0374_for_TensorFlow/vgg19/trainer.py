...
class Trainer(object):
    ...
    def train(self):
        print ('training steps: %d' % self.args.nstep)  #5004    #add 4500
        self.classifier.train( input_fn=lambda:self.data.get_train_input_fn(),
                               max_steps = self.args.nstep-self.args.nstep_less,
    ...
    def train_and_evaluate(self):
        epochs_between_evals = self.args.epochs_between_evals   #150----1
        ...
        for i in range(self.args.max_epochs // epochs_between_evals):
            ...
            print("steps:",self.args.nsteps_per_epoch*epochs_between_evals)
            self.classifier.train(input_fn=lambda:self.data.get_train_input_fn(),
                            steps = self.args.nsteps_per_epoch*epochs_between_evals, #5004*150=750600
                            hooks = self.training_hook )
