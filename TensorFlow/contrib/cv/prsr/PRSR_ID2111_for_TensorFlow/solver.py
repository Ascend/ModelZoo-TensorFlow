...
class Solver(object):
	def __init__(self):
		...
		# ------------------- NPU LossScale -------------------------
		self.loss_scale_manager = ExponentialUpdateLossScaleManager(init_loss_scale = 2 ** 32, incr_every_n_steps = 1000,
	...
	def train(self):
		...
		try:
			...
			while not (coord.should_stop()):
				...
				# 		(iters, loss, self.batch_size / (t2 - t1), (t2 - t1), scale_value)))
				iters += 1
				if iters % 10 == 0:
