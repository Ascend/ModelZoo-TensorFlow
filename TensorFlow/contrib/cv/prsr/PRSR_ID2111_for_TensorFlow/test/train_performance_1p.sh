...
#基础参数，需要模型审视修改
#网络名称，同目录名称
Network="PRSR_ID2111_for_TensorFlow"
#训练batch_size
batch_size=32
...
sed -i "s@'./celebA'@'${data_path}/data/CelebA/Img/img_align_celeba'@g"  tools/create_img_lists.py
#sed -i "s@'iters == 380'@'iters == 80'@g" solver.py
python3 tools/create_img_lists.py
echo "strat training"
python3 train.py > ${cur_path}/test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log 2>&1
sed -i "s@'${data_path}/data/CelebA/Img/img_align_celeba'@'./celebA'@g"  tools/create_img_lists.py
#sed -i "s@'iters == 80'@'iters == 380'@g" solver.py
wait
