...
class MergerConfig(object):
    ...
    def ask_settings(self):
        self.sharpen_mode = 0  # io.input_int ("", 0, valid_list=self.sharpen_dict.keys(), help_message="Enhance details by applying sharpen filter.")
...
class MergerConfigMasked(MergerConfig):
    ...
    def ask_settings(self):
        mode = 1  # self.default_mode # io.input_int ("", mode_str_dict.get(self.default_mode, 1) )
        self.mode = mode_dict.get(mode, self.default_mode )
        ...
        if 'raw' not in self.mode:
            ...
            if self.mode == 'hist-match' or self.mode == 'seamless-hist-match':
                self.hist_match_threshold = 255  # np.clip ( io.input_int("Hist match threshold", 255, add_info="0..255"), 0, 255)

        self.mask_mode = 1  # io.input_int("", 1, valid_list=mask_mode_dict.keys() )

        if 'raw' not in self.mode:
            self.erode_mask_modifier = 0  # np.clip ( io.input_int ("Choose erode mask modifier", 0, add_info="-400..400"), -400, 400)
            self.blur_mask_modifier = 0  # np.clip ( io.input_int ("Choose blur mask modifier", 0, add_info="0..400"), 0, 400)
            self.motion_blur_power = 0  # np.clip ( io.input_int ("Choose motion blur power", 0, add_info="0..100"), 0, 100)

        self.output_face_scale = 0  # np.clip (io.input_int ("Choose output face scale modifier", 0, add_info="-50..50" ), -50, 50)

        if 'raw' not in self.mode:
            self.color_transfer_mode = None  # io.input_str ( "Color transfer to predicted face", None, valid_list=list(ctm_str_dict.keys())[1:] )
            self.color_transfer_mode = ctm_str_dict[self.color_transfer_mode]

        super().ask_settings()

        self.super_resolution_power = 0 # np.clip ( io.input_int ("Choose super resolution power", 0, add_info="0..100", help_message="Enhance details by applying superresolution network."), 0, 100)

        if 'raw' not in self.mode:
            self.image_denoise_power = 0  # np.clip ( io.input_int ("Choose image degrade by denoise power", 0, add_info="0..500"), 0, 500)
            self.bicubic_degrade_power = 0  # np.clip ( io.input_int ("Choose image degrade by bicubic rescale power", 0, add_info="0..100"), 0, 100)
            self.color_degrade_power = 0  # np.clip (  io.input_int ("Degrade color power of final image", 0, add_info="0..100"), 0, 100)
