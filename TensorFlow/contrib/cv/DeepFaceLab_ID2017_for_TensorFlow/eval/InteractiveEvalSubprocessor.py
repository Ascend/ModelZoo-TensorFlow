...
from eval import MergeFaceAvatar, MergeMasked, MergerConfig
...
class InteractiveEvalSubprocessor(Subprocessor):
        ...
        def process_data(self, pf): #pf=ProcessingFrame
            ...
            if len(frame_info.landmarks_list) == 0:
                ...
                if cfg.type == MergerConfig.TYPE_MASKED:
                    try:
                        ssim = MergeMasked (self.predictor_func, self.predictor_input_shape,
                                            cfg=cfg, frame_info=frame_info)
                        if pf.need_return_image:
                            return ssim
    ...
    def __init__(self, is_interactive, merger_session_filepath, predictor_func, predictor_input_shape,
                 merger_config, frames,
                 frames_root_path, output_path, output_mask_path, model_iter, subprocess_count=4):
        if len(frames) == 0:
            raise ValueError ("len (frames) == 0")

        super().__init__('Merger', InteractiveEvalSubprocessor.Cli, io_loop_sleep_time=0.001)
    ...
    def on_clients_initialized(self):
        # io.progress_bar("Evaluating: ", len(self.frames_idxs)+len(self.frames_done_idxs), initial=len(self.frames_done_idxs) )
    ...
    def on_tick(self):
        ...
        return (self.is_interactive and self. _quitting) or \
    ...
    #override
    def on_result (self, host_dict, pf_sent, ssim_result):
        if ssim_result:
            frame = self.frames[pf_sent.idx]
            frame.is_processing = False
            frame.is_done = True
            self.eval_result.append(ssim_result)
    ...
    def get_data(self, host_dict):
        ...
        for i in range ( min(len(self.frames_idxs), self.prefetch_frame_count) ):
            ...
            if not frame.is_done and not frame.is_processing and frame.cfg is not None:
                ...
                return InteractiveEvalSubprocessor.ProcessingFrame(idx=frame.idx,
                                                           cfg=frame.cfg.copy(),
