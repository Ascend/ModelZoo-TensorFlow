...
class QModel(ModelBase):
    ...
    def on_initialize(self):
        ...
        io.log_info('******* models_opt_on_gpu: ' + models_opt_device)
        ...
        with tf.device(models_opt_device):
            ...
            if self.is_training:
                ...
                # self.src_dst_opt = nn.AdaBelief(lr=5e-5, beta_1=0.5, beta_2=0.999, lr_dropout=0.3, name='src_dst_opt')
        ...
        if self.is_training:
            ...
            for gpu_id in range(gpu_count):
                with tf.device(f'/{devices[gpu_id].tf_dev_type}:{gpu_id}' if len(devices) != 0 else f'/CPU:0'):
                    ...
                    gpu_src_loss = tf.reduce_mean(10 * nn.dssim(gpu_target_src_masked_opt, gpu_pred_src_src_masked_opt,
                                                                max_val=1.0, filter_size=int(resolution / 11.6)),
                                                  axis=[1])
                    gpu_src_loss += tf.reduce_mean(10 * tf.square(gpu_target_src_masked_opt -
                                                                  gpu_pred_src_src_masked_opt), axis=[1, 2, 3])

                    gpu_src_loss += tf.reduce_mean(10 * tf.square(gpu_target_srcm - gpu_pred_src_srcm), axis=[1, 2, 3])

                    gpu_dst_loss = tf.reduce_mean(10 * nn.dssim(gpu_target_dst_masked_opt, gpu_pred_dst_dst_masked_opt,
                                                                max_val=1.0, filter_size=int(resolution / 11.6)),
                                                  axis=[1])
                    gpu_dst_loss += tf.reduce_mean(10 * tf.square(gpu_target_dst_masked_opt -
                                                                  gpu_pred_dst_dst_masked_opt), axis=[1, 2, 3])
            ...
            def src_dst_train(warped_src, target_src, target_srcm, warped_dst, target_dst, target_dstm):
            ...
            def AE_eval(warped_dst):
                return nn.tf_sess.run([pred_src_dst, pred_dst_dstm, pred_src_dstm],
    ...
    # override
    def onSave(self):
        ckpt_file_path = self.get_strpath_storage_for_file("DFL_ckpt_save")
        if not Path(ckpt_file_path).exists():
            Path(ckpt_file_path).mkdir(exist_ok=True, parents=True)
        ckpt_save_path = ckpt_file_path + "/DFL"
        nn.tf.train.Saver(nn.tf.trainable_variables()).save(nn.tf_sess, ckpt_save_path)
    ...
    def onTrainOneIter(self):
        if self.get_iter() % 3 == 0 and self.last_samples is not None:
            ((warped_src, target_src, target_srcm), (warped_dst, target_dst, target_dstm)) = self.last_samples
            warped_src = target_src
            warped_dst = target_dst
        else:
            samples = self.last_samples = self.generate_next_samples()
            ((warped_src, target_src, target_srcm), (warped_dst, target_dst, target_dstm)) = samples
    ...
    def onGetSSIM(self, samples):
        ...
        result = []
        for i in range(n_samples):
            dst_masked = (D[i] * target_dstm[i] * 255).astype(np.uint8)
            predict_masked = (SD[i] * (DDM[i] * SDM[i]) * 255).astype(np.uint8)
            result.append(ssim.ssim(dst_masked, predict_masked))
        return result
    ...
    def eval_func(self, face=None, target_dstm=None):
        # face = np.expand_dims(face, 0)
        face_temp = face
        face = nn.to_data_format(face[None, ...], self.model_data_format, "NHWC")

        SD, DDM, SDM = [ np.clip(nn.to_data_format(x, "NHWC", self.model_data_format), 0.0, 1.0)
                         for x in self.AE_eval(face) ]
        DDM = np.repeat(DDM, (3,), -1)
        SDM = np.repeat(SDM, (3,), -1)
        target_dstm = np.expand_dims(target_dstm, 2)
        target_dstm = np.repeat(target_dstm, (3,), -1)

        dst_masked = (face_temp * target_dstm * 255).astype(np.uint8)
        predict_masked = (SD[0] * (DDM[0] * SDM[0]) * 255).astype(np.uint8)
        ...
        cv2_imwrite(filename, dst_masked)
        cv2_imwrite(filename1, predict_masked)

        return ssim.ssim(dst_masked, predict_masked)

    # override
    def get_MergerConfig(self):
        import merger
        return self.predictor_func, (self.resolution, self.resolution, 3), \
               merger.MergerConfigMasked(face_type=self.face_type, default_mode='overlay',)

    # override
    def get_EvalConfig(self):
        import eval
        return self.eval_func, (self.resolution, self.resolution, 3), \
               eval.MergerConfigMasked(face_type=self.face_type, default_mode='overlay',)
