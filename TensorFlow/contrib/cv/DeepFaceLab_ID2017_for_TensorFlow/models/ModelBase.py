...
class ModelBase(object):
    ...
    def ask_override(self):
        return self.is_training and self.iter != 0 and io.input_in_time("Press enter in 2 seconds to override \
                                                                        model settings.", 5 if io.is_colab() else 2)
    ...
    def ask_autobackup_hour(self, default_value=0):
        ...
        self.options['autobackup_hour'] = io.input_int(f"Autobackup every N hour", default_autobackup_hour,
                                                       add_info="0..24",
                                                       help_message="Autobackup model files with preview every N hour."
    ...
    def ask_random_src_flip(self):
        ...
        self.options['random_src_flip'] = io.input_bool("Flip SRC faces randomly", default_random_src_flip,
    ...
    def ask_random_dst_flip(self):
        ...
        self.options['random_dst_flip'] = io.input_bool("Flip DST faces randomly", default_random_dst_flip,
    ...
    def get_eval_samples(self):
        eval_samples = []
        for _ in range(25):
            eval_samples += [self.generate_next_samples()]
        self.eval_samples = eval_samples

    def get_ssim(self):
        ssim_result = []
        for samples in self.eval_samples:
            ssim_result += self.onGetSSIM(samples)
        return ssim_result
    ...
    def get_summary_text(self):
        ...
        width_value = max([len(str(x)) for x in visible_options.values()] + [len(str(self.get_iter())),
                                                                             len(self.get_model_name())]) + 1  # Single space buffer to right edge
        ...
        for key in visible_options.keys():
            summary_text += [
                f'=={key: >{width_name}}: {str(visible_options[key]): <{width_value}}==']  # visible_options key/value pairs
        summary_text += [f'=={" " * width_total}==']
    ...
    def get_loss_history_preview(loss_history, iter, w, c):
        ...
        if len(loss_history) != 0:
            ...
            plist_max = [[max(0.0, loss_history[int(col * l_per_col)][p],
                          ...
                          for p in range(loss_count)] for col in range(w)]
