...
# limitations under the License.
"""
data transmission between modelarts and obs
"""
...
def obs_checkpoint2modelarts(config):
    """
    Copy checkpoint from obs to modelarts by using moxing api.
    """
    start = datetime.datetime.now()
    print("===>>>Copy files from obs:{} to modelarts dir:{}".format(config.checkpoint_url,
                                                                    config.modelarts_checkpoint_dir))
    mox.file.copy_parallel(src_url=config.checkpoint_url, dst_url=config.modelarts_checkpoint_dir)
    end = datetime.datetime.now()
    print("===>>>Copy from obs to modelarts, time use:{}(s)".format((end - start).seconds))
    files = os.listdir(config.modelarts_checkpoint_dir)
    print("===>>>Files:", files)
...
def modelarts_checkpoint2obs(FLAGS):
    """
    Copy checkpoint from modelarts to obs.
    According to the swich flags, the debug data may contains auto tune repository,
    dump data for precision comparision, even the computation graph and profiling data.
    """
    work_dir = os.getcwd()
    ...
    ## copy result from modelarts to obs
    obs_checkpoint_dir = "obs://prototypical/checkpoint"
    if not mox.file.exists(obs_checkpoint_dir):
        mox.file.make_dirs(obs_checkpoint_dir)
    mox.file.copy_parallel(src_url=FLAGS.result, dst_url=obs_checkpoint_dir)
    print("===>>>Copy Event or Checkpoint from modelarts dir:{} to obs:{}".format(FLAGS.result, obs_checkpoint_dir))
