    ...
    def build(self, input_shape):
        ...
        with tf.keras.backend.name_scope(self.name):  # name scope used to make sure weights get unique names
            ...
            for k in range(2):
                ...
                self._build_layer(tf.keras.layers.SpatialDropout1D(rate=self.dropout_rate))
            ...
            if self.nb_filters != input_shape[-1]:
                ...
                self.shape_match_conv = tf.keras.layers.Lambda(lambda x: x, name=name)      #identity map
...
class TCN(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.slicer_layer = tf.keras.layers.Lambda(lambda tt: tt[:, self.output_slice_index, :])
    ...
    def call(self, inputs, training=None):
        ...
        for layer in self.residual_blocks:
            try:
                ...
                x, skip_out = layer(tf.keras.backend.cast(x, 'float32'), training=training)
...
def compiled_tcn(num_feat,  # type: int
    ...
    input_layer = tf.keras.Input(shape=(max_len, num_feat))
