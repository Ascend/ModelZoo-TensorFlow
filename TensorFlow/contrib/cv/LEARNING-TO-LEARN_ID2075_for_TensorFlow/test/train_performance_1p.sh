...
#基础参数，需要模型审视修改
#网络名称，同目录名称
Network="LEARNING-TO-LEARN_ID2075_for_TensorFlow"
#训练epoch
train_epochs=10
#训练batch_size
batch_size=128
...
do
    ...
    python3 evaluate3.py \
	--data_path=${data_path}/MNIST-data \
	--output_path=./result \
	--num_epochs=${train_epochs} \
	--step=10 > ${cur_path}test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log 2>&1
...
#输出性能FPS，需要模型审视修改
#倒数第一个时间
#TrainingTime1=`grep "Mean epoch time:" $cur_path/test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log |awk 'END {print $4}'`
#倒数第二个时间
#TrainingTime2=`grep "Mean epoch time:" $cur_path/test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log |awk '{print $4}'|tail -2|head -n -1`
#TrainingTime=`awk 'BEGIN{printf "%.4f\n", '${TrainingTime1}'-'${TrainingTime2}'}'`
