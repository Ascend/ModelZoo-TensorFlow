...
parser.add_argument('--save_ckpt', type=str, default='./save_ckpt',
                    help='save ckpt')
parser.add_argument('--data_path', type=str, required=True, default='',
                    help='Directiory contains required dataset')
...
market = {}
market['TEST'] = os.path.join(DATASET, 'bounding_box_test')
market['TEST_NUM'] = 19732
market['QUERY'] = os.path.join(DATASET, 'query')
market['QUERY_NUM'] = 3368

duke = {}
duke['TEST'] = os.path.join(DATASET, 'bounding_box_test')
duke['TEST_NUM'] = 17661
duke['QUERY'] = os.path.join(DATASET, 'query')
duke['QUERY_NUM'] = 2228
...
for i in range(25, 26):
   model = os.path.join(save_ckpt,'%d.ckpt') % i
   evaluate(dataset, model)

# model = './dataset/Market/checkpoint/25.ckpt'
# evaluate(dataset, model)
sess.close()
