...
class StarGAN_v2() :
    def __init__(self, sess, args):
        ...
        self.dataset_path = os.path.join(args.data_path, self.dataset_name)
        ...
        self.dataset_path_test = os.path.join(self.dataset_path, 'test') #add
...
#                 with tf.device(tf.DeviceSpec(device_type="GPU", device_index=gpu_id)):
    ...
    def train(self):
        ...
        for idx in range(start_batch_id, self.iteration):
            ...
            time_one_iter = time.time() - start_time
            fps = 1.0 * self.batch_size / time_one_iter
            print("iter: [%6d/%6d] time: %4.4f d_loss: %.8f g_loss: %.8f fps %.8f" % (idx, self.iteration, time_one_iter, d_loss, g_loss, fps))
    ...
    def test(self):
        ...
        test_files = glob(os.path.join(self.dataset_path_test + '/*.jpg')) + glob(os.path.join(self.dataset_path_test +'/*.png')) #add  性能看护传参所需，勿改
        t_vars = tf.trainable_variables()
        ...
        for sample_file in tqdm(test_files):
            ...
            for i in range(self.num_style) :
                ...
                save_single_img(i, fake_img, self.result_dir, '{}'.format(os.path.basename(sample_file)))
