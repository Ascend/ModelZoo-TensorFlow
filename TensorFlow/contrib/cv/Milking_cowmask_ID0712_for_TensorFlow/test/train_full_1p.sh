...
echo "**************************"
sed -i "s|steps_per_epoch -=48000|steps_per_epoch -=0|g" train.py
python3 train.py \
...
wait
sed -i "s|steps_per_epoch -=0|steps_per_epoch -=48000|g" train.py
end=$(date +%s)
e2e_time=$(( $end - $start ))
...
accuracy=`grep "accuracy:" ${cur_path}/test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|awk 'END {print $7}'|awk -F "%" '{print $1 / 100}'`
##打印
