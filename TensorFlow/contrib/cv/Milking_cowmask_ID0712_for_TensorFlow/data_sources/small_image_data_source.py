...
class AbstractSmallImageDataSource(object):
  ...
  def __init__(self, n_val, n_sup, train_batch_size, eval_batch_size,
               augment_twice, subset_seed=12345, val_seed=131, dataset_path= './dataset'):
    """Constructor.
    Args:
    ...
    base_train_ds = self._load_train_set(dataset_path=dataset_path)

    @tf.function
    def get_train():
      return next(iter(base_train_ds.batch(self.TRAIN_IMAGES, drop_remainder=True)))
    trainval = get_train()
    ...
    with tf.Session(config=npu_config_proto()) as sess1:
      if n_val > 0:
        train_val_splitter = StratifiedShuffleSplit(
          1, test_size=n_val, random_state=val_seed)
        train_label = trainval['label'].eval()
        train_image = trainval['image'].eval()
        train_ndx, val_ndx = next(train_val_splitter.split(
          train_label, train_label))
        X_train = train_image[train_ndx]  # pylint: disable=invalid-name
        y_train = train_label[train_ndx]
        X_val = train_image[val_ndx]  # pylint: disable=invalid-name
        y_val = train_label[val_ndx]
      else:
        train_label = trainval['label'].eval()
        train_image = trainval['image'].eval()
        X_train = train_image  # pylint: disable=invalid-name
        y_train = train_label
...
###.cache()
    ...
    if n_val > 0:
      ...
      val_ds = val_ds.batch(eval_batch_size, drop_remainder=True)
    ...
    #
    # .cache()
    test_ds = self._load_test_set(dataset_path=dataset_path).cache()
    ...
    if n_val == 0:
        ...
        self.n_eval = self.n_test
    else:
        self.eval_ds = self.val_ds
        self.n_eval = self.n_val

  def _load_train_set(self, dataset_path):
    raise NotImplementedError('Abstract')

  def _load_test_set(self, dataset_path):
    raise NotImplementedError('Abstract')
...
class CIFAR10DataSource(AbstractSmallImageDataSource):
  ...
  def _load_train_set(self, dataset_path):
    ...
    return tfds.load('cifar10',split="train",download=False, data_dir=dataset_path)

  def _load_test_set(self, dataset_path):
    return tfds.load('cifar10',split="test",download=False, data_dir=dataset_path)
...
class CIFAR100DataSource(AbstractSmallImageDataSource):
  ...
  def _load_train_set(self, dataset_path):
    return tfds.load('cifar100', split='train', data_dir=dataset_path)

  def _load_test_set(self, dataset_path):
    return tfds.load('cifar100', split='test', data_dir=dataset_path)
...
class SVHNDataSource(AbstractSmallImageDataSource):
  ...
  def _load_train_set(self, dataset_path):
    return tfds.load('svhn_cropped', split='train', data_dir=dataset_path)

  def _load_test_set(self, dataset_path):
    return tfds.load('svhn_cropped', split='test', data_dir=dataset_path)
