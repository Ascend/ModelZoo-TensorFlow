...
#基础参数，需要模型审视修改
#网络名称，同目录名称
Network="ADA-Net_ID1245_for_TensorFlow"
#训练epoch
train_epochs=1
#训练batch_size
batch_size=16
...
do
	...
	--data_dir=${data_path}/ \
	--num_epochs=2000 \
	--epoch_decay_start=1500 \
	--save_model_secs=150 \
    --logdir=./svhn_log_1/ \
	--dataset_seed=1 >  ${cur_path}test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log   2>&1
	
	python3 test_svhn.py \
	--data_dir=${data_path}/ \
	--logdir=./svhn_log_1/  >  ${cur_path}test/output/${ASCEND_DEVICE_ID}/val_${ASCEND_DEVICE_ID}.log   2>&1
...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
grep "elapsed_time:"  $cur_path/test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log  |awk '{print $6}'|tail -n +2 > $cur_path/test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}_traintime.txt
...
#输出训练精度,需要模型审视修改
train_acc=`grep "accuracy:" $cur_path/test/output/${ASCEND_DEVICE_ID}/val_${ASCEND_DEVICE_ID}.log|awk  '{print $4}'|awk -F":" '{print $2}'`
#打印，不需要修改
...
grep 'CE_loss_train' $cur_path/test/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log|awk  '{print $4}'  > $cur_path/test/output/$ASCEND_DEVICE_ID/train_${CaseName}_loss.txt
