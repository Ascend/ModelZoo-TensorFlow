...
sed -i "s#dave/db_48_2.0216_2.5701.h5#${data_path}/DifferentiableBinarization/datasets/total_text/dave/db_48_2.0216_2.5701.h5#g" inference.py
sed -i "s#datasets/total_text/test_images#${data_path}/DifferentiableBinarization/datasets/total_text/test_images#g" inference.py
...
then
    python3.7 ./train.py \
        --train_generator=${data_path}/DifferentiableBinarization/datasets/total_text \
        --val_generator=${data_path}/DifferentiableBinarization/datasets/total_text \
        --epochs=${train_epoch} \
        --output_path=${output_path} 1>${print_log} 2>&1
else
    python3.7 ./train.py \
        --train_generator=${data_path}/DifferentiableBinarization/datasets/total_text \
        --val_generator=${data_path}/DifferentiableBinarization/datasets/total_text \
...
# 性能相关数据
StepTime=`grep "200/200" ${print_log} | awk '{print $5}' | tr -d "s/step" | tail -n +2 | awk '{sum+=$1} END {print sum/NR}'`
FPS=`awk 'BEGIN{printf "%.2f\n", '${batch_size}'/'${StepTime}'}'`

# 精度相关数据计算
train_accuracy=`grep "200/200" ${print_log} | awk '{print $8}' | awk 'END {print $NF}'`
# 提取所有loss打印信息
grep "200/200" ${print_log} | awk '{print $8}' > ./test/output/${ASCEND_DEVICE_ID}/my_output_loss.txt
