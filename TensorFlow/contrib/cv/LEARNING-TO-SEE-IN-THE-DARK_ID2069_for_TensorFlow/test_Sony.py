...
if not os.path.isdir(FLAGS.result_dir + 'final/'):
    os.makedirs(FLAGS.result_dir + 'final/')
...
for test_id in test_ids:
    ...
    for k in range(len(in_files)):
        ...
        Image.fromarray((output * 255).astype('uint8')).convert('RGB').save(
            FLAGS.result_dir + 'final/%5d_00_%d_out.png' % (test_id, ratio))
        # scipy.misc.toimage(scale_full * 255, high=255, low=0, cmin=0, cmax=255).save(
        Image.fromarray((scale_full * 255).astype('uint8')).convert('RGB').save(
            FLAGS.result_dir + 'final/%5d_00_%d_scale.png' % (test_id, ratio))
        # scipy.misc.toimage(gt_full * 255, high=255, low=0, cmin=0, cmax=255).save(
        Image.fromarray((gt_full * 255).astype('uint8')).convert('RGB').save(
            FLAGS.result_dir + 'final/%5d_00_%d_gt.png' % (test_id, ratio))
