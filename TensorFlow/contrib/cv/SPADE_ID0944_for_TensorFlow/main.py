...
def parse_args():
    ...
    parser.add_argument('--data_path', type=str, default='./data/', help='数据集及预训练模型目录')
    parser.add_argument('--output_path', type=str, default='./output/', help='输出目录')
    parser.add_argument('--use_npu', type=int, default=0, help='是否使用NPU')
    ...
    parser.add_argument('--train_epochs', type=int, default=1, help='The number of epochs to run')
    ...
    args.result_dir = os.path.join(args.output_path, args.result_dir)
    args.checkpoint_dir = os.path.join(args.output_path, args.checkpoint_dir)
    args.log_dir = os.path.join(args.output_path, args.log_dir)
    args.sample_dir = os.path.join(args.output_path, args.sample_dir)
...
def main():
    ...
    from migrate import get_session_config
    session_config = get_session_config(args.use_npu)
