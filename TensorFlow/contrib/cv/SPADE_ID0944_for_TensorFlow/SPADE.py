...
class SPADE(object):
    def __init__(self, sess, args):
        ...
        self.dataset_path = os.path.join(args.data_path, 'dataset', self.dataset_name)
    ...
    def build_model(self):
        ...
        img_and_segmap = img_and_segmap.map(img_class.image_processing).batch(self.batch_size, drop_remainder=True).prefetch(1)
        segmap_test = segmap_test.map(img_class.test_image_processing).shuffle(self.test_dataset_num).batch(self.batch_size, drop_remainder=True).prefetch(1)

        self.img_and_segmap_iterator = img_and_segmap.make_initializable_iterator()
        self.segmap_test_iterator = segmap_test.make_initializable_iterator()

        self.real_x, self.real_x_segmap, self.real_x_segmap_onehot = self.img_and_segmap_iterator.get_next()
        self.real_x_segmap_test, self.real_x_segmap_test_onehot = self.segmap_test_iterator.get_next()
    ...
    def train(self):
        ...
        for epoch in range(start_epoch, self.epoch):
            self.img_and_segmap_iterator.initializer.run(session=self.sess)
            self.segmap_test_iterator.initializer.run(session=self.sess)
            ...
            while True:
                try:
                    ...
                    tf.logging.log_every_n(tf.logging.INFO, 'Train Step %d  sec/step: %.3f  step_loss: %.6f  step_loss_g: %.6f', 1, counter, step_period / 1e3, d_loss, g_loss)
                    counter += 1
    ...
    def random_test(self):
        ...
        segmap_files = glob(os.path.join(self.dataset_path, 'segmap_test', '*.*'))
