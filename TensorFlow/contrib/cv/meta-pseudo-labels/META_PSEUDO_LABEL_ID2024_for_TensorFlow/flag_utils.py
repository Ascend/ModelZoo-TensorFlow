...
define_string('load_ema_checkpoint', None, '')
...
define_string('data_path', None, 'dataset directory')
define_integer('num_shards_per_worker', None, '')
define_string('task_mode', None, '')
define_string('output_path', None, '')
...
def build_params_from_flags():
  ...
  # Figure out `output_path`
  output_path = FLAGS.output_path
  logging.info(f'Checkpoints are at: {output_path}')

  # Create or delete `output_path`
  if not gfile.IsDirectory(output_path):
    logging.info(f'Path `{output_path}` does not exist. Creating')
    gfile.MakeDirs(output_path)
  elif FLAGS.reset_output_dir:
    logging.info(f'Path `{output_path}` exists. Removing')
  ...
  if params.task_mode not in ['inference', 'evals', 'eval_forever']:
    params_filename = os.path.join(params.output_path, 'hparams.json')
