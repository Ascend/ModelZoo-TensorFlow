...
#基础参数，需要模型审视修改
#网络名称，同目录名称
Network="META_PSEUDO_LABEL_ID2024_for_TensorFlow"
#训练epoch
train_epochs=10
#训练batch_size
batch_size=64
...
do
	...
	--uda_temp=0.7 \
	--uda_threshold=0.6 \
	--uda_weight=8	|tee  ${cur_path}/test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log   2>&1
	...
	--uda_temp=0.7 \
	--uda_threshold=0.6 \
	--uda_weight=8	|tee  ${cur_path}/test/output/${ASCEND_DEVICE_ID}/eval_${ASCEND_DEVICE_ID}.log   2>&1
...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
grep "cost="  $cur_path/test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log  |awk '{print $7}'|tail -n +2 > $cur_path/test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}_traintime.txt
...
#最后一个迭代loss值，不需要修改
grep 'eemmmaa_loss' $cur_path/test/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log|awk  '{print $10}'  > $cur_path/test/output/$ASCEND_DEVICE_ID/train_${CaseName}_loss.txt
