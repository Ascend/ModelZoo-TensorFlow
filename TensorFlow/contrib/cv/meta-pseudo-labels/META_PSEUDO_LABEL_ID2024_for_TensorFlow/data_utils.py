...
def _add_val_repeat(params, dataset, num_images):
  ...
  dataset = dataset.take(num_images).cache().repeat()
  return dataset
...
def cifar10_4000_uda(params, batch_size=None):
  ...
  filenames = [os.path.join(params.data_path, "data_batch_{}.bin".format(i)) for i in range(1, 6)]
  record_bytes = 1 + (3 * 32 * 32)
...
def cifar10_eval(params, batch_size=None, eval_mode=None,
  ...
  if eval_mode == 'valid':
    ...
    filenames = [os.path.join(params.data_path, 'test_batch.bin')]
  else:
...
def imagenet_eval(params, batch_size=None, eval_mode=None, num_workers=None,
  ...
  dataset = tf.data.Dataset.range(128).shard(int(os.getenv('RANK_SIZE', '1')), int(os.getenv('RANK_ID', '0')))
