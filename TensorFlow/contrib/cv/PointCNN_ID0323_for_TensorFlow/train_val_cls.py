...
def main():
    ...
    parser.add_argument('--platform', '-p', help='GPU or NPU to use', default='NPU')
    ...
    if args.platform == 'GPU' and setting.keep_remainder:
    ...
    batch_num = batch_num_per_epoch * num_epochs                                                                  #234*1024=239616
    ...
    iterator = tf.data.Iterator.from_string_handle(handle, dataset_train.output_types, dataset_train.output_shapes) # training_dataset.output_shapes 修改
    ...
    if args.platform == 'NPU':
        ...
        #     '{"output":"/cache/result/models/cls/profiling","task_trace":"on","aicpu":"on"}')
    ...
    with tf.Session(config=config) as sess:
        ...
        perf_list=[]
        fps_list=[]
        for batch_idx_train in range(batch_num):   # batch_num
            ######################################################################
            # Validation
            ...
            if batch_idx_train > 1:  #qu掉第一次数据
                perf = time.time() - start_time
                perf_list.append(perf)
                perf_ = np.mean(perf_list)
                fps = batch_size_train / perf
                fps_list.append(fps)
                ...
                if batch_idx_train % 10 == 0:
                    ...
                    print('{}-[Train]-Iter: {:06d} perf: {:.4f} fps: {:.4f} Loss: {:.4f}  T-1 Acc: {:.4f}  T-1 mAcc: {:.4f}'
                        .format(datetime.now(), step, perf_, fps_, loss, t_1_acc, t_1_per_class_acc))
                    sys.stdout.flush()
