...
python3.7 trainer.py > ${print_log} 2>&1

# 性能相关数据计算
FPS=`grep "instances/sec" ${print_log} | tail -n 10 | awk '{print $19}' | awk '{sum+=$1} END {print"",sum/NR}'`

# 精度相关数据计算
train_accuracy=`grep "Accuracy:" ${print_log} | tail -n 10 | awk '{print $16}' | awk '{sum+=$1} END {print"",sum/NR}'`

# 提取所有loss打印信息
grep "Supervised loss" ${print_log} | awk '{print $8}' > ./test/output/${ASCEND_DEVICE_ID}/my_output_loss.txt
