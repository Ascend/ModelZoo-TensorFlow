...
do
    ...
    nohup python3 src/train.py \
		...
		--max_steps=10 \
        --checkpoint_step=5 > ${cur_path}/test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log 2>&1
done
wait
...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
#TrainingTime=`grep 'sec/batch' $cur_path/test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|awk 'END {print $10}'|tail -2 |head -n 1`
#FPS=`grep 'images/sec' $cur_path/test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|awk 'END {print $8}'|tail -2 |head -n 1`
