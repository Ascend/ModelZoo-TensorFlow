...
# 性能相关数据计算
start=`grep " 1/  100] time:" ${print_log}|  cut -d ":" -f 3| cut -d " " -f 2`
end=`grep " 99/  100] time:" ${print_log}|  cut -d ":" -f 3| cut -d " " -f 2`

#StepTime = end - start/steps; FPS=BATCH_SIZE * ITPS
StepTime=`awk 'BEGIN{printf "%.2f", ('${end}' - '${start}')/'99'}'`
