...
class UGATIT(object) :
    def __init__(self, sess, args):
        ...
        if self.phase == 'train':
            self.checkpoint_dir_save = os.path.join(args.output_path, 'checkpoint')
            self.checkpoint_dir_read = os.path.join(args.data_path,'init_model')

        if self.phase == 'test':
            self.checkpoint_dir_save = None
            self.checkpoint_dir_read = os.path.join(args.data_path, 'init_model')

        self.result_dir = os.path.join(args.output_path, 'results')
        self.log_dir = os.path.join(args.output_path, 'logdir')
        self.dataset_dir = args.data_path
        ...
        self.sample_dir = os.path.join(args.output_path, 'sample', self.model_dir)
        check_folder(self.sample_dir)

        self.trainA_dataset = glob('{}/*.*'.format(self.dataset_dir + '/trainA'))
        self.trainB_dataset = glob('{}/*.*'.format(self.dataset_dir + '/trainB'))
    ...
    def build_model(self):
        if self.phase == 'train' :
            ...
            self.G_optim = npu_tf_optimizer(tf.train.AdamOptimizer(self.lr, beta1=0.5, beta2=0.999)).minimize(self.Generator_loss, var_list=G_vars)
            self.D_optim = npu_tf_optimizer(tf.train.AdamOptimizer(self.lr, beta1=0.5, beta2=0.999)).minimize(self.Discriminator_loss, var_list=D_vars)
        ...
        for epoch in range(start_epoch, self.epoch):
            ...
            for idx in range(start_batch_id, self.iteration):
                ...
                if np.mod(idx + 1, self.save_freq) == 0:
                    self.save(self.checkpoint_dir_save, counter)
    ...
    def model_dir(self):
        ...
        return "{}_{}_{}_{}_{}_{}_{}_{}_{}{}{}".format(self.model_name,
    ...
    def test(self):
        tf.global_variables_initializer().run()
        test_A_files = glob('{}/*.*'.format(self.dataset_dir + '/testA'))
        test_B_files = glob('{}/*.*'.format(self.dataset_dir + '/testB'))
