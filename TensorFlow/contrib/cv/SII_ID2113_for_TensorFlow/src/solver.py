...
class Solver(object):
    def __init__(self, flags):
        ...
        run_config.graph_options.rewrite_options.remapping = RewriterConfig.OFF  # 必须显式关闭
        run_config.graph_options.rewrite_options.memory_optimization = RewriterConfig.OFF  # 必须显式关闭
    ...
    def _make_folders(self):
        if self.flags.is_train:
            if self.flags.load_model is None:
                ...
                self.model_out_dir = "/cache/{}/model/{}".format(self.flags.dataset, cur_time)
            ...
            self.sample_out_dir = "/cache/{}/sample/{}".format(self.flags.dataset, cur_time)
            if not os.path.isdir(self.sample_out_dir):
                os.makedirs(self.sample_out_dir)

            self.train_writer = tf.summary.FileWriter("/cache/{}/logs/{}".format(self.flags.dataset, cur_time),
                                                      graph_def=self.sess.graph_def)
        elif not self.flags.is_train:
            self.model_out_dir = "/cache/{}/model/{}".format(self.flags.dataset, self.flags.load_model)
            self.test_out_dir = "/cache/{}/test/{}".format(self.flags.dataset, self.flags.load_model)
    ...
    def train(self):
        ...
        while self.iter_time < self.flags.iters:
            ...
            if np.mod(self.iter_time, self.flags.print_freq) == 0:
                print('Perf : ', time.time() - start_train_time)
