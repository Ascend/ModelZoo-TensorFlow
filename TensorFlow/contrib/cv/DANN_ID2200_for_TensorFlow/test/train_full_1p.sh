...
#基础参数，需要模型审视修改
#网络名称，同目录名称
Network="DANN_ID2200_for_TensorFlow"
#训练epoch
train_epochs=1
#训练batch_size
batch_size=64
...
do
    ...
    --num_steps  8600  >  ${cur_path}test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log   2>&1
...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
step_time_sum=`grep "train_time_sum"  $cur_path/test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log |awk  'END {print $17}'`
TrainingTime=`awk 'BEGIN{printf "%.4f\n", '${step_time_sum}'/10}'`
#打印，不需要修改


#输出训练精度,需要模型审视修改
train_acc=`grep "Domain accuracy:" $cur_path/test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|awk  'END {print $3}'`
#打印，不需要修改
