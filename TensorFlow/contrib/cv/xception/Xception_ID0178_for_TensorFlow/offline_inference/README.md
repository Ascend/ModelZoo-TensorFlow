推理情况表   
| 模型 |数据集| 输入shape | 输出shape | 单张推理时长| msame精度 | 目标精度 |
|--|--|--|---| -- | --| -- |
| xception | imageNet val  | `50*299*299*3` | `50*1000`  | ~4ms | 78.80% | 78.76% | 

## 1、原始模型
[下载地址](https://e-share.obs-website.cn-north-1.myhuaweicloud.com?token=), 找到对应文件夹,下载对应的ckpt，使用改文件夹下面的convert脚本转成pb模型。(提取码为 66666) 

## 2、转om模型
[obs链接](https://e-share.obs-website.cn-north-1.myhuaweicloud.com?token=
)  (提取码为666666),找到对应的om的文件

atc转换命令参考：

```sh
atc --model=xception.pb  --framework=3 --input_shape="inputx:50,299,299,3" --output=./xception --out_nodes="dense/BiasAdd:0" --soc_version=Ascend310
```

## 3、编译msame推理工具
参考https://gitee.com/ascend/tools/tree/master/msame, 编译出msame推理工具


## 4、全量数据集精度测试：

### 4.1 下载预处理后的imageNet val数据集

[obs链接](https://e-share.obs-website.cn-north-1.myhuaweicloud.com?token=)  (提取码为666666) 找到`val.tfrecord` 文件   

### 4.2 预处理

数据预处理在制作数据集时已经完成。wideresnet数据预处理为  
执行shell脚本: ./xception_data_preprocess.sh 生成bin文件  
```txt
resize  : 299 299
img /= 127.5 
img -= 1.0 
```

### 4.3 执行推理和精度计算
执行shell脚本： ./start_inference.sh
```log
/mnt/sdc/data/imageNet/inference
[INFO] output data success
[INFO] create model output success
[INFO] start to process file:/mnt/sdc/data/imageNet/data/998.bin
[INFO] model execute success
Inference time: 398.377ms
/mnt/sdc/data/imageNet/inference
[INFO] output data success
[INFO] create model output success
[INFO] start to process file:/mnt/sdc/data/imageNet/data/999.bin
[INFO] model execute success
Inference time: 398.472ms
/mnt/sdc/data/imageNet/inference
[INFO] output data success
Inference average time without first time: 398.44 ms
[INFO] unload model success, model Id is 1
[INFO] Execute sample success.
Test Finish!
******************************
[INFO] end to destroy stream
[INFO] end to destroy context
[INFO] end to reset device is 0
[INFO] end to finalize acl
/mnt/sdc/data/imageNet/inference
[INFO] 推理结果生成结束
>>>>> 共 50000 测试样本    accuracy:0.788060
```
