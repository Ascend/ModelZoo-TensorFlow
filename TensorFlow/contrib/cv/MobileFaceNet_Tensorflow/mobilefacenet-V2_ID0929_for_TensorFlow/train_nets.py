...
def get_parser():
    ...
    parser.add_argument('--max_epoch', default=10,  type=int, help='epoch to train the network')
    ...
    parser.add_argument('--summary_interval', default=400,  type=int, help='interval to save summary')
...
#         inference_loss, logit = triplet_loss(prelogits, labels, args.num_output, margin=0.5)
        ...
        custom_op.parameter_map["fusion_switch_file"].s = \
            tf.compat.as_bytes("./fusion_switch.cfg")
        ...
        for i in range(args.max_epoch):
            ...
            while True:
                try:
                    ...
                    perf = end - start
                    fps = args.train_batch_size / perf
                    count += 1
                    # print training information
                    if count > 0 and count % args.show_info_interval == 0:
                        print('epoch %d, step %d  perf: %.2f fps: %.2f  total loss is: %.2f  inference loss is: %.2f, reg_loss is: %.2f' % \
                              (i, count, perf, fps, total_loss_val, inference_loss_val, np.sum(reg_loss_val)))
                    ...
                    if count > 0 and args.train_batch_size * count > 3328001:  #3804846/64=59650     add 3328001/64=52000
                        break
