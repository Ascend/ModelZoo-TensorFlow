...
class SlotAttention(layers.Layer):
  ...
  def call(self, inputs):
    ...
    print("______________slotattentioninputs_____________")
    print(inputs)
    ...
    for _ in range(self.num_iterations):
      ...
      # Attention.
      print("__________slotattentionslots___________")
      q = self.project_q(slots)  # Shape: [batch_size, num_slots, slot_size].
      print(q.shape)
      q *= self.slot_size ** -0.5  # Normalization.
      print("____________________________slotattentionkandq.shape__________________")
      print(k.shape)
      print(q.shape)
      attn_logits = tf.keras.backend.batch_dot(k, q, axes=[2,2])
...
class SlotAttentionAutoEncoder(layers.Layer):
  ...
  def call(self, image):
    ...
    print("_________xshape___________")
    print(x.shape)
    # Slot Attention module.
    slots = self.slot_attention(x)
    # `slots` has shape: [batch_size, num_slots, slot_size].
    print("-----------slot--------",type(slots))
