...
# 修改参数
sed -i "s#/mnt/home/test_user01/pr_slot_attention/graphcheckp#${output_path}#" ./code/train_slot.py
sed -i "s#/home/test_user01/slot-attention/data#${data_path}#" ./code/data.py
...
if [ x"${modelarts_flag}" != x ];
then
    python3.7 ./code/train_slot.py
else
    python3.7 ./code/train_slot.py --model_dir=${output_path} --num_train_steps=500 > ${print_log} 2>&1
fi
# 性能相关数据计算
StepTime=`grep "Time" ${print_log} | awk '{print $NF}' | tail -n 10 | awk '{sum+=$1} END {print sum/NR}' | sed s/[[:space:]]//g`
...
# 提取所有loss打印信息
grep "Step" ${print_log} | awk '{print $8}' | tr -d "," > ./test/output/${ASCEND_DEVICE_ID}/my_output_loss.txt
