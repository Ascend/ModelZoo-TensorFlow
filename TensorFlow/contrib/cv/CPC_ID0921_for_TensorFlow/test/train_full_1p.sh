...
# 修改参数
sed -i "s#resources#${data_path}/data#" ./data_utils.py
...
if [ x"${modelarts_flag}" != x ];
then
    python3.7 ./train_model.py
else
    python3.7 ./train_model.py > ${print_log} 2>&1
fi
# 性能相关数据计算
StepTime=`grep -Eo "[0-9]*ms/step" ${print_log} | tr -d "ms/step" | tail -n 5 | awk '{sum+=$1} END {print sum/NR}' | sed s/[[:space:]]//g`
...
# 提取所有loss打印信息
grep -Eo "loss: 0.[0-9]*" ${print_log} | awk '{print $NF}' > ./test/output/${ASCEND_DEVICE_ID}/my_output_loss.txt
