...
do
    ...
    python3 Non_local_Net.py --data_path=${data_path}/MNIST_data \
	--epoch=${epochs} \
...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
grep "global_step/sec:"  $cur_path/test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log  |awk '{print $6}'|tail -n +2 > $cur_path/test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}_traintime.txt
...
#输出训练精度,需要模型审视修改
train_accuracy=`grep 'Accuarcy' $cur_path/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log | awk END'{print $5}'`
#打印，不需要修改
