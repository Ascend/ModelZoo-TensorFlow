...
def NonLocalBlock(input_x, output_channels, sub_sample=False, is_bn=True, is_training=True, scope="NonLocalBlock"):
    batch_sizes, height, width, in_channels = input_x.get_shape().as_list()
    with tf.variable_scope(scope):
        with tf.variable_scope("g"):
            g = tf.layers.conv2d(inputs=input_x, filters=output_channels, kernel_size=1,
                                 strides=1, padding="same", name="g_conv")
            if sub_sample:
                g = tf.layers.max_pooling2d(inputs=g, pool_size=2, strides=2, padding="valid", name="g_max_pool")
                print(g.shape)

        with tf.variable_scope("phi"):
            phi = tf.layers.conv2d(inputs=input_x, filters=output_channels,
                                   kernel_size=1, strides=1, padding="same", name="phi_conv")
            if sub_sample:
                phi = tf.layers.max_pooling2d(inputs=phi, pool_size=2, strides=2, padding="valid", name="phi_max_pool")
                #print(phi.shape)

        with tf.variable_scope("theta"):
            theta = tf.layers.conv2d(inputs=input_x, filters=output_channels, kernel_size=1,
                                     strides=1, padding="same", name="theta_conv")
            print(theta.shape)
        ...
        with tf.variable_scope("w"):
            w_y = tf.layers.conv2d(inputs=y, filters=in_channels, kernel_size=1, strides=1,
                                   padding="same", name="w_conv")
            if is_bn:
...
def residual(x, output_channels, strides, types, is_training):
    short_cut = x
