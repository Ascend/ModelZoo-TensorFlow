...
def parse_args():
    ...
    parser.add_argument('--num_gpus', type=int, default=1, help='The number of gpu')
...
def Build_ResNet(x, resnet_size, is_training = True):
    ...
    for i in range(len(stages)):
        with tf.compat.v1.variable_scope("stage_%d_block_%d"%(i, 0)):
            ...
            for j in range(1, stages[i]):
                with tf.compat.v1.variable_scope("stage_%d_block_%d"%(i, j)):
                    if stages[i] == 4 or stages[i] == 6:
...
with tf.Session() as sess:
    ...
    for epoch in range(num_epoches):
        ...
        print("Epoch: {}\tLoss:{:.9f}\tAccuarcy: {:.2%}\t sec:{}".format(epoch+1, avg_cost, avg_acc, end - start))
