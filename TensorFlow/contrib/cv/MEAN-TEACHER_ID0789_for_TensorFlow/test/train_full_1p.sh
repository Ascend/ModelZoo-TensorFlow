...
if [ x"${modelarts_flag}" != x ];
then
    python3.7 ./train_cifar10.py --data_path=${data_path} --output_path=${output_path} --training_length=${train_steps}
else
    python3.7 ./train_cifar10.py --data_path=${data_path} --output_path=${output_path} --training_length=${train_steps} 1>${print_log} 2>&1
fi

# 性能相关数据计算
StepTime=`grep "Perf: " ${print_log} |awk 'END {print $11}'`
FPS=`awk 'BEGIN{printf "%.2f\n", '${batch_size}'/'${StepTime}'}'`

# 精度相关数据计算
train_accuracy=`grep "Evaluate" ${print_log} |awk 'END {print $5}'|sed 's/,//g'|sed 's/%//g'`
# 提取所有loss打印信息
grep "train/class_cost/1:" ${print_log} | awk '{print $7}'|sed 's/,//g'|sed 's/%//g' > ./test/output/${ASCEND_DEVICE_ID}/my_output_loss.txt
