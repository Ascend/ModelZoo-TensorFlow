...
class Model:
    ...
    def train(self, training_batches, evaluation_batches_fn):
        ...
        self.run(self.train_init_op, self.feed_dict(next(training_batches)))
        # vari = [x for x in slim.get_variables(scope='initialization')]
        #
        # for i in vari:
        #     self.run(tf.variables_initializer([i]), self.feed_dict(inp))
        # self.run(tf.variables_initializer(slim.get_variables(scope='initialization')), self.feed_dict(inp))
        # print('init is clear')
        # self.run(tf.variables_initializer(slim.get_variables(scope='primary')), self.feed_dict(inp))
        # print('primary is clear')
        # self.run(tf.variables_initializer(slim.get_variables(scope='secondary')), self.feed_dict(inp))
        # print('secondary is clear')
        # self.run(tf.variables_initializer(slim.get_variables(scope='train_step')), self.feed_dict(inp))
        # print('train_step is clear')
        ...
        for batch in training_batches:
            ...
            if step_control['time_to_print']:
                LOG.info("step Train %5d:   %s Perf: %.4f",  step_control['step'], self.result_formatter.format_dict(results), time.time()-start_time)
    ...
    def evaluate(self, evaluation_batches_fn):
        ...
        LOG.info("step %5d:  Evaluate %s", step, self.result_formatter.format_dict(results))
