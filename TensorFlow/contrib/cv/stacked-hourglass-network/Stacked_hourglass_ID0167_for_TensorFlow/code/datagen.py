...
class DataGenerator():
	...
	def _aux_generator(self, batch_size = 16, stacks = 4, normalize = True, sample_set = 'train'):
		...
		for step in range(1000): 
			...
			#img_all.append(train_img)
			#gtamp_all.append(train_gtmap)
			#weight_all.append(train_weights)
			yield train_img, train_gtmap, train_weights
	
	#def _test_aux_generator(self, batch_size = 16, stacks = 4, normalize = True):
		#""" Auxiliary Generator
	#	Args:
		#	See Args section in self._generator
	#	"""
	#	for i in range(len(self.valid_set)):
		#	name = self.valid_set[i]


		#	train_img = np.zeros((1, 256,256,3), dtype = np.float32)
		#	train_gtmap = np.zeros((1, stacks, 64, 64, len(self.joints_list)), np.float32)
		#	train_weights = np.zeros((1, len(self.joints_list)), np.float32)
			
		#	joints = self.data_dict[name]['joints']
		#	box = self.data_dict[name]['box']
		#	weight = np.asarray(self.data_dict[name]['weights'])
		#	train_weights[0] = weight 
		#	img = self.open_img(name)
		#	padd, cbox = self._crop_data(img.shape[0], img.shape[1], box, joints, boxp = 0.2)
		#	new_j = self._relative_joints(cbox,padd, joints, to_size=64)
		##	hm = self._generate_hm(64, 64, new_j, 64, weight)
		...
		#	hm = np.repeat(hm, stacks, axis = 0)
		#	if normalize:
		#		train_img[0] = img.astype(np.float32) / 255
		#	else :
		#		train_img[0] = img.astype(np.float32)
		#	train_gtmap[0] = hm  
		#	train_img.tofile('../train_img_1/test_img%d.bin'%i)
		##	train_gtmap.tofile('../train_gtmap_1/test_gt%d.bin'%i) 
