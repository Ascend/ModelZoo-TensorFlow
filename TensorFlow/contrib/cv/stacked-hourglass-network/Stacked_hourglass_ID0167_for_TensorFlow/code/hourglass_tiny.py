...
class HourglassModel():
	...
	def generate_model(self):
		...
		self.rmsprop = NPULossScaleOptimizer(opt_tmp, loss_scale_manager)
		...
		with tf.name_scope('Train'):
			...
			for epoch in range(nEpochs):
				...
				for i in range(epochSize):
					...
					sys.stdout.flush()
					#indexss = random.randint(0,epochSize-1)
				...
				for i in range(validIter):
					...
					#print(i,accuracy_pred,'/n',img_valid,'/n',gt_valid)                    
				print('--Avg. Accuracy =', str((np.sum(accuracy_array) / len(accuracy_array)) * 100)[:6], '%' )
				#with tf.name_scope('save'):
				#	self.saver.save(self.Session, os.path.join(os.getcwd(),str(self.name + '_' + str(epoch + 1)+ '_' + str((np.sum(accuracy_array) / len(accuracy_array)) * 100)[:6])))
			...
			#for i in range(validIter):
			#	indexss = random.randint(0,epochSize-1)
			#	img_valid, gt_valid = img_all[indexss] , gtamp_all[indexss]
	...
	#def _test(self, nEpochs = 10, epochSize = 1000, saveStep = 500, validIter = 10):
		"""
		"""
		#with tf.name_scope('Test'):
		#	accuracy_array_test = np.array([0.0]*len(self.joint_accur))
		#	img_all, gtamp_all, weight_all = self.dataset._aux_generator(self.batchSize, self.nStack, normalize = True, sample_set = 'valid')
		#	startTime = time.time()
		#	for i in range(validIter):
