...
class SAGAN(object):
    ...
    def __init__(self, sess, args):
        ...
        if self.dataset_name == 'mnist' :
            ...
            self.c_dim = 3
            dataset_path = os.path.join(args.data_path, 'dataset', self.dataset_name)
            self.data = load_data(dataset_name_or_path=dataset_path, size=self.img_size)
    ...
    def build_model(self):
        ...
        if self.custom_dataset:
            ...
            inputs = inputs.map(Image_Data_Class.image_processing).shuffle(self.dataset_num).batch(self.batch_size, drop_remainder=True).prefetch(1)

            self.inputs_iterator = inputs.make_initializable_iterator()

            self.inputs = self.inputs_iterator.get_next()
    ...
    def train(self):
        ...
        for epoch in range(start_epoch, self.epoch):
            self.inputs_iterator.initializer.run(session=self.sess)
