...
def load_mnist(size=64):
    ...
    x = np.asarray([cv2.resize(x_img, (size, size)) for x_img in x])
    x = np.expand_dims(x, axis=-1)
    return x
...
def load_data(dataset_name_or_path, size=64) :
    if dataset_name_or_path == 'mnist' :
        x = load_mnist(size)
    elif dataset_name_or_path == 'cifar10' :
        x = load_cifar10(size)
    else :

        x = glob(os.path.join(dataset_name_or_path, '*.*'))

    return x


def preprocessing(x, size):
    x = cv2.imread(x, flags=cv2.IMREAD_COLOR)
