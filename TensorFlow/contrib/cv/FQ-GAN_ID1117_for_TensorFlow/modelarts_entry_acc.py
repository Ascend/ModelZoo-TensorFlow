...
if __name__ == '__main__':
    ...
    bash_header=os.path.join(code_dir,'train_full_1p.sh.sh')
    arg_url='%s '%(code_dir)
    bash_command='bash %s %s' %(bash_header,arg_url)
    print('bash command:',bash_command)
    os.system(bash_command)
    #bash_header = os.path.join(code_dir, 'run_self.sh')
    ...
    # local_dir = '/cache/samples_quant'
    # files = os.listdir(local_dir)
    # print(files)
    # print(files[0])
    remote_dir=config.train_url
    ...
    mox.file.copy_parallel("/cache/profiling", remote_dir)
    mox.file.copy_parallel("/cache/results",remote_dir)
