...
class Network:
    def __init__(self, dataset, config, val_steps, max_epoch):
        ...
        if self.config.saving:
            if self.config.saving_path is None:
                self.saving_path = time.strftime('/home/ma-user'
                                                 '/modelarts/outputs/train_url_0/result/Log_%Y-%m-%d_%H-%M-%S', time.gmtime())
            else:
        ...
        with tf.variable_scope('inputs'):
            ...
            self.Log_file = open('/home/ma-user/modelarts/outputs/train_url_0/result/' + 'log_train_' + dataset.name + str(dataset.val_split) + '.txt', 'a')
        ...
        custom_op = c_proto.graph_options.rewrite_options.custom_optimizers.add()
        custom_op.name = "NpuOptimizer"
        c_proto.graph_options.rewrite_options.remapping = RewriterConfig.OFF  # 必须显式关闭
        c_proto.graph_options.rewrite_options.memory_optimization = RewriterConfig.OFF  # 必须显式关闭
        #c_proto = npu_tf_config.session_dump_config(c_proto, action='fusion_off')
    ...
    def train(self, dataset):
        ...
        while self.training_epoch < self.max_epoch:
            t_start = time.time()
            try:
                ops = [self.train_op,
    ...
    def evaluate(self, dataset):
        ...
        for step_id in range(self.val_steps):
            if step_id % 5 == 0:
                print(str(step_id) + ' / ' + str(self.val_steps))
            try:
