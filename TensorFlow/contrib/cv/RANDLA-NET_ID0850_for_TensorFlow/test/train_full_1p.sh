...
#基础参数，需要模型审视修改
#网络名称，同目录名称
Network="RANDLA-NET_ID0850_for_TensorFlow"
#训练epoch
train_epochs=1
#训练batch_size
batch_size=3
...
#输出训练精度,需要模型审视修改
train_acc=`grep "Mean IoU" $cur_path/test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log |awk  'END {print $4}'|awk -F'[ %]+' '{print $1}'`
#打印，不需要修改
...
grep 'L_out' $cur_path/test/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log|awk  '{print $3}'|awk -F'=' '{print $2}'  > $cur_path/test/output/$ASCEND_DEVICE_ID/train_${CaseName}_loss.txt
