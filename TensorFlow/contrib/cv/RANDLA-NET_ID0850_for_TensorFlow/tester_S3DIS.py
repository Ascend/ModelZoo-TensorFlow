...
class ModelTester:
    def __init__(self, model, dataset, restore_snap=None):
        ...
        makedirs('/home/ma-user/modelarts/outputs/train_url_0/test') if not exists('/home/ma-user/modelarts/outputs/train_url_0/test') else None
        self.Log_file = open('/home/ma-user/modelarts/outputs/train_url_0/test/log_test_' + str(dataset.val_split) + '.txt', 'a')
        ...
        self.sess = tf.Session(config=npu_config_proto(config_proto=c_proto))
    ...
    def test(self, model, dataset, num_votes=100):
        ...
        test_path = join('/cache/train_out/test', saving_path.split('/')[-1])
