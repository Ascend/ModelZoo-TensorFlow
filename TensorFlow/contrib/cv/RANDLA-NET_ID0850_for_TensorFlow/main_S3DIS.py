...
class S3DIS:
    def __init__(self, test_area_idx):
        self.name = 'S3DIS'
        self.path =FLAGS.data_path #'/home/ma-user/modelarts/outputs/data_url_0/data_retry'
    ...
    def get_batch_gen(self, split):
        if split == 'training':
            num_per_epoch = FLAGS.train_steps * FLAGS.batch_size
        elif split == 'validation':
            num_per_epoch = FLAGS.val_steps * FLAGS.val_batch_size
    ...
    def init_input_pipeline(self):
        ...
        self.batch_train_data = self.train_data.batch(FLAGS.batch_size, drop_remainder=True)
        self.batch_val_data = self.val_data.batch(FLAGS.val_batch_size, drop_remainder=True)
        map_func = self.get_tf_mapping2()

        self.batch_train_data = self.batch_train_data.map(map_func=map_func)
        self.batch_val_data = self.batch_val_data.map(map_func=map_func)

        self.batch_train_data = self.batch_train_data.prefetch(FLAGS.batch_size)
        self.batch_val_data = self.batch_val_data.prefetch(FLAGS.val_batch_size)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_path', type=str, help='the path model saved')
    parser.add_argument('--data_path', type=str, help='the data on modelarts mirror')
    parser.add_argument('--train_steps', type=int, help='Number of steps per epochs')
    parser.add_argument('--batch_size', type=int, help='batch_size during training')
    parser.add_argument('--val_steps', type=int, help='Number of validation steps per epoch')
    parser.add_argument('--val_batch_size', type=int, help='batch_size during validation and test')
    parser.add_argument('--max_epoch', type=int, help='maximum epoch during training')
    ...
    if Mode == 'train':
        model = Network(dataset, cfg, FLAGS.val_steps, FLAGS.max_epoch)
        model.train(dataset)
    elif Mode == 'test':
        cfg.saving = False
        ...
        if FLAGS.model_path is not 'None':
            ...
            chosen_snapshot = -1
            logs = np.sort([os.path.join('/cache/train_out/result', f) for f in os.listdir('/cache/train_out/result') if
                            f.startswith('Log')])
