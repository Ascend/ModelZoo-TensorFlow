...
sed -i s#"/home/dingwei/yolov5"#"${data_path}"#g ./2007_train.txt
sed -i s#"/home/dingwei/yolov5"#"${data_path}"#g ./2007_val.txt
...
# 性能相关数据计算
#StepTime=`grep "each step time" ${print_log} | tail -n 10 | awk '{print $NF}' | awk '{sum+=$1} END {print sum/NR}'`
#FPS=`awk 'BEGIN{printf "%.2f\n", '${batch_size}'/'${StepTime}'}'`

# 精度相关数据计算
train_accuracy=`grep "mAP =" ${print_log} | awk '{print $NF}'`
# 提取所有loss打印信息
grep "loss:" ${print_log} | awk '{print $NF}' > ./test/output/${ASCEND_DEVICE_ID}/my_output_loss.txt
