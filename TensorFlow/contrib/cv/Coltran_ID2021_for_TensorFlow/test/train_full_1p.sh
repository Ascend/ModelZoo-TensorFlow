...
mkdir -p ./test/output/${ASCEND_DEVICE_ID}/model.ckpt

# 修改参数
sed -i 's/300000/200000/' ./configs/spatial_upsampler.py
...
if [ x"${modelarts_flag}" != x ];
then
    python3.7 ./train.py --train_url=${output_path} --data_url=${data_path} --logdir=${output_path}
else
    python3.7 ./train.py --train_url=${output_path} --data_url=${data_path} --logdir=${output_path} --data_dir=${data_path} 1>${print_log} 2>&1
fi

# 性能相关数据计算
FPS=`grep "examples/sec" ${print_log} | awk '{print $10}' | tr -d "(" | tail -n 10 | awk '{sum+=$1} END {print"",sum/NR}' | sed s/[[:space:]]//g`
StepTime=`awk 'BEGIN{printf "%.2f\n", '${batch_size}'/'${FPS}'}'`

# 提取所有loss打印信息
grep "Avgloss" ${print_log} | awk '{print $9}' > ./test/output/${ASCEND_DEVICE_ID}/my_output_loss.txt
