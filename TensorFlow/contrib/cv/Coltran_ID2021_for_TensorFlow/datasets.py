...
def resize_to_square(image, resolution=32, train=True):
    ...
    Args:
      ...
      train: A boolean indicating whether the preprocessing is for training.
      Returns:
        A preprocessed image `Tensor` of the same type as `image`.
    """
    ...
    side_size = tf.math.minimum(height, width)
    ...
    image = datasets_utils.change_resolution(
        image, res=resolution, method='area')
    return image
...
def preprocess(example, train=True, resolution=256):
    ...
    Args:
        example: A `Tensor` representing an image of arbitrary size.
        train: A boolean indicating whether the preprocessing is for training.
        resolution: The resolution of the output image.
    Returns:
        A preprocessed image `Tensor` of the same type as `example`.
    """
    image = example
...
def get_gen_dataset(data_dir, batch_size):
    ...
    Args:
        data_dir: The directory where the TFRecords are stored.
    ...
    def parse_example(example_proto, res=64):
        features = {'image': tf.compat.v1.io.FixedLenFeature([res*res*3], tf.int64)}
        example = tf.compat.v1.io.parse_example(example_proto, features=features)
    ...
    def tf_record_name_to_num(x):
        """
        Converts a TFRecord name to a number.
        Args:
            x: The name of the TFRecord.
            Returns:
                The number.
        """
        x = x.split('.')[0]
    ...
    tf_dataset = tf.compat.v1.data.TFRecordDataset(records)
    tf_dataset = tf_dataset.map(parse_example)
    tf_dataset = tf_dataset.batch(drop_remainder=True, batch_size=batch_size)
...
def create_gen_dataset_from_images(image_dir):
    ...
    def load_image(path):
        ...
        return tf.io.decode_image(image_str, channels=3)
...
def get_imagenet(subset, read_config, data_dir):
    """
    Gets imagenet dataset.
    Args:
        subset: The subset of the dataset to be used.
        read_config: The read configuration.
    ...
    if subset == 'test':
        ds = tfds.load('imagenet2012', data_dir=data_dir,split='validation', shuffle_files=False,download=False)
    else:
        # split 10000 samples from the imagenet dataset for validation.
        ds, info = tfds.load('imagenet2012', data_dir=data_dir,download=False, split='train', with_info=True,
...
def get_dataset(name,
    ...
    * Setting `config.downsample` to be True, adds:.
    ...
    if name == 'imagenet':
        ds = get_imagenet(subset, read_config, data_dir)
    elif name == 'custom':
        assert data_dir is not None
