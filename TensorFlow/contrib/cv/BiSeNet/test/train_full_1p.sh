...
# 性能相关数据计算
StepTime=`grep -a 'step time' ${print_log}|awk -F " " '{print $21}'|awk 'END {print}'`
FPS=`grep -a 'FPS' ${print_log}|awk -F " " '{print $17}'|awk 'END {print}'`

# 精度相关数据计算
train_accuracy=`grep -a 'accuracy' ${print_log}|awk -F " " '{print $10}'|awk 'END {print}'`
# 提取所有loss打印信息
