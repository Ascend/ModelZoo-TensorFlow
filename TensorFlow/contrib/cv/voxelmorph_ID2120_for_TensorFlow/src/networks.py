...
from tensorflow.python.keras.layers import LeakyReLU, Reshape, Lambda
...
sys.path.append(os.path.join(project_path, 'ext/neuron'))
sys.path.append(os.path.join(project_path, 'ext/pynd-lib'))
sys.path.append(os.path.join(project_path, 'ext/pytools-lib'))

# other vm functions


def unet_core(vol_size, enc_nf, dec_nf, x_in, full_size=True):
...
def cvpr2018_net(vol_size, enc_nf, dec_nf, src, tgt, full_size=True, indexing='ij'):
    """
    ...
    # get the core model
    x = unet_core(vol_size, enc_nf, dec_nf, x_in, full_size=full_size)
    ...
    y = nrn_layers.SpatialTransformer(
        interp_method='linear', indexing=indexing)([src, flow])
    ...
    if use_miccai_int:
        ...
        for _ in range(int_steps):
            v1 = nrn_layers.SpatialTransformer(
                interp_method='linear', indexing=indexing)([v, v])
        ...
        z_sample = flow
        flow = nrn_layers.VecInt(
            method='ss', name='flow-int', int_steps=int_steps)(z_sample)
        if bidir:
            rev_z_sample = Negate()(z_sample)
            neg_flow = nrn_layers.VecInt(
                method='ss', name='neg_flow-int', int_steps=int_steps)(rev_z_sample)
...
def nn_trf(vol_size, indexing='xy'):
    ...
    nn_output = nrn_layers.SpatialTransformer(
        interp_method='nearest', indexing=indexing)
...
def cvpr2018_net_probatlas(vol_size, enc_nf, dec_nf, nb_labels,
    ...
    unet_model = unet_core(vol_size, enc_nf, dec_nf,
                           full_size=full_size, tgt_feats=nb_labels)
    [src_img, src_atl] = unet_model.inputs
    x = unet_model.output

    # transform the results into a flow field.
    Conv = getattr(KL, 'Conv%dD' % ndims)
    flow1 = Conv(ndims, kernel_size=3, padding='same',
                 name='flow', kernel_initializer=weaknorm)(x)
    if diffeomorphic:
        flow2 = nrn_layers.VecInt(
            method='ss', name='flow-int', int_steps=8)(flow1)
    else:
        flow2 = flow1
    if full_size:
        flow = flow2
    ...
    # warp atlas
    if warp_method == 'WARP':
        warped_atlas = nrn_layers.SpatialTransformer(
            interp_method='linear', indexing=indexing, name='warped_atlas')([src_atl, flow])
    else:
    ...
    if init_mu is not None:
        ...
        stat_mu = Lambda(lambda x: network_stat_weight *
                         x + init_mu, name='comb_mu')(stat_mu)

    # combine sigma with initializtion
    if init_sigma is not None:
        init_logsigmasq = np.array([2*np.log(f) for f in init_sigma])
        stat_logssq = Lambda(lambda x: network_stat_weight *
                             x + init_logsigmasq, name='comb_sigma')(stat_logssq)
    ...
    uloglhood = KL.Lambda(lambda x: unnorm_loglike(
        *x), name='unsup_likelihood')([src_img, stat_mu, stat_logssq])
...
def diff_net(vol_size, enc_nf, dec_nf, int_steps=7, src_feats=1,
    ...
    # get unet
    unet_model = unet_core(vol_size, enc_nf, dec_nf,
                           full_size=full_size, src=src, tgt=tgt, src_feats=src_feats)
    [src, tgt] = unet_model.inputs
    ...
    # new implementation in neuron is cleaner.
    flow = nrn_layers.VecInt(
        method='ss', name='flow-int', int_steps=int_steps)(vel)
    if bidir:
        # rev_z_sample = Lambda(lambda x: -x)(z_sample)
        neg_vel = Negate()(vel)
        neg_flow = nrn_layers.VecInt(
            method='ss', name='neg_flow-int', int_steps=int_steps)(neg_vel)
    ...
    # transform
    y = nrn_layers.SpatialTransformer(
        interp_method='linear', indexing=indexing, name='warped_src')([src, flow])
    if bidir:
        y_tgt = nrn_layers.SpatialTransformer(
            interp_method='linear', indexing=indexing, name='warped_tgt')([tgt, neg_flow])
    ...
    if ret_flows:
        outputs += [model.get_layer('diffflow').output,
...
def cond_img_atlas_diff_model(vol_shape, nf_enc, nf_dec,
    ...
    for i in range(extra_conv_layers):
        last_tensor = Conv(nb_conv_features, kernel_size=cond_conv_size,
                           padding='same', name='atlas_ec_%d' % i)(last_tensor)
    ...
    if not templcondsi:
        ...
        # this is just tmp. Do not use me.
        tmp_layer = KL.Lambda(lambda x: K.softmax(x[..., 1:]))(atlas_tensor)
        cl = Conv(1, kernel_size=1, padding='same', use_bias=False,
                  name='atlas_gen', kernel_initializer=RandomNormal(mean=0, stddev=1e-5))
        ximg = cl(tmp_layer)
        if templcondsi_init is not None:
            w = cl.get_weights()
            w[0] = templcondsi_init.reshape(w[0].shape)
            cl.set_weights(w)
        atlas_tensor = KL.Lambda(lambda x: K.concatenate(
            [x[0], x[1][..., 1:]]), name='atlas')([ximg, atlas_tensor])

    pheno_model = keras.models.Model(
        [pheno_tmp_model.input, atlas_input], atlas_tensor)
    ...
    if do_mean_layer:
        mean_layer = nrn_layers.MeanStream(
            name='mean_stream', cap=mean_cap)(neg_diffflow_out)
...
def img_atlas_diff_model(vol_shape, nf_enc, nf_dec,
    ...
    # pre-warp model (atlas model)
    # Wait I'm confused....
    pw = atl_img_model(vol_shape, mult=atl_mult,
                       src=mn.inputs[0], atl_layer_name=atl_layer_name)
...
class LocalParamWithInput(Layer):
    ...
    def build(self, input_shape):
        self.kernel = self.add_weight(name='kernel',
                                      ...
                                      trainable=True)
        super(LocalParamWithInput, self).build(
