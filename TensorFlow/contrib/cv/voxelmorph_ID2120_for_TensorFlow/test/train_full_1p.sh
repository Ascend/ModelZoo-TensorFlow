...
export PYTHONPATH=$PYTHONPATH:${cur_path}/../ext
export RANK_SIZE=1
export RANK_ID=0
export JOB_ID=10087
...
if [ x"${modelarts_flag}" != x ];
then
    python3.7 ./src/train_all.py ${data_path}/train/ --atlas_file=${data_path}/atlas_abide_brain_crop.nii.gz --model_dir=${output_path} --tensorboard_log_dir=${output_path} --batch_size=batch_size
else
    python3.7 ./src/test_zyh.py ${data_path}/Dataset-ABIDE --model_path=${data_path}/Dataset-ABIDE/models 1>${print_log} 2>&1
    python3.7 ./src/train_all.py ${data_path}/Dataset-ABIDE/train/ --atlas_file=${data_path}/Dataset-ABIDE/atlas_abide_brain_crop.nii.gz --model_dir=${output_path} --tensorboard_log_dir=${output_path} --batch_size=${batch_size} 1>>${print_log} 2>&1

fi

# 性能相关数据计算
StepTime=`grep "time" ${print_log} | awk '{print $11}' | tr -d "{" | tr -d "}" | awk '{sum+=$1} END {print sum/NR}'`
FPS=`awk 'BEGIN{printf "%.2f\n", '${batch_size}'/'${StepTime}'}'`

# 精度相关数据计算
train_accuracy=`grep "dice" ${print_log} | awk '{print $NF}'`
# 提取所有loss打印信息
grep "loss:" ${print_log} | awk '{print $5}' | tr -d "{" | tr -d "}," > ./test/output/${ASCEND_DEVICE_ID}/my_output_loss.txt
