...
class PrecisionTool(object):
    ...
    def do_vector_compare(self, argv=None):
        """do vector compare"""
        parser = argparse.ArgumentParser()
        parser.add_argument('-lt', '--left', dest='lt',
                            default=None, help='left path(npu dump path)')
        parser.add_argument('-rt', '--right', dest='rt',
                            default=None, help='right path(cpu/npu dump path)')
        parser.add_argument('-g', '--graph', dest='graph',
                            required=False, default=None, help='graph json file')
        args = parser.parse_args() if argv is None else parser.parse_args(argv)
        # 1. compare npu_debug0 - tf  dump data (auto)
        # 2. compare npu_debug0 - npu_debug1 dump data
        # 3. compare dir - dir dump data
        result_dir = os.path.join(cfg.VECTOR_COMPARE_PATH, time.strftime(
            "%Y%m%d%H%M%S", time.localtime()))
        if args.lt is None:
            debug_0_dump_root = self.dump_manager.get_dump_root_dir(
                Constant.DEFAULT_DEBUG_ID)
            if util.empty_dir(debug_0_dump_root):
                raise PrecisionToolException(
                    "NPU debug_0 dump dir is empty, no files to compare.")
            if not util.empty_dir(cfg.TF_DUMP_DIR):
                self.log.info(
                    "Tf dump dir is not empty, will compare npu dump data with tf dump data.")
                ...
                self.log.warning("Tf dump dir is empty, maybe run [python3 precision_tool/cli.py tf_dump] to decode"
                                 " tf debug data.")
            debug_1_dump_root = self.dump_manager.get_dump_root_dir(
                Constant.NPU_DEBUG_ID_1)
            if debug_1_dump_root is not None and not util.empty_dir(debug_1_dump_root):
                self.log.info(
                    "NPU debug_1 dump dir is not empty, will compare two npu dump data.")
                self.compare.npu_vector_compare(
                    debug_0_dump_root, debug_1_dump_root)
        else:
            lh_path = args.lt
            rh_path = args.rt
            graph_json = args.graph
            self.compare.vector_compare(
                lh_path, rh_path, result_dir, graph_json)
        self.compare.vector_summary(result_dir)

    @catch_tool_exception
    def do_vector_compare_summary(self, argv=None):
        parser = argparse.ArgumentParser(
            description="show vector compare result summary.")
        parser.add_argument('-f', '--file', dest='file', default=None,
                            required=False, help='compare_result file/path')
        parser.add_argument('-c', '--cos_sim', dest='cos_sim',
                            type=float, help='cos_sim_threshold', default=0.98)
        parser.add_argument('-l', '--limit', dest='limit',
                            type=int, help='limit', default=3)
        args = parser.parse_args() if argv is None else parser.parse_args(argv)
        error_ops = self.compare.vector_summary(
            args.file, args.cos_sim, args.limit)
        # parse error_ops
    ...
    def do_list_nodes(self, argv):
        ...
        parser.add_argument('-t', '--type', dest='type',
                            default='', help='list by op type')
        parser.add_argument('-n', '--name', dest='name',
                            default='', help='list by op name')
        parser.add_argument('-f', '--fusion', dest='fusion',
                            default='', help='list by op fusion pass')
        parser.add_argument('-k', '--kernel_name', dest='kernel_name',
                            default='', help='list by op kernel_name')
        args = parser.parse_args(argv)
        self.graph_manager.print_op_list(
            args.type, args.name, args.fusion, args.kernel_name)
    ...
    def do_node_info(self, argv):
        ...
        npu_op_summary, tf_op_summary = self.graph_manager.op_graph_summary(
            npu_ops, args.attr)
        npu_dump_summary, tf_dump_summary = self.dump_manager.op_dump_summary(
            npu_ops)
    ...
    def do_compare_data(self, argv):
        ...
        parser.add_argument('-n', '--name', dest='names',
                            type=str, default=[], help='op name', nargs='+')
        parser.add_argument('-p', '--print', dest='count',
                            default=20, type=int, help='print err data num')
        parser.add_argument('-s', '--save', dest='save',
                            action='store_true', help='save data in txt format')
        parser.add_argument('-al', '--atol', dest='atol',
                            default=0.001, type=float, help='set rtol')
        parser.add_argument('-rl', '--rtol', dest='rtol',
                            default=0.001, type=float, help='set atol')
        args = parser.parse_args(argv)
        if len(args.names) != 2:
            self.log.error("compare files should be 2.")
        else:
            self.compare.compare_data(
                args.names[0], args.names[1], args.save, args.rtol, args.atol, args.count)
    ...
    def do_convert_npu_dump(self, argv):
        ...
        parser.add_argument('-f', '--format', dest='format',
                            default=None, required=False, help='target format')
        parser.add_argument('-o', '--output', dest='output',
                            required=False, default=None, help='output path')
        args = parser.parse_args(argv)
    ...
    def single_cmd(self, argv):
        ...
        if argv[1] in cmd_func_map:
            ...
            return func(argv[2:])
        raise PrecisionToolException(
            "cmd %s is not supported or cmd should be run in interactive mode." % argv[1])
