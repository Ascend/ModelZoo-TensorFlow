...
#基础参数，需要模型审视修改
#网络名称，同目录名称
Network="DCGAN_ID1471_for_TensorFlow"
#训练epoch
epochs=2
#训练batch_size
batch_size=32
...
--dataset materials \
--data_dir=${data_path}/data/cifar10/ \
--input_height=64 \
--crop --train_svm False > ${cur_path}/test/output/${ASCEND_DEVICE_ID}/test1_${ASCEND_DEVICE_ID}.log 2>&1
wait
python3  classifier_svm.py \
 --train_svm True > ${cur_path}/test/output/${ASCEND_DEVICE_ID}/test_${ASCEND_DEVICE_ID}.log 2>
...
grep 'd_loss:' $cur_path/test/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log|awk  '{print $14}' >> $cur_path/test/output/$ASCEND_DEVICE_ID/train_${CaseName}_loss.txt
#最后一个迭代loss值，不需要修改
