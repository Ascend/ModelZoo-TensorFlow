...
os.system("bash ./test/train_full_1p.sh --data_path=%s --output_path=%s %s " % (config.data_url, config.train_url, config.modelarts_ckpt_dir))
# os.system("bash ./test/train_full_1p.sh --data_path=%s --output_path=%s %s " % (config.data_url, config.train_url, config.modelarts_ckpt_dir))
print("[CANN-Modelzoo] finish run train shell")

print("[CANN-Modelzoo] after train - list my code_dir files:")
