...
output_path="/home/test_user03/xx2"
...
then
  ...
  --num_train_images=20000 \
  --num_eval_images=1000 \
  --iterations_per_loop=1251 1>${print_log} 2>&1
fi

# 性能相关数据计算
StepTime=`grep "step/sec:" ${print_log} | tail -n 2 | awk '{print $NF}' | awk '{sum+=$1} END {print NR/sum}'`
