...
#模型训练参数
Network="BicycleGAN_ID1287_for_TensorFlow"
data_path=''
result_path=${cur_path}/test/output/$ASCEND_DEVICE_ID/ckpt/
batch_size=1
epochs=2
...
if [[ $1 == --help || $1 == -h ]];then
    ...
    --result_path            output path, default is ${cur_path}/test/output/$ASCEND_DEVICE_ID/ckpt/
...
echo "------------------ Final result ------------------"
#输出性能
TrainingTime=`grep "time" ${cur_path}/test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|awk 'END {print $6*1000}'`
...
#echo "Accuracy = ${accuracy}" >> $cur_path/test/output/$ASCEND_DEVICE_ID/${CaseName}.log
