...
# 拷贝文件
cp -r ${data_path}/CS_GAN_ID2064/* ./

# 修改参数
sed -i "s#/home/ma-user/modelarts/outputs/train_url_0/#${output_path}#" ./main.py
sed -i "s#/home/ma-user/modelarts/user-job-dir/code/cifar/cifar-10-batches-py#${data_path}/cifar/cifar-10-batches-py#" ./utils.py
...
then
    ...
    python3.7 ./main.py --num_training_iterations=4000  --output_dir=${output_path} > ${print_log} 2>&1
fi
# 性能相关数据计算
StepTime=`grep "time" ${print_log} | awk '{print $8}' | tr -d "INFO:tensorflow:global_step/sec:" | tail -n 10 | awk '{sum+=$1} END {print sum/NR}' | sed s/[[:space:]]//g`
FPS=`awk 'BEGIN{printf "%.2f\n", '${batch_size}'/'${StepTime}'}'`

# 精度相关数据计算
# train_accuracy=`grep -Eo "binary_accuracy: 0.[0-9]*" ${print_log} | awk '{print $NF}' | awk 'NR==1{max=$1;next}{max=max>$1?max:$1}END{print max}'`

# 提取所有loss打印信息
grep "step" ${print_log} | awk '{print $4}' | tr -d "," > ./test/output/${ASCEND_DEVICE_ID}/my_output_loss.txt
