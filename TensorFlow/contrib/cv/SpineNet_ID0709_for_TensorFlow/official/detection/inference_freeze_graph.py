...
flags.DEFINE_string(
    'freezegraph_dir', '', 'The path to the freeze graph file.')
...
def main(unused_argv):
  ...
  with tf.Session(graph=tf.Graph()) as sess:
    print(' - Loading freeze graph...')
    f = open(FLAGS.freezegraph_dir, "rb")
    input_graph_def = tf.GraphDef()
    input_graph_def.ParseFromString(f.read())
    tf.import_graph_def(input_graph_def, name="")

    image_node = sess.graph.get_tensor_by_name('Placeholder:0')

    output_nodes = {
        'num_detections': sess.graph.get_tensor_by_name('NumDetections:0'),
        'detection_boxes': sess.graph.get_tensor_by_name('DetectionBoxes:0'),
        'detection_classes': sess.graph.get_tensor_by_name('DetectionClasses:0'),
        'detection_scores': sess.graph.get_tensor_by_name('DetectionScores:0'),
    }

    if not FLAGS.use_normalized_coordinates:
      output_nodes['image_info'] = 'ImageInfo:0'
...
if __name__ == '__main__':
  flags.mark_flag_as_required('freezegraph_dir')
