...
#modelarts
flags.DEFINE_string("result",
                    "/cache/result",
                    "The result directory where the model checkpoints will be written.")

flags.DEFINE_string("platform",
                    "modelarts",
                    "Run on linux/apulis/modelarts platform. Modelarts Platform has some extra data copy operations")

flags.DEFINE_boolean("profiling",
                     True,
...
def main(argv):
  ...
  if FLAGS.mode == 'train':
    ...
    executor.train(train_input_fn, params.train.total_steps, hooks=npu_hooks_append(hooks_list=params.train.total_steps))
    ...
    for cycle in range(num_cycles):
      ...
      executor.train(train_input_fn, hooks=npu_hooks_append(hooks_list=current_cycle_last_train_step))
      executor.evaluate(eval_input_fn, eval_times)
  else:
