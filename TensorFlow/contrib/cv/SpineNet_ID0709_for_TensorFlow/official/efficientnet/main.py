...
def main(unused_argv):
  ...
  est = tf.estimator.tpu.TPUEstimator(
      use_tpu=False,
      model_fn=model_fn,
      config=npu_run_config_init(run_config=config),
      train_batch_size=FLAGS.train_batch_size,
      eval_batch_size=FLAGS.eval_batch_size,
      export_to_tpu=False,
      params=params, eval_on_tpu=False)
    ...
    if FLAGS.mode == 'train':
      ...
      while current_step < FLAGS.train_steps:
        ...
        next_checkpoint = min(current_step + FLAGS.steps_per_eval,
                              FLAGS.train_steps)
        est.train(input_fn=imagenet_train.input_fn, max_steps=next_checkpoint, hooks=npu_hooks_append())
