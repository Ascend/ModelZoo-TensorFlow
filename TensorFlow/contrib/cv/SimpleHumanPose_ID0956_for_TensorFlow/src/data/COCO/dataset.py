...
current_path = os.path.dirname(os.path.realpath(__file__))  # 对应于COCO
project_root = os.path.dirname(os.path.dirname(current_path))  # 对应于TF-Simple
main_path = os.path.join(project_root, "main", "train.py")
...
class Dataset(object):
    ...
    # train_annot_path = osp.join(cur_dir, 'annotations', 'person_keypoints_train2017.json')
    # val_annot_path = osp.join(cur_dir, 'annotations', 'person_keypoints_val2017.json')
    # test_annot_path = osp.join(cur_dir, 'data', dataset_name, 'annotations', 'image_info_test-dev2017.json')

    # tf_flags = tf.app.flags.FLAGS
    data_path = "cache/dataset"
    img_path = osp.join(data_path, 'images')
    human_det_path = osp.join(data_path, 'dets', 'human_detection.json')
    train_annot_path = osp.join(data_path, 'annotations', 'person_keypoints_train2017.json')
    val_annot_path = osp.join(data_path, 'annotations', 'person_keypoints_val2017.json')
    test_annot_path = osp.join(data_path, 'annotations', 'image_info_test-dev2017.json')
    ...
    def evaluation(self, result, gt, result_dir, db_set):
        ...
        with open(result_path, 'w') as f:
            json.dump(result, f)  # 把result中的东西写到result.json里面
