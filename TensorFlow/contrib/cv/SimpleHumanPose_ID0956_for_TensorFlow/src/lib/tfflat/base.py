...
class Base(object):
    ...
    def next_feed(self):
        ...
        for inputs in self._input_list:  # (image, target_coord, valid)
            blobs = next(self._data_iter)
            blobs = [blobs[i][int(rank_id * (self.cfg.batch_size / rank_size)):int(
                (rank_id + 1) * self.cfg.batch_size / rank_size)] for i in range(3)]
...
class Trainer(Base):
    ...
    def _make_data(self):
        ...
        d = Dataset()
        train_data = d.load_train_data()  # [dict()]
        ...
        data_load_thread = BatchData(data_load_thread, self.cfg.batch_size)  # 按batch获取数据shuffle
    ...
    def train(self):
        ...
        print("---------------"+str(self.cfg.end_epoch))
        print("---------------"+str(self.itr_per_epoch))
        for epoch in range(self.cfg.end_epoch):
            for step in range(self.itr_per_epoch):
                self.tot_timer.tic()
                ...
                screen = [
                    'Epoch %d itr %d/%d:' % (self.cur_epoch, epoch * step, self.itr_per_epoch * self.cfg.end_epoch),
                    'lr: %g' % self.lr_eval,
                    'speed: %.2f(%.2fs r%.2f)s/epoch * step' % (
                ...
                # TODO(display stall?)
                if (epoch * step) % self.cfg.display == 0:
                    self.logger.info(' '.join(screen))

                if (epoch * step) % self.itr_per_epoch == 0:
