...
mkdir -p ./logs/miniimagenet1shot/

# 修改路径
sed -i "s#./data/miniImagenet/train#${data_path}/data/miniImagenet/train#" ./data_generator.py
sed -i "s#./data/miniImagenet/test#${data_path}/data/miniImagenet/test#" ./data_generator.py
sed -i "s#./data/miniImagenet/val#${data_path}/data/miniImagenet/val#" ./data_generator.py
...
then
    ...
    python3.7 ./main.py --logdir=logs/miniimagenet1shot/ 1>${print_log} 2>&1
fi

# 性能相关数据计算
StepTime=`grep "step_time" ${print_log} | awk '{print $6}' | tail -n 10 | awk '{sum+=$1} END {print"",sum/NR}' | sed s/[[:space:]]//g`
FPS=`awk 'BEGIN{printf "%.2f\n", '${batch_size}'/'${StepTime}'}'`

# 精度相关数据计算
train_accuracy=`grep "Totally results:" ${print_log} | awk '{print $4}' | awk 'NR==1{max=$1;next}{max=max>$1?max:$1}END{print max}'`
# 提取所有loss打印信息
grep "Iteration" ${print_log} | awk '{print $3}' | tr -d "," > ./test/output/${ASCEND_DEVICE_ID}/my_output_loss.txt
