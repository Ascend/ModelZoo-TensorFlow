...
class DataGenerator(object):
    ...
    def __init__(self, num_samples_per_class, batch_size, config={}):
        ...
        if FLAGS.datasource == 'sinusoid':
            ...
            data_folder = config.get('data_folder', './data/miniImagenet')
            # for files_yzh in os.listdir(data_folder):
            # print(files_yzh)
            # for family in os.listdir(data_folder):
            #     print(os.path.join(data_folder, family))
            #     if os.path.isdir(os.path.join(data_folder, family)):
            #         print("nb")
            character_folders = [os.path.join(data_folder, family, character) for family in os.listdir(data_folder) if
                                 os.path.isdir(os.path.join(data_folder, family)) for character in
                                 os.listdir(os.path.join(data_folder, family))]
            random.seed(1)
            ...
            metatrain_folders = [os.path.join(metatrain_folder, label) for label in os.listdir(metatrain_folder) if
                                 os.path.isdir(os.path.join(metatrain_folder, label))]
    ...
    def make_data_tensor(self, train=True):
        ...
        dataset = dataset.map(image_process).batch(batch_image_size, drop_remainder=True).repeat(-1).prefetch(1)
        ...
        return all_image_batches, all_label_batches, iterator
