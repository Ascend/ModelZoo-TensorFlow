...
class SBN(object):  # REINFORCE
  ...
  def partial_fit(self, X, n_samples=1):
    ...
    _, res1, grad_variance1, step1, temperature1 = self.sess.run(
    ...
    return res1, grad_variance1, step1
  ...
  def partial_eval(self, X, n_samples=5):
    if n_samples < 1000:
      ...
      for i in range(int(n_samples/100)):
        print('-'*100, '开始partial_eval_2')
        logF, = self.sess.run(
            (self.logF,),
            feed_dict={self.x: X, self.n_samples: 100})
        print('-'*100, '结束partial_eval_2')
  ...
  def get_muprop_gradient(self):
    ...
    for i in range(self.hparams.n_layer):
      ...
      learning_signals.append(learning_signal - scaling_baseline_0 * muELBO - scaling_baseline_1 * dfDiff
                            - self._create_baseline())
  ...
  def get_dynamic_rebar_gradient(self):
    ...
    return total_grads, variance_objective, variance_objective_grad
...
class SBNTrackGradVariances(SBN):
  ...
  def _create_loss(self):
    ...
    return ELBO, gradient_to_follow, variance_objective_grad
