...
def resnet_v2(inputs,
  ...
  with tf.variable_scope(scope, 'resnet_v2', [inputs], reuse=reuse) as sc:
    ...
    #     end_points['predictions'] = slim.softmax(net, scope='predictions')
      ...
      # net = L.linear(out_dim=512, nl=L.leaky_relu, name='linear1')
      W_fc1 = weight_variable([net.shape[1], 512])
      b_fc1 = bias_variable([512])
      # 将中间层与输出层10个结果全连接
      # y_conv = tf.nn.softmax()
      ynet = tf.matmul(net, W_fc1) + b_fc1 # [B,512]
      ynet = leaky_relu(ynet)
      W_fc2 = weight_variable([512,embedding_size])
      b_fc2 = bias_variable([embedding_size])
      
      res = tf.matmul(ynet, W_fc2) + b_fc2 # [B,num_classes]
      # net = slim.batch_norm(res, activation_fn=tf.nn.relu)
