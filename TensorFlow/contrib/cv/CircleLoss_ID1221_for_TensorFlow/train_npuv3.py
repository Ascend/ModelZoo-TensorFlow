...
def parse_args():
    ...
    parser.add_argument('-epoch','--epoch_size',type=int,default=120)
    parser.add_argument('-w','--warmup',type=int,default=5)
    parser.add_argument('--init_lr',type=float,default=0.08)
    parser.add_argument('--lamda',type=float,default=1)
    parser.add_argument('--data_path', type=str, default="./dataset/")
    parser.add_argument('--result_dir', type=str, default="./result/")
    parser.add_argument('--ckptpname', type=str, default="resnet_v1_50.ckpt")
    parser.add_argument('-e','--expriment_name',type=str,default=f'test_{timestamp}')
...
ckptpname = args.ckptpname
result_dir = args.result_dir
expriment_name = args.expriment_name
...
if __name__ == '__main__':
    ...
    datasetpath = os.path.join(data_dir, 'bounding_box_train')
    print(datasetpath)
    ckptpath = os.path.join(data_dir,ckptpname)
    expriment_dir = os.path.join(result_dir,expriment_name)
    log_dir = os.path.join(expriment_dir, 'logs')
    ckpt_output_dir = os.path.join(expriment_dir, 'ckpt')
    model_save_path = ckpt_output_dir + expriment_name
    ...
    # valdataset = tf_val_dataset(batch_size=batch_size,epoch_size=None)
    # testiterator = valdataset.make_one_shot_iterator()
    # testnext = testiterator.get_next()
    
    # 定义计算图
    # y_logit = None
    # with slim.arg_scope(resnet_arg_scope(is_training=False)):
    #     y_logit = resnet_v2_50(x, num_classes=embedding_size,global_pool=True)
    # y_norm = tf.math.l2_normalize(y_logit,axis=1,name='features',epsilon=1e-12)
    # loss_op,sn,sp = get_loss_and_metric(y_norm,y_)
    ...
    with tf.variable_scope('Logits'):
        ...
        with slim.arg_scope([slim.conv2d, slim.fully_connected],
            ...
            logits = slim.fully_connected(drop1, num_outputs=num_classes,activation_fn=None,weights_initializer=tf.random_normal_initializer(mean=0,stddev=0.001))
    ...
    def adjust_LR(step,epoch):
        ...
        if step < 32000//args.batch_size: # freeze bonee
            lr_b = 0
        return lr_l,lr_b
    ...
    with tf.Session(config=npu_config_proto(config_proto=config)) as sess:
        ...
        while epoch < epoch_size:
            ...
            print(
                "Epoch:{} Step:{} TrainLoss:{:.4f} SoftmaxLoss:{:.4f} CircleLoss:{:.4f} SoftmaxAcc:{:.4f} PairwiseF1:{:.4f} TimePerStep:{:.4f}s TotalTiming:{}m{}s".format(
                    epoch, step_counter, loss_, softmaxlossterm, circlelossterm, acc_, f1, timingperstep, (timing) // 60,
                    timing % 60))
