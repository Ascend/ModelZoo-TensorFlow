...
ckptpath = os.path.join(log_dir,'ckpt')
makedirifnotexist(ckptpath)
num_bottleneck = 512
...
# 读入数据
imnum, dataset,le = tf_dataset(erasing_prob=args.erasing_prob,batch_size=batch_size,epoch_size=None,path='/home/nanshen/xutan/yixin/market1501/Market-1501-v15.09.15/bounding_box_train')
...
least_graph_path = None
least_param_path = None
...
with tf.Session(config=config) as sess:
    ...
    while epoch < epoch_size:
        ...
        if (step_counter+1)%(imnum//batch_size) == 0:
            ckptpathnew = os.path.join(ckptpath,str(step_counter+1))
            makedirifnotexist(ckptpathnew)
            saver.save(sess, os.path.join(ckptpathnew,args.modelname)+'.ckpt', global_step=step_counter+1)
            least_graph_path = os.path.join(ckptpathnew,args.modelname)+f'.ckpt-{str(step_counter+1)}.meta'
            least_param_path = ckptpathnew
...
train_writer.close()
print(least_graph_path)
print(least_param_path)
syscall = f'python /home/nanshen/xutan/yixin/circlesoftmax/code/test.py --graphpath {least_graph_path} --parampath {least_param_path} --logdir {log_dir}'
print('测试阶段')
print(syscall)
os.system(syscall)
