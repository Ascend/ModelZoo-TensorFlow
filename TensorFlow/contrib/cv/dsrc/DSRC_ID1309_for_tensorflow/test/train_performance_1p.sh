...
#基础参数，需要模型审视修改
#网络名称，同目录名称
Network="DSRC_ID1309_for_TensorFlow"
#训练batch_size
batch_size=2000
...
python3 dsrc_usps_npu.py \
    --data_path=${data_path}/data/ \
    --max_step=2000 > ${cur_path}/test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log 2>&1
wait
...
FPS=`grep 'fps:' $cur_path/test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|awk 'END {print $5}'`
#打印，不需要修改
...
grep 'train_total_loss:' $cur_path/test/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log|awk  '{print $2}' >> $cur_path/test/output/$ASCEND_DEVICE_ID/train_${CaseName}_loss.txt
#最后一个迭代loss值，不需要修改
