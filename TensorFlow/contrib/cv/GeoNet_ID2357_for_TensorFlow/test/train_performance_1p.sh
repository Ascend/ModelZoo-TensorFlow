...
if [ x"${modelarts_flag}" != x ];
then
    python3.7 geonet_main.py --mode=train_rigid --learning_rate=0.00001 --seq_length=3 --batch_size=4 --dataset_dir=${data_path}/stageone/train/ --checkpoint_dir=${output_path}/ckpt/ --max_steps=${train_steps}
else
    python3.7 geonet_main.py --mode=train_rigid --learning_rate=0.00001 --seq_length=3 --batch_size=4 --dataset_dir=${data_path}/stageone/train/ --checkpoint_dir=${output_path}/ckpt/ --max_steps=${train_steps} > ${print_log} 2>&1
fi
end_time=$(date +%s)
# 性能相关数据计算
StepTime=`grep "s/iter" ${print_log} | tail -n 30 | awk '{print $6}' | awk '{gsub(/s\/iter/, ""); print}'   | awk '{sum+=$1} END {print sum/NR}'`
FPS=`awk 'BEGIN{printf "%.2f\n", '${batch_size}'/'${StepTime}'}'`

# 精度相关数据计算
train_accuracy=`grep "ATE mean" ${print_log}  | awk '{print $3,$5}'`
# 提取所有loss打印信息
