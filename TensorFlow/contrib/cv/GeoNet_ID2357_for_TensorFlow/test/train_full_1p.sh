...
eval_log="./test/output/${ASCEND_DEVICE_ID}/eval_${ASCEND_DEVICE_ID}.log"
...
if [ x"${modelarts_flag}" != x ];
then
    python3.7 geonet_main.py --mode=train_rigid --learning_rate=0.00001 --seq_length=3 --batch_size=4 --dataset_dir=${data_path}/stageone/train/ --checkpoint_dir=${output_path}/ckpt/pose_depth/ --max_steps=${train_steps}
    end_time=$(date +%s)
    python3.7 geonet_main.py --mode=test_pose --dataset_dir=${data_path}/stageone/test/ --init_ckpt_file=${output_path}/ckpt/pose_depth/model-185000 --batch_size=1 --seq_length=3 --pose_test_seq=9 --output_dir=${output_path}/pose_depth/185000/ 
    python3.7 kitti_eval/eval_pose.py --gtruth_dir=${data_path}/stageone/eval/09_snippets/09/ --pred_dir=${output_path}/pose_depth/185000/
else
    python3.7 geonet_main.py --mode=train_rigid --learning_rate=0.00001 --seq_length=3 --batch_size=4 --dataset_dir=${data_path}/stageone/train/ --checkpoint_dir=${output_path}/ckpt/pose_depth/ --max_steps=${train_steps} > ${print_log} 2>&1
    end_time=$(date +%s)
    python3.7 geonet_main.py --mode=test_pose --dataset_dir=${data_path}/stageone/test/ --init_ckpt_file=${output_path}/ckpt/pose_depth/model-185000 --batch_size=1 --seq_length=3 --pose_test_seq=9 --output_dir=${output_path}/pose_depth/185000/ 
    python3.7 kitti_eval/eval_pose.py --gtruth_dir=${data_path}/stageone/eval/09_snippets/09/ --pred_dir=${output_path}/pose_depth/185000/ > ${eval_log} 2>&1
fi

# 性能相关数据计算
StepTime=`grep "s/iter" ${print_log} | tail -n 50 | awk '{print $6}' | awk '{gsub(/s\/iter/, ""); print}'   | awk '{sum+=$1} END {print sum/NR}'`
FPS=`awk 'BEGIN{printf "%.2f\n", '${batch_size}'/'${StepTime}'}'`

# 精度相关数据计算
train_accuracy=`grep "ATE mean" ${eval_log}  | awk '{print $3,$5}'`
# 提取所有loss打印信息
