...
python3  ./train_softmax.py \
  ...
  --weight=${WEIGHT} \
  --alpha=${ALPHA} \
  --network=${NETWORK} ${FC_BN} \
  --image_height=${IMAGE_HEIGHT} \
  --image_width=${IMAGE_WIDTH} \
  --embedding_size=${EMBEDDING_SIZE} > ${cur_path}/test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log 2>&1
wait
...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
TrainingTime=`grep 'fps' $cur_path/test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|awk 'END {print $10}'`
...
grep 'Total_Loss' $cur_path/test/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log|awk  '{print $12}' >> $cur_path/test/output/$ASCEND_DEVICE_ID/train_${CaseName}_loss.txt
#最后一个迭代loss值，不需要修改
