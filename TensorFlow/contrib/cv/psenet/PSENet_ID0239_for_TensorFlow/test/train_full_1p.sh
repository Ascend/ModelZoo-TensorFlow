...
#基础参数，需要模型审视修改
#网络名称,同目录名称
Network="PSENet_ID0239_for_TensorFlow"
#训练batch_size
batch_size=14
#训练epochs或者steps
train_steps=100000
...
#输出训练精度,需要模型审视修改
train_accuracy=`grep -a "Final Train Accuracy" ${cur_path}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log | awk -F" " '{print $4}'`
#打印，不需要修改
...
grep "] Step " $cur_path/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log|awk -F',' '{print $3}'|awk '{print $3}' > $cur_path/output/$ASCEND_DEVICE_ID/train_${CaseName}_loss.txt
