...
then
    python3.7 ./main_point_cloud_boostPerf.py \
                               --method=svd \
    ...
    python3.7 ./main_point_cloud_boostPerf.py \
                               ...
                               --train_steps=2600000 \
                               --save_checkpoints_steps=100000 \
                               --eval_examples=39900 > ${print_log} 2>&1
fi


# 性能相关数据计算
Step_per_sec=`grep "global_step/sec" ${print_log} | grep -v "INFO" | awk '{print $6}' | awk '{sum+=$1} END {print sum/NR}'`
Traintime=`awk 'BEGIN{printf "%.2f\n", 1/'${Step_per_sec}'}'`
FPS=`awk 'BEGIN{printf "%.2f\n", '${batch_size}'/'${Traintime}'}'`
# 精度相关数据计算
train_accuracy=`grep "mean_degree_err" ${print_log} | grep -v "INFO" | awk 'END {print $19}'`
# 提取所有loss打印信息
grep "loss" ${print_log} | grep -v "INFO" |  awk '{print $7}' | head -n -1  > ./test/output/${ASCEND_DEVICE_ID}/my_output_loss.txt
