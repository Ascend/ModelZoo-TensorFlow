...
flags.DEFINE_integer('eval_examples', 39900, 'Number of test examples.')
...
def model_fn(features, labels, mode, params):
  ...
  if mode == tf.estimator.ModeKeys.TRAIN:
    ...
    batch_pts1 = tf.tile(data, [num_rot, 1, 1])
  ...
  unit_one = tf.constant(1.0,dtype=tf.float32)

  rotation_matrix = tf.multiply(r,unit_one,name='rotation_matrix')
  # Compute the loss.
  loss = tf.nn.l2_loss(rot - r)

  # Compute the relative angle in radians.
  theta = utils.relative_angle(rot, r)
  ...
  if mode == tf.estimator.ModeKeys.TRAIN:
    ...
    with tf.control_dependencies(update_ops):
      ...
      #train_op = util.set_graph_exec_config(train_op, dynamic_input=True,dynamic_graph_execute_mode="lazy_recompile")
