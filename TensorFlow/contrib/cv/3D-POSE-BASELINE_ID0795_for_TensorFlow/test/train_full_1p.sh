...
nohup python3 predict_3dpose.py \
    --data_dir ${data_path} \
...
#输出性能FPS，需要模型审视修改
TrainingTime=`grep 'Working on epoch' $cur_path/test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|awk 'END {print $14}'| tail -2 | head -n 1`
FPS=$(awk 'BEGIN{printf "%d\n",'${batch_size}'/'${TrainingTime}'*1000}')
#打印，不需要修改
echo "Final Performance TrainingTime : $TrainingTime"
echo "Final Performance images/sec : $FPS"

train_accuracy=$(grep Average $cur_path/test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log | awk 'END {print $2}')
#打印，不需要修改
...
grep 'Working on epoch' $cur_path/test/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log|awk  '{print $10}' >> $cur_path/test/output/$ASCEND_DEVICE_ID/train_${CaseName}_loss.txt
#最后一个迭代loss值，不需要修改
