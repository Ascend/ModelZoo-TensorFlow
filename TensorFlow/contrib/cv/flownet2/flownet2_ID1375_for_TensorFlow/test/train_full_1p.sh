...
#基础参数，需要模型审视修改
#网络名称，同目录名称
Network="flownet2_ID1375_for_TensorFlow"
#训练epoch
train_epochs=1
#训练batch_size
batch_size=4
...
do
    ...
    python3 flownet2_train.py \
	    --dataset=${data_path} \
        --train_step=10000 \
		--pretrained=${ckpt_path}/flownet-2.ckpt-0 \
		--resume_path=${cur_path}/test/output/$ASCEND_DEVICE_ID/ckpt > ${cur_path}/test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log 2>&1
done
wait
...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
TrainingTime=`grep 'ms' $cur_path/test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|awk 'END {print}'|awk -F ":" '{print $5}'|awk '{print$1}'`
