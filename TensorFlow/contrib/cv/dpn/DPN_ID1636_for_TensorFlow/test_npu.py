...
with tf.Session(config=config) as sess:
    ...
    with slim.arg_scope(DPN_model.vgg_arg_scope()):
        out = DPN_model.dpn_model(inputx, inputd=spectral_distance, is_training=is_training, _HEIGHT=_HEIGHT, _WIDTH=_WIDTH)
    ...
    try:
        ...
        for step in range(int(img_N / batch_size)):
            ...
            print("------------------step----------------------",step)
            tra_out = sess.run(out, feed_dict={inputx: x_in})
