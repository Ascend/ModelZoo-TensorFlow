...
#基础参数，需要模型审视修改
#网络名称，同目录名称
Network="DPN_ID1636_for_TensorFlow"
#训练epoch
train_epochs=57                                                                                                     #changed epoch (before57)
#训练batch_size
batch_size=8
#训练step
...
if [[ $data_path == "" ]];then
    ...
    exit 1
elif [[ $more_path1 == "" ]];then
    echo "[Error] para \"more_path1\" must be confing"
    exit 1
fi
...
do
    ...
    nohup python3 train_npu.py \
        ...
        --model_save=${cur_path}/test/output/$ASCEND_DEVICE_ID/ckpt >> ${cur_path}test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log 2>&1 &
done
wait
...
do
    ...
    nohup python3 test_npu.py \
        --data_path=${more_path1}/val.tfrecords \
        --model_path=${cur_path}/test/output/$ASCEND_DEVICE_ID/ckpt/ckpt_model.ckpt \
        --output_path=${cur_path}/model_save  >>  ${cur_path}test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log 2>&1 &
done
wait
