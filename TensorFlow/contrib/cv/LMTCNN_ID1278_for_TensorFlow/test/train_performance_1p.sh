...
do
	...
	--batch_size=128 \
	--model_dir=./models |tee  ${cur_path}test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log   2>&1
...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
grep "sec/batch"  $cur_path/test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log  |awk '{print $17}'|tail -n +2 > $cur_path/test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}_traintime.txt
