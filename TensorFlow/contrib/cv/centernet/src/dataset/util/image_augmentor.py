...
def image_augmentor(image,
    ...
    if keep_aspect_ratios:
        if fill_mode in ['NEAREST_NEIGHBOR', 'BILINEAR', 'BICUBIC']:
            zoom_ratio = tf.cond(tf.less(zoom_or_output_h / input_h, zoom_or_output_w / input_w),
            ...
            resize_h, resize_w = tf.cond(
                tf.less(zoom_or_output_h / input_h, zoom_or_output_w / input_w), lambda:
                (zoom_or_output_h, tf.cast(tf.cast(input_w, tf.float32) * zoom_ratio, tf.int32)), lambda:
                (tf.cast(tf.cast(input_h, tf.float32) * zoom_ratio, tf.int32), zoom_or_output_w))
            image = tf.image.resize_images(
                image,
                [resize_h, resize_w],
                fill_mode_project[fill_mode],
                align_corners=True,
            )
            if ground_truth is not None:
                ymin, ymax = ymin * zoom_ratio, ymax * zoom_ratio
                xmin, xmax = xmin * zoom_ratio, xmax * zoom_ratio
            image = tf.pad(image, [[0, zoom_or_output_h - resize_h], [0, zoom_or_output_w - resize_w], [0, 0]],
                           mode='CONSTANT',
                           constant_values=constant_values)
        else:
            image = tf.pad(image, [[0, zoom_or_output_h - input_h], [0, zoom_or_output_w - input_w], [0, 0]],
                           mode='CONSTANT',
                           constant_values=constant_values)
    else:
        image = tf.image.resize_images(image, [zoom_or_output_h, zoom_or_output_w],
                                       fill_mode_project[fill_mode],
                                       align_corners=True,
    ...
    if zoom_size is not None:
        ...
        image = tf.slice(image, [crop_h, crop_w, 0], [output_h, output_w, input_c])
        if ground_truth is not None:
    ...
    if flip_prob is not None:
        ...
        if ground_truth is not None:
            ymax, ymin = tf.cond(tf.less(flip_td_prob, flip_prob[0]), lambda:
                                 (output_h - ymin - 1., output_h - ymax - 1.), lambda: (ymax, ymin))
            xmax, xmin = tf.cond(tf.less(flip_lr_prob, flip_prob[1]), lambda:
                                 (output_w - xmin - 1., output_w - xmax - 1.), lambda: (xmax, xmin))
    if color_jitter_prob is not None:
        bcs = tf.random_uniform([3], 0., 1.)
        image = tf.cond(bcs[0] < color_jitter_prob,
                        lambda: tf.image.adjust_brightness(image, tf.random_uniform([], 0., 0.3)), lambda: image)
        image = tf.cond(bcs[1] < color_jitter_prob,
                        lambda: tf.image.adjust_contrast(image, tf.random_uniform([], 0.8, 1.2)), lambda: image)
        image = tf.cond(bcs[2] < color_jitter_prob,
                        lambda: tf.image.adjust_hue(image, tf.random_uniform([], -0.1, 0.1)), lambda: image)
    ...
    if rotate is not None:
        ...
        if ground_truth is not None:
            ...
            xmin = tf.reduce_min(tf.concat([xminymin_x, xmaxymax_x, xminymax_x, xmaxymin_x], axis=-1),
                                 axis=-1,
                                 keepdims=True)
            ymin = tf.reduce_min(tf.concat([xminymin_y, xmaxymax_y, xminymax_y, xmaxymin_y], axis=-1),
                                 axis=-1,
                                 keepdims=True)
            xmax = tf.reduce_max(tf.concat([xminymin_x, xmaxymax_x, xminymax_x, xmaxymin_x], axis=-1),
                                 axis=-1,
                                 keepdims=True)
            ymax = tf.reduce_max(tf.concat([xminymin_y, xmaxymax_y, xminymax_y, xmaxymin_y], axis=-1),
                                 axis=-1,
                                 keepdims=True)
    if data_format == 'channels_first':
    ...
    if ground_truth is not None:
        ...
        if tf.shape(ground_truth_)[0] == 0:
            if pad_truth_to is not None:
                ...
                ground_truth = tf.pad(ground_truth_, [[0, pad_truth_to - tf.shape(ground_truth)[0]], [0, 0]],
                                      constant_values=-1.0)
            return image_copy, ground_truth
        else:
            if pad_truth_to is not None:
                ground_truth = tf.pad(ground_truth_, [[0, pad_truth_to - tf.shape(ground_truth_)[0]], [0, 0]],
                                      constant_values=-1.0)
...
def basic(image, input_shape, data_format, output_shape, ground_truth=None, pad_truth_to=100, constant_values=0.0):
    if data_format == 'channels_first':
        image = tf.transpose(image, [1, 2, 0])
    input_h, input_w, input_c = input_shape[0], input_shape[1], input_shape[2]
    output_h, output_w = output_shape

    zoom_ratio = tf.cond(tf.less(output_h / input_h,
                                 output_w / input_w), lambda: tf.cast(output_h / input_h, tf.float32),
                         lambda: tf.cast(output_w / input_w, tf.float32))
    resize_h, resize_w = tf.cond(tf.less(output_h / input_h, output_w / input_w), lambda:
                                 (output_h, tf.cast(tf.cast(input_w, tf.float32) * zoom_ratio, tf.int32)), lambda:
                                 (tf.cast(tf.cast(input_h, tf.float32) * zoom_ratio, tf.int32), output_w))
    image = tf.image.resize_images(
        image,
        [resize_h, resize_w],
    ...
    image = tf.pad(image, [[0, output_h - resize_h], [0, output_w - resize_w], [0, 0]],
                   mode='CONSTANT',
                   constant_values=constant_values)
    ...
    if pad_truth_to is not None:
        ground_truth = tf.pad(ground_truth, [[0, pad_truth_to - tf.shape(ground_truth)[0]], [0, 0]],
                              constant_values=-1.0)
    return image, ground_truth


def revert_bbox(bbox, input_shape, output_shape):
    ...
    y = tf.reshape(bbox[:, 0], [-1, 1])
    x = tf.reshape(bbox[:, 1], [-1, 1])
    h = tf.reshape(bbox[:, 2], [-1, 1])
    w = tf.reshape(bbox[:, 3], [-1, 1])
    class_id = tf.reshape(bbox[:, 4], [-1, 1])
    ...
    bbox = tf.concat([ymin, ymax, xmin, xmax, class_id], axis=-1)
    return bbox
