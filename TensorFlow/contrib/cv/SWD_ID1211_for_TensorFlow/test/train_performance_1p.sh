...
#基础参数，需要模型审视修改
#网络名称，同目录名称
Network="SWD_ID1211_for_TensorFlow"
#训练epoch
train_epochs=10
#训练batch_size
batch_size=300
...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
grep "step_time"  $cur_path/test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log  |awk '{print $8}'|tail -n +2 > $cur_path/test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}_traintime.txt
...
#最后一个迭代loss值，不需要修改
grep 'loss_s:' $cur_path/test/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log|awk  '{print $2}'  > $cur_path/test/output/$ASCEND_DEVICE_ID/train_${CaseName}_loss.txt
