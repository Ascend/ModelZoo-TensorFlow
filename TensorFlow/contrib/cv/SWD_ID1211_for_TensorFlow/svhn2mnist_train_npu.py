...
#     saver = tf.train.Saver()
#     save_dir = "model-adapt_mcd/graph.ckpt"
#     saver.restore(sess, save_dir)
    # Training
    for step in range(20):
        # Forward and backward propagation
#         batch_size = x_t.shape[0]
        total_batch = int(x_t.shape[0] / batch_size)
        for i in range(0, x_t.shape[0], batch_size):
            ...
            if batch_x_s.shape[0] < batch_size or batch_x_s.shape[0] < batch_size:
                break
            _, loss_s_tmp, loss_dis_tmp = sess.run([train, loss_s, loss_dis], feed_dict={
                                   X: batch_x_s, Y: batch_y_s, X_target: batch_x_t})
            
            for j in range(5):
                _, loss_s_tmp, loss_dis_tmp = sess.run([optim_dis3, loss_s, loss_dis], feed_dict={X: batch_x_s, Y: batch_y_s, X_target: batch_x_t})
#             print(f"Batch: {i / batch_size} / {total_batch}, loss_s is {loss_s_tmp}, loss_dis is {loss_dis_tmp}")
            
            ####### train one by one #################
#             for j in range(3):
#                 _, loss_s_tmp, loss_dis_tmp = sess.run([optim_s, loss_s, loss_dis], feed_dict={
#                                        X: batch_x_s, Y: batch_y_s, X_target: batch_x_t})
#                 print(f"在S上训练所有，Batch: {i / batch_size} / {total_batch}, loss_s is {loss_s_tmp}, loss_dis is {loss_dis_tmp}")

#             _, loss_s_tmp, loss_dis_tmp = sess.run([optim_dis1, loss_s, loss_dis], feed_dict={
#                                    X: batch_x_s, Y: batch_y_s, X_target: batch_x_t})
#             print(f"训练C1，Batch: {i / batch_size} / {total_batch}, loss_s is {loss_s_tmp}, loss_dis is {loss_dis_tmp}")
#             _, loss_s_tmp, loss_dis_tmp = sess.run([optim_dis2, loss_s, loss_dis], feed_dict={
#                                    X: batch_x_s, Y: batch_y_s, X_target: batch_x_t})
#             print(f"训练C2，Batch: {i / batch_size} / {total_batch}, loss_s is {loss_s_tmp}, loss_dis is {loss_dis_tmp}")
        
#             for j in range(3):
#                 _, loss_s_tmp, loss_dis_tmp = sess.run([optim_dis3, loss_s, loss_dis], feed_dict={
#                                        X: batch_x_s, Y: batch_y_s, X_target: batch_x_t})
#                 print(f"训练G，Batch: {i / batch_size} / {total_batch}, loss_s is {loss_s_tmp}, loss_dis is {loss_dis_tmp}")
#             print("--"*10)
            ####### train one by one #################
#             if i > 1000:
#                 break 
        ...
        if step % 1 == 0:
            print(f"Iteration: {step} / 10000, loss_s is {loss_s_tmp}, loss_dis is {loss_dis_tmp}" )
#             print(loss_s_tmp)
            predicted_y_test_t = []
            predicted_y_test_s = []
