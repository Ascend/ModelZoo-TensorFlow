...
class DeltaEncoder(object):
    ...
    def encoder(self, features, reference_features):
        # features = tf.nn.dropout(features, rate=self.drop_out_rate_input_pl)
        ...
        for i, size in enumerate(self.encoder_size):
            ...
            # input = tf.nn.dropout(self.lrelu(input_lin), rate=self.drop_out_rate_pl)
    ...
    def train(self, verbose=False):
        ...
        #     '{"output":"/home/ma-user/modelarts/user-job-dir/code/test/profiling2","training_trace":"on","task_trace":"on","aicpu":"on","fp_point":"resnet_model/conv2d/Conv2Dresnet_model/batch_normalization/FusedBatchNormV3_Reduce","bp_point":"gradients/AddN_70","aic_metrics":"PipeUtilization"}')
        ...
        with tf.Session(config=config) as self.session:
            ...
            for epoch in xrange(self.num_epoch):
                ...
                print("epoch %d use time %f" % (epoch, end_time-start_time))
                print("-----")
            self.session.close()
