...
logger = logging.getLogger("DeltaEncoder Logger")
# handler = logging.StreamHandler()
formatter = logging.Formatter('%(asctime)s - %(name)s - %(levelname)s - %(message)s')

handler = logging.FileHandler('./test/output/loss+perf_gpu.txt')
...
class linear_classifier(object):
    def __init__(self, features_train, labels_train, features_test, labels_test,
                 learning_rate=0.0005, number_epoch=25, batch_size=128):
...
#         print(self.features_pl)
#         print(self.labels_pl)
...
#         self.features_test_temp = self.features_test
        ...
        for i in xrange(self.number_epoch):
            mean_loss_d = 0.0
            for count in xrange(0, self.features_train.shape[0] -self.batch_size, self.batch_size):
...
#         print(self.x_pl)
#         print(self.z_pl)
#         print(self.reference_features_pl)
    ...
    def encoder(self, features, reference_features):
        features = npu_ops.dropout(features, keep_prob=1 -  self.drop_out_rate_input_pl)
        ...
        for i, size in enumerate(self.encoder_size):
            ...
            input = npu_ops.dropout(self.lrelu(input_lin), keep_prob=1 -  self.drop_out_rate_pl)
...
#         print(name)
    ...
    def train(self, verbose=True):
        ...
        with tf.Session(config=config) as self.session:
            ...
            last_loss_epoch = None
            acc = self.val()
            logger.info('Unseen classes accuracy without training: {}'.format(acc))
#             print('Unseen classes accuracy without training: {}'.format(acc))
#             print("-----")
            for epoch in xrange(self.num_epoch):
                mean_loss_e = 0.0
                for count in xrange(0, self.features.shape[0] - self.batch_size, self.batch_size):
                    ...
                    if verbose:
                        if np.mod(c,10)==1:
                            logger.info('Batch#{0} Loss {1}'.format(c,mean_loss_e/(c+1e-7)))
#                             print('Batch#{0} Loss {1}'.format(c,mean_loss_e/(c+1e-7)))


                mean_loss_e /= (self.features.shape[0] / self.batch_size)
                if verbose:
                    logger.info('epoch : {}: Loss : {}'.format(epoch+1, mean_loss_e))
#                     print('epoch : {}: E : {}'.format(epoch, mean_loss_e))
                ...
                if epoch == self.num_epoch-1:
                    tf.train.Saver().save(self.session, "./ckpt_npu/model.ckpt")
                    tf.io.write_graph(self.session.graph, './ckpt_npu', 'graph.pbtxt', as_text=True)
...
#                     self.save_npy(self.session, self.last_file_name)
                    ...
                    logger.info('epoch {}: Higher unseen classes accuracy reached: {} '.format(epoch+1, acc))
#                     print('epoch {}: Higher unseen classes accuracy reached: {} (Saved in {}.npy)'.format(epoch+1, acc, self.last_file_name))
                else:
                    logger.info(print('epoch {}: Lower unseen classes accuracy reached: {} (<={})'.format(epoch+1, acc,self.best_acc)))
    ...
    def val(self, verbose = False):
        ...
        for episode_data in self.episodes:
            ...
            lin_model = linear_classifier(features, labels, self.features_test,
                                                     self.labels_test)
            with tf.Session(config=config) as linear_sess:
                acc_ = lin_model.learn(linear_sess)
#                 print(acc_)
#                 logger.info('acc:{}'.format(acc_))
                acc.append(acc_)
