...
def undersample(x, mask, centred=False, norm_method='ortho', noise=0):
    """
    ...
    noise_power = noise
    nz = np.sqrt(.5) * (np.random.normal(0, 1, x.shape) +
                        1j * np.random.normal(0, 1, x.shape))
    nz = nz * np.sqrt(noise_power)

    if norm_method == 'ortho':
...
def data_consistency(x, y, mask, centered=False):
    """
    x is in image space,
    y is in k-space
    """
