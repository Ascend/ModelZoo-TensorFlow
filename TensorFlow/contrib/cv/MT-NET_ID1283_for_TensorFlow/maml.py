...
class MAML:
    ...
    def construct_model(self, input_tensors=None, prefix='metatrain_'):
        ...
        with tf.variable_scope('model', reuse=None) as training_scope:
            ...
            # NPU does not support tf.map_fn
            # refer to link:https://gitee.com/ascend/modelzoo/blob/bd9a768f476f6662578b6ff800c4eaa8d6f26ec5/contrib/TensorFlow/Research/cv/META-SGD_ID1236_for_TensorFlow/maml.py
            if FLAGS.chip == 'npu':
                if self.classification:
        ...
        if 'train' in prefix:
            ...
            # loss scale
            # refer to link1:https://www.bilibili.com/video/BV1ZP4y187ET?spm_id_from=444.41.0.0 0:54:40
            # refer to link2:https://support.huaweicloud.com/tfmigr-cann504alpha2training/atlasmprtg_13_0038.html
            if FLAGS.chip == 'gpu':
                self.pretrain_op = tf.train.AdamOptimizer(self.meta_lr).minimize(total_loss1)
            if FLAGS.chip == 'npu':
                opt = tf.train.AdamOptimizer(self.meta_lr)
                loss_scale_manager = FixedLossScaleManager(1024)
                loss_scale_optimizer = NPULossScaleOptimizer(opt, loss_scale_manager, is_distributed=False)
                self.pretrain_op = loss_scale_optimizer.minimize(total_loss1)
