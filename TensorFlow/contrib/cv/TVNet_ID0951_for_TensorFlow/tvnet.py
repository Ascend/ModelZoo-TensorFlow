...
class TVNet(object):
    ...
    def variable_with_weight_loss(self, shape, stddev):
        var = tf.Variable(tf.truncated_normal(shape, stddev=stddev))  # tensorflow::ops::TruncatedNormal
        return var
    ...
    def centered_gradient(self, x, name):
        ...
        with tf.variable_scope('centered_gradient'):
            x_ker_init = tf.constant_initializer([-0.5, 0, 0.5])  # tf.constant_initializer([-0.5,0, 0.5])
            ...
            # x_ker_init = self.variable_with_weight_loss(shape=[1, 3, 1, 1], stddev=5e-2)
            # diff_x = tf.nn.conv2d(x, x_ker_init, [1, 2, 2, 1], padding='SAME', name=name + '_diff_y')

            test1 = tf.slice(x, [0, 0, 1, 0], [batch_size, x.shape[1].value, 1, x.shape[3].value])
            test2 = tf.slice(x, [0, 0, 0, 0], [batch_size, x.shape[1].value, 1, x.shape[3].value])
            first_col = 0.5 * (tf.slice(x, [0, 0, 1, 0], [batch_size, x.shape[1].value, 1, x.shape[3].value]) -
                               tf.slice(x, [0, 0, 0, 0], [batch_size, x.shape[1].value, 1, x.shape[3].value]))

            last_col = 0.5 * (
                    tf.slice(x, [0, 0, x.shape[2].value - 1, 0], [batch_size, x.shape[1].value, 1, x.shape[3].value]) -
                    tf.slice(x, [0, 0, x.shape[2].value - 2, 0], [batch_size, x.shape[1].value, 1, x.shape[3].value]))
            diff_x_valid = tf.slice(diff_x, begin=[0, 0, 1, 0],
                                    size=[batch_size, x.shape[1].value, x.shape[2].value - 2, x.shape[3].value])
            diff_x = tf.concat(axis=2, values=[first_col, diff_x_valid, last_col])

            first_row = 0.5 * (tf.slice(x, [0, 1, 0, 0], [batch_size, 1, x.shape[2].value, x.shape[3].value]) -
                               tf.slice(x, [0, 0, 0, 0], [batch_size, 1, x.shape[2].value, x.shape[3].value]))
            last_row = 0.5 * (
                    tf.slice(x, [0, x.shape[1].value - 1, 0, 0], [batch_size, 1, x.shape[2].value, x.shape[3].value]) -
                    tf.slice(x, [0, x.shape[1].value - 2, 0, 0], [batch_size, 1, x.shape[2].value, x.shape[3].value]))
            diff_y_valid = tf.slice(diff_y, begin=[0, 1, 0, 0],
                                    size=[batch_size, x.shape[1].value - 2, x.shape[2].value, x.shape[3].value])
    ...
    def forward_gradient(self, x, name):
        ...
        with tf.variable_scope('forward_gradient'):
            ...
            diff_x = tf.layers.conv2d(x, x.shape[-1].value, [1, 2], padding='same',
                                      kernel_initializer=x_ker_init, use_bias=True, name=name + '_diff_x',
                                      trainable=True)

            y_ker_init = tf.constant_initializer([[-1], [1]])
            diff_y = tf.layers.conv2d(x, x.shape[-1].value, [2, 1], padding='same',
                                      kernel_initializer=y_ker_init, use_bias=True, name=name + '_diff_y',
                                      trainable=True)
            # refine the boundary
            diff_x_valid = tf.slice(diff_x, begin=[0, 0, 0, 0],
                                    size=[batch_size, x.shape[1].value, x.shape[2].value - 1, x.shape[3].value])
            ...
            diff_y_valid = tf.slice(diff_y, begin=[0, 0, 0, 0],
                                    size=[batch_size, x.shape[1].value - 1, x.shape[2].value, x.shape[3].value])
    ...
    def divergence(self, x, y, name):
        ...
        with tf.variable_scope('divergence'):
            ...
            y_valid = tf.slice(y, begin=[0, 0, 0, 0],
                               size=[batch_size, y.shape[1].value - 1, y.shape[2].value, y.shape[3].value])
    ...
    def dual_tvl1_optic_flow(self, x1, x2, u1, u2,
        ...
        l_t = lbda * theta
        taut = tau / theta
        diff2_x, diff2_y = self.centered_gradient(x2, 'x2')  # conv and slice
        p11 = p12 = p21 = p22 = tf.zeros_like(x1)
        for warpings in range(warps):  # 1
            with tf.variable_scope('warping%d' % (warpings,)):
                ...
                diff2_x_sq = tf.square(diff2_x_warp)  # square mat
                ...
                for ii in range(max_iterations):  # 50
                    with tf.variable_scope('iter%d' % (ii,)):
                        ...
                        u1 = v1 + theta * self.divergence(p11, p12, 'div_p1')  # slice->concat->conv->add
    ...
    def tvnet_flow(self, x1, x2,
        ...
        n_scales = 1 + np.log(np.sqrt(height ** 2 + width ** 2) / 4.0) / np.log(1 / zfactor);  # calculate n_scales
        n_scales = min(n_scales, max_scales)
        # n_scales = 1
        with tf.variable_scope('tvl1_flow'):

            grey_x1 = self.grey_scale_image(x1)  # conv and floor
            grey_x2 = self.grey_scale_image(x2)

            norm_imgs = self.normalize_images(grey_x1, grey_x2)  # normalize to 0-255

            smooth_x1 = self.gaussian_smooth(norm_imgs[0])  # conv
                ...
                with tf.variable_scope('scale%d' % ss):
                    ...
                    if ss == n_scales - 1:
                        ...
                        u2 = tf.get_variable('u2', shape=[1, down_height, down_width, 1], dtype=tf.float32,
                                             initializer=tf.zeros_initializer)
                        # print ([tf.shape(smooth_x1)[0], 1, 1, 1])
