...
if [ x"${etp_flag}" != xtrue ];
then
    python3.7 ./train_epe_sintel.py --data_path=${data_path} --output_path=${output_path} --steps=${train_steps}
else
    python3.7 ./train_epe_sintel.py --data_path=${data_path} --output_path=${output_path} --steps=${train_steps} > ${print_log} 2>&1
fi
...
# 精度相关数据计算
train_accuracy=`grep "loss:" ${print_log} | awk '{print $NF}' |grep -v skipping | tail -n 1`
# 提取所有loss打印信息
grep "loss:" ${print_log} | awk '{print $NF}' |grep -v skipping > ./test/output/${ASCEND_DEVICE_ID}/my_output_loss.txt
