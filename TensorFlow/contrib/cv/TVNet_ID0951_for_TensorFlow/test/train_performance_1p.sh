...
if [ x"${etp_flag}" != xtrue ];
then
    python3.7 ./train_epe_sintel.py --data_path=${data_path}/dataset --output_path=${output_path} --steps=${train_steps}
else
    python3.7 ./train_epe_sintel.py --data_path=${data_path}/dataset --output_path=${output_path} --steps=${train_steps} > ${print_log} 2>&1
fi

# 性能相关数据计算
StepTime=`cat ${print_log} | grep "sec/batch" | tail -n +2 | awk '{print $8}' | awk '{sum+=$1} END {print sum/NR}'`
FPS=`awk 'BEGIN{printf "%.2f\n", '${batch_size}'/'${StepTime}'}'`

# 精度相关数据计算
train_accuracy=''
# 提取所有loss打印信息
grep " loss =" ${print_log} | awk '{print $5}' > ./test/output/${ASCEND_DEVICE_ID}/my_output_loss.txt
