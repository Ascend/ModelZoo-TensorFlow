...
python3.7 ./generative.py \
   ...
   --mutation_rate=0.0 \
   --reg_weight=0.0 \
   --in_tr_data_dir=${data_path}/before_2011_in_tr \
   --in_val_data_dir=${data_path}/between_2011-2016_in_val \
   --ood_val_data_dir=${data_path}/between_2011-2016_ood_val \
   --out_dir=${output_path} > ${print_log} 2>&1
...
# 精度相关数据计算
train_accuracy=`grep "auc" ${print_log} | tr -d "auc=" | tail -n 100 | awk '{sum+=$1} END {print"",sum/NR}' | sed 's/[[:space:]]//g'`

# 提取所有loss打印信息
grep -Eo "in_tr_loss=[0-9].[0-9]*" ${print_log} | tr -d "in_tr_loss=" > ./test/output/${ASCEND_DEVICE_ID}/my_output_loss.txt
