...
then
    ...
    python3 ./train.py --datapath=${data_path}/meshes --training_steps=${training_steps} --precision_mode=default > ${print_log} 2>&1
fi

wait
# 性能相关数据计算，按照关键字，取最后n行中每行的最后一列（默认空格分隔），求平均
StepTime=`grep "StepTime:" ${print_log} | awk '{print $NF}'`
