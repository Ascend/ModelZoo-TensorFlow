...
class CondGANTrainer(object):
    ...
    def load_bin(self):
        # all = sorted(os.listdir('/home/TestUser07/stackgan/offline_inference/output_binfile/20211201_155309'),
        #              key=lambda x: int(x[-5]))
        a = sorted(os.listdir('/home/TestUser07/stackgan/offline_inference/output_binfile2/20211201_205753'),
                   key=lambda x: int(x[4:6]))
        b = sorted(os.listdir('/home/TestUser07/stackgan/offline_inference/output_binfile/20211201_204200'),
                   key=lambda x: int(x[4:6]))
        # output0=sorted(a,key = lambda x: int(x[4:6]))
        # output1=sorted(b,key = lambda x: int(x[4:6]))

        self.fake_images = []
        self.hr_fake_images = []
        for item in a:
            temp = np.fromfile('/home/TestUser07/stackgan/offline_inference/output_binfile2/20211201_205753/' + item,
                               dtype=np.float32)
            temp.shape = 64, 64, 64, 3
            self.fake_images.append(temp)
        for item in b:
            temp = np.fromfile('/home/TestUser07/stackgan/offline_inference/output_binfile/20211201_204200/' + item,
                               dtype=np.float32)
            temp.shape = 64, 256, 256, 3
            self.hr_fake_images.append(temp)
        self.fake_images = np.array(self.fake_images)
        # print(self.fake_images.shape)
        self.hr_fake_images = np.array(self.hr_fake_images)
        # print(self.hr_fake_images.shape)
    ...
    def build_test(self, input):
        with tf.variable_scope("g_net"):  # For testing
            c, _ = self.sample_encoded_context(input)
            z = tf.random_normal([self.batch_size, cfg.Z_DIM])
            fake_images = self.model.get_generator(tf.concat([c, z], 1), False)
        with tf.variable_scope("hr_g_net"):  # For testing
            hr_c, _ = self.sample_encoded_context(input)
            hr_fake_images = self.model.hr_get_generator(fake_images, hr_c, False)
    ...
    def train(self):
        ...
        custom_op.parameter_map["fusion_switch_file"].s = tf.compat.as_bytes(
            "stageII/fusion_switch.cfg")                                                               #add1
        ...
        with tf.Session(config=config) as sess:
            with tf.device("/gpu:%d" % cfg.GPU_ID):
                ...
                updates_per_epoch = int(number_example / self.batch_size) #138
                # int((counter + lr_decay_step/2) / lr_decay_step)
                decay_start = cfg.TRAIN.PRETRAINED_EPOCH
                epoch_start = int(counter / updates_per_epoch)  #164000/138=1188
                print("-------------epoch_start------------",epoch_start)  #1200-1188=12  12*138=1656
    ...
    def drawCaption(self, img, caption):
        ...
        # get a font
        fnt = ImageFont.truetype('freemono/FreeMono.ttf', 50) #add1
                ...
                for j in range(numSamples):
                    hr_samples, samples = sess.run([self.hr_fake_images, self.fake_images],
                                                   {self.embeddings: embeddings_batchs[
                                                       i]})  # (64, 256, 256, 3)   (64, 64, 64, 3)

                    # print("#######")
                    print("####")
                    print(hr_samples.shape)
                    print(samples.shape)
                    print("####")
                    # print("#######")
    ...
    def eval_one_dataset_from_bin(self, dataset, save_dir, subset='train'):
        count = 0
        count_file = 0
        print('num_examples:', dataset._num_examples)
        while count < dataset._num_examples:
            ...
            for i in range(len(embeddings_batchs)):
                ...
                numSamples = np.minimum(16, cfg.TRAIN.NUM_COPY)
                for j in range(numSamples):
                    samples,hr_samples = self.fake_images[count_file], self.hr_fake_images[count_file]
                    # print("####")
                    # print(hr_samples.shape)
                    # print(samples.shape)
                    # print("####")
        ...
        with tf.Session(config=config) as sess:
            with tf.device("/gpu:%d" % cfg.GPU_ID):
                if self.model_path.find('.ckpt') != -1:
                    ...
                    self.eval_one_dataset_from_bin(self.dataset.test, self.log_dir, subset='test')
                else:
