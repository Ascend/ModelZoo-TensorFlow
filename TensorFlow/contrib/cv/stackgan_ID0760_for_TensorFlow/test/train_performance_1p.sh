...
do
    ...
    sed -i "/MAX_EPOCH:/s@600@2@g" stageI/cfg/birds.yml
    sed -i "/SNAPSHOT_INTERVAL:/s@2000@200@g" stageI/cfg/birds.yml
    sed -i "s@'Data/%s'@'${data_path}/Data/%s'@g" stageI/run_exp.py
   
    python3 stageI/run_exp.py \
	    --cfg stageI/cfg/birds.yml > ${cur_path}/test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log 2>&1
done
wait
    sed -i "/MAX_EPOCH:/s@2@600@g" stageI/cfg/birds.yml
    sed -i "/SNAPSHOT_INTERVAL:/s@200@2000@g" stageI/cfg/birds.yml
    sed -i "s@'${data_path}/Data/%s'@'Data/%s'@g" stageI/run_exp.py
...
grep 'd_loss' $cur_path/test/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log|awk  '{print $7}'|awk -F ";" '{print $1}'  >> $cur_path/test/output/$ASCEND_DEVICE_ID/train_${CaseName}_loss.txt
#最后一个迭代loss值，不需要修改
