...
class COCOReader(Base2DReader):
    def __init__(self, name='COCO', mode='training', objtype=0, shuffle=True, batch_size=1, crop_noise=False):
        ...
        if name == 'COCO':
            self.image_root = '/tmp/COCO_data/train2017/'
            ...
            for i, image_data in enumerate(db_data['root']):
                ...
                img_dirs.append(os.path.join(self.image_root, image_data['img_paths'][-16:]))
                mask_dirs.append(os.path.join(self.mask_root, 'train2017_mask_all_'+image_data['img_paths'][-16:-3] + 'png'))
                ...
                human['body_valid'].append(valid.astype(np.bool))
            ...
            for i in range(total_num):
                ...
                human['body_valid'].append(body[:, 2].astype(np.int64))
            human['img_dirs'] = img_dirs
            human['mask_dirs'] = mask_dirs
