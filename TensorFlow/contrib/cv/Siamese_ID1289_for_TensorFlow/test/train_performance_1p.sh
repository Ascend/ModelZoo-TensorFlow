...
then
    ...
    python3.7 ./experiments/train-appearance-network.py --data_path=${data_path} --output_path=${output_path} --steps=${1000}
else
    sed -i s#/home/ysp/data/#${data_path}#g configuration.py
    sed -i s#/home/ysp/output/#${data_path}#g configuration.py
    python3.7 ./experiments/train-appearance-network.py --data_path=${data_path} --output_path=${output_path} --steps=${1000} 1>${print_log} 2>&1
fi

# 性能相关数据计算
StepTime=`grep "examples/sec" ${print_log} | tail -n 10 | awk '{print $18}' | awk '{sum+=$1} END {print sum/NR}'`
FPS=`grep "examples/sec" ${print_log} | tail -n 10 | awk '{print $20}' | awk '{sum+=$1} END {print sum/NR}'`

# 精度相关数据计算
train_accuracy=`grep "total loss =" ${print_log} | tail -n 10 | awk '{print $12}' | awk '{sum+=$1} END {print sum/NR}'`
# 提取所有loss打印信息
grep "batch loss =" ${print_log} | awk '{print $12}' > ./test/output/${ASCEND_DEVICE_ID}/my_output_loss.txt
