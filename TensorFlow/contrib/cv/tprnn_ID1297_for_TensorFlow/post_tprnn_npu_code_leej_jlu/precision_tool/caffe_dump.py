...
class CaffeProcess:
    def __init__(self):
        ...
        parse.add_argument(
            "-o", dest="output_path", help="<Required> the output path", required=True
        )
        parse.add_argument(
            "-i",
            "--input_bins",
            dest="input_bins",
            help="input_bins bins. e.g. './a.bin;./c.bin'",
            required=True,
        )
        parse.add_argument(
            "-n",
    ...
    def _load_inputs(self, net):
        ...
        for layer_name, blob in net.blobs.items():
            if layer_name in self.input_names:
                ...
                if self.calDataSize(input_bin_shape) == self.calDataSize(
                    input_bin.shape
                ):
                    input_bin = input_bin.reshape(input_bin_shape)
                else:
                    print(
                        "Error: input node data size %d not match with input bin data size %d.",
                        self.calDataSize(input_bin_shape),
                        self.calDataSize(input_bin.shape),
                    )
                    exit(-1)
    ...
    def process(self):
        ...
        net = caffe.Net(self.model_file_path, self.weight_file_path, caffe.TEST)
