...
class NpuDumpDecodeFile(object):
    ...
    def summary(self):
        txt = [
            "[yellow][%s][TaskID: %d][/yellow][green][%s][/green] %s"
...
class NpuDump(object):
    ...
    def get_dump_files_by_op(self, op):
        ...
        for f in self.dump_files:
            ...
            if re.match(match_name, f) and (
                op.graph_name in self.dump_files[f].path or cfg.NET_TYPE == "infer"
            ):
    ...
    def op_dump_summary(self, op):
        ...
        npu_dump_files = sorted(
            npu_dump_files.values(), key=lambda x: (x.idx, x.timestamp)
        )
    ...
    def _parse_dump_files(self):
        ...
        sub_dir = (
            os.path.join(self.dump_root, sub_dir) if sub_dir != "" else self.dump_root
        )
