...
class Op(object):
    ...
    def compare(self, right_op):
        ...
        res_str = [
            "LeftOp(Type/Name) : [green][%s][/green] %s" % (self.type(), self.name()),
            "RightOp(Type/Name): [green][%s][/green] %s"
            % (right_op.type(), right_op.name()),
        ]
        similar = True
    ...
    def summary(self, origin_txt=False):
        ...
        res_str = [
            "Op(Type/Name): [green][%s][/green] %s" % (self.type(), self.name()),
    ...
    def _parse_inputs(self):
        ...
        if "input" not in self.op_json:
            if self.type() not in NO_INPUT_NODES:
                self.log.warning(
                    "Parse Op[%s][%s] inputs error." % (self.type(), self.name())
                )
            return self.input_list
        desc_index = 0
    ...
    def _parse_outputs(self):
        ...
        if "dst_index" not in self.op_json:
            if self.type() not in NO_OUTPUT_NODES:
                self.log.warning(
                    "Parse Op[%s][%s] outputs error." % (self.type(), self.name())
                )
            return self.output_list
        desc_index = 0
