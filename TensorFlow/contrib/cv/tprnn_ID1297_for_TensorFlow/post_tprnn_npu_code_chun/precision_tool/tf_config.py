...
def estimator_dump_config(action=None):
    ...
    if _is_overflow(action):
        config = DumpConfig(
            enable_dump_debug=True, dump_path=cfg.NPU_OVERFLOW_DUMP_DIR, dump_mode="all"
        )
    elif _is_dump(action):
        config = DumpConfig(
            enable_dump=True,
            dump_path=cfg.DEFAULT_NPU_DUMP_DIR,
            dump_step=cfg.TF_DUMP_STEP,
            dump_mode="all",
        )
    else:
        config = DumpConfig()
...
def _is_dump(action):
    ...
    if (
        cfg.PRECISION_TOOL_DUMP_FLAG in os.environ
        and os.environ[cfg.PRECISION_TOOL_DUMP_FLAG] == "True"
    ):
...
def _is_overflow(action):
    ...
    if (
        cfg.PRECISION_TOOL_OVERFLOW_FLAG in os.environ
        and os.environ[cfg.PRECISION_TOOL_OVERFLOW_FLAG] == "True"
    ):
