...
try:
    ...
    rich_print = print
    print(
        "Failed to import rich. some function may disable. Run 'pip3 install rich' to fix it.",
        import_err,
    )
...
try:
    ...
    print(
        "Unable to import module: readline. Run 'pip3 install gnureadline pyreadline' to fix it."
    )
...
OFFLINE_DUMP_DECODE_PATTERN = r"^([A-Za-z0-9_-]+)\.([A-Za-z0-9_-]+)\.([0-9]+)(\.[0-9]+)?\.([0-9]{1,255})\.([a-z]+)\.([0-9]{1,255})\.npy$"
...
CPU_DUMP_DECODE_PATTERN = (
    r"^([A-Za-z0-9_-]+)\.([0-9]+)(\.[0-9]+)?\.([0-9]{1,255})\.npy$"
)
...
class Util(object):
    ...
    def convert_proto_to_json(self, src_file, dst_file):
        ...
        if os.path.exists(dst_file) and os.path.getmtime(dst_file) > os.path.getmtime(
            src_file
        ):
    ...
    def list_dump_files(self, path, sub_path=""):
        ...
        for dir_path, dir_names, file_names in os.walk(
            os.path.join(path, newest_sub_path), followlinks=True
        ):
        ...
        for dir_path, dir_names, file_names in os.walk(path, followlinks=True):
            ...
            for item in mapping:
                ...
                if not os.path.isfile(src_file):
                    self.log.warning(
                        "Can not find file %s in mapping.csv, dir: %s.",
                        item[0],
                        dir_path,
                    )
                    continue
                ...
                dst_file_name = ".".join(
                    [
                        file_desc.op_type,
                        file_desc.file_name,
                        str(file_desc.task_id),
                        str(file_desc.stream_id),
                        str(file_desc.timestamp),
                    ]
                )
    ...
    def list_npu_dump_files(self, path, extern_pattern=""):
        npu_dump_files = self._list_file_with_pattern(
            path, OFFLINE_DUMP_PATTERN, extern_pattern, self._gen_dump_file_info
        )
        npu_dump_files.update(
            self.parse_mapping_csv(path, OFFLINE_DUMP_PATTERN, extern_pattern)
        )
    ...
    def list_ge_graph_files(self, path, extern_pattern=""):
        return self._list_file_with_pattern(
            path,
            Constant.Pattern.GE_PROTO_GRAPH_PATTERN,
            extern_pattern,
    ...
    def list_cpu_graph_files(self, path, extern_pattern=""):
        return self._list_file_with_pattern(
            path, CKPT_META_SHUFFIX, extern_pattern, self._gen_cpu_graph_files_info
        )

    def list_vector_compare_result_files(self, path, extern_pattern=""):
        return self._list_file_with_pattern(
            path,
            VECTOR_COMPARE_RESULT_PATTERN,
            extern_pattern,
    ...
    def list_numpy_files(self, path, extern_pattern=""):
        return self._list_file_with_pattern(
            path, NUMPY_PATTERN, extern_pattern, self._gen_numpy_file_info
        )
    ...
    def print_npy_summary(self, path, file_name, is_convert=False, extern_content=""):
        ...
        for i in range(min(16, int(np.ceil(flatten_data.size / 8)))):
            ...
            table.add_row(
                str(i * 8),
                " ".join(flatten_data[i * 8 : last_idx].astype("str").tolist()),
            )
        summary = [
            "[yellow]%s[/yellow]" % self.gen_npy_info_txt(data),
            "Path: %s" % target_file,
        ]
        if is_convert:
        ...
        self.print_panel(
            self.create_columns([table, Constant.NEW_LINE.join(summary)]), file_name
        )
        if is_convert:
    ...
    def _detect_file_if_not_exist(self, target_file):
        ...
        if len(res) == 0:
            raise PrecisionToolException(
                "Cannot find any file named %s in dir %s"
                % (target_file, cfg.CMD_ROOT_PATH)
            )
    ...
    def get_newest_dir(self, path: str):
        ...
        self.log.info(
            "Sub path num:[%d]. Dirs[%s], choose[%s]",
            len(sub_paths),
            str(sub_paths),
            newest_sub_path,
        )
    ...
    def _gen_build_graph_file_info(name, match, dir_path):
        return BuildGraphFileDesc(
            name, dir_path, -1, int(match.group(1)), match.group(2)
        )

    @staticmethod
    def _gen_dump_file_info(name, match, dir_path):
        return NpuDumpFileDesc(
            name,
            dir_path,
            int(match.groups()[-1]),
