...
if [ x"${modelarts_flag}" != x ];
then
    python3.7 ./nets/c3ae_npu.py --dataset=${data_path} -s=${output_path} -white -se
else
    python3.7 ./nets/c3ae_npu.py --dataset=${data_path} -s=${output_path} -white -se &> ${print_log}
fi

# 性能相关数据计算
StepTime=`grep "ms/step" ${print_log} | tail -n 10 | awk '{print $5}' | awk '{sum+=$1} END {print sum/NR/1000}'`
FPS=`awk 'BEGIN{printf "%.2f\n", '${batch_size}'/'${StepTime}'}'`

# 精度相关数据计算
train_accuracy=`grep "val_age_mean_absolute_error did not improve from" ${print_log} | tail -n 10 | awk '{print $8}' | sort |head -n 1`
# 提取所有loss打印信息
