...
if [ x"${modelarts_flag}" != x ];
then
    python3.7 ./nets/c3ae_npu.py --dataset=${data_path} -s=${output_path} -epochs=1 -white -se
else
    python3.7 ./nets/c3ae_npu.py --dataset=${data_path} -s=${output_path} -epochs=1 -white -se &> ${print_log}
fi
...
# 精度相关数据计算
train_accuracy=`grep "val_age_mean_absolute_error:" ${print_log} | awk '{print $NF}' | sort | head -n 1`
# 提取所有loss打印信息
