...
"""
HMR trainer.
From an image input, train a model that outputs 85D latent vector
...
from benchmark.eval_util import compute_errors_w_mask
...
class HMRTrainer(object):
    ...
    def use_pretrained(self):
        ...
        Returns true only if:
          ...
          3. model_dir is NOT empty, meaning we're picking up from previous pretrained model.
        """
        if ('resnet' in self.model_type) and (self.pretrained_model_path is
                                              not None):
            # Check if model_dir is empty
            import os
    ...
    def draw_results(self, result):
        ...
        for img_id, (img, gt_kp, verts, joints, cams) in enumerate(
            ...
            bio = BytesIO()
            plt.imsave(bio, combined, format='png')
            vis_sum = tf.Summary.Image(
                encoded_image_string=bio.getvalue(),
        ...
        with self.sv.managed_session(config=sess_config) as sess:
            ...
            while not self.sv.should_stop():
                fetch_dict = {
                    ...
                    # For metrics
                    "gt_joints": self.gt_joints,
                    "pred_joints": self.pred_joints,
                    "has_gt3d_joints": self.has_gt3d_joints,
                }
                if not self.encoder_only:
                    fetch_dict.update({
                        # For D:
