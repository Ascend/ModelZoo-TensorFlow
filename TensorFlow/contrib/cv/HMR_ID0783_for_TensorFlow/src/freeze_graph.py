...
class Inference(object):
    def __init__(self, config, sess, images_pl):
        """
        Args:
          config
        """
    ...
    def predict_dict(self):
        ...
        joints = fetch_dict['joints']
        fetch_dict['joints'] = ((joints + 1) * 0.5) * self.img_size

        return fetch_dict['joints3d']
...
if __name__ == '__main__':
    ...
    with tf.Session() as sess:
        input_node = tf.placeholder(tf.float32, [None, config.img_size, config.img_size, 3], 'the_inputs')
        model = Inference(config, sess, input_node)
        output_node = model.predict_dict()
        flow = tf.identity(output_node, 'the_outputs')
        tf.train.write_graph(sess.graph_def, './pb_model', 'model.pb')
        freeze_graph.freeze_graph('./pb_model/model.pb', '', False, config.load_path, 'the_outputs',
                                'save/restore_all', 'save/Const:0', './pb_model/inference.pb', False, "")