...
if __name__ == "__main__":
    ...
    parser.add_argument("--train_url", type=str, default='/root/code/dhaa/LAJ_DHAA/Jobs_morph/')
    parser.add_argument("--data_url", type=str, default='/root/code/dhaa/LAJ_DHAA/morph/')
    ...
    dataset_test = Dataset(args, image_list, label_list, lms_list, mode='test', test_flip=False)
    epoch_size = len(test_image_list)
    print("epoch_Size",epoch_size)
    with tf.Session() as sess:
        for i in range(len(test_image_list)):
            image_batch, label_batch, filename_batch, thetas = sess.run(list(dataset_test.nextit))
            #########################
            inner_path = filename_batch[0].decode(encoding='UTF-8').split('/')[-3:]
            inner_path[-1] = inner_path[-1].split('.')[0]
            out_path = os.path.join('/root/code/dhaa/LAJ_DHAA/ATC_DHAA/data_bin/{}/'.format(args.phase), '_'.join(inner_path) + ".bin")
