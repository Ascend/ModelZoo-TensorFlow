...
from util.utils import instance_evaluation
...
def get_bin_label_dict(path, label_file):
    ...
    with open(label_file) as f:
        for eachline in f:
            ...
            final_name = final_name_ + '_output_0.bin'
            # print(final_name)
            # input()
...
def main(test_bin_path, data_url, fid):
    #### Set GPU options ###
    ...
    for batch_it in range(test_batches):
        ...
        for i in tmp_img_list:
            tmp_feature = np.fromfile(i, dtype='float32').reshape([1, 35])
            img_feat_list.append(tmp_feature)
            img_label_list.append(img_label_dict[i])
        img_feats = np.array(img_feat_list).reshape([-1, 35])
        img_labels = np.array(img_label_list).reshape([-1, 35])
