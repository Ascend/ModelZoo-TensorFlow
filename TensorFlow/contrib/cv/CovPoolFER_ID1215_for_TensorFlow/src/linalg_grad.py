...
def _SelfAdjointEigV2Grad(op, grad_e, grad_v):
  ...
  with ops.control_dependencies([grad_e, grad_v]):
    if compute_v:
      ...
      tmp_data = array_ops.expand_dims(e, -2) - array_ops.expand_dims(e, -1)
      tmp_abs = math_ops.abs(tmp_data)
      condition = math_ops.less(tmp_abs, 1e-8)
      tmp_where = array_ops.where(condition, tmp_data + 1e-3, tmp_data)
      f = array_ops.matrix_set_diag(math_ops.reciprocal(tmp_where), array_ops.zeros_like(e))
