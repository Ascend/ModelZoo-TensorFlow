...
from npu_bridge.estimator.npu.npu_loss_scale_manager import ExponentialUpdateLossScaleManager, FixedLossScaleManager
...
#os.environ["ASCEND_DEVICE_ID"] = "2"


# os.environ["DUMP_GE_GRAPH"] = "2"
# os.environ["DUMP_GRAPH_PATH"] = "/home/test_user02/yuxh/dumpgraph2/"
# os.environ["ASCEND_PROCESS_LOG_PATH"] = "/home/test_user02/yuxh/STGAN_GPU/log/"
# ==============================================================================
# =                                    param                                   =
# ==============================================================================
def boolean(s):
...
# settings
dataroot_default = '/home/test_user02/yuxh/celeba/'
...
parser.add_argument('--init_epoch', type=int, default=70, help='# of epochs with init lr.')
...
#data.py需要的参数
parser.add_argument('--train_image_num', type=int, default=182000,help='train image num')
...
tr_data = data.Celeba(args, dataroot, atts, img_size, batch_size, part='train', sess=sess, crop=crop_)
val_data = data.Celeba(args, dataroot, atts, img_size, n_sample, part='val', shuffle=False, sess=sess, crop=crop_)
...
_a = tf.cast(_a, tf.float32)
_b = tf.cast(_b, tf.float32)
# generate
z = Genc(xa)
...
xa__loss_rec = tf.reduce_mean(tf.abs(xa - xa_))
...
# d_step = tf.train.AdamOptimizer(lr, beta1=0.5)
# d_step = NPULossScaleOptimizer(d_step, loss_scale_manager)
# d_step = d_step.minimize(d_loss, var_list=d_var)
d_step = tf.train.AdamOptimizer(lr, beta1=0.5).minimize(d_loss, var_list=d_var)
# d_step = tf.train.GradientDescentOptimizer(lr).minimize(d_loss, var_list=d_var)
# d_step = tf.train.RMSPropOptimizer(lr).minimize(d_loss, var_list=d_var)

g_var = tl.trainable_variables('G')
# g_step = tf.train.AdamOptimizer(lr, beta1=0.5)
# g_step = NPULossScaleOptimizer(g_step, loss_scale_manager)
# g_step = g_step.minimize(g_loss, var_list=g_var)
...
try:
    ...
    for it in range(sess.run(it_cnt), max_it):
        with pylib.Timer(is_output=False) as t:
            ...
            # display
            if (it + 1) % 1 == 0:
                print("Epoch: (%3d) (%5d/%5d)  d_loss: %.8f  Time: %s " % (epoch, it_in_epoch, it_per_epoch,sess.run(d_loss), t))
