...
if [ x"${etp_flag}" != xtrue ];
then
    python3.7 ./jshdr_train.py --data_path=${data_path} --output_path=${output_path} --steps=${train_steps}
else
    python3.7 ./jshdr_train.py --data_path=${data_path} --output_path=${output_path} --steps=${train_steps} > ${print_log} 2>&1
fi
