...
if [ x"${modelarts_flag}" != x ];
then
    python3.7 ./training.py --srcdir=${data_path}/dataset --modeldir=${data_path} --outdir=${output_path} --logdir=${output_path} --epochs=${train_epoch} 1>${print_log} 2>&1
else
    python3.7 ./training.py --srcdir=${data_path}/dataset --modeldir=${data_path}/pretrained_semantic_model --outdir=${output_path} --logdir=${output_path} --epochs=${train_epoch} 1>${print_log} 2>&1
fi

# 性能相关数据计算
StepTime=`grep "sec/step" ${print_log} | awk '{print $4}' | tail -n 50 | awk '{sum+=$1} END {print sum/NR}'`
FPS=`awk 'BEGIN{printf "%.2f\n", '${batch_size}'/'${StepTime}'}'`

# 精度相关数据计算
train_accuracy=`grep "valid loss" ${print_log} | tail -n 50 | awk '{print $5}' | awk '{sum+=$1} END {print sum/NR}'`
# 提取所有loss打印信息
grep "loss" ${print_log} | grep -v "valid" | awk '{print $4}' >./test/output/${ASCEND_DEVICE_ID}/my_output_loss.txt
