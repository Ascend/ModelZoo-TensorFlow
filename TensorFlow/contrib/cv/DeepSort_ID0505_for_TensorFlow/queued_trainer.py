...
class QueuedTrainer(object):
    ...
    def run(self, feed_iterator, train_op, log_dir="/tmp/slim_trainer/",
        ...
        with tf.Session(config=config) as session:
            ...
            feed_iterator.initializer.run()

            number_of_steps = kwargs['number_of_steps']
            # log_interval_steps = kwargs['log_interval_steps']
            ...
            while True:
                ...
                print("========global step: {:d}".format(step_index))
                print("========each step time: {:.2f}".format((time.time() - train_start)*1000))
                print("========loss: ", total_loss)
            saver.save(session, os.path.join(log_dir, "mode.ckpt"))
