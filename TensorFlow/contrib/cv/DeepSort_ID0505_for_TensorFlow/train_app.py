...
def train_loop(preprocess_fn, network_factory, train_x, train_y,
    ...
    def process(batch_x, batch_y):
        ...
        with tf.device('/cpu:0'):
            image_var = tf.map_fn(
                ...
                batch_x, back_prop=False, dtype=tf.uint8)
            ...
            image_var.set_shape((batch_size, None, None, None))
            label_var.set_shape((batch_size))

        return image_var, label_var

    dataset = tf.data.Dataset.from_generator(queued_trainer.random_sample_identities_forever,
            (tf.string, tf.int64), ((128,),  (128,)), (batch_size, num_images_per_id, train_x, train_y))
    dataset = dataset.map(process).prefetch(batch_size)
    feed_iterator = dataset.make_initializable_iterator()
    trainer, train_op = create_trainer(
        feed_iterator, preprocess_fn, network_factory, read_from_file, image_shape, batch_size,
    ...
    trainer.run(
        feed_iterator, train_op, log_dir, restore_path=restore_path,
...
def create_trainer(feed_iterator, preprocess_fn, network_factory, read_from_file, image_shape,
    ...
    #         filename_var, label_var = feed_iterator.get_next()
    #         image_var = tf.map_fn(
    #             lambda x: tf.image.decode_jpeg(
    #                 tf.read_file(x), channels=num_channels),
    ...
    #         image_var, label_var = feed_iterator.get_next()
