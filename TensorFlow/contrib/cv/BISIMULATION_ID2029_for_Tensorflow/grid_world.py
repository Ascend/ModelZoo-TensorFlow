...
class GridWorld(object):
  ...
  def _build_train_op(self, optimizer):
    ...
    self.target_network = tf.make_template('Target',
                                           self._network_template)
    # float64
    self.s1_ph = tf.placeholder(tf.float32, (self.batch_size, 2),
                                name='s1_ph')
    # float64
    self.s2_ph = tf.placeholder(tf.float32, (self.batch_size, 2),
                                name='s2_ph')
    ...
    # float64
    self.rewards_ph = tf.placeholder(tf.float32, (self.batch_size,))
    ...
    larger_targets_count = tf.reduce_sum(
        tf.cast(larger_targets > 0., tf.float32))
  ...
  def _build_eval_metric(self):
    ...
    """
    # float64
    self.eval_states_ph = tf.placeholder(tf.float32, (self.num_states, 2),
                                         name='eval_states_ph')
  ...
  def learn_metric(self, verbose=False):
    ...
    for state in range(self.num_states):
      ...
      # float64
    eval_states = np.array(eval_states, dtype=np.float32)
    ...
    with tf.Session(config=config) as sess:
      ...
      for i in range(self.num_iterations):
        ...
        if i % 1 == 0:
          ...
          print('ONE_ITER_TIME: {}'.format(one_iter_time))
  ...
  def sample_distance_pairs(self, num_samples_per_cell=2, verbose=False):
    ...
    """
    # float64
    paired_states_ph = tf.placeholder(tf.float32, (1, 4),
