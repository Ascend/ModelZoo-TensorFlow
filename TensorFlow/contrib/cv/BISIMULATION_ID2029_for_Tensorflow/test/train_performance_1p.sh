...
if [ x"${modelarts_flag}" != x ];
then
    python3.7 -m compute_metric_profiling --base_dir=/tmp/grid_world_profiling --grid_file=configs/mirrored_rooms.grid --gin_files=configs/mirrored_rooms.gin
else
    python3.7 -m compute_metric_profiling --base_dir=/tmp/grid_world_profiling --grid_file=configs/mirrored_rooms.grid --gin_files=configs/mirrored_rooms.gin > ${print_log} 2>&1
fi
