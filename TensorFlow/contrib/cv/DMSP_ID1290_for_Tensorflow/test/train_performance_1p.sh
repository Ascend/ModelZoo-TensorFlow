...
python3.7 ./src/demo_DMSP.py --data_url=${data_path}/BSDS300/images/train > ${print_log} 2>&1

# 性能相关数据计算
StepTime=`grep "sec/batch" ${print_log} | tail -n 10 | awk '{print $7}' | awk '{sum+=$1} END {print sum/NR}'`
FPS=`grep "examples/sec" ${print_log} | tail -n 10 | awk '{print $5}' | tr -d '(' | awk '{sum+=$1} END {print sum/NR}'`
...
echo "Final Performance examples/sec : $FPS"
echo "Final Performance sec/batch : $StepTime"
