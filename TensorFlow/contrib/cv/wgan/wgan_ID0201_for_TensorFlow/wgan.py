...
from skimage.metrics import structural_similarity as ssim  #add
...
class WassersteinGAN(object):
    ...
    def save(self,save_dir):
        ...
        sess = self.sess
        saver.save(sess, os.path.join(save_dir,'save_dir')) 
    ...
    def eval(self, batch_size=1, num_batches=60000 // 1):
        saver = tf.train.import_meta_graph(os.path.join(args.ckpt, "save_dir.meta"))
        saver.restore(self.sess,tf.train.latest_checkpoint('./ckpt'))
        print('Evaluating {}...'.format(args.ckpt))
        current = []
       
        for t in range(0, num_batches):
            bx = self.x_sampler(batch_size).astype(np.float32).reshape(64,64,3)
            print(bx.shape)
            bz = self.z_sampler(batch_size, self.z_dim).astype(np.float32)
            d_ = self.sess.run(self.x_, feed_dict={self.z: bz})

            
            test_result = d_.reshape(1, 64, 64, 3)
            test_recon_image = self.data2img(test_result)
            test_recon_image = np.reshape(test_recon_image, (64, 64, 3))
            ss = ssim(test_recon_image, bx, multichannel=True)
            print(ss)
            current.append(math.fabs(ss))

        print("ssim_mean:",self.Average(current))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--iters', type=int, default=18750)   # 60000 // 64 * 20 for mnist                  300000 // 64 * 60 for lsun
    parser.add_argument('--use', type=str, default='train',help="train or eval")
    ...
    if args.use == 'train':
        ...
        wgan.eval(num_batches=args.iters)
