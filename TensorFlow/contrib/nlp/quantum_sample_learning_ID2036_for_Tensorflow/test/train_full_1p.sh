...
then
    python3.7 ./run_lm.py \
        --probabilities_path=${data_path}/q12c0.txt \
        --checkpoint_dir=${PWD}/training_checkpoints_lm \
        --epochs=20 \
        --eval_samples=500000 \
        --training_eval_samples=4000 \
        --experimental_bitstrings_path=${PWD}/data/experimental_samples_q12c0d14.txt \
        --train_size=500000  > ${print_log} 2>&1
...
# 性能相关数据计算
StepTime=`grep "7812/7812" ${print_log} | grep -v "ETA" | awk '{print $5}' | tail -n +2 | tr -d "ms/step" | awk '{sum+=$1} END {print sum/NR}'`
Traintime=`awk 'BEGIN{printf "%.2f\n", '${StepTime}'/'1000'}'`
FPS=`awk 'BEGIN{printf "%.2f\n", '${batch_size}'/'${Traintime}'}'`

# 精度相关数据计算
train_accuracy=`grep "linear_xeb:" ${print_log} | awk 'END {print $NF}'`
# 提取所有loss打印信息
grep "loss:" ${print_log} | grep -v "ETA" | awk '{print $NF}' > ./test/output/${ASCEND_DEVICE_ID}/my_output_loss.txt
...
echo "Final Performance sec/step : $Traintime"
...
echo "TrainingTime = ${Traintime}" >> $cur_path/output/$ASCEND_DEVICE_ID/${CaseName}.log
