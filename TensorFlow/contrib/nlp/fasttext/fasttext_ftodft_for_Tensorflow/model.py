...
def parse_batch_func(batch_size):
    ...
    def parse(batch):
        ...
        # inputs = tf.reshape(inputs, [batch_size, -1]) # npu-cann need set -1 to fixed 
        inputs = tf.reshape(inputs, [batch_size, 46])
...
class TrainingModel():
    ...
    def __init__(self, settings, cluster=None):
        ...
        with self.graph.as_default():
            ...
            # with tf.device(device): # npu-cann need set device to cpu
