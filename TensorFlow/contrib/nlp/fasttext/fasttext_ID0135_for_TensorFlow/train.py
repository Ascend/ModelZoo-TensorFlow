...
def run_train(data, train_specific, train_params, data_specific, train_history, train_history_path):
    ...
    train_url = train_params["train_url"]
    ...
    with tf.Session(config=config) as sess:
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            if comparable > best_score:
                ...
                freeze_save_graph(sess, train_url, "model_best.pb", "prediction")
                logs["best"] = epoch

            if save_all_models:
                freeze_save_graph(sess, train_url, "model_ep{}.pb".format(epoch), "prediction")
            else:
                if epoch == num_epochs:
                    freeze_save_graph(sess, train_url, "model_ep{}.pb".format(epoch), "prediction")
            iteration += 1
        ...
        print("The model is stored at {}".format(train_url), flush=flush)
        if use_test:
