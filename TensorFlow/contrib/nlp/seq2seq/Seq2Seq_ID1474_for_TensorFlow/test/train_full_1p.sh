...
#基础参数，需要模型审视修改
#网络名称，同目录名称
Network="Seq2Seq_ID1474_for_TensorFlow"
#训练epoch
train_epochs=10
#训练batch_size
batch_size=128
...
do
	...
	python3 translate.py \
	--compute_bleu_dev_set=True \
	--data_dir=${data_path}  > ${cur_path}/test/output/${ASCEND_DEVICE_ID}/test_${ASCEND_DEVICE_ID}.log   2>&1
done 
wait
...
train_acc=`grep 'The average score of Bleu' $cur_path/test/output/$ASCEND_DEVICE_ID/test_${ASCEND_DEVICE_ID}.log |awk '{print $7}'`
#train_acc=`awk 'BEGIN{printf "%.3f\n",  '${train_acc}'}'`
...
#最后一个迭代loss值，不需要修改
grep 'global step' $cur_path/test/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log|awk  '{print $10}'  > $cur_path/test/output/$ASCEND_DEVICE_ID/train_${CaseName}_loss.txt
