...
def prepare_wmt_data(data_dir, en_vocabulary_size, fr_vocabulary_size, use_rev_sou_sen, tokenizer=None, ):
    ...
    Args:
      ...
      use_rev_sou_sen: boolean.If true, we use the training set of inverted source sentences.
      tokenizer: a function to use to tokenize each data sentence;
        if None, basic_tokenizer will be used.
    ...
    if use_rev_sou_sen:
        # Method parameter 'train_path' is before reversal.
        # Return value parameter 'train_path' is the reverse.
        train_path_en = rev_sou_sen(train_path)
        from_train_path = train_path_en + ".en"
    else:
        from_train_path = train_path + ".en"
    print("The English training set is:", from_train_path)
...
# Extract the original test set and use it to calculate Bleu score
def extract_dev_set(data_dir):
    dev_set_str_list = []
    with open(os.path.join(data_dir, 'newstest2013.en'), 'r', encoding='utf-8') as f_en:
        with open(os.path.join(data_dir, 'newstest2013.fr'), 'r', encoding='utf-8') as f_fr:
            dev_set_en_str_list = f_en.readlines()
            dev_set_fr_str_list = f_fr.readlines()
            for i in range(len(dev_set_en_str_list)):
                dev_set_str_list.append(
                    [dev_set_en_str_list[i].replace('\n', ""), dev_set_fr_str_list[i].replace('\n', "")])

    # print(len(dev_set_str_list))
    # print(dev_set_str_list)
    return dev_set_str_list


# Reverse source sentence
def rev_sou_sen(train_normal_path):
    rev_sen_path = train_normal_path + ".reversed"
    if not gfile.Exists(rev_sen_path + ".en"):
        with open(train_normal_path + ".en", 'r', encoding='utf-8') as ft:
            with open(rev_sen_path + ".en", 'w', encoding='utf-8') as fr:
                normal_sen = ft.readline().replace('\n', "")
                while normal_sen:
                    rev_sen_list = normal_sen.split()
                    rev_sen_list.reverse()
                    rev_sen = " ".join(rev_sen_list)
                    fr.write(rev_sen + '\n')
                    normal_sen = ft.readline().replace('\n', "")
    return rev_sen_path
