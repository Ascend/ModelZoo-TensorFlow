...
def create_vocab_tables2(vocablist, UNK_ID, UNK):
  """Creates vocab tables."""
  # this implementation is c++-friendly in freezed model
  vocab2id = collections.OrderedDict()
  vocab_size = 0
...
class CPUVocab(object):
  def __init__(self, vocablist, default_value):
    """Creates vocab tables."""
    # this implementation is c++-friendly in freezed model
    self.vocab2id = collections.OrderedDict()
    vocab_size = 0
    for v in vocablist:
      self.vocab2id[v] = vocab_size
      vocab_size += 1
    self.default_value = default_value
  def __call__(self, xx):
    """lookup"""
    if isinstance(xx, list):
      return [self.vocab2id.get(x, self.default_value) for x in xx]
    elif instance(xx, str):
      return [self.vocab2id.get(xx, self.default_value)]
    else:
      raise ValueError("Unsupported string type: %s" % (type(xx)))