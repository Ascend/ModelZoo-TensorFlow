...
class _LogSessionRunHook(tf.train.SessionRunHook):
  ...
  def before_run(self, run_context):
    ...
    return tf.train.SessionRunArgs(
                #fetches=[tf.train.get_global_step(), 'total_loss:0', 'loss_scale:0', 'overflow_status_reduce_all:0'])
                fetches=[tf.train.get_global_step(), 'total_loss:0'])

  def after_run(self, run_context, run_values):

    # global_step, total_loss, loss_scaler, isFinite = run_values.results
    global_step, total_loss = run_values.results
    print('###Test print### Step = %6i Loss = %9.6f ' % (global_step, total_loss), flush=True)

training_hooks = []
training_hooks.append(_LogSessionRunHook(1))
...
def main(_):
  ...
  # 此部分注释掉的为调试精度问题时添加的  混合精度、打开/关闭融合规则，以及加入黑名单等操作dump数据的配置
  """
  ...
  custom_op.parameter_map["fusion_switch_file"].s = tf.compat.as_bytes("/home/test_user06/tc_workspace/tmp/fusion_switch_off.cfg")
  #custom_op.parameter_map["enable_dump_debug"].b = True
  #custom_op.parameter_map["dump_path"].s = tf.compat.as_bytes('/home/test_user08/tc_workspace/tmp/smith')
  #custom_op.parameter_map["dump_debug_mode"].s = tf.compat.as_bytes("all")
  
  session_config = npu_config_proto(config_proto=config)
  ######### Add dump config by:TC #################
  """
  ...
  if FLAGS.schedule == "train":
    ...
    run_config = tf.estimator.RunConfig(
        model_dir=FLAGS.output_dir,
        save_checkpoints_steps=exp_config.train_eval_config
        .save_checkpoints_steps, save_summary_steps=0)
