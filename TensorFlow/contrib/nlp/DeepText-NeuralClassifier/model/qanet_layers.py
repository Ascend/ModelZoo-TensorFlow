...
def highway(x, size=None, activation=None,
    ...
    with tf.variable_scope(scope, reuse):
        ...
        for i in range(num_layers):
            ...
            H = npu_ops.dropout(H, 1.0 - dropout)
            x = H * T + x * (1.0 - T)
        return x
...
def layer_dropout(inputs, residual, dropout):
    ...
    return tf.cond(pred, lambda: residual,
                   lambda: npu_ops.dropout(inputs, 1.0 - dropout) + residual)
...
def conv_block(inputs, num_conv_layers, kernel_size, num_filters,
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        for i in range(num_conv_layers):
            ...
            if (i) % 2 == 0:
                outputs = npu_ops.dropout(outputs, 1.0 - dropout)
...
def dot_product_attention(q,
    ...
    with tf.variable_scope(scope, default_name="dot_product_attention",
        ...
        weights = npu_ops.dropout(weights, 1.0 - dropout)
...
def optimized_trilinear_for_attention(args, c_maxlen, q_maxlen,
    ...
    dtype = args[0].dtype
    droped_args = [npu_ops.dropout(arg, input_keep_prob) for arg in args]
...
def trilinear(args,
    ...
    with tf.variable_scope(scope):
        ...
        flat_args = [npu_ops.dropout(arg, input_keep_prob) for arg in flat_args]
