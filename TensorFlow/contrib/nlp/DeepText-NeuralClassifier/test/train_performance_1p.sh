...
# export AOE_MODE=1
# export TUNE_BANK_PATH=/home/test_user08/adq/custom_tune_bank
...
python3.7 run_train.py \
    ...
    --train_epochs=${train_epochs} 1>${print_log} 2>&1


# 性能相关数据计算
# StepTime=`grep "" ${print_log} | tail -n 10 | awk '{print $NF}' | awk '{sum+=$1} END {print sum/NR}'`
step_per_sec=grep 'global_step/sec' ${print_log} | tail -1 | awk '{print $NF}'
StepTime=`echo "scale=2; 1/$step_per_sec" | bc`
...
train_accuracy=`grep 'precision: .*, recall' ${print_log} -o | tail -1 | grep -P '[0-9|\.]+' -o`
