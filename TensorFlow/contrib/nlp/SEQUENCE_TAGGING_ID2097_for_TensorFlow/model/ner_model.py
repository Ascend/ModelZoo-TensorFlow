...
class NERModel(BaseModel):
    ...
    def add_embeddings_op(self):
        ...
        self.word_embeddings = npu_ops.dropout(word_embeddings, self.dropout)
    ...
    def add_logits_op(self):
        ...
        with tf.compat.v1.variable_scope("bi-lstm"):
            ...
            self.word_embeddings = npu_ops.dropout(output, self.dropout)

        self.logits = tf.layers.dense(output, self.config.ntags)
        self.crf_params = tf.get_variable("crf", [self.config.ntags, self.config.ntags], dtype=tf.float32)
    ...
    def add_loss_op(self):
        ...
        if self.config.use_crf:
            log_likelihood, trans_params = tf.contrib.crf.crf_log_likelihood(
                self.logits, self.labels, self.sequence_lengths, self.crf_params)
