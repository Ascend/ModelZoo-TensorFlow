...
if [ x"${modelarts_flag}" != x ];
then
    python3.7 ./train.py --data_path=${data_path} --output_path=${output_path} --epochs=${train_epochs}
else
    python3.7 ./train.py --data_path=${data_path} --output_path=${output_path} --epochs=${train_epochs} 1>${print_log} 2>&1
fi
...
# 精度相关数据计算
train_accuracy=`grep "Final best score" ${print_log}  | awk '{print $NF}'`
# 提取所有loss打印信息
