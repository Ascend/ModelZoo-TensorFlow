...
class Network(object):
    ...
    @layer
    def anchor_target_layer(self, input,input1, _feat_stride, anchor_scales, name):
        if isinstance(input[0], tuple):
            input[0] = input[0][0]
        rpn_labels, rpn_bbox_targets, rpn_bbox_inside_weights, rpn_bbox_outside_weights = \
            anchor_target_layer_py(
                       input1[0], input1[1], input1[2], input1[3],
                        input1[4], _feat_stride, anchor_scales)
    ...
    def build_loss(self, ohem=False):
        ...
        rpn_bbox_targets = self.get_output('rpn_bbox_targets')
        rpn_bbox_inside_weights = self.get_output('rpn_bbox_inside_weights')
        rpn_bbox_outside_weights = self.get_output('rpn_bbox_outside_weights')
